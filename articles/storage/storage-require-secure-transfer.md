...
ms.date: 08/14/2017
ms.author: v-haiqya
ms.openlocfilehash: 4c3c0a59dc440f148d67a9bfc67c454518f2204d
...
“需要安全传输”选项通过仅允许从安全连接将请求发送到存储帐户可增强存储帐户的安全性。 例如，在调用 REST API 访问存储帐户时，必须使用 HTTPS 进行连接。 启用“需要安全传输”后，使用 HTTP 的任何请求都将被拒绝。

使用 Azure 文件服务时，如果启用了“需要安全传输”，未加密的任何连接都将失败。 这包括使用 SMB 2.1、未加密的 SMB 3.0 以及某些版本的 Linux SMB 客户端的方案。 
...
在 [Azure 门户](https://portal.azure.cn)中创建存储帐户时，以及对于现有存储帐户，都可以启用“需要安全传输”设置。

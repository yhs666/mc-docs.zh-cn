...
18. **如何针对 Azure 文件启用服务器端加密？**
    ...
    针对 Azure 文件的[服务器端加密](storage-service-encryption.md)目前提供预览版。 在预览期间，只能在使用 [Azure 门户预览](https://portal.azure.cn)新建的 Azure Resource Manager 存储帐户上启用此功能。 启用该功能没有额外收费。 针对 Azure 文件存储启用存储服务加密以后，系统会自动加密数据。 

...
* 使用 Azure 门户预览版或 PowerShell 创建新的 Azure 文件共享、添加目录、将本地文件上载到该共享，以及列出该目录中的文件。
...
## <a name="use-the-azure-portal-preview-to-manage-a-file-share"></a>使用 Azure 门户预览版管理文件共享
[Azure 门户预览版](https://portal.azure.cn)提供了方便客户管理文件共享的用户界面。 在门户中，你可以：
...
1. 首先，按照[在 Azure 门户预览版中创建 Windows 虚拟机](../virtual-machines/virtual-machines-windows-hero-tutorial.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)中的说明创建新的 Azure 虚拟机。
2. 接下来，按照[使用 Azure 门户预览版登录到 Windows 虚拟机](../virtual-machines/virtual-machines-windows-connect-logon.md)中的说明远程登录到虚拟机。
...
可以从 [Azure 门户预览版](https://portal.azure.cn)为文件存储启用指标。 你还可以通过 REST API 或存储客户端库中的类似物之一调用“设置文件服务属性”操作，以编程方式启用指标。

...
ms.openlocfilehash: 8c3598146d5943309f3bb4d8b1cd309703830668
ms.sourcegitcommit: 6a19227dcc0c6e0da5b82c4f69d0227bf38a514a
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 11/22/2019
ms.locfileid: "74328777"
---
...
> [!TIP]
> 适用于 Node.js 的 Azure 存储客户端库的最新版本是 v10。 我们建议尽可能使用最新版本的客户端库。 若要开始使用 v10，请参阅[快速入门：使用适用于 JavaScript v10 的 Azure 存储客户端库上传、下载、列出和删除 Blob（预览版）](storage-quickstart-blobs-nodejs-v10.md)。
...
*listContainers* 函数调用 [listContainersSegmented](https://docs.microsoft.com/javascript/api/azure-storage/azurestorage.services.blob.blobservice.blobservice?view=azure-node-latest)，后者返回组中容器的集合。
...
*createContainer* 函数调用 [createContainerIfNotExists](https://docs.microsoft.com/javascript/api/azure-storage/azurestorage.services.blob.blobservice.blobservice?view=azure-node-latest) 并为 Blob 设置相应的访问级别。
...
*uploadString* 函数调用 [createBlockBlobFromText](https://docs.microsoft.com/javascript/api/azure-storage/azurestorage.services.blob.blobservice.blobservice?view=azure-node-latest)，以便将任意字符串写入（或覆盖）到 Blob 容器。
...
```
其他适用于将内容上传到 Blob 中的方法涉及到使用[文本](https://docs.microsoft.com/javascript/api/azure-storage/azurestorage.services.blob.blobservice.blobservice?view=azure-node-latest-string--string--string---buffer--errororresult-blobresult--)和[流](https://docs.microsoft.com/javascript/api/azure-storage/azurestorage.services.blob.blobservice.blobservice?view=azure-node-latest#createblockblobfromstream-string--string--stream-readable--number--errororresult-blobresult--)。 为了验证文件是否已上传到 Blob 存储，可以使用 [Azure 存储资源管理器](https://azure.microsoft.com/features/storage-explorer/)查看帐户中的数据。
...
*downloadBlob* 函数使用 [getBlobToText](https://docs.microsoft.com/javascript/api/azure-storage/azurestorage.services.blob.blobservice.blobservice?view=azure-node-latest) 将 Blob 的内容下载到给定的绝对文件路径。
...
```
此处显示的实现更改了源并将 Blob 的内容作为字符串返回。 也可将 Blob 作为[流](https://docs.microsoft.com/javascript/api/azure-storage/azurestorage.services.blob.blobservice.blobservice?view=azure-node-latest)来下载，或者直接下载到[本地文件](https://docs.microsoft.com/javascript/api/azure-storage/azurestorage.services.blob.blobservice.blobservice?view=azure-node-latest)。
...
> [Azure Storage SDK for Node.js and JavaScript for Browsers](https://github.com/Azure/azure-storage-node)（Azure Storage SDK for Node.js 和 JavaScript for Browsers）

...
最后，若要删除 Blob，请调用 **deleteBlob**。 以下代码示例删除名为 **myblob**的 Blob。
...
以下代码示例生成了一个新的共享访问策略，该策略允许共享访问签名持有者对 **myblob** Blob 执行读取操作，并且在创建后 100 分钟过期。
...
然后，客户端应用程序共享访问签名用于 **BlobServiceWithSAS** ，以便针对 Blob 执行操作。 以下语句获取有关 **myblob**的信息。

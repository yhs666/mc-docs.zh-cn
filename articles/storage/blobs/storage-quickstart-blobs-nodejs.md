...
*listContainers* 函数调用 [listContainersSegmented](https://docs.microsoft.com/javascript/api/azure-storage/azurestorage.services.blob.blobservice.blobservice?view=azure-node-latest#listcontainerssegmented)，后者返回组中容器的集合。
...
*uploadString* 函数调用 [createBlockBlobFromText](https://docs.microsoft.com/javascript/api/azure-storage/azurestorage.services.blob.blobservice.blobservice?view=azure-node-latest#createblockblobfromtext)，以便将任意字符串写入（或覆盖）到 Blob 容器。
...
*downloadBlob* 函数使用 [getBlobToText](https://docs.microsoft.com/javascript/api/azure-storage/azurestorage.services.blob.blobservice.blobservice?view=azure-node-latest#getblobtotext) 将 Blob 的内容下载到给定的绝对文件路径。
...
```
此处显示的实现更改了源并将 Blob 的内容作为字符串返回。 也可将 Blob 作为[流](https://docs.microsoft.com/javascript/api/azure-storage/azurestorage.services.blob.blobservice.blobservice?view=azure-node-latest#getblobtostream)来下载，或者直接下载到[本地文件](https://docs.microsoft.com/javascript/api/azure-storage/azurestorage.services.blob.blobservice.blobservice?view=azure-node-latest#getblobtolocalfile)。

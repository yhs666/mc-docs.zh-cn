...
此示例使用 [createContainerCreateIfNotExists](https://docs.microsoft.com/nodejs/api/azure-storage/blobservice#azure_storage_BlobService_createContainerIfNotExists)，因为我们想要每次运行示例时都创建新容器。 在整个应用程序中使用相同容器的生产环境中，建议仅调用一次 CreateIfNotExists。 或者可以提前创建容器，这样就无需在代码中创建它。
...
若要上传文件至 blob，请使用 [createBlockBlobFromLocalFile](https://docs.microsoft.com/nodejs/api/azure-storage/blobservice?view=azure-node-2.2.0#azure_storage_BlobService_createBlockBlobFromLocalFile) 方法。 此操作会创建 blob（若尚不存在），或者覆盖它（若已存在）。
...
Blob 存储支持多种上传方法。 例如，若有一个内存流，则可以使用 [createBlockBlobFromStream](https://docs.microsoft.com/nodejs/api/azure-storage/blobservice?view=azure-node-2.2.0#azure_storage_BlobService_createBlockBlobFromStream) 方法来替换 [createBlockBlobFromLocalFile](https://docs.microsoft.com/nodejs/api/azure-storage/blobservice?view=azure-node-2.2.0#azure_storage_BlobService_createBlockBlobFromLocalFile)。

### <a name="list-the-blobs-in-a-container"></a>列出容器中的 Blob

下一步，此应用程序使用 [listBlobsSegmented](https://docs.microsoft.com/nodejs/api/azure-storage/blobservice?view=azure-node-2.2.0#azure_storage_BlobService_listBlobsSegmented) 获取容器中的文件列表。 下面的代码检索 blob 列表，然后循环访问它们，显示找到的 blob 的 URI。 可以从命令窗口中复制 URI，然后将其粘贴到浏览器以查看文件。
...
使用 [getBlobToLocalFile](https://docs.microsoft.com/nodejs/api/azure-storage/blobservice?view=azure-node-2.2.0#azure_storage_BlobService_getBlobToLocalFile) 方法将 blob 下载到本地磁盘。

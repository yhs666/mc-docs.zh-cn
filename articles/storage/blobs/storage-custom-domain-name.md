...
将自定义域映射到 Blob 终结点的过程会导致在 [Azure 门户](https://portal.azure.cn)中注册域时出现短暂的停机时间。 如果自定义域目前所支持的应用程序的服务级别协议 (SLA) 要求不能有故障时间，则可以使用 Azure asverify 子域作为中间注册步骤。 此中间步骤可确保用户能够在 DNS 映射进行时访问域。

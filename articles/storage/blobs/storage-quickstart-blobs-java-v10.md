---
title: Azure 快速入门 - 使用 Java 存储 SDK V10 在对象存储中创建 Blob | Azure
...
ms.date: 07/30/2018
ms.author: v-jay
ms.openlocfilehash: 51c343000c213fce76775580a92c170193b93a8a
ms.sourcegitcommit: 878351dae58cf32a658abcc07f607af5902c9dfa
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 07/27/2018
ms.locfileid: "39295860"
---
...
此示例使用 [containerURL.create]() ，因为我们想要每次运行示例时都创建新容器。 或者可以提前创建容器，这样就无需在代码中创建它。
...
为了将文件上传到 blob，需获取对目标容器中 blob 的引用。 有了 Blob 引用以后，即可将一个文件上传到其中，方法是在 BlockBlobURL 的实例中使用低级别 API，例如 Upload（又称 PutBlob）、StageBlock（又称 PutBLock），或者使用 [TransferManager](https://docs.microsoft.com/en-us/java/api/com.microsoft.azure.storage.blob._transfer_manager?view=azure-java-preview) 类中提供的高级别 API，例如 TransferManager.uploadFileToBlockBlob 方法。 此操作会创建 blob（若尚不存在），或者覆盖它（若已存在）。
...
可以使用 containerURL.listBlobsFlatSegment 获取容器中对象的列表。 此方法一次最多返回 5000 个对象，如果容器中还有更多需要列出的对象，则还会返回一个继续操作标记（后续操作标记）。 为此，我们创建一个帮助程序函数，只要在上一 listBlobsFlatSegment 响应中存在后续操作标记，该函数就会重复进行自身调用。
...
使用 blobURL.download 将 Blob 下载到本地磁盘。
...
如果不再需要在本快速入门中上传的 Blob，可使用 containerURL.delete 删除整个容器。 此方法也会删除容器中的文件。

...
ms.date: 08/28/2017
ms.author: v-haiqya
ms.openlocfilehash: e02a4fb06fd881e7c2d66b60a61a3fc004049d26
...
本指南将演示如何使用 Blob 存储执行常见方案。 相关示例是使用 Ruby API 编写的。 涉及的任务包括上传、列出、下载和删除 Blob。
...
Azure 模块会读取环境变量 AZURE_STORAGE_ACCOUNT 和 AZURE_STORAGE_ACCESS_KEY，获取连接到 Azure 存储帐户所需的信息。 如果未设置这些环境变量，则在使用 **Azure::Blob::BlobService** 之前必须通过以下代码指定帐户信息：
...
只需修改 create_container()<strong></strong> 调用即可传递 :public_access_level 选项：
...
若要将内容上传到 blob，请使用 create_block_blob() 方法创建 blob，将文件或字符串用作 blob 的内容。 
...
若要列出容器中的 blob，请使用 list_blobs() 方法。 

这会输出帐户的所有容器中的所有 Blog 的 URL。
...
以下代码示例演示了如何使用 get_blob() 下载“image-blob”的内容并将其写入本地文件。

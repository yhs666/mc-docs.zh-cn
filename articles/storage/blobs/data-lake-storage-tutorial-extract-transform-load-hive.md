...
ms.date: 01/14/2019
ms.author: v-jay
ms.openlocfilehash: 17ac1765656353c87094b769d4951112d890af16
ms.sourcegitcommit: 5eff40f2a66e71da3f8966289ab0161b059d0263
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 01/10/2019
ms.locfileid: "54192960"
---
...
![在 Azure HDInsight 上使用 Apache Hive 执行 ETL 操作](./media/data-lake-storage-tutorial-extract-transform-load-hive/hdinsight-etl-architecture.png "在 Azure HDInsight 上使用 Apache Hive 执行 ETL 操作")

如果没有 Azure 订阅，可在开始前创建一个 [1 元人民币试用帐户](https://www.azure.cn/zh-cn/pricing/1rmb-trial-full/?form-type=identityauth)。

## <a name="prerequisites"></a>先决条件

* **HDInsight 上基于 Linux 的 Hadoop 群集**。 有关创建新的基于 Linux 的 HDInsight 群集的步骤，请参阅[在 HDInsight 中使用 Hadoop、Spark、Kafka 等设置群集](./data-lake-storage-quickstart-create-connect-hdi-cluster.md)。
...
* **Azure CLI**。 如果尚未安装 Azure CLI，请参阅[安装 Azure CLI](/cli/install-azure-cli?view=azure-cli-latest)了解详细步骤。
...
可通过多种方式将数据上传到与 HDInsight 群集相关的存储。 本部分使用 `scp` 上传数据。 若要了解上传数据的其他方式，请参阅[使用 Distcp 在现有存储帐户和启用了 Data Lake Storage Gen2 的新存储帐户之间复制数据](data-lake-storage-use-distcp.md)。
...
1. 打开命令提示符，使用以下命令将 zip 文件上传到 HDInsight 群集头节点：
   ...
   > [!NOTE]
   > 如果使用密码对 SSH 登录名进行身份验证，系统会提示输入密码。 如果使用公钥，可能需要使用 `-i` 参数并指定匹配私钥的路径。 例如，`scp -i ~/.ssh/id_rsa FILE_NAME.zip USER_NAME@CLUSTER_NAME-ssh.azurehdinsight.cn:`。
...
如果已拥有 SQL 数据库，必须获取服务器的名称。 要在 [Azure 门户](https://portal.azure.cn)中查找服务器名称，请选择“SQL 数据库”，然后筛选要使用的数据库的名称。 服务器名称在“服务器名称”列中列出。
...
在前面的部分中，已经在 `abfs://<FILE_SYSTEM_NAME>@<ACCOUNT_NAME>.dfs.core.chinacloudapi.cn/tutorials/flightdelays/output` 复制了转换后的数据。 本部分使用 Sqoop 将数据从 `abfs://<FILE_SYSTEM_NAME>@<ACCOUNT_NAME>.dfs.core.chinacloudapi.cn/tutorials/flightdelays/output` 导出到在 Azure SQL 数据库中创建的表。 
...
2. 使用以下命令将 hivesampletable 中的数据导出到 delays 表：
    ...
    ```bash
    sqoop export --connect 'jdbc:sqlserver://<SERVER_NAME>.database.chinacloudapi.cn:1433;database=<DATABASE_NAME>' --username <ADMIN_LOGIN> --password <ADMIN_PASSWORD> --table 'delays' --export-dir 'abfs://<FILE_SYSTEM_NAME>@.dfs.core.chinacloudapi.cn/tutorials/flightdelays/output' 
...
[azure-member-offers]: http://azure.cn/pricing/member-offers/
[azure-1rmb-trial]: http://azure.microsoft.com/pricing/free-trial/

...
ms.date: 03/03/2017
ms.author: v-johch
ms.openlocfilehash: fe9c0eba4177b85b9d724198c26f70d374690768
...
---
# 启用存储指标并查看指标数据
<a id="enabling-storage-metrics-and-viewing-metrics-data" class="xliff"></a>
...
启用存储指标时，必须为数据选择保留期：此期限用于确定存储服务保留指标并针对存储指标所需的空间向你收费的时长。 通常，由于分钟指标需要大量额外的空间，因此，应对分钟指标而非小时指标使用较短的保留期。 你应该选择恰当的保留期，以便有足够的时间分析数据，并下载任何需要保留下来进行脱机分析或报告的度量值。 请记住，从存储帐户下载指标数据时，你也需要付费。

## 如何使用 Azure 经典管理门户启用存储指标
<a id="how-to-enable-storage-metrics-using-the-azure-classic-management-portal" class="xliff"></a>
...
## 如何使用 PowerShell 启用存储指标
<a id="how-to-enable-storage-metrics-using-powershell" class="xliff"></a>

...
ms.date: 01/06/2017
ms.author: v-johch
ms.openlocfilehash: b0a99fc69347e4930e9cfc48c5d4d1370abadf79
...
本文介绍通过使用 Visual Studio 中的“添加连接服务”对话框在 ASP.NET 5 项目中创建或引用 Azure 存储帐户之后，如何开始在 Visual Studio 中使用 Azure 表存储。
...
有关使用 Azure 表存储的更多常规信息，请参阅 [Get started with Azure Table storage using .NET](./storage-dotnet-how-to-use-tables.md)（通过 .NET 开始使用 Azure 表存储）。

若要开始，首先需要在存储帐户中创建表。 我们将向你展示如何使用代码创建 Azure 表。 此外，我们将展示如何执行基本的表和实体操作，例如添加、修改、读取和删除表实体。 示例采用 C\# 代码编写，并使用了适用于 .NET 的 Azure 存储客户端库。
...
若要访问 ASP.NET 5 项目中的表，需要将以下项包含在访问 Azure 表存储的任何 C# 源文件中。
...
若要创建 Azure 表，只需添加对 **CreateIfNotExistsAsync()** 的调用。
...
您可以通过单个写入操作将多个实体插入表中。 以下代码示例将创建两个实体对象（“Jeff Smith”和“Ben Smith”），使用 **Insert** 方法将它们添加到 **TableBatchOperation** 对象，然后通过调用 CloudTable.ExecuteBatchAsync 来启动操作。

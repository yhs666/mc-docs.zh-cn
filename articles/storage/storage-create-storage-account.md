...
> [!NOTE]
> 创建 Azure 虚拟机时，如果在部署位置中还没有存储帐户，则会在该位置自动创建一个存储帐户。 因此，没有必要按照下面的步骤来创建虚拟机磁盘的存储帐户。 存储帐户名称基于虚拟机名称。 请参阅 [Azure 虚拟机文档](../virtual-machines/index.md)了解更多详细信息。
...
5. 选择存储帐户的类型：“常规用途”或“Blob 存储”。 “常规用途”是默认值。
    ...
    如果已选择“Blob 存储”，则指定访问层：“经常访问”或“不常访问”。 默认值为“经常访问”。 有关如何使用存储帐户名称在 Azure 存储中定位你的对象的详细信息，请参阅 [Azure Blob 存储：不常访问和经常访问的层](./storage-blob-storage-tiers.md)了解更多详细信息。
...
对于 Blob 存储帐户，更改访问层除了会更改定价之外，可能还会产生更改费用。 有关更多详细信息，请参阅 [Blob 存储帐户 — 定价和计费](storage-blob-storage-tiers.md#pricing-and-billing)。

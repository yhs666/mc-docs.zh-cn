...
* 数据恢复：恢复存储在 Blob 存储中的大量数据，并将其递送到本地位置。
...
* 在你想要将当前作为 Blob 存储于存储帐户中的数据传输到运送给硬盘驱动器时创建导出作业。

创建作业时，需通知导入/导出服务：要将一个或多个硬盘驱动器运送到 Azure 数据中心。
...
只支持将 2.5 英寸 SSD、2.5/3.5 英寸 SATA II/III 或 SAS 内部 HDD 用于导入/导出服务。 单个导入/导出作业最多可有 10 个 HDD/SSD，且每个 HDD/SSD 大小不受限制。 大量驱动器可在多个作业中分布，且对可创建的作业数量没有限制。 若要使用用于连接磁盘阵列控制器的磁盘，请将磁盘阵列控制器设置为直通模式。
...
Azure 导入/导出服务支持将数据复制到中国东部或中国北部的 Azure 存储帐户，以及从后者进行复制。 可以将硬盘驱动器寄送到以下位置之一。 如果存储帐户所在的 Azure 位置未在此处指定，则使用 Azure 门户或导入/导出 REST API 创建作业时，系统会提供备用的寄送位置。
...
可以使用 Azure 导入/导出服务在本地站点和 Azure Blob 存储之间传输数据，只需创建作业，然后将硬盘驱动器寄送到 Azure 数据中心即可。 寄送的每个硬盘驱动器都与单个作业相关联。 每个作业都与单个存储帐户相关联。 请仔细查看 [“先决条件”部分](#pre-requisites) ，以了解此服务的具体情况，例如支持的 Blob 类型、磁盘类型、位置和寄送方式。

此部分深入介绍导入和导出作业所涉及的步骤。 随后，会在[“快速启动”部分](#quick-start)提供分步说明，指导创建导入和导出作业。
...
处理导入/导出作业的时间各不相同，取决于不同的因素，例如寄送时间、作业类型、要复制的数据的类型和大小，以及所提供磁盘的大小。 导入/导出服务没有 SLA。 可以通过 REST API 更密切地跟踪作业进度。 在“列出作业”操作中有一个完成百分比参数，该参数指示复制进度。 如果需要估算何时才能完成时间要求紧的导入/导出作业，请联系我们。
...
2. 根据数据总大小确定所需驱动器数目。 采购所需数目的 2.5 英寸 SSD、2.5/3.5 英寸 SATA II/III 或 SAS 硬盘驱动器。
...
5. 为数据集和驱动器集创建 CSV 文件。
    ...
    **数据集 CSV 文件**
    ...
    在上面的示例中，100M_1.csv.txt 会复制到名为“containername”的根容器中。 如果名为“containername”的容器不存在，系统会创建一个。 50M_original 下的所有文件和文件夹以递归方式复制到 containername。 文件夹结构保持不变。
...
1. 准备好驱动器后，在 Azure 门户中导航到用户的存储帐户，并查看“仪表板”。 在“速览”下，单击“创建导入作业”。 查看相关步骤，并选择指示用户已准备好驱动器并提供了驱动器日志文件的复选框。
2. 在步骤 1 中，提供负责该导入作业的人员的联系信息，以及有效的回寄地址。 如果希望保存导入作业的详细日志数据，则选中“将详细日志保存在我的‘waimportexport’Blob 容器中”选项。
3. 在步骤 2 中，上传在驱动器准备步骤中获取的驱动器日志文件。 需要为已准备好的每个驱动器上传一个文件。
   
   ![创建导入作业 - 步骤 3](./media/storage-import-export-service/import-job-03.png)
4. 在步骤 3 中，输入导入作业的描述性名称。 请注意，输入的名称只能包含小写字母、数字、连字符和下划线，必须以字母开头并且不得包含空格。 在作业进行中以及作业完成后，你使用所选名称来跟踪作业。
   
   接下来，从列表中选择数据中心区域。 数据中心区域会指示必须将包裹运送到的数据中心和地址。 有关更多信息，请参见下面的“常见问题”。
5. 在步骤 4 中，从列表中选择回程快递商，并输入快递商帐户号码。 当你的导入作业完成后，Microsoft 会使用此帐户寄回驱动器。
   
   如果有跟踪号码，则从列表中选择快递商，并输入跟踪号码。
   
   如果用户还没有跟踪号码，请选择“我将在发运我的包裹后提供此导入作业的发运信息”，并完成导入过程 。
...
1. 如果要创建导出作业，请导航到 Azure 门户中的存储帐户，并查看“仪表板”。 在“速览”下，单击“创建导出作业”，然后继续完成向导。
2. 在步骤 2 中，提供负责该导出作业的人员的联系信息。 如果希望保存导出作业的详细日志数据，则选中“将详细日志保存在我的‘waimportexport’Blob 容器中”选项。
...
5. 在步骤 5 中，从列表中选择回程承运人，并输入承运人帐号。 当你的导出作业完成后，Microsoft 会使用此帐户寄回你的驱动器。
   
   如果有跟踪号码，则从列表中选择快递商，并输入跟踪号码。
   
   如果用户还没有跟踪号码，请选择“我将在发运我的包裹后提供此导出作业的发运信息”，并完成导出过程 。
6. 若要在寄送包裹后输入跟踪号，请在 Azure 门户中返回到存储帐户的“导入/导出”页面，从列表中选择作业并选择“寄送信息”。 在向导中导航并在步骤 2 中输入跟踪号码。

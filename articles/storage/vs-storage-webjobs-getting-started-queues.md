...
在下面的示例中，队列包含一个字符串消息，因此将 QueueTrigger 应用到包含队列消息内容的 logMessage 字符串参数。 该函数 [向仪表板写入一条日志消息](#how-to-write-logs)。
...
属性修饰 Stream 对象时，另一个构造函数参数会将 FileAccess 模式指定为读取、写入或读取/写入。

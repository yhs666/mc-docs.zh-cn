...
Azure 队列存储是一项可存储大量消息的服务，用户可以通过经验证的呼叫，使用 HTTP 或 HTTPS 从世界任何地方访问这些消息。 有关详细信息，请参阅 [Azure 队列简介](storage-queues-introduction.md)。 本教程介绍常见的队列存储操作。 你将学习如何执行以下操作：
...
如果你不知道要使用哪个位置，可以列出可用的位置。 显示列表后，找到要使用的位置。 本教程将使用“中国东部”。 将此内容存储在变量 location 中，以供以后使用。

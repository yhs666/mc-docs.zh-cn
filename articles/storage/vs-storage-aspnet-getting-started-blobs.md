...
ms.date: 02/24/2017
ms.author: v-johch
ms.openlocfilehash: 4b03f706100a2ceb33098050058637ef429b4430
ms.sourcegitcommit: 033f4f0e41d31d256b67fc623f12f79ab791191e
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 06/21/2017
---
# <a name="get-started-with-azure-blob-storage-and-visual-studio-connected-services-aspnet"></a>Azure blob 存储和 Visual Studio 连接服务入门 (ASP.NET)
...
1. 添加名为 CreateBlobContainer 的方法，用于返回 ActionResult。
...
1. 如果容器不存在，则调用 **CloudBlobContainer.CreateIfNotExists** 方法来创建容器。 如果容器原本不存在，但已成功创建，CloudBlobContainer.CreateIfNotExists 方法将返回 true。 否则返回 **false** 。    
...
1. 运行该应用程序，并选择“创建 Blob 容器”  查看类似于以下屏幕截图所示的结果：
    ...
    如前所述，仅当容器不存在但已创建时，CloudBlobContainer.CreateIfNotExists 方法才返回 true。 因此，如果在容器存在的情况下运行该应用，此方法会返回 **false**。 若要多次运行应用，必须在再次运行应用之前删除容器。 可通过 **CloudBlobContainer.Delete** 方法删除容器。 还可以使用 [Azure 门户](http://go.microsoft.com/fwlink/p/?LinkID=525040)或 [Azure 存储资源管理器](../vs-azure-tools-storage-manage-with-storage-explorer.md)删除容器。  
...
1. 添加名为 UploadBlob 的方法，用于返回 EmptyResult。
...
1. 如前所述，Azure 存储支持不同的 blob 类型。 若要检索页 blob 引用，请调用 CloudBlobContainer.GetPageBlobReference 方法。 若要检索块 blob 引用，请调用 CloudBlobContainer.GetBlockBlobReference 方法。 通常情况下，推荐使用块 blob 类型。 （将 <blob-name>* 更改为上传 blob 后需要提供给 blob 的名称。）

    ```csharp
    CloudBlockBlob blob = container.GetBlockBlobReference(<blob-name>);
    ```

1. 有了 blob 引用后，可通过调用 blob 引用对象的 UploadFromStream 方法，将任何数据流上传到该引用。 UploadFromStream 方法会根据 blob 是否存在来覆盖或创建 blob。 （将 &lt;file-to-upload> 更改为需上传文件的完全限定的路径。）
...
本部分说明如何列出 Blob 容器中的 Blob。 示例代码引用在 *创建 Blob 容器* 部分创建的 [test-blob-container](#create-a-blob-container)。
...
1. 添加名为 ListBlobs 的方法，用于返回 ActionResult。
...
1. 若要列出 Blob 容器中的 Blob，请使用 **CloudBlobContainer.ListBlobs** 方法。 CloudBlobContainer.ListBlobs 方法返回 IListBlobItem 对象，该对象可强制转换为 CloudBlockBlob、CloudPageBlob 或 CloudBlobDirectory 对象。 以下代码片段将枚举 Blob 容器中的所有 Blob。 每个 blob 将基于其类型强制转换为适当的对象，其名称（对于 CloudBlobDirectory 为 URI）将添加到列表。
...
本部分演示了如何下载 Blob，以及如何将其保存到本地存储或将内容读入字符串中。 示例代码引用在 *创建 Blob 容器* 部分创建的 [test-blob-container](#create-a-blob-container)。

1. 打开 `BlobsController.cs` 文件。

1. 添加名为 DownloadBlob 的方法，用于返回 ActionResult。
...
1. 运行该应用程序，然后选择“下载 blob”下载 blob。 在 CloudBlobContainer.GetBlockBlobReference 方法调用中指定的 blob 将下载到 File.OpenWrite 方法调用中指定的位置。 
...
1. 添加名为 DeleteBlob 的方法，用于返回 ActionResult。

...
---
# <a name="frequently-asked-questions-about-azure-iaas-vm-disks-and-managed-and-unmanaged-premium-disks"></a>有关 Azure IaaS VM 磁盘以及托管和非托管高级磁盘的常见问题
...
是的。 可以使用 Azure 门户预览、PowerShell 或 Azure CLI 更改托管磁盘的存储帐户类型。

**是否可将托管磁盘复制或导出到专用存储帐户？**

是的，可以通过 Azure 门户预览、PowerShell 或 Azure CLI 导出托管磁盘。
...
为何客户必须使用 Azure 托管磁盘在端口 8443 上为 VM 解除阻止出站流量？
...
第一个 VM 会显示部署失败，因为未成功部署扩展。 不会部署第二个 VM。 

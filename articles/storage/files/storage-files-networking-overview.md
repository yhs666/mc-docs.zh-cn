...
ms.author: v-jay
ms.subservice: files
ms.openlocfilehash: bd313127c84d8f0c4bbe368c2207082ac86b0c0c
ms.sourcegitcommit: 6a19227dcc0c6e0da5b82c4f69d0227bf38a514a
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 11/22/2019
ms.locfileid: "74328781"
---
# <a name="azure-files-networking-considerations"></a>Azure 文件存储的网络注意事项 
可以直接通过 SMB 或 FileREST 协议访问共享。 要删除尽可能多的本地服务器时，主要使用此访问模式。

本文重点介绍如何在用例要求访问 Azure 文件共享时配置网络。
...
可以将一个存储帐户添加到一个或多个虚拟网络中。 若要详细了解如何将存储帐户添加到虚拟网络或配置其他防火墙设置，请参阅[配置 Azure 存储防火墙和虚拟网络](../common/storage-network-security.md?toc=%2fstorage%2ffiles%2ftoc.json)。
...
若要从本地访问 Azure 文件共享，最简单方法是为端口 445（SMB 使用的端口）打开本地网络，并装载 Azure 门户提供的 UNC 路径。 此操作不需要特殊网络。 许多客户都不愿意打开端口 445，因为关于 SMB 1.0 的安全指南已过时，Azure 不会将其视为 Internet 安全协议。 Azure 文件存储不实现 SMB 1.0。 

SMB 3.0 的设计明确要求采用 Internet 安全文件共享协议。 因此，使用 SMB 3.0 及更高版本时，从计算机网络的角度来看，打开端口 445 与打开用于 HTTPS 连接的端口 443 是一样的。 Azure 建议你按照以下步骤操作，而不是通过阻止端口 445 来防止不安全的 SMB 1.0 流量：

> [!Important]  
> 即使你决定对出站流量关闭端口 445，Azure 仍建议按照以下步骤从环境中删除 SMB 1.0。
...
1. 请确认组织内没有需要 SMB 1.0 的产品并将其删除。 我们提供 [SMB1 产品交换所](https://aka.ms/stillneedssmb1)，其中包含需要 SMB 1.0 的所有 Azure 已知的第一和第三方产品。 

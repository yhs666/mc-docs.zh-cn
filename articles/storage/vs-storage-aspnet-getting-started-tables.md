...
ms.date: 04/06/2017
ms.author: v-johch
ms.openlocfilehash: 69a1a9e168eab7b6079a758ebd4904baebb4d3be
ms.sourcegitcommit: 033f4f0e41d31d256b67fc623f12f79ab791191e
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 06/21/2017
---
# <a name="get-started-with-azure-table-storage-and-visual-studio-connected-services-aspnet"></a>Azure 表存储和 Visual Studio 连接服务入门 (ASP.NET)
...
本文中的许多示例使用名为 CustomerEntity 的 TableEntity 派生类。 以下步骤会引导你将此类声明为模型类：
...
1. 运行该应用程序，并选择“创建表”  以查看类似于以下屏幕截图所示的结果：
    ...
    如前所述，仅当表不存在但已创建时，CloudTable.CreateIfNotExists 方法才返回 true。 因此，如果在表存在的情况下运行该应用，此方法会返回 **false**。 若要多次运行应用，必须在重新运行应用前删除表格。 可通过 **CloudTable.Delete** 方法完成表的删除。 也可以使用 [Azure 门户](https://portal.azure.cn/)或 [Azure 存储资源管理器](../vs-azure-tools-storage-manage-with-storage-explorer.md)删除表。  

## <a name="add-an-entity-to-a-table"></a>将实体添加到表

实体使用派生自 TableEntity 的自定义类映射到 C\# 对象。 若要将实体添加到表，请创建用于定义实体的属性的类。 在本部分中，将介绍如何定义将客户的名字和姓氏分别用作行键和分区键的实体类。 实体的分区键和行键共同唯一地标识表中的实体。 相对于查询分区键不同的实体，分区键相同的实体的查询速度更快，但使用不同的分区键能够增加并行操作的可伸缩性。 对于应在表服务中存储的所有属性，该属性必须是受支持类型的公共属性且公开设置和检索值。
实体类 *必须* 声明不带参数的公共构造函数。
...
1. 运行该应用程序，并选择“添加实体”  以查看类似于以下屏幕截图所示的结果：
    ...
    可按照 [获取单个实体](#get-a-single-entity)部分中的步骤验证已添加该实体。 还可使用 [Azure 存储资源管理器](../vs-azure-tools-storage-manage-with-storage-explorer.md)查看表的所有实体。
...
1. 根据 **向表中添加实体** 部分所述的 [CustomerEntity](#add-an-entity-to-a-table)模型类实例化某些客户对象。
...
1. CloudTable.ExecuteBatch 方法返回 TableResult 对象的列表，可检查其中的每个 TableResult 对象，确定每次操作是成功还是失败。 在本示例中，将列表传递给视图并让视图显示每个操作的结果。 
...
1. 添加名为 GetSingle 的方法，用于返回 ActionResult。
...
1. 实例化 TableQuery 对象，指定 Where 子句中的查询。 以下代码片段使用[向表添加一批实体](#add-a-batch-of-entities-to-a-table)部分提供的 CustomerEntity 类和数据，查询表中 PartitionKey（客户的姓氏）值为“Smith”的所有实体：
...
1. 在循环中调用 **CloudTable.ExecuteQuerySegmented** 方法，传递在上一步实例化的查询对象。  CloudTable.ExecuteQuerySegmented 方法返回 TableContinuationToken 对象，该对象为 null 时表示没有更多可检索的实体。 在循环中，使用另一个循环来循环访问返回的实体。 在以下代码示例中，返回的值均添加到列表中。 循环结束后，列表将传递给视图进行显示： 
...
1. 创建删除操作对象，该对象采用派生自 **TableEntity**的实体对象。 本示例中使用 **向表添加一批实体** 部分提供的 [CustomerEntity](#add-a-batch-of-entities-to-a-table)类和数据。 实体的 **ETag** 必须设置为有效值。  

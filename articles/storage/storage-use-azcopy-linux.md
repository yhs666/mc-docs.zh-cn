...
ms.date: 06/26/2017
ms.author: v-johch
ms.openlocfilehash: 2fbf48c6073e3bc54572b77f99b5f6cdb2c8bfc3
...
<a id="transfer-data-with-azcopy-on-linux" class="xliff"></a>
Linux 上的 AzCopy 是一个命令行实用程序，专用于使用具有优化性能的简单命令将数据复制到 Microsoft Azure Blob 和文件存储以及从这些位置复制数据。 可在存储帐户中将数据从一个对象复制到另一个对象，或者在存储帐户之间复制。
...
### Linux 上的安装
<a id="installation-on-linux" class="xliff"></a>
...
### Ubuntu 上的其他安装
<a id="alternative-installation-on-ubuntu" class="xliff"></a>
...
或者，如果希望排除其上次修改时间与目标文件相同或早于目标文件的 blob，则添加 `--exclude-older` 选项：
...
默认情况下，AzCopy 将目标 blob 的内容类型设置为 `application/octet-stream`。 但可以通过选项 `--set-content-type [content-type]` 显式指定内容类型。 此语法将在上传操作中设置所有 blob 的内容类型。
...
### 使用配置文件指定命令行参数
<a id="use-a-configuration-file-to-specify-command-line-parameters" class="xliff"></a>
...
请注意，AzCopy 当前仅支持[帐户 SAS](storage-dotnet-shared-access-signature-part-1.md)。
...
### 输出详细日志
<a id="output-verbose-logs" class="xliff"></a>
...
### 错误: 系统中未找到 .NET Core。
<a id="error-net-core-is-not-found-in-the-system" class="xliff"></a>
...
### 安装 AzCopy 时出错
<a id="error-installing-azcopy" class="xliff"></a>
...
AzCopy 旨在最大程度上利用计算机资源来加快数据传输，如果需要更多并发操作，建议在一台计算机上只运行一个 AzCopy 实例并指定选项 `--parallel-level`。 有关详细信息，请在命令行中键入 `AzCopy --help parallel-level`。
...
* [使用存储资源管理器管理 Blob](https://docs.azure.cn/zh-cn/vs-azure-tools-storage-explorer-blobs)

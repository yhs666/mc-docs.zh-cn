...
随后可在该 VM 对象内引用此要附加的磁盘对象。 创建 VM 时，通过指定在 `managedDisk` 属性中创建的托管磁盘的资源 ID，允许附加磁盘。 VM 资源的 `apiVersion` 必须为 `2016-04-30-preview` 才可使用此功能。 另请注意，我们已在磁盘资源上创建了一个依赖项，确保它在创建 VM 之前已成功创建。 

---
title: 在 Azure Active Directory 中删除企业应用中的用户或组分配 | Azure
...
ms.reviewer: asteen
ms.custom: it-pro
ms.openlocfilehash: 67aaa6088577b350d6b0f5391aafdd37cdb1924e
...
1. 以提升的权限打开 Windows PowerShell 命令提示符。
    ...
    >[!NOTE] 
    > 需要安装 AzureAD 模块（使用命令 `Install-Module -Name AzureAD`）。 出现安装 NuGet 模块的提示时，请键入 Y 并按 ENTER。

2. 运行 `Connect-AzureAD -AzureEnvironment AzureChinaCloud` 并使用全局管理员用户帐户登录。

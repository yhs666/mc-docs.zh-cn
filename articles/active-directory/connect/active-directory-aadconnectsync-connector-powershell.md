...
| [导出类型][exportT] |确定呈现给导出脚本的对象类型。 <li>AttributeReplace – 包括属性更改时多值属性的完整值集。</li><li>AttributeUpdate – 仅包括属性更改时多值属性的增量。</li><li>MultivaluedReferenceAttributeUpdate - 包括非引用多值属性的完整值集，仅包括多值引用属性的增量。</li><li>ObjectReplace – 包括任意属性更改时某个对象的所有属性</li> |
| [数据规范化][DataNorm] |指示同步服务先将定位点属性规范化，再提供给脚本。 |
| [对象确认][oconf] |在同步服务中配置挂起导入行为。 <li>Normal – 默认行为，将通过导入确认所有已导出更改</li><li>NoDeleteConfirmation – 删除某个对象时，不生成挂起的导入。</li><li>NoAddAndDeleteConfirmation – 创建或删除某个对象时，不生成挂起的导入。</li> |
...
脚本必须将单个 [Partition][part] 对象或 Partition 对象的 List[T] 返回到管道中。
...
| ConfigParameters |[KeyedCollection][keyk][string, [ConfigParameter][cp]] |连接器的配置参数表。 |
| 凭据 |[PSCredential][pscred] |包含管理员在“连接”选项卡上输入的任何凭据。 |
| 分区 |[Partition][part] |CSEntry 所在的目录分区。 |

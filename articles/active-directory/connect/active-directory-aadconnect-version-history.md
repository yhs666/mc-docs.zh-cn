...
ms.date: 03/07/2017
ms.author: v-junlch
ms.openlocfilehash: f6788699e2c29bc0a1d91e4a3e5253d50aac4079
...
- 如果进行安装，并在安装向导结束时取消选择“启动同步过程”， 则重新运行安装向导不会启用计划程序。
...
- 支持使用安装向导中的 MFA 和 PIM 来提供全局管理员。
  - 如果使用 MFA，则代理也需要允许发往 https://secure.aadcdn.microsoftonline-p.com 的流量。
  - 需将 https://secure.aadcdn.microsoftonline-p.com 添加到受信任站点列表，这样 MFA 才能正常工作。
- 允许在初始安装之后更改用户的登录方法。
...
- 可[配置 ADFS](active-directory-aadconnect-get-started-custom.md#configuring-federation-with-ad-fs)
...
- 现在支持使用基于属性的筛选执行密码同步。 有关更多详细信息，请参阅[使用筛选进行密码同步](active-directory-aadconnectsync-configure-filtering.md)。

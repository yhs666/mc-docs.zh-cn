...
ms.openlocfilehash: 399db0df1b04e3533c71c8781fce886ff90d177d
...
1. 具有特定属性配置的对象继续收到导出错误，而不是重复属性被隔离。  
   例如：
    ...
    a.将新的虚拟硬盘附加到 VM。 新用户在 AD 中创建，其 UPN 为 **Joe@contoso.com**，ProxyAddress 为 **smtp:Joe@contoso.com**。
...
2. UPN 冲突的详细错误消息对已更改/隔离其 UPN 的用户显示不正确的 displayName。 例如：
    ...
    a.将新的虚拟硬盘附加到 VM。 **用户 A** 首先使用 **UPN = User@contoso.com** 同步。

...
ms.date: 01/17/2018
ms.author: v-junlch
ms.openlocfilehash: 7275b4f9e5eea1f9f96440427dab3669454eb128
...
1. 针对所有对象的完全同步  
   选择顶部的“连接器”。 标识在前面部分中进行过更改的连接器（在本例中为 Active Directory 域服务），并选中它。 从“操作”中选择“运行”，然后选择“完全同步”和“确定”。
...
在启用 PreferredDataLocation 属性同步之前，必须：
 ...
 - 首先，确定要用作源属性的本地 Active Directory 属性。 它应该采用**单值字符串**类型。 下面的步骤中使用一个 extensionAttributes。
...
源属性已从本地 Active Directory 和 Azure AD 中的 PreferredDataLocation 导入相应的连接器空间。 在继续执行完全同步步骤之前，我们建议在本地 AD 连接器空间中的现有用户对象上执行“预览”。 选择的对象应具有填充的源属性。 成功**预览**填充在 Metaverse 中的 PreferredDataLocation 是一个很好的指标，表示已配置同步规则正确。 有关如何执行**预览**的信息，请参阅[验证更改](#verify-the-change)部分。
...
默认情况下，UserType 属性未启用同步，因为在本地 Active Directory 中没有相应的 UserType 属性。 必须手动启用同步。 在启用 UserType 属性同步之前，必须注意 Azure AD 实施的以下行为：

- Azure AD 仅接受 UserType 属性的两个值：**Member** 和 **Guest**。
- 如果没有为 Azure AD Connect 中的同步启用 UserType 属性，则通过目录同步创建的 Azure AD 用户的 UserType 属性将设置为 **Member**。
- Azure AD 不允许在现有 Azure AD 用户中使用会被 Azure AD Connect 更改的 UserType 属性。 只能在创建 Azure AD 用户的过程中设置该属性。

在启用 UserType 属性同步之前，必须先确定如何从本地 AD 派生 UserType 属性。 两种常见方法包括：

- 指定将某个未使用的本地 AD 属性（例如 extensionAttribute1）用作源属性。 指定的本地 AD 属性应为**字符串**类型，为单值且包含值 **Member** 或 **Guest**。 如果选择此方法，在启用 UserType 属性同步之前，必须确保指定的属性中填充了本地 Active Directory 中已同步到 Azure AD 的所有现有用户对象的正确值。
- 或者，可以从其他属性派生 UserType 属性的值。 例如，如果所有用户的本地 AD UserPrincipalName 属性以域部分“@partners.fabrikam123.org”结尾，则可将这些用户作为来宾同步。 如前所述，Azure AD Connect 不允许在现有 Azure AD 用户中使用会被 Azure AD Connect 更改的 UserType 属性。 因此，必须确保所确定的逻辑与针对租户中所有现有 Azure AD 用户配置 UserType 属性的方式一致。
...
1. 禁用**同步计划程序**，并确认是否没有正在进行的同步操作
2. 将**源属性**添加到本地 AD 连接器架构
...
### <a name="step-3-add-usertype-to-the-azure-ad-connector-schema"></a>步骤 3：将 UserType 添加到 Azure AD 连接器架构
...
6. 转到“转换”选项卡并实现所需的转换规则。 例如，如果已指定某个未使用的本地 AD 属性（例如 extensionAttribute1）作为 UserType 的源属性，则可以实现一个直接属性流：

    | 流类型 | 目标属性 | 源 | 应用一次 | 合并类型 |
    | --- | --- | --- | --- | --- |
    | 直接 | UserType | extensionAttribute1 | 未选中 | 更新 |

    另一个示例 - 从其他属性派生 UserType 属性的值。 例如，如果所有用户的本地 AD UserPrincipalName 属性以域部分“@partners.fabrikam123.org”结尾，则可将这些用户作为来宾同步。 可以实现以下表达式：
...
3. 验证现有 User 对象上的同步规则更改：
    ...
    源属性已从本地 Active Directory 和 Azure AD 中的 UserType 导入到相应的连接器空间。 然后再继续进行完全同步步骤。我们建议对本地 AD 连接器空间中的现有用户对象执行**预览**。 选择的对象应具有填充的源属性。 如果已成功完成**预览**并且 UserType 已填充到 Metaverse 中，则可以合理地判断同步规则已正确配置。 有关如何执行**预览**的信息，请参阅[验证更改](#verify-the-change)部分。

...
- 在包含某些有冲突服务计划的产品许可证之间进行较复杂的迁移，例如，在 Office 365 企业版 E1 与 Office 365 企业版 E3 之间进行迁移。 有关冲突的详细信息，请参阅[有冲突的服务计划](/active-directory/users-groups-roles/licensing-groups-resolve-problems#conflicting-service-plans)和[无法同时分配的服务计划](/active-directory/users-groups-roles/licensing-service-plan-reference#service-plans-that-cannot-be-assigned-at-the-same-time)。
...
4.  验证基于组的许可是否已完全处理用户批。 确认是否为每个用户分配了目标许可证。  检查用户最终是否未进入错误状态，例如，与其他产品相冲突，或缺少足够的许可证。 有关错误的详细信息，请参阅[解决 Active Directory 许可组问题](/active-directory/users-groups-roles/licensing-groups-resolve-problems)。
...
4.  验证基于组的许可是否已完全处理用户批。 确认已记录每个用户发生的冲突错误。 检查某些用户是否最终未进入意外的错误状态。 有关错误的详细信息，请参阅[解决 Active Directory 许可组问题](/active-directory/users-groups-roles/licensing-groups-resolve-problems)。
...
若要执行该代码，请参考 [Azure AD PowerShell v1.0 库](https://docs.microsoft.com/powershell/azure/active-directory/install-msonlinev1?view=azureadps-1.0)中的说明。 在执行脚本之前，请先运行 `Connect-MsolService -AzureEnvironment AzureChinaCloud` cmdlet 登录到租户。

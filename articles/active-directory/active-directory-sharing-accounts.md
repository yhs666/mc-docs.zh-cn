...
ms.reviewer: jeffsta
ms.custom: it-pro
ms.openlocfilehash: c66d7cb47046bf54c12b38c78eb5a7c49672483c
...
- 每个共享的应用程序可能都需要唯一的一组共享凭据，从而要求用户记住多组凭据。 在用户必须记住许多凭据时，他们会依靠有风险的做法，风险就会随之增加。 （例如写下密码）。
- 不知道谁有权访问应用程序。
- 不知道谁 *访问了* 应用程序。
...
Azure AD 支持任何 Enterprise Mobility Suite (EMS)、高级或基本许可用户的共享帐户，包括所有类型的密码单一登录应用程序。 可以共享应用程序库中数千个预先集成的应用程序的帐户，并可以使用自定义 SSO 应用添加自己的密码身份验证应用程序。
...
- 可选：在某些应用程序（例如 LinkedIn）中，可以启用 [Azure AD 自动轮换密码](http://blogs.technet.com/b/ad/archive/2015/02/20/azure-ad-automated-password-roll-over-for-facebook-twitter-and-linkedin-now-in-preview.aspx)选项

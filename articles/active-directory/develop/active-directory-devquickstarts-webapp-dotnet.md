...
ms.date: 03/13/2017
ms.author: v-junlch
ms.openlocfilehash: 0957a9f9cb941bb0909aba99248128b46f9b0d5d
...
4. 打开 App_Start\Startup.Auth.cs 文件，然后实现 ConfigureAuth(...) 方法。 在 *OpenIDConnectAuthenticationOptions* 中提供的参数充当应用与 Azure AD 通信时使用的坐标。 还需要设置 Cookie 身份验证，因为 OpenID Connect 中间件将在后台使用 Cookie。
...
2. 构建并运行应用程序。 如果尚未使用 partner.onmschina.cn 域在租户中创建一个新的用户，则现在可以执行此操作。 方法如下：
  ...
  a.将新的虚拟硬盘附加到 VM。 以该用户的身份登录，然后注意该用户的标识在顶部栏中的显示方式。

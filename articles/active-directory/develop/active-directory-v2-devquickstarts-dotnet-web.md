...
ms.date: 03/13/2017
ms.author: v-junlch
ms.openlocfilehash: 3ffb6b6ff5a3ea96a8397c31aa2c372236ad2217
...
1. 首先，打开项目根目录中的 `web.config` 文件，并在 `<appSettings>` 部分输入应用的配置值。
...
使用 OpenID Connect 对用户进行身份验证时，v2.0 终结点将向应用返回 id_token，其中包含有关用户的声明或断言。  你可以使用这些声明来个性化应用：
...
[此处以 .zip 格式提供了](https://github.com/AzureADQuickStarts/AppModelv2-WebApp-OpenIdConnect-DotNet/archive/complete.zip)完整示例（无配置值）以供参考，也可以从 GitHub 克隆该示例：

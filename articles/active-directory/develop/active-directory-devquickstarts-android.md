...
ms.date: 02/07/2017
ms.author: v-junlch
ms.openlocfilehash: 919b3c93d839ade3c594c50dd9ca3ec0cf78c69f
...
- 使用 [OAuth 2.0 身份验证协议](./active-directory-protocols-oauth-code.md)获取用于调用待办事项列表 API 的访问令牌。
- 获取用户的待办事项列表
- 将用户注销。

若要开始，你需要一个可在其中创建用户和注册应用程序的 Azure AD 租户。  如果没有租户，请[了解如何获取租户](./active-directory-howto-tenant.md)。
...
*Microsoft Active Directory 支持添加两种类型的应用程序。Web API，用于向访问这些 Web API 的用户和应用程序（在 Web 上或者在设备中运行的应用程序上）提供服务。在此步骤中，将注册在本地运行的用于测试此示例的 Web API。通常，此 Web API 是一项 REST 服务，它提供应用需要访问的功能。Azure Active Directory 可以保护任何终结点！*
...
9. 对于应用 ID URI，请输入 `https://<your_tenant_name>/TodoListService`，并将 `<your_tenant_name>` 替换为 Azure AD 租户的名称。  单击“确定”完成注册。
...
*如前所述，Azure Active Directory 支持添加两种类型的应用程序。Web API，用于向访问这些 Web API 的用户和应用程序（在 Web 上或者在设备中运行的应用程序上）提供服务。在此步骤中，要将应用程序注册到此示例。只有执行了此操作，此应用程序才能请求访问刚刚注册的 Web API。除非注册了应用程序，否则 Azure Active Directory 甚至可能会拒绝应用程序请求登录！这是模型安全功能的一部分。*

*此处我们假设你要注册上面引用的这个示例应用程序，但这也适用于你正在开发的任何应用程序。*

**为什么要将应用程序和 Web API 放在一个租户中？**

*如你可能猜到的那样，可以生成一个从其他租户访问 Azure Active Directory 中注册的外部 API 的应用。如果这样做，系统会提示客户许可使用该应用程序中的 API。令人欣慰的是，适用于 iOS 的 Active Directory 身份验证库将负责处理此许可！在了解更高级的功能后，你会发现，这是从 Azure 和 Office 以及任何其他服务提供程序访问 Microsoft API 套件所需工作的重要部分。现在，由于已将 Web API 和应用程序注册到同一个租户下，因此，不会出现任何许可提示。如果只是在为自己的公司开发要使用的应用程序，则通常就是这种情况。*
...
#### <a name="option-5-jar-package-inside-libs-folder"></a>选项 5：libs 文件夹中的 jar 包
...
8. **中转站**：Microsoft Intune 的公司门户应用将提供中转站组件。 如果在验证器中创建了一个用户帐户并且开发人员选择不跳过中转站帐户，ADAL 将使用中转站帐户。 开发人员可以使用以下操作跳过中转站用户：
...
### <a name="authority-url-and-adfs"></a>机构 URL 和 ADFS
...
显然这是第一项诊断。 我们将尝试提供有用的错误消息。 如果你发现某个错误消息没有作用，请记录相应的问题并告诉我们。 此外，请提供设备信息，例如型号和 SDK 编号。
...
消息可写入自定义日志文件，如下所示。 遗憾的是，没有标准的方法可从设备中获取日志。 有些服务可帮助你实现此目的。 你也可以创造自己的方法，例如，将文件发送到服务器。
...
可以将日志从 logcat 提取到文件，如下所示：
 ...
 更多关于 adb 命令的示例：https://developer.android.com/tools/debugging/debugging-log.html#startingLogcat
...
AuthenticationParameters 类提供通过 Oauth2 持有者质询获取 authorization_uri 的功能。

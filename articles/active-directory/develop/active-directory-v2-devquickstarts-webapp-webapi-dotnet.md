---
title: "Azure AD v2.0 .NET Web 应用调用 API 入门 | Azure"
...
ms.date: 03/13/2017
ms.author: v-junlch
ms.openlocfilehash: 777bb64bad7494a7ce7fe13684d01a6effe60539
...
- 打开 `TodoList-WebApp` 项目根目录中的 `web.config` 文件，然后在 `<appSettings>` 部分输入应用的配置值。
  - `ida:ClientId` 是在注册门户中为应用分配的 **应用程序 ID** 。
  - `ida:ClientSecret` 是在注册门户中创建的 **应用密码** 。
  - `ida:RedirectUri` 是在门户中输入的 **重定向 URI** 。
- 打开 `TodoList-Service` 项目根目录中的 `web.config` 文件，然后将 `ida:Audience` 替换为上述相同的应用程序 ID。
...
## <a name="use-msal-to-get-access-tokens"></a>使用 MSAL 获取访问令牌
在 `AuthorizationCodeReceived` 通知中，建议使用[与 OpenID Connect 串联的 OAuth 2.0](./active-directory-v2-protocols.md)，以兑换待办事项列表服务的访问令牌的 authorization_code。  MSAL 可以简化此过程：
...
现在可以实际使用在步骤 3 中获取的 access_token。  打开 Web 应用的 `Controllers\TodoListController.cs` 文件，此文件可向待办事项列表 API 发出所有 CRUD 请求。

- 此处可再次使用 MSAL 从 MSAL 缓存提取 access_token。  首先，将 MSAL 的 `using` 语句添加到此文件。
...
- 如果待办事项列表服务返回 `401 Unauthorized` 响应，则 MSAL 中的 access_token 由于某种原因而失效。  在此情况下，应删除所有来自 MSAL 缓存的 access_token，并向用户显示消息告知其可能需要重新登录，而重新登录会重启令牌获取流程。
...
- 同样，如果 MSAL 由于任何原因而无法返回 access_token，则应该指示用户重新登录。  这就像获取任何 `MSALException`一样简单：
...
- 在 `Create` 和 `Delete` 操作中实现的 `AcquireTokenSilentAsync` 调用完全一样。  在 Web 应用中，只要应用需要，就可以使用此 MSAL 方法获取 access_token。  MSAL 将为用户获取、缓存和刷新令牌。

最后，生成并运行应用程序！  使用 Microsoft 帐户或 Azure AD 帐户登录，随后你会看到该用户的标识在顶部导航栏中的显示方式。  在用户的“待办事项列表”中添加和删除一些项，以查看 OAuth 2.0 保护的 API 调用的运行情况。  Web 应用和 Web API 现在都使用行业标准协议进行保护，该协议可以使用个人和工作/学校帐户来对用户进行身份验证。

[此处](https://github.com/AzureADQuickStarts/AppModelv2-WebApp-WebAPI-OpenIdConnect-DotNet/archive/complete.zip)提供了已完成示例（无配置值）以供参考。  

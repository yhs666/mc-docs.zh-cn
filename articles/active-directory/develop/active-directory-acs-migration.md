...
ms.openlocfilehash: e0268d85756f1db0757a4990f8e2d032a70c806f
ms.sourcegitcommit: 579d4e19c2069ba5c7d5cb7e9b233744cc90d1f5
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 12/11/2018
ms.locfileid: "53219636"
---
...
所有接受访问控制颁发的令牌的 Microsoft 云服务现在都支持至少一种备用身份验证。 每个服务的身份验证机制各不相同。 建议参阅每个服务的特定文档，获取官方指南。 为方便起见，下面列出了每套文档：
...
- 支持以下身份验证协议：OAuth 2.0 Draft 13、WS-Trust，以及 Web Services 联合身份验证 (WS-Federation)。
...
一个主要例子是与社交标识提供者（例如 Facebook、Google 和 Yahoo）联合。 如果用户使用这些类型的凭据进行登录，那么 Azure AD 并不适用。 
...
| Windows Server Active Directory 和 ADFS 帐户 |- 支持（通过与 Azure AD 租户联合） <br />- 支持（通过与 AD FS 直接联合） | 只能通过与 Azure AD 租户的联合来支持 | 
| 其他企业标识管理系统中的帐户 |- 可能支持（通过与 Azure AD 租户联合） <br />- 支持（通过直接联合） | 可以通过与 Azure AD 租户的联合来支持 |
| 供个人使用的 Microsoft 帐户 | 支持 | 支持（只通过 Azure AD v2.0 OAuth 协议） | 
| Facebook、Google、Yahoo 帐户 | 支持 | 都不支持 |
| **协议和 SDK 兼容性** | | |
| WIF | 支持 | 支持，但说明有限 |
| WS 联合身份验证 | 支持 | 支持 |
| OAuth 2.0 | 草案 13 支持 | 支持最新规范 RFC 6749 |
| WS-Trust | 支持 | 不支持 |
| **令牌格式** | | |
| JWT | Beta 版支持 | 支持 |
| SAML 1.1 | 支持 | 预览 |
...
| SWT | 支持 | 不支持 |
| **自定义** | | |
| 可自定义的主页领域发现/帐户选取 UI | 可整合到应用中的可下载代码 | 不支持 |
| 上传自定义令牌签名证书 | 支持 | 支持 |
...
如果采用这种方法，需要了解 [Azure AD 中的签名密钥滚动更新](https://docs.microsoft.com/azure/active-directory/develop/active-directory-signing-key-rollover)。 此方法使用 Azure AD 全局签名密钥来颁发令牌。 默认情况下，WIF 不会自动刷新签名密钥。 如果 Azure AD 轮用其全局签名密钥，则 WIF 实现需要做好接受更改的准备。 有关详细信息，请参阅[有关 Azure AD 中签名密钥滚动更新的重要信息](https://msdn.microsoft.com/library/azure/dn641920.aspx)。
...
| 其他企业标识管理系统中的帐户 | 支持（通过使用 WS 联合身份验证进行直接联合） | 支持（通过使用自定义策略实现 SAML 联合） |
...
| SWT | 支持 | 不支持 |
| **自定义** | | |
| 可自定义的主页领域发现/帐户选取 UI | 可整合到应用中的可下载代码 | 完全可自定义 UI（通过自定义 CSS） |

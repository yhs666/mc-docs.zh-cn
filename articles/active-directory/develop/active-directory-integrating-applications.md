...
有关同意框架的更多详细信息，请参阅 [Azure AD 中的 OAuth 2.0](./active-directory-protocols-oauth-code.md)、[Azure AD 的身份验证方案](./active-directory-authentication-scenarios.md)和 Office 365 主题[了解如何使用 Office 365 API 进行身份验证](https://msdn.microsoft.com/office/office365/howto/common-app-authentication-tasks)。
...
默认情况下，为应用程序禁用了 OAuth 2.0 隐式授权。可以通过在[应用程序清单](./active-directory-application-manifest.md)（表示应用程序标识配置的 JSON 文件）中设置 `oauth2AllowImplicitFlow` 值，来为应用程序启用 OAuth 2.0 隐式授权。

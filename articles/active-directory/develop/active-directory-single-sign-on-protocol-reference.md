...
ms.custom: aaddev
ms.reviewer: hirsin
ms.openlocfilehash: f67b218d1c31e047f0bed73857644a5261f9e241
ms.sourcegitcommit: 7cdf4633aea04e524cb48cb1990b750ae8be841c
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 08/07/2018
ms.locfileid: "39584255"
---
...
| 参数 |  | 说明 |
| --- | --- | --- |
| ID | 必须 | Azure AD 使用此属性来填充返回的响应的 `InResponseTo` 属性。 ID 的开头不能是数字，因此常见的策略是在 GUID 的字符串表示形式前面加上类似于“id”的字符串。 例如， `id6c1c178c166d486687be4aaf5e482730` 是有效的 ID。 |
| 版本 | 必须 | 此参数应设置为 **2.0**。 |
| IssueInstant | 必须 | 这是具有 UTC 值并采用[往返格式（“o”）](https://msdn.microsoft.com/library/az4se3k1.aspx)的 DateTime 字符串。 Azure AD 需要这种类型的日期时间值，但不评估或使用该值。 |
...
包含一系列标识提供者的 `Scoping` 元素在发送到 Azure AD 的 `AuthnRequest` 元素中是可选项。
...
`NotBefore` 和 `NotOnOrAfter` 属性指定断言有效的间隔时间。

- `NotBefore` 属性值等于或略晚于（一秒以内）`Assertion` 元素的 `IssueInstant` 属性值。 Azure AD 不考虑自身与云服务（服务提供者）之间的任何时间差，并且不对此时间添加任何缓冲。
...
与 `Issuer` 值一样，`Audience` 值必须与 Azure AD 云服务中表示云服务的某一个服务主体名称完全匹配。 但是，如果 `Issuer` 元素值不是 URI 值，则响应中的 `Audience` 值应该是带有 `spn:` 前缀的 `Issuer` 值。

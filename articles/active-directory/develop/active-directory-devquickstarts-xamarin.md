...
ms.date: 02/07/2017
ms.author: v-junlch
ms.openlocfilehash: daa41c91c64dfbd94bda672e54030edd3a2906c9
ms.sourcegitcommit: 033f4f0e41d31d256b67fc623f12f79ab791191e
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 06/21/2017
---
# <a name="integrate-azure-ad-into-a-xamarin-app"></a>将 Azure AD 集成到 Xamarin 应用中
[!INCLUDE [active-directory-devquickstarts-switcher](../../../includes/active-directory-devquickstarts-switcher.md)]

[!INCLUDE [active-directory-devguide](../../../includes/active-directory-devguide.md)]

借助 Xamarin 可以使用 C# 编写可在 iOS、Android 和 Windows（移动设备和电脑）上运行的移动应用。 如果你要使用 Xamarin 生成应用，Azure AD 可让你简单直接地使用用户的 Active Directory 帐户对其进行身份验证。 它还可以让应用程序安全地使用 Azure AD 保护的任何 Web API，例如 Office 365 API 或 Azure API。
...
若要开始，请[下载框架项目](https://github.com/AzureADQuickStarts/NativeClient-MultiTarget-DotNet/archive/skeleton.zip)或[下载已完的成示例](https://github.com/AzureADQuickStarts/NativeClient-MultiTarget-DotNet/archive/complete.zip)。 每个下载项目都是 Visual Studio 2013 解决方案。 你还需要一个可在其中创建用户和注册应用程序的 Azure AD 租户。 如果没有租户，请[了解如何获取租户](./active-directory-howto-tenant.md)。
...
- 另外，请在“配置”选项卡中，找到“针对其他应用程序的权限”部分  。 对于“Azure Active Directory”应用程序，在“委派权限”下添加“访问组织的目录”权限。 这样，你的应用程序便可以在图形 API 中查询用户。
...
- 在 DirectorySearcherLib 项目中，打开 `DirectorySearcher.cs`。 更改类成员值以反映你在 Azure 门户中输入的值。 只要使用 ADAL，你的代码就会引用这些值。
  ...
  - `returnUri` 是在门户中输入的 redirectUri，例如 `http://DirectorySearcher`。

## <a name="3-use-adal-to-get-tokens-from-aad"></a>3.使用 ADAL 从 AAD 获取令牌
应用几乎所有的身份验证逻辑都位于 `DirectorySearcher.SearchByAlias(...)`。 在特定于平台的项目中，所要做的一切就是将上下文参数传递到 `DirectorySearcher` PCL。
...
- `AcquireTokenAsync(...)` 首先会在不提示用户输入其凭据（通过缓存或刷新旧令牌）的情况下，尝试返回请求资源（在本例中为图形 API）的令牌。 仅在必要时，它才会在获取请求的令牌之前，向用户显示 Azure AD 登录页。
...
- 在 `MainWindow.xaml.cs` 中，只需调用 `SearchByAlias(...)` 即可在桌面的 `PlatformParameters` 对象中传递 `WindowInteropHelper`：
...
- 在 Windows Universal 中，打开 `MainPage.xaml.cs` 并实现 `Search` 方法，该方法会根据需要，使用共享项目中的帮助器方法来更新 UI。

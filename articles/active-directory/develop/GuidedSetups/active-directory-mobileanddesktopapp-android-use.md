...
ms.author: v-junlch
ms.custom: aaddev
ms.openlocfilehash: 0f75de1c72b020348e4619e59697a8e829e8e9d7
...
调用 `AcquireTokenAsync` 方法将出现提示用户登录的窗口。 当应用程序首次需要访问受保护的资源或获取令牌的静默操作失败（如用户密码过期）时，通常会要求用户以交互方式登录。
...
2.  应用程序还可以直观地提示用户以交互方式登录，用户可以选择在合适的时间登录，或者应用程序可以稍后重试 `AcquireTokenSilentAsync`。 如果用户能在不中断的情况下访问应用程序的功能（例如，应用程序中有可用的脱机内容），则通常会使用此方法。 在这种情况下，用户可以决定何时登录并访问受保护的资源，或何时刷新过期信息，或在网络暂时不可用得到还原后，应用程序可以决定重试 `AcquireTokenSilentAsync`。
<!--end-collapse-->
...
虽然此示例中的应用程序支持单个用户，但 MSAL 也支持可同时注册多个帐户的方案（例如，用户可以在一个电子邮件应用程序中包含多个帐户）。
<!--end-collapse-->

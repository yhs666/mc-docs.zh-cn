...
ms.author: v-junlch
ms.date: 11/08/2016
ms.openlocfilehash: e5c96cb1962fd80861597cd9f7db8fd904abe9e1
...
祝贺你！ 现在，你已创建一个有效的 .NET WPF 应用，它可以使用 OAuth 2.0 对用户进行身份验证并安全调用 Web API。  运行两个项目，并以个人的 Microsoft 或工作/学校的帐户登录。  将任务添加到该用户的待办事项列表。  注销，然后以其他用户的身份重新登录，以查看其他用户的待办事项列表。  关闭应用程序，然后重新运行它。  请注意，用户的会话保持不变 - 这是因为应用在本地文件中缓存了令牌。
...
[此处以 .zip 格式提供了](https://github.com/AzureADQuickStarts/AppModelv2-NativeClient-DotNet/archive/complete.zip)完整示例（无配置值）以供参考，也可以从 GitHub 克隆该示例：

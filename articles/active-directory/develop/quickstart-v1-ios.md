...
ms.openlocfilehash: 5db151adf1d753d9583ed172169c21e2016a2fd0
ms.sourcegitcommit: 562cde32fc2271238f3d1ef5d2cc5ed037bdec2d
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/04/2018
ms.locfileid: "43531618"
---
...
若要开始，请[下载应用框架](https://github.com/AzureADQuickStarts/NativeClient-iOS/archive/skeleton.zip)或[下载已完成的示例](https://github.com/AzureADQuickStarts/NativeClient-iOS/archive/complete.zip)。 还需要一个可在其中创建用户和注册应用程序的 Azure AD 租户。 如果没有租户，请[了解如何获取租户](quickstart-create-new-tenant.md)。
...
2. 现在，我们需要使用此令牌在图形中搜索用户。 找到 `// TODO: implement SearchUsersList` 注释。 此方法向 Azure AD 图形 API 发出 GET 请求，以查询其 UPN 以给定搜索词开头的用户。 要查询 Azure AD Graph API，需要在请求的 `Authorization` 标头中包含 access_token。 这是 ADAL 的进入位置。
...
## <a name="5-build-and-run-the-application"></a>5.生成并运行应用程序
祝贺！ 现在，已有了一个可正常工作的 iOS 应用程序，它可以对用户进行身份验证，使用 OAuth 2.0 安全地调用 Web API，并获取有关用户的基本信息。 如果尚未这样做，可以在租户中填充一些用户。 启动快速入门应用，并使用这些用户之一进行登录。 根据用户的 UPN 搜索其他用户。 关闭应用，然后重新启动它。 请注意，用户的会话将保持不变。

...
ms.author: v-junlch
ms.custom: aaddev
ms.openlocfilehash: b79352a98a5902533b1a77ebd1a752ee88be860c
...
**Passport** 是 Node.js 的身份验证中间件。 Passport 很灵活并且采用模块化结构，可以在不造成干扰的情况下放入任何基于 Express 的应用程序或 Resitify Web 应用程序。 在 Passport 中，有一套综合性策略支持使用用户名和密码或其他选项进行身份验证。 我们针对 Azure AD 开发了一项策略。 本文介绍了如何安装该模块及添加 Azure AD `passport-azure-ad` 插件。
...
1.  在项目的根目录中，打开 Config.js 文件。 在 `exports.creds` 部分中，输入应用的配置值。
...
添加向用户显示信息的路由和视图。 路由和视图还会处理你创建的 `/logout` 和 `/login` 路由。
...
7.  使用 Microsoft 个人帐户或工作/学校帐户登录。 请注意，用户的标识反映在 /account 列表中。 

现在，你拥有了使用行业标准协议进行保护的 Web 应用。 你可使用用户的个人和工作/学校帐户，在应用中对用户进行身份验证。

...
ms.openlocfilehash: af8828eb7f2f6c653d7cd162a8529ffc77bfdf8f
ms.sourcegitcommit: 9d5fd3184b6a47bf3b60ffdeeee22a08354ca6b1
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 06/21/2019
ms.locfileid: "67305842"
---
...
借助 Microsoft 身份验证库 (MSAL)，应用程序开发人员可以使用 [Azure Active Directory (Azure AD) B2C](/active-directory-b2c/) 通过社交和本地标识对用户进行身份验证。 Azure Active Directory (Azure AD) B2C 是一项标识管理服务，用于自定义和控制客户在使用应用程序时的注册、登录和管理配置文件的方式。
...
如果尚未创建自己的 [Azure AD B2C 租户](/active-directory-b2c/tutorial-create-tenant)，请立即创建一个。 可以使用现有的 Azure AD B2C 租户。 
...
本教程中使用的[用户流](/active-directory-b2c/active-directory-b2c-reference-policies)的名称为 B2C_1_signupsignin1。 如果你使用了不同的用户流名称，请在 authority 值中使用你的用户流名称。


### <a name="configure-application-to-use-b2clogincn"></a>将应用程序配置为使用 `b2clogin.cn`

在你的 Azure Active Directory (Azure AD) B2C 应用程序中设置用于注册和登录的标识提供者时，你可以使用 `b2clogin.cn` 代替 `login.partner.microsoftonline.cn` 作为重定向 URL。

**`b2clogin.cn`** 即 
`https://your-tenant-name.b2clogin.cn/your-tenant-guid` 用于以下项：

- Microsoft 服务在 cookie 标头中使用的空间就会减少。
- URL 不再包括对 Microsoft 的引用。 例如，Azure AD B2C 应用程序可能会引用 login.partner.microsoftonline.cn


 若要使用“b2clogin.cn”，需要更新应用程序的配置。  

1. 更新 ValidateAuthority：将 **validateAuthority** 属性设置为 `false`。 当 **validateAuthority** 设置为 false 时，允许重定向到 b2clogin.cn。

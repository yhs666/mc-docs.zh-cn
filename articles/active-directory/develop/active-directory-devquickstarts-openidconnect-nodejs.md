...
ms.date: 02/07/2017
ms.author: v-junlch
ms.openlocfilehash: 5bb9d57f82dbba96c687e2ba4708190bb773c0ec
...
- 根据提示创建一个新的 **Web 应用程序和/或 WebAPI**。
    ...
    - “应用程序 ID URI”是应用程序的唯一标识符  。  约定使用 `https://<tenant-domain>/<app-name>`，例如 `https://contoso.partner.onmschina.cn/my-first-aad-app`
- 完成注册后，AAD 将为应用分配唯一的客户端标识符。  在后面的部分中将会用到此值，因此，请从“配置”选项卡复制此值。

## <a name="2-add-pre-requisities-to-your-directory"></a>2.向目录添加先决条件
...
- 首先，打开项目根目录中的 `config.js` 文件，并在 `exports.creds` 部分输入应用的配置值。
...
- 然后，使用我们刚刚提到的策略来处理登录请求
    ...
    Passport 对其所有策略（Twitter、Facebook 等）都使用相似的模式，所有策略写入器都遵循该模式。 查看该策略可以发现，我们已将它作为函数来传递，其中包含一个令牌和一个用作参数的 done。 策略完成所有工作之后，便尽责地返回。 完成后，需要存储用户和令牌，以免再次请求它。
...
- 我们详细探讨一下：
  ...
  - `/account` 路由首先确保我们已进行身份验证（将在下面的部分中进行实现），然后在请求中传递用户，以便我们可以获取有关该用户的其他信息。
...
- 在 `app.js` 的最后一部分中，添加上述 `/account` 中使用的 EnsureAuthenticated 方法。
...
[此处以 .zip 格式提供了](https://github.com/AzureADQuickStarts/WebApp-OpenIDConnect-NodeJS/archive/complete.zip)完整示例（无配置值）以供参考，也可以从 GitHub 克隆该示例：

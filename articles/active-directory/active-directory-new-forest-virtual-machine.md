...
description: 本教程介绍了如何在 Azure 虚拟网络上的虚拟机 (VM) 中创建新的 Active Directory 林。
...
1. 在 Azure 门户中，选择“新建” > “计算”，然后选择一个虚拟机。 使用以下值来完成向导操作。 除非建议或必须使用其他值，否则请接受默认的设置值。
...
使用在本地所用的相同例程[安装 AD DS](https://technet.microsoft.com/library/jj574166.aspx)（也就是说，可以使用 UI、应答文件或 Windows PowerShell）。 需要提供管理员凭据来安装新林。 要指定 Active Directory 数据库、日志和 SYSVOL 的位置，请将默认存储位置从操作系统驱动器更改为已附加到 VM 的额外数据磁盘。

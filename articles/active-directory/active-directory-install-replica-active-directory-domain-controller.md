...
ms.openlocfilehash: 50d453f6cdaba242b6f5acca178369be96e699cc
...
应用程序服务器和 DC 将部署在独立的云服务中以分散计算处理，并部署在[可用性集](../virtual-machines/windows/manage-availability.md)中以改进容错功能。
...
2. 接下来，将配置虚拟网络网关，以便创建安全的站点到站点 VPN 连接。 有关说明，请参阅[配置虚拟网关](../vpn-gateway/vpn-gateway-configure-vpn-gateway-mp.md)。
...
若要使用 Windows PowerShell 而不是 UI 创建 VM，请参阅[使用 Azure PowerShell 创建和预配置基于 Windows 的虚拟机](../virtual-machines/windows/classic/create-powershell.md)。
...
2. 将磁盘附加到要运行 DC 服务器角色的每个 VM。 需要提供额外的磁盘来存储 AD 数据库、日志和 SYSVOL。 指定磁盘的大小（例如 10 GB）并将“主机缓存首选项”设置为“无”。 有关步骤，请参阅[如何将数据磁盘附加到 Windows 虚拟机](../virtual-machines/windows/classic/attach-disk.md)。

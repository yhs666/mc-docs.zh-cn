...
ms.openlocfilehash: 72c1277e41f52e1c15e6af664fcd7c0ad85732a8
...
使用 Azure 命令行接口 (CLI) 管理 RBAC 之前，必须具备以下先决条件：
...
以下示例在订阅范围内将读者角色分配给 Christine Koch 的团队。
...
以下示例在 Pharma-Sales-ProjectForcast 资源组范围内将虚拟机参与者角色授予 samert@aaddemo.com 用户。
...
以下示例在 Pharma-Sales-ProjectForcast 资源组上从 sammert@aaddemo.com 用户删除虚拟机参与者角色分配。
然后，该示例从订阅上的组中删除角色分配。
...
若要修改自定义角色，请先使用 `azure role list` 命令检索角色定义。 然后，对角色定义文件做出所需更改。 最后，使用 `azure role set` 保存修改后的角色定义。

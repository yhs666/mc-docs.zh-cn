...
ms.date: 02/21/2017
ms.author: v-dazen
ms.openlocfilehash: 3308a52a215d4a50fc71df934a9ab53d3cf73bf2
...
例如，以下控制器代码会导致 Swashbuckle 生成三个 Contact_Get 操作 ID。
...
默认情况下，Swashbuckle 假设 Web API 方法的唯一合法响应是 HTTP 200 (OK)。 在某些情况下，可能需要在不导致客户端引发异常的情况下返回其他响应代码。  例如，以下 Web API 代码演示了想要客户端接受 200 或 404 作为有效响应的方案。
...
由于 Visual Studio 使用 Swagger API 定义来为客户端生成代码，因此为任何 HTTP 200 以外的响应所创建的客户端代码将引发异常。 以下代码来自为本示例 Web API 方法生成的 C# 客户端。
...
2. 在在 *SwaggerConfig.cs* 文件中添加说明，指示 Swashbuckle 使用 XML 文档文件。
...
使用 Visual Studio 为 REST API 重新生成客户端代码时，C# 代码同时接受 HTTP OK 和 Not Found 状态代码，并且不会引发异常，这样用户可以使用代码来决定如何处理返回 null Contact 记录。 

...
ms.date: 02/21/2017
ms.author: v-dazen
ms.openlocfilehash: 07c68be746e3b4eefd2be413a7d474a3188e2fdd
...
* 如何通过 Visual Studio 2015 中的内置工具在 Azure App Service 中创建和部署 [API 应用](app-service-api-apps-why-best-platform.md)。
...
2. 按 F5 或单击“调试”>“开始调试”，在调试模式中运行项目。

    浏览器将打开并显示“HTTP 403 错误”页。
3. 在浏览器的地址栏中，于行尾处添加 `swagger/docs/v1`，然后按回车键。 （URL 为 `http://localhost:45914/swagger/docs/v1`。）
...
6. 再次运行该项目。
7. 在浏览器的地址栏中，于行尾处添加 `swagger`，然后按回车键。 （URL 为 `http://localhost:45914/swagger`。）
...
11. 单击“发布”，然后单击“模型架构”下面的框。

    单击模型架构会预先填充输入框，可以在该框中指定 Post 方法的参数值。 （如果这不适用于 Internet Explorer，请使用其他浏览器或在下一步骤中手动输入参数值。）  
...
本部分使用已集成到 Visual Studio“发布 Web”向导中的 Azure 工具，在 Azure 中创建新的 API 应用。 然后将 ToDoListDataAPI 项目部署到新的 API 应用，并通过运行 Swagger UI 来调用 API。
...
2. 在“发布 Web”向导的“配置文件”选项卡中，单击“Azure App Service”。
...
4. 在“应用服务”对话框中，选择要使用的 Azure 订阅，然后单击“新建”。
    ...
    由于部署的 Web API 项目已安装 Swashbuckle，因此 Visual Studio 假设你要创建 API 应用。 “API 应用名称”标题指出了这一点，另外，从“更改类型”下拉列表已设置为“API 应用”也能看出这一点。
...
6. 在“资源组”下拉列表中单击“新建”，然后输入“ToDoListGroup”或其他喜好的名称。
    ...
    使用此框可以选择现有[资源组](../azure-resource-manager/resource-group-overview.md)，或通过键入与订阅中任何现有资源组不同的名称来新建资源组。
7. 单击“应用服务计划”下拉列表旁边的“新建”按钮。

    屏幕截图显示了“API 应用名称”、“订阅”和“资源组”的示例值 - 可以使用其他的值。
...
12. 在“创建应用服务”对话框中，单击“创建”。
    ...
    Visual Studio 将创建 API 应用，并创建包含 API 应用全部所需设置的发布配置文件。 然后，将打开“发布 Web”向导来部署项目。

    会在“连接”选项卡（如下所示）上打开“发布 Web”向导。
...
13. 单击“下一步”。
    ...
    * 从 App_Data 文件夹中排除文件
    ...
    在本教程中，不需要使用这些选项。 有关这些选项作用的详细说明，请参阅[如何：在 Visual Studio 中使用一键式发布来部署 Web 项目](https://msdn.microsoft.com/library/dd465337.aspx)。
14. 单击“下一步”。

    ![在“发布 Web”的“设置”选项卡中单击“下一步”](./media/app-service-api-dotnet-get-started/settingsnext.png)

    接下来是“预览”选项卡（如下所示），用于查看哪些文件即将从项目复制到 API 应用。 如果要将项目部署到前面已部署到的 API 应用，则只会复制已更改的文件。 如需查看要复制的项的列表，请单击“开始预览”按钮。
15. 单击“发布”。
...
6. 在 ToDoListAPI 项目中，打开 Controllers\ToDoListController.cs，查看位于第 40 行的代码，该代码使用生成的客户端调用 API。
...
7. 将会根据 API 应用名称，以不同的名称生成客户端类；更改 Controllers\ToDoListController.cs 中的代码，使类型名称与项目中生成的内容匹配。 例如，如果将 API 应用命名为 ToDoListDataAPI071316，请将以下代码：
...
2. 在“发布 Web”向导的“配置文件”选项卡中，单击“Azure App Service”。
3. 在“应用服务”对话框中，单击“新建”。
4. 在“创建应用服务”对话框的“托管”选项卡中，接受默认的“API 应用名称”或者输入在 chinacloudsites.cn 域中具有唯一性的名称。
5. 选择一直使用的 Azure **订阅** 。
...
3. 在 API 应用的“应用程序设置”边栏选项卡中，向下滚动到“应用设置”部分并添加以下密钥和值。 值将是在本教程中发布的第一个 API 应用的 URL。

   | **键** | toDoListDataAPIURL |
   | --- | --- |
   | **值** |https://{数据层 API 应用名称}.chinacloudsites.cn |
...
* 如果在企业网络中并尝试通过防火墙部署到 Azure 应用服务，请确保已针对 Web 部署打开端口 443 和 8172。 如果无法打开这些端口，可以使用其他部署方法。  请参阅[将应用部署到 Azure App Service](../app-service-web/web-sites-deploy.md)。
* “路由名称必须唯一”错误 -- 如果不小心将错误的项目部署到 API 应用，稍后再将正确的项目部署到其中，可能会出现此错误。 若要更正此问题，请将正确的项目重新部署到 API 应用，然后在“发布 Web”向导的“设置”选项卡上选择“删除目标位置的其他文件”。
...
本文已介绍如何将现有 Web API 项目部署到 API 应用、生成 API 应用的客户端代码，以及从 .NET 客户端使用 API 应用。 本系列教程的下一篇文章介绍如何[借助 CORS 从 JavaScript 客户端使用 API 应用](app-service-api-cors-consume-javascript.md)。
...
Azure API 应用项目模板相当于选择空的 ASP.NET 4.5.2 模板，单击复选框以添加 Web API 支持，然后安装 Swashbuckle NuGet 包。 此外，该模板添加了一些 Swashbuckle 配置代码，旨在避免创建重复的 Swagger 操作 ID。 创建 API 应用项目之后，可以使用本教程中所述的相同方式将它部署到 API 应用。

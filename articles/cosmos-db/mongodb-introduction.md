...
**MongoDB 兼容性**：可使用现有 MongoDB 专业知识、应用程序代码和工具，因为 Azure Cosmos DB 实现 MongoDB 网络协议。 可使用 MongoDB 开发应用程序，并使用完全托管的多区域分布式 Azure Cosmos DB 服务将其部署到生产环境。 有关受支持版本的详细信息，请参阅 [MongoDB 协议支持](mongodb-feature-support.md#mongodb-protocol-support)。
<!-- Notice: 全球 to 多个区域 -->
...
**多区域复制：** Azure Cosmos DB 以透明方式将数据复制到与 MongoDB 帐户关联的所有区域，使用户能够开发那些对多区域数据访问有要求的应用程序，与此同时还在一致性、可用性和性能方面做出权衡，所有这些都有相应的保证。 Azure Cosmos DB 通过多宿主 API 提供透明区域故障转移，还可以弹性缩放多区域吞吐量和存储。 在[分发多区域数据](distribute-data-globally.md)中了解详细信息。

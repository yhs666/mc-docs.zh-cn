---
title: "Azure Cosmos DB: API for MongoDB 简介 | Azure"
...
ms.date: 07/17/2017
ms.author: v-yeche
ms.openlocfilehash: 8906bed2b699077a9903a8e9771801d6f05821a0
...
<a id="introduction-to-azure-cosmos-db-api-for-mongodb" class="xliff"></a>
...
可以将 Cosmos DB 数据库用作为 [MongoDB](https://docs.mongodb.com/manual/introduction/) 编写的应用的数据存储。 这意味着，通过使用现有[驱动程序](https://docs.mongodb.org/ecosystem/drivers/)，为 MongoDB 编写的应用程序现在可与 Cosmos DB 通信，并使用 Cosmos DB 数据库而不是 MongoDB 数据库。 在许多情况下，只需更改连接字符串便可从使用 MongoDB 切换到使用 Cosmos DB。 使用此功能，可以在 Azure 云中轻松生成和运行 MongoDB 数据库应用程序（利用 Azure Cosmos DB 在全球的分布以及[内容广泛且行业领先的 SLA](https://www.azure.cn/support/sla/cosmos-db)），并可继续使用熟悉的技能和 MongoDB 工具。

## 使用适用于 MongoDB 应用程序的 Azure Cosmos DB 有什么好处？
<a id="what-is-the-benefit-of-using-azure-cosmos-db-for-mongodb-applications" class="xliff"></a>
...
可调整的一致性级别：从五个妥善定义的一致性级别中选择，实现一致性与性能之间的最佳平衡。 对于查询和读取操作，Cosmos DB 提供五种不同的一致性级别：强、有限过时、会话、一致前缀和最终。 通过这些细化的定义完好的一致性级别，你可以在一致性、可用性和延迟之间实现合理的平衡。 有关详细信息，请参阅[使用一致性级别最大化可用性和性能](consistency-levels.md)。
...
按照 MongoDB 快速入门教程创建 Cosmos DB 帐户并迁移现有 Mongo DB 应用程序以使用 Cosmos DB，或生成一个新的 Mongo DB 应用程序：

---
title: "Azure Cosmos DB DocumentDB API 的 SQL 查询指标 | Azure"
...
在 Azure Cosmos DB 中，数据存储在容器中，容器可以增长到任何[存储大小或请求吞吐量](partition-data.md)。 Azure Cosmos DB 在幕后无缝地在物理分区之间缩放数据来以预配的吞吐量处理数据增长。 可以使用 REST API 或受支持的 [Cosmos DB SDK](documentdb-sdk-dotnet.md) 之一向任何容器发出 SQL 查询。
...
有关 REST API 请求标头和选项的详细信息，请参阅[使用 Cosmos DB REST API 查询资源](https://docs.microsoft.com/rest/api/documentdb/querying-documentdb-resources-using-the-rest-api)。
...
有关 SDK 发行说明和已实现的类和方法的详细信息，请参阅 [Cosmos DB SDK](documentdb-sdk-dotnet.md)

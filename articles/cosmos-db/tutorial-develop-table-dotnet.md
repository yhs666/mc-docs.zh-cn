---
title: "Azure Cosmos DB：在 .NET 中使用表 API 进行开发 | Azure"
...
ms.openlocfilehash: f9e97db2a1c86eaff20c88a41e8c401998138776
...
- 单个区域内可用性达 99.99%，可添加更多区域以提高可用性，以及获得正式发布版中[行业领先的综合 SLA](https://www.azure.cn/support/sla/documentdb/)
- 使用现有的 Azure 存储 .NET SDK，而不会更改应用程序的任何代码

预览期间，Azure Cosmos DB 使用 .NET SDK 支持表 API。 可从 NuGet 下载 [Azure 存储预览版 SDK](https://aka.ms/premiumtablenuget)，其具有与 [Microsoft Azure 存储 SDK](https://www.nuget.org/packages/WindowsAzure.Storage) 相同的类和方法签名，但也可以使用表 API 连接到 Azure Cosmos DB 帐户。

若要详细了解复杂的 Azure 表存储任务，请参阅：

* [Azure Cosmos DB 简介：表 API](table-introduction.md)
* 有关可用 API 的完整详细信息的表服务参考文档[用于 .NET 的存储客户端库参考](https://msdn.microsoft.com/library/mt347887.aspx&clcid=0x409)

### <a name="about-this-tutorial"></a>关于本教程
本教程供熟悉 Azure 表存储 SDK 并想要通过 Azure Cosmos DB 使用高级功能的开发人员使用。 本教程基于[通过 .NET 开始使用 Azure 表存储](../storage/storage-dotnet-how-to-use-tables.md)，并演示如何利用辅助索引、预配的吞吐量和多宿主等附加功能。 涵盖如何使用 Azure 门户预览创建 Azure Cosmos DB 帐户，然后生成并部署表应用程序。 还将演练用于创建和删除表，以及插入、更新、删除和查询表数据的 .NET 示例。 
...
1. 在 [Azure 门户预览](http://portal.azure.cn/)的 Azure Cosmos DB 帐户的左侧导航栏中，单击“密钥”，然后单击“读写密钥”。 将在下一步使用屏幕右侧的复制按钮将连接字符串复制到 app.config 文件中。
...
| TablePreferredLocations | 以逗号分隔的首选（多主页）位置列表，用于读取。 每个 Azure Cosmos DB 帐户可与 1-30+ 个区域关联。 每个客户端实例可按首选顺序指定这些区域的一个子集以实现低延迟读取。 必须使用这些区域的[显示名称](https://msdn.microsoft.com/library/azure/gg441293.aspx)命名这些区域，例如 `China North`。 另请参阅[多宿主 API](tutorial-global-distribution-table.md)。
...
> [!NOTE]
> 虽然表存储 SDK 当前不支持修改吞吐量，但可在任何时候通过 Azure 门户预览或 Azure CLI 即时更改吞吐量。
...
用于 Azure Cosmos DB 的表 API 处于预览状态。 在正式发布版中，由 SLA（其他 Azure Cosmos DB API 等）支持 p99 延迟保证。 

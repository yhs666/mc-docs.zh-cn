---
title: "使用 Java 创建 Azure Cosmos DB 文档数据库 | Azure | Azure"
...
ms.date: 11/27/2017
ms.author: v-yeche
ms.openlocfilehash: a0d4f98a90f226348960c521e1312d39297b268d
...
Azure Cosmos DB 由 Microsoft 提供，是全球分布的多模型数据库服务。 使用 Azure Cosmos DB，可以快速创建和查询托管的文档和表数据库。
...
现在，让我们转到如何使用代码上来。 从 GitHub 克隆 DocumentDB API 应用，设置连接字符串，并运行该应用。 你会看到以编程方式处理数据是多么容易。 
...
* `DocumentClient` 初始化。 [DocumentClient](https://docs.azure.cn/java/api/com.microsoft.azure.documentdb._document_client) 为 Azure Cosmos DB 数据库服务提供客户端逻辑表示。 此客户端用于对服务配置和执行请求。
...
* 使用 [createDocument](https://docs.azure.cn/java/api/com.microsoft.azure.documentdb._document_client.createdocument) 方法创建文档。
...
* 使用 [queryDocuments](https://docs.azure.cn/java/api/com.microsoft.azure.documentdb._document_client.querydocuments) 方法，对 JSON 执行 SQL 查询。
...
1. 在 [Azure 门户](http://portal.azure.cn/)中，单击“密钥”。 

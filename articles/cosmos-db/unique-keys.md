...
使用唯一键策略创建容器后，该策略会根据唯一键约束的指定，阻止在逻辑分区中创建新的或更新的重复项。 分区键与唯一键的组合可保证某个项在容器范围内的唯一性。
...
若要创建具有相同电子邮件地址但不是相同名字、姓氏和电子邮件地址的项，请将其他路径添加到唯一键策略。 如果不基于电子邮件地址创建唯一键，还可以使用名字、姓氏和电子邮件的组合创建唯一键。 此键称为复合唯一键。 在这种情况下，给定的 `CompanyID` 中允许这三个值的每种唯一组合。 

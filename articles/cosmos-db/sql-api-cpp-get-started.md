...
创建 Azure Cosmos DB 帐户。 如果已有一个可以使用的帐户，可以直接跳到[设置 C++ 应用程序](#SetupC++)。
...
5. 在“NuGet: hellodocumentdb”选项卡中，单击“浏览”，然后搜索“documentdbcpp”。 在结果中选择“DocumentDbCPP”，如以下屏幕截图中所示。 此程序包安装 C++ REST SDK 的引用，该 SDK 是DocumentDbCPP 的依赖项。  
...
Azure Cosmos DB 支持替换 JSON 文档，如以下代码所示。 请 executesimplequery 函数的后面添加此代码。
...
为此，可将应用程序的 main 函数替换为以下代码。 这样会覆盖步骤 3 中复制到代码的 account_configuration_uri 和 primary_key，因此请再次在门户中保存此代码行或复制这些值。 

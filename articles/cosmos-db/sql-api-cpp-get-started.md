...
创建 Azure Cosmos DB 帐户。 如果已经有想要使用的帐户，可以跳到 [安装 C++ 应用程序](#SetupC++)。
...
4. 创建项目后，在“解决方案资源管理器”中右键单击“hellodocumentdb”项目，然后单击“管理 NuGet 包”，打开 NuGet 包管理器。 
    ...
    ![在项目菜单上显示“管理 NuGet 包”的屏幕截图](media/sql-api-cpp-get-started/nuget.png)
5. 在“NuGet: hellodocumentdb”选项卡中，单击“浏览”，然后搜索“documentdbcpp”。 在结果中选择 DocumentDbCPP，如下面的屏幕截图所示：   

    ![突出显示 DocumentDbCpp 包的屏幕截图](media/sql-api-cpp-get-started/cpp.png)

    此程序包安装 C++ REST SDK 的引用，该 SDK 是DocumentDbCPP 的依赖项。 将包添加到项目后，已准备好开始编写一些代码。   
...
打开 [Azure 门户](https://portal.azure.cn)，导航到已创建的 Azure Cosmos DB 帐户。 下一步需要 Azure 门户中的 URI 和主密钥，以便使用 C++ 代码片段建立连接。 
...
2. 接下来，将以下代码添加到 main 函数，并根据步骤 3 中的 Azure Cosmos DB 设置替换帐户配置和主密钥。 
    ...
    现在，你已完成了用于初始化客户端的代码，接下来看看如何处理 Azure Cosmos DB 资源。

<a name="CreateDBColl"></a>
## <a name="step-5-create-a-c-database-and-collection"></a>步骤 5：创建 C++ 数据库和集合
...
<a name="CreateDoc"></a>
## <a name="step-6-create-a-document"></a>步骤 6：创建文档
[文档](sql-api-resources.md#documents)是用户定义的（任意）JSON 内容。 现在，可以将文档插入 Azure Cosmos DB 中。 可以通过将以下代码复制到主函数的末尾来创建一个文档。 
...
总而言之，此代码创建可在 Azure 门户中使用数据资源管理器查询的 Azure Cosmos DB 数据库、集合与文档。 

![C++ 教程 - 说明帐户、数据库、集合和文档间层次关系的关系图](media/sql-api-cpp-get-started/docs.png)
...
<a name="Replace"></a>
## <a name="step-8-replace-a-document"></a>步骤 8：替换文档
Azure Cosmos DB 支持替换 JSON 文档，如以下代码所示。 在 executesimplequery 函数后面添加此代码。
...
<a name="Delete"></a>
## <a name="step-9-delete-a-document"></a>步骤 9：删除文档
...
<a name="DeleteDB"></a>
## <a name="step-10-delete-a-database"></a>步骤 10：删除数据库
...
<a name="Run"></a>
## <a name="step-11-run-your-c-application-all-together"></a>步骤 11：运行整个 C++ 应用程序！
...
## <a name="get-the-complete-c-tutorial-solution"></a>获取完整的 C++ 教程解决方案

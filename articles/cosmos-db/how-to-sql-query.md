...
ms.date: 12/03/2018
ms.author: v-yeche
ms.openlocfilehash: d57a736745e3af24f4ec467f91774bba2cf3ab9e
ms.sourcegitcommit: 59db70ef3ed61538666fd1071dcf8d03864f10a9
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 11/30/2018
ms.locfileid: "52676788"
---
...
下面是另一个有一个细微差异的项 - 使用了 `givenName` 和 `familyName`，而不是 `firstName` 和 `lastName`。
...
**查询 2**：现在假设我们需要将 JSON 输出的格式重新设置为一种不同的形式。 地址的城市名称与省/自治区/直辖市名称相同时，此查询使用两个选定的字段 Name 和 City 表示新的 JSON 对象。 在这种情况下，“NY, NY”匹配。
...
在 SQL API 与在 ANSI SQL 中使用 BETWEEN 的主要不同之处在于，前者支持对混合类型的属性执行快速范围查询 - 例如，可以在某些项中将“grade”设置为数字 (5)，并在其他项中将其设置为字符串（“grade4”）。 在这些情况下（如在 JavaScript 中），在两种不同类型之间进行比较的结果为“undefined”，会跳过项。
...
TOP 关键字可用于限制来自查询中的值的数量。 当 TOP 与 ORDER BY 子句配合使用时，结果集被限制为有序值的前 N 个数；否则，它会返回未定义排序的结果中的前 N 数。 在 SELECT 语句中，最佳做法是始终使用带有 TOP 子句的 ORDER BY 子句。 组合这两个子句是可预测指示受 TOP 影响的行的唯一方法。 
...
现在，让我们来看看对容器中的子女执行遍历的另一个查询。 请注意输出数组中的差异。 此示例拆分 `children` 并将结果合并为单个数组。  
...
在传统 SQL 不同，在 SQL API 中，在从数据库中检索出值之前，值类型经常是未知的。 为了高效执行查询，大多数运算符具有严格的类型要求。
...
类型检查函数允许检查 SQL 查询内表达式的类型。 当类型是变量或未知时，可使用类型检查函数动态确定项内属性的类型。 以下是支持的内置类型检查函数表。

| **使用情况** | **说明** |
|-----------|------------|
| [IS_ARRAY (expr)](../cosmos-db/sql-api-sql-query-reference.md#bk_is_array) | 返回一个布尔值，它指示值的类型是否为数组。 |
| [IS_BOOL (expr)](../cosmos-db/sql-api-sql-query-reference.md#bk_is_bool) | 返回一个布尔值，它指示值的类型是否为布尔。 |
| [IS_NULL (expr)](../cosmos-db/sql-api-sql-query-reference.md#bk_is_null) | 返回一个布尔值，它指示值的类型是否为 null。 |
| [IS_NUMBER (expr)](../cosmos-db/sql-api-sql-query-reference.md#bk_is_number) | 返回一个布尔值，它指示值的类型是否为数字。 |
| [IS_OBJECT (expr)](../cosmos-db/sql-api-sql-query-reference.md#bk_is_object) | 返回一个布尔值，它指示值的类型是否为 JSON 对象。 |
| [IS_STRING (expr)](../cosmos-db/sql-api-sql-query-reference.md#bk_is_string) | 返回一个布尔值，它指示值的类型是否为字符串。 |
| [IS_DEFINED (expr)](../cosmos-db/sql-api-sql-query-reference.md#bk_is_defined) | 返回一个布尔，它指示属性是否已经分配了值。 |
| [IS_PRIMITIVE (expr)](../cosmos-db/sql-api-sql-query-reference.md#bk_is_primitive) | 返回一个布尔值，它指示值的类型是否为字符串、数字、布尔或 null。 |
...
| T_WITHIN (point_expr, polygon_expr) | 返回一个布尔表达式，该表达式指示第一个 GeoJSON 对象（点、多边形或 LineString）是否在第二个 GeoJSON 对象（点、多边形或 LineString）里面。 |
...
可以将以上运算符组合在一起，形成功能更强大的查询。 由于 Cosmos DB 支持嵌套的容器，因此可以连接或嵌套运算符组合。
...
还可以通过使用 `IQueryable` 对象创建 `IDocumentQueryable`，并读取 ` ResponseContinuationToken` 值并将它们作为 `FeedOptions` 中的 `RequestContinuationToken` 向回传递，从而显式控制分页。 `EnableScanInQuery` 以启用扫描。 对于分区容器，可以使用 `PartitionKey` 针对单个分区运行查询（尽管 Azure Cosmos DB 可以自动从查询文本中提取此内容），还可以使用 `EnableCrossPartitionQuery` 来运行可能需要针对多个分区运行的查询。

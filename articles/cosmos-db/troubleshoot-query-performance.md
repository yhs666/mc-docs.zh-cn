...
多值子查询可以通过在 `WHERE` 子句中的每个 select-many 表达式后面（而不是所有 cross-join 后面）推送谓词，来优化 `JOIN` 表达式。 有关详细示例，请参阅[优化 Join 表达式](/cosmos-db/sql-query-subquery#optimize-join-expressions)一文。
...
* 对于多属性 `ORDER BY` 表达式，请在排序字段中定义[组合索引](/cosmos-db/index-policy#composite-indexes)。  

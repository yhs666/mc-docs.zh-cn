---
title: "Azure Cosmos DB 诊断日志记录 | Azure"
...
开始使用一个或多个 Azure Cosmos DB 数据库后，可能需要监视数据库的访问方式和时间。 可利用 Azure Cosmos DB 中的诊断日志记录执行此监视。 可以通过启用诊断日志记录，将日志发送到 [Azure 存储](https://www.azure.cn/home/features/storage/)、将日志流式传输到 [Azure 事件中心](https://www.azure.cn/home/features/event-hubs/)，以及/或者将日志导出到 [Log Analytics](https://www.azure.cn/home/features/log-analytics/)，后者是 [Operations Management Suite](https://www.microsoft.com/cloud-platform/operations-management-suite) 的一部分。
...
1. 在 [Azure 门户](https://portal.azure.cn)的 Azure Cosmos DB 帐户中，单击左侧导航栏中的“诊断日志”，然后单击“启用诊断”。
...
2. 在“诊断设置”页上，执行以下操作： 
    ...
    * **发送到 Log Analytics**。     若要使用此选项，请使用现有工作区，或者创建新的 Log Analytics 工作区，只需在门户中执行[创建新工作区](../log-analytics/log-analytics-quick-collect-azurevm.md#create-a-workspace)的步骤即可。 若要详细了解如何在 Log Analytics 中查看日志，请参阅[在 Log Analytics 中查看日志](#view-in-loganalytics)。
    * **记录 DataPlaneRequests**。 选择此选项可记录 Cosmos DB、Graph 和表 API 帐户的诊断。 若要存档到存储帐户，可以选择诊断日志的保留期。 保留期到期后自动删除日期。
    * **记录 MongoRequests**。 选择此选项可记录 MongoDB API 帐户的诊断。 若要存档到存储帐户，可以选择诊断日志的保留期。 保留期到期后自动删除日期。
...
3. 单击“保存” 。
    ...
    在将来的任意时间，若要更改诊断日志的保存方式，可随时回到此页修改帐户的诊断日志设置。
...
为进一步简化管理，我们在本教程中使用包含 Azure Cosmos DB 数据库的同一资源组。 将 ContosoResourceGroup、contosocosmosdblogs 和“中国北部”的值替换为自己的值（如适用）：
...
在每个此类查询中返回的 blob 以文本形式存储，格式化为 JSON blob，如以下代码所示。 
...
## <a name="view-logs-in-log-analytics"></a>查看 Log Analytics 中的日志

如果在启用日志记录时选择了“发送到 Log Analytics”选项，则会在两小时内将集合中的诊断数据转发到 Log Analytics。 这意味着，如果在启用日志记录后立即查看 Log Analytics，将看不到任何数据。 请等待两小时，然后重试。 

在查看日志之前，需查看 Log Analytics 工作区是否已升级，否则无法使用新的 Log Analytics 查询语言。 为此，请打开 [Azure 门户](https://portal.azure.cn)，单击最左侧的“Log Analytics”，然后选择工作区名称，如下图所示。 此时会显示“OMS 工作区”页，如下图所示。

![Azure 门户中的 Log Analytics](./media/logging/azure-portal.png)

如果在“OMS 工作区”页上看到以下消息，则说明工作区尚未升级，无法使用新的语言。 有关升级到新查询语言的进一步信息，请参阅[将 Azure Log Analytics 工作区升级到新的日志搜索](../log-analytics/log-analytics-log-search-upgrade.md)。 

![Log Analytics 升级通知](./media/logging/upgrade-notification.png)

若要查看 Log Analytics 中的诊断数据，请打开左侧菜单中的“日志搜索”页或该页的“管理”区域，如下图所示。

![Azure 门户中的日志搜索选项](./media/logging/log-analytics-open-log-search.png)

启用数据收集以后，即可使用新的查询语言运行以下日志搜索示例，以便查看最新的十个日志 `AzureDiagnostics | take 10`。

![“take 10”日志搜索示例](./media/logging/log-analytics-query.png)

<a id="#queries"></a>
### <a name="queries"></a>查询

下面是一些可以输入到“日志搜索”框中的其他查询，用于监视 Azure Cosmos DB 容器。 这些查询使用[新语言](../log-analytics/log-analytics-log-search-upgrade.md)。 

若要了解每个日志搜索返回的数据的含义，请参阅[解释 Azure Cosmos DB 日志](#interpret)。

* 指定时段内 Azure Cosmos DB 中的所有诊断日志。
...
* 所有用户活动，按资源分组。 请注意，这是活动日志，不是诊断日志。
...
若要详细了解如何使用新日志搜索语言，请参阅[了解 Log Analytics 中的日志搜索](../log-analytics/log-analytics-log-search-new.md)。 

## <a id="interpret"></a>解释日志

存储在 Azure 存储和 Log Analytics 中的诊断数据使用非常类似的架构。 
...
| resourceTokenUserRid | resourceTokenUserRid_s | [资源令牌](/cosmos-db/secure-access-to-data#resource-tokens)用于身份验证并指向用户的资源 ID 时，此项非空。 |

## <a name="next-steps"></a>后续步骤

- 若要了解如何启用日志记录，以及各种 Azure 服务支持的指标和日志类别，请参阅 [Azure 中的指标概述](../monitoring-and-diagnostics/monitoring-overview-metrics.md)和 [Azure 诊断日志概述](../monitoring-and-diagnostics/monitoring-overview-of-diagnostic-logs.md)这两篇文章。
- 阅读以下文章，了解事件中心：
   - [什么是 Azure 事件中心？](../event-hubs/event-hubs-what-is-event-hubs.md)
   - [事件中心入门](../event-hubs/event-hubs-csharp-ephcs-getstarted.md)
- 阅读[从 Azure 存储下载指标和诊断日志](../storage/blobs/storage-dotnet-how-to-use-blobs.md#download-blobs)

...
ms.date: 05/13/2019
ms.author: v-yeche
ms.openlocfilehash: 6c1b2a03c1bc50e7fa1e673f5459d13f191e837f
ms.sourcegitcommit: 71172ca8af82d93d3da548222fbc82ed596d6256
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 05/15/2019
ms.locfileid: "65669055"
---
...
* 相关子查询 - 相关子查询是引用外部查询中的值的子查询。 将针对外部查询处理的每个行求值该子查询一次。

* 非相关子查询 - 非相关子查询是独立于外部查询的子查询，它可以独立执行，而不依赖于外部查询。
...
可以根据子查询返回的行数和列数进一步分类子查询。 有三种不同的类型：
1. **Table**：返回多个行和多个列
2. **多值**：返回多个行和单个列
3. **标量**：返回单个行和单个列

> [!NOTE]
> Azure Cosmos DB 支持多值和标量子查询

Azure Cosmos DB SQL 查询始终返回单个列（一个简单的值，或一个复杂文档）。 因此，Azure Cosmos DB 中仅适用上述多值子查询和标量子查询。 多值子查询只能作为关系表达式在 FROM 子句中使用，而标量子查询可以作为标量表达式在 SELECT 或 WHERE 子句中使用，或者作为关系表达式在 FROM 子句中使用。
...
对于此查询，索引将匹配其中的标记包含“infant formula”、nutrient 项的值为 0 到 10、serving 项的数量大于 1 的任何文档。 但是，此处的 JOIN 表达式将针对应用任何筛选器之前的每个匹配文档，执行所有 tags、nutrients 和 servings 数组项的叉积计算。 然后，WHERE 子句将针对每个 <c, t, n, s> 元组应用筛选谓词。 例如，如果在匹配文档的三个数组中，每个数组包含 10 个项，则此子句将扩展为 1 x 10 x 10 x 10（即 1000）个元组。 在与下一个表达式联接之前，使用此处的子查询可帮助筛选出联接的数组项。
...
假设 tags 数组中只有一个项与筛选器相匹配，而 nutrients 和 servings 数组各有 5 个项，则 JOIN 表达式将扩展为 1 x 1 x 5 x 5 = 25 个项，而不是第一个查询中的 1000 个项。
...
> [!NOTE] 
> 在确定 JOIN 表达式叉积行为的情况下，如果 UDF 表达式可能会求值为 undefined（未确定），则应该通过从子查询返回对象，而不是直接返回值，来确保 JOIN 表达式始终生成单个行。
>
...
此方法并不局限于 UDF，而是适用于任何可能存在较高开销的表达式。 例如，我们可以对数学函数 avg 采用相同的方法：
...
我们经常需要引用极少发生变化的静态数据，例如度量单位或国家/地区代码。 最好是不要针对每个文档复制此类数据。 避免这种复制可以节省存储空间，并通过保持较小的文档大小来提高写入性能。 在此处可以使用子查询通过引用数据的集合来模拟内部联接语义。
例如，假设存在以下引用数据集。
...
但是，使用标量子查询并不总是有助于优化。 例如，将标量子查询作为参数传递给系统定义的或用户定义的函数并不能在 RU 消耗量与延迟方面带来好处。
...
编写此查询的更好方法是在子查询中进行联接，并在 SELECT 和 WHERE 子句中引用子查询别名。 此查询更有效，因为我们只需执行 join 语句中的子查询，而无需同时执行投影和筛选器中的子查询。
...
但是，如果省略上述子查询中的 VALUE 关键字，则该查询将求值为 true：
```sql
SELECT EXISTS (SELECT undefined) 
```

该子查询在对象中的 select 列表内括住值列表。 如果选定的列表不包含任何值，该子查询将返回定义的单个值 '{}'，因此 EXISTS 求值为 true。
...
此外，ARRAY_CONTAINS 只能检查某个值是否等于数组中的任何元素。 如果数组属性中需要更复杂的筛选器，则需要使用 JOIN。
...
在本例中，我们根据 EXISTS 子查询中的数组元素进行筛选。 如果某个数组元素与筛选器匹配，则我们会投影该元素，而 `EXISTS` 将求值为 true。
...
使用 `ARRAY` 表达式可将查询结果投影为数组。 此表达式只能在查询的 SELECT 子句中使用。
...
<!--Update_Description: new articles on how to sql subquery -->
<!--ms.date: 05/20/2019-->
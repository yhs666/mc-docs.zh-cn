...
ms.date: 12/25/2017
ms.author: v-yeche
ms.openlocfilehash: 402b8509139a8c732824fd65944ecbc9dce2d5e2
...
将 **getCollection** 函数复制并粘贴到 app.js 文件中 **getDatabase** 函数下面，以便使用 ```config``` 对象中指定的 ```id``` 创建新集合。 同样，我们首先检查以确保不存在具有相同 ```FamilyCollection``` ID 的集合。 如果确实存在，我们将返回该集合而不是创建新的。

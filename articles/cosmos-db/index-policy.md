...
ms.date: 04/15/2019
ms.author: v-yeche
ms.openlocfilehash: e667d9e4ae4049db591400217b9107a3b1efdbff
...
Azure Cosmos DB 支持两种索引模式，可以通过索引策略在 Azure Cosmos 容器上配置这些模式：

* **一致**：如果 Azure Cosmos 容器的策略设置为“一致”，则特定容器上的查询将按照为点读取指定的一致性级别进行（例如，非常、有限过期性、会话或最终）。 

  更新项时，会同步更新索引。 例如，对项执行插入、替换、更新和删除操作将导致更新索引。 一致索引支持一致的查询，但代价是影响写入吞吐量。 写入吞吐量的降低取决于“索引中包含的路径”和“一致性级别”。 设计一致的索引模式是为了能够随时使用所有更新对索引进行更新，以及让索引能够即时处理查询。
...
所有索引转换都是联机进行的。 按照旧策略编制索引的项可以按照新策略有效转换，而不会影响写入可用性或在容器上预配的吞吐量。 在索引转换期间，使用 REST API、SDK 或存储过程和触发器执行的读取和写入操作的一致性不会受到影响。
...
更改索引策略时，更改得到应用，从旧索引移动到新索引，主要基于索引模式配置。 与其他属性（如包括/排除路径，索引种类和精度）相比，索引模式配置起主要作用。

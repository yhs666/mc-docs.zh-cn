---
title: "快速入门：将表 API 与 Node.js 配合使用 - Azure Cosmos DB | Azure"
...
2. 运行以下命令，在本地安装 [azure]、[node-uuid]、[nconf] 和 [async] 模块，并为它们在 package.json文件中保存一个条目

   ```
   npm install azure-storage node-uuid async nconf --save
   ```

2. 在 git 终端窗口运行以下命令，启动 Node 应用程序。

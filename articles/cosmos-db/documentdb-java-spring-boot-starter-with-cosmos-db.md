...
1. 浏览到 Azure 门户（网址：<https://portal.azure.cn/>），单击“+新建”。

   ![Azure 门户][AZ01]

1. 依次单击“数据库”、“Azure Cosmos DB”。

   ![Azure 门户][AZ02]

1. 在“Azure Cosmos DB”页面上，输入以下信息：

   * 输入一个唯一的 **ID**，作为数据库的 URI。 例如：*wingtiptoysdata.documents.azure.cn*。
   * 为 API 选择“SQL(文档 DB)”。
   * 选择要用于数据库的“订阅”。
...
1. 当显示数据库的属性页面时，单击“访问密钥”，并复制数据库的 URI 和访问密钥，在 Spring Boot 应用程序中会用到这些值。
...
1. 指定希望使用 Java 生成 Maven 项目，输入应用程序的“组”名称和“项目”名称，单击“生成项目”按钮。
...
1. 在文本编辑器中打开 *pom.xml* 文件，并将以下行添加到 `<dependencies>` 列表中：
...
1. 在应用的 *resources* 目录中找到 *application.properties* 文件，例如：
...
1. 在文本编辑器中打开 *application.properties* 文件，将以下行添加到文件中，并将示例值替换为数据库的相应属性：
...
1. 应用程序会显示多个运行时消息，当显示 `User: testFirstName testLastName` 消息时，表示已成功在数据库中存储和检索值。
...
* [Azure Cosmos DB：使用 Java 和 Azure 门户生成 Cosmos DB API 应用][Build a DocumentDB API app with Java]
...
* 在 Azure 容器服务中运行 Kubernetes 群集上的 Spring Boot 应用程序

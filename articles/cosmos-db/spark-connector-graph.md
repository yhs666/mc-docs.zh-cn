---
title: "Azure Cosmos DB：使用 Spark 和 Apache TinkerPop Gremlin 执行图形分析 | Azure"
...
ms.date: 07/17/2017
ms.author: v-yeche
ms.openlocfilehash: ff0ddd2f569f099b79e9d8ef3ce3626cf8e4ad7c
ms.sourcegitcommit: b15d77b0f003bef2dfb9206da97d2fe0af60365a
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 07/07/2017
---
# Azure Cosmos DB：使用 Spark 和 Apache TinkerPop Gremlin 执行图形分析
<a id="azure-cosmos-db-perform-graph-analytics-by-using-spark-and-apache-tinkerpop-gremlin" class="xliff"></a>
...
[Spark](http://spark.apache.org/) 是 Apache Software Foundation 项目，专注于通用联机分析处理 (OLAP) 数据处理。 Spark 提供混合的内存/磁盘型分布式计算模型，类似于 Hadoop MapReduce 模型。 可以使用 [Azure HDInsight](https://www.azure.cn/home/features/hdinsight/apache-spark/) 在云中部署 Apache Spark。
...
## 创建 Azure Cosmos DB 数据库帐户
<a id="create-an-azure-cosmos-db-database-account" class="xliff"></a>
...
## 获取 Apache TinkerPop
<a id="get-apache-tinkerpop" class="xliff"></a>

获取 Apache TinkerPop，方法如下：

1. 远程连接到 HDInsight 群集 `ssh tinkerpop3-cosmosdb-demo-ssh.azurehdinsight.cn` 的主节点。
...
3. 安装 Spark-Gremlin 插件 
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 插件安装由 Grape 负责。 为 Grape 填充存储库信息，使之能够下载插件及其依赖项。 
    ...
    b.保留“数据库类型”设置，即设置为“共享”。 启动 Gremlin 控制台 `bin/gremlin.sh`。
...
## 准备 TinkerPop3 依赖项
<a id="prepare-tinkerpop3-dependencies" class="xliff"></a>
...
## 获取 Azure Cosmos DB Spark 连接器
<a id="get-the-azure-cosmos-db-spark-connector" class="xliff"></a>
...
2. 也可在本地生成。 由于最新版本的 Spark-Gremlin 是使用 Spark 1.6.1 生成的，不兼容目前在 Azure Cosmos DB Spark 连接器中使用的 Spark 2.0.2，因此可以手动生成最新的 TinkerPop3 代码并安装 jar。 请执行以下操作：
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 克隆 Azure Cosmos DB Spark 连接器。

    b.保留“数据库类型”设置，即设置为“共享”。 生成 TinkerPop3（已在前述步骤中完成）。 在本地安装所有 TinkerPop 3.3.0-SNAPSHOT jar。
    ...
    d.单击“下一步”。 使用 Maven 生成。 所需 jar 置于 `target` 和 `target/alternateLocation` 中。
...
## 将依赖项分发到 Spark 辅助角色节点
<a id="distribute-the-dependencies-to-the-spark-worker-nodes" class="xliff"></a> 
...
2. 将前述 Gremlin 依赖项、CosmosDB Spark 连接器 jar 和 CosmosDB Java SDK 复制到辅助角色节点，方法如下：
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 将所有 jar 复制到 `~/azure-documentdb-spark` 中。
    ...
    b.保留“数据库类型”设置，即设置为“共享”。 获取所有 Spark 辅助角色节点的列表，该列表位于 Ambari 仪表板 `Spark2` 部分的 `Spark2 Clients` 列表中。
...
## 设置环境变量
<a id="set-up-the-environment-variables" class="xliff"></a>

1. 查找 Spark 群集的 HDP 版本。 它是 `/usr/hdp/` 下的目录名（如，2.5.4.2-7）。

2. 为所有节点设置 hdp.version。 在 Ambari 仪表板中，转到“YARN 部分” > “配置” > “高级”，然后执行以下操作： 

    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 在 `Custom yarn-site` 中添加新属性 `hdp.version`，其值设置为主节点上的 HDP 版本。 

    b.保留“数据库类型”设置，即设置为“共享”。 保存配置。 出现警告，但可将其忽略。 
...
## 准备图形配置
<a id="prepare-the-graph-configuration" class="xliff"></a>
...
## 加载 TinkerPop 图形，并将其保存到 Azure Cosmos DB 中
<a id="load-the-tinkerpop-graph-and-save-it-to-azure-cosmos-db" class="xliff"></a>
...
3. 启动 Gremlin 控制台并创建以下计算步骤，将数据保存到配置的 Azure Cosmos DB 集合：  
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 创建图形 `graph = GraphFactory.open("conf/hadoop/gremlin-spark.properties")`。

    b.保留“数据库类型”设置，即设置为“共享”。 使用 SparkGraphComputer 写入 `graph.compute(SparkGraphComputer.class).result(GraphComputer.ResultGraph.NEW).persist(GraphComputer.Persist.EDGES).program(TraversalVertexProgram.build().traversal(graph.traversal().withComputer(Computer.compute(SparkGraphComputer.class)),"gremlin-groovy","g.V()").create(graph)).submit().get()`。
...
## 从 Azure Cosmos DB 加载图形，并运行 Gremlin 查询
<a id="load-the-graph-from-azure-cosmos-db-and-run-gremlin-queries" class="xliff"></a>
...
2. 通过执行以下步骤加载图形、遍历数据并对其运行 Gremlin 查询：
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 启动 Gremlin 控制台 `bin/gremlin.sh`。

    b.保留“数据库类型”设置，即设置为“共享”。 使用配置 `graph = GraphFactory.open('conf/hadoop/gremlin-spark.properties')` 创建图形。

    c. 使用 SparkGraphComputer `g = graph.traversal().withComputer(SparkGraphComputer)` 创建图形遍历。

    d.单击“下一步”。 运行以下 Gremlin 图形查询：

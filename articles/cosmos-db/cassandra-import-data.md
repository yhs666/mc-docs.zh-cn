...
* 增加吞吐量：数据迁移的持续时间取决于为表预配的吞吐量。 请确保对于较大的数据迁移增加吞吐量。 完成迁移后，减少吞吐量以节约成本。 有关在 [Azure 门户](https://portal.azure.cn)中增加吞吐量的详细信息，请参阅[设置 Azure Cosmos DB 容器的吞吐量](set-throughput.md)。
...
1. 在 [Azure 门户](https://portal.azure.cn)的最左侧，单击“Azure Cosmos DB”。
...
1. 预创建和缩放表：
    * 默认情况下，Azure Cosmos DB 会预配一个新的 Cassandra API 表，每秒钟有 1,000 个请求单元 (RU/s)（基于 CQL 的创建预配有 400 RU/s）。 通过使用 cqlsh 开始迁移之前，从 [Azure 门户](https://portal.azure.cn)或 cqlsh 预先创建所有表。 

    * 在迁移期间，从 [Azure 门户](https://portal.azure.cn)将表的吞吐量从默认吞吐量（400 或 1000 RU/s）增加到 10,000 RU/s。 提高吞吐量后，可避免发生限制，并缩短迁移时间。 由于 Azure Cosmos DB 采用按小时计费，因此可以在迁移后立即降低吞吐量，以节省成本。

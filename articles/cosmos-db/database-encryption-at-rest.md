---
title: "数据库静态加密：Azure Cosmos DB | Azure"
...
---
# Azure Cosmos DB 数据库静态加密
<a id="azure-cosmos-db-database-encryption-at-rest" class="xliff"></a>
...
## 实现静态加密
<a id="implement-encryption-at-rest" class="xliff"></a>
...
### 问：如果启用存储服务加密，Azure 存储的费用将会高多少？
<a id="q-how-much-more-does-azure-storage-cost-if-storage-service-encryption-is-enabled" class="xliff"></a>
答：没有任何额外费用。
...
答：不会影响或改变性能 SLA，因为已对所有现有的和新的帐户启用静态加密。 可以在 [Cosmos DB SLA](https://www.azure.cn/support/sla/cosmos-db) 页上阅读详细信息，查看最新保证。

### 问：本地模拟器是否支持静态加密？
<a id="q-does-the-local-emulator-support-encryption-at-rest" class="xliff"></a>
答：模拟器是一项独立的开发/测试工具，不使用托管 Cosmos DB 服务使用的密钥管理服务。 建议在要存储敏感的模拟器测试数据的驱动器上启用 BitLocker。 [模拟器支持更改默认数据目录](local-emulator.md)以及使用已知位置。

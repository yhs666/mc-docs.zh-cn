...
Azure Cosmos DB 多主数据库提供多区域冲突解决模式，确保数据在所有复制到的区域中保持一致。
...
> [!NOTE]
> SQL API 用户可以从三种不同的冲突解决模式中选择。 对于所有其他的 API 模型 (MongoDB )，可以使用“最后写入者优先”来解决冲突。

<!-- Not Available on Cassandra, Graph and Table-->
...
<!-- Update_Description: new articles on multi master conflict resolution -->
<!--ms.date: 11/05/2018-->
...
ms.openlocfilehash: a91e3ddf70a618fab858d88da88798d1b94d1445
...
本文介绍如何使用 Azure 门户预览设置 Azure Cosmos DB 全局分发，然后使用图形 API（预览版）进行连接。
...
为了利用[全局分发](../documentdb/documentdb-distribute-data-globally.md)，客户端应用程序可以指定要用于执行文档操作的区域优先顺序列表。 可通过设置连接策略来实现此目的。 SDK 根据 Azure Cosmos DB 帐户配置、当前区域可用性和指定的优先顺序列表，选择最佳的终结点来执行写入和读取操作。

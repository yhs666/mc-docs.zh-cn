...
可在 Azure 门户中以*固定*或*无限制*模式创建 Azure Cosmos DB 容器。 固定大小的容器上限为 10 GB，10,000 RU/s 吞吐量。 若要以无限制模式创建容器，必须指定一个分区键，以及最低 1,000 RU/秒的吞吐量。 也可将 Azure Cosmos DB 容器配置为在一组容器之间共享吞吐量，该组中的每个容器必须指定一个分区键，可以无限增长。

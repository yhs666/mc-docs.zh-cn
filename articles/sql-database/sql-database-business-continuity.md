...
ms.date: 04/17/2018
ms.author: v-johch
ms.openlocfilehash: 3fcbb4b8a0fa927592aec36993f0823f4db25326
ms.sourcegitcommit: 8b36b1e2464628fb8631b619a29a15288b710383
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 06/26/2018
ms.locfileid: "36947956"
---
...
SQL 数据库每周自动执行完整数据库备份，每小时自动执行差异数据库备份，每 5 到 10 分钟自动执行事务日志备份，防止企业丢失数据。 如果使用的是[基于 DTU 的购买模型](sql-database-service-tiers-dtu.md)，则对于“标准”和“高级”服务层中的数据库，这些备份会在 RA-GRS 存储中存储 35 天；对于“基本”服务层中的数据库，则存储 7 天。 如果服务层的保留期不符合企业需求，可以通过[更改服务层](sql-database-service-tiers-dtu.md#choosing-a-service-tier-in-the-dtu-based-purchasing-model)来延长保留期。 如果使用的是[基于vCore 的购买模型（预览版）](sql-database-service-tiers-vcore.md)，则在“常规用途”和“业务关键（预览版）”层中，备份保留期是可配置的，最长为 35 天。 完整和差异数据库备份也会复制到配对的数据中心，以防数据中心中断。 有关更多详细信息，请参阅[自动数据库备份](sql-database-automated-backups.md)。

如果支持的最长 PITR 保留期对你的应用程序而言不足，可以通过为数据库配置长期保留 (LTR) 策略来延长保留期。 有关详细信息，请参阅 [长期保留](sql-database-long-term-retention.md)。

通过这些自动数据库备份，可以在自己的数据中心以及其他数据中心内，从各种干扰性事件中恢复数据库。 使用自动数据库备份时，估计恢复时间取决于若干因素，包括在相同区域同时进行恢复的数据库总数、数据库大小、事务日志大小以及网络带宽。 恢复时间通常少于 12 小时。 恢复到另一个数据区域时，因为每小时会针对异地冗余存储进行差异数据库备份，因此最多只可能丢失 1 小时的数据。
...
若要启用自动和透明故障转移，应使用 SQL 数据库的[自动故障转移组](sql-database-geo-replication-overview.md)功能（预览），将异地复制数据库整理成组。

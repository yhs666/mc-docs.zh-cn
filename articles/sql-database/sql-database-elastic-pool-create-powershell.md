...
ms.date: 12/26/2016
ms.author: v-johch
ms.openlocfilehash: 41a8af43ac4db29e51db9ebafc315c2c5693b6f9
...
[New-AzureRmSqlElasticPool](https://msdn.microsoft.com/zh-cn/library/azure/mt619378.aspx) cmdlet 将创建新池。 每个池的 eDTU 值、最小和最大 DTU 受服务层值（基本、标准或高级）的约束。 请参阅[弹性池和弹性数据库的 eDTU 和存储限制](./sql-database-elastic-pool.md#eDTU-and-storage-limits-for-elastic-pools-and-elastic-databases)。
...
使用 [New-AzureRmSqlDatabase](https://msdn.microsoft.com/zh-cn/library/azure/mt619339.aspx) cmdlet 并将 **ElasticPoolName** 参数设置为目标池。 要将现有数据库移动到池，请参阅[将数据库移动到弹性池](./sql-database-elastic-pool-manage-powershell.md#Move-a-database-into-an-elastic-pool)。
...
此脚本创建新的 Azure 资源组和新的服务器。 出现提示时，请提供新服务器的管理员用户名和密码（不是你的 Azure 凭据）。
...
- [管理池](./sql-database-elastic-pool-manage-powershell.md)
- [使用 Azure SQL 数据库进行横向扩展](./sql-database-elastic-scale-introduction.md)：使用弹性数据库工具进行横向扩展。
...
ms.date: 
ms.author: v-johch
ms.openlocfilehash: 52f25729c2ec24bc8cb23d094560ded57f51bd46
...
你可能会遇到这种情况，不小心修改了 SQL 数据库中的某些数据，而现在你希望恢复单个受影响的表。 本文介绍了如何在数据库中从某个 SQL 数据库[自动备份](sql-database-automated-backups.md)还原单个表。
...
2. 使用[时间点还原](sql-database-recovery-using-backups.md#point-in-time-restore)步骤，将数据库的一个备份还原到想要恢复到的一个时间点。
   ...
   > [!NOTE]
   > 还原的数据库的名称格式为 DBName+TimeStamp；例如，**Adventureworks2012_2016-01-01T22-12Z**。 此步骤不会覆盖服务器上现有的数据库名称。 这是一项安全措施，目的是让用户在删除其当前数据库之前确认还原的数据库，然后重命名此还原的数据库供生产之用。
...
3. 在“连接到服务器”  对话框中应用以下设置：
   ...
   * 数据库：**Master 数据库（列出所有数据库）**
   ...
   > [!NOTE]
   > 默认情况下，此向导将保存你的登录信息。 如果不想保存，请选择“忘记登录信息”。
   >
...
6. 在“脚本向导摘要”页中，当系统询问是否准备好生成 SQL 脚本时，单击“是”。 你还可以选择保存此 TSQL 脚本供以后使用。
...
- 查询并测试最近复制的表，以确保数据完好无损。 确认后，你可以删除**准备步骤**部分中已重命名的表。 （例如，&lt;表名称&gt;_old）。

...
ms.topic: article
ms.tgt_pltfrm: NA
ms.openlocfilehash: 0a627efacde9a8143ccb7ae9902887892b67ad31
...
---
# 复制 Azure SQL 数据库
<a id="copy-an-azure-sql-database" class="xliff"></a>
...
## 数据库副本中的登录名
<a id="logins-in-the-database-copy" class="xliff"></a>

将某个数据库复制到同一逻辑服务器时，可以在这两个数据库上使用相同的登录名。 用于复制该数据库的安全主体将成为新数据库上的数据库所有者 (DBO)。 所有数据库用户、其权限及安全标识符 (SID) 都将复制到数据库副本中。  

将数据库复制到不同的逻辑服务器时，新服务器上的安全主体将成为新数据库上的数据库所有者。 如果使用[包含的数据库用户](sql-database-manage-logins.md)进行数据访问，请确保主数据库和辅助数据库始终具有相同的用户凭据，这样在复制完成后，便可以使用相同的凭据立即访问。 
...
## 使用 Azure 门户复制数据库
<a id="database-copy-using-the-azure-portal" class="xliff"></a>
...
## 使用 PowerShell 复制数据库
<a id="database-copy-using-powershell" class="xliff"></a>
...
<a id="database-copy-using-transact-sql" class="xliff"></a>
...
### 将 SQL 数据库复制到同一台服务器
<a id="copy-a-sql-database-to-the-same-server" class="xliff"></a>
...
### 将 SQL 数据库复制到不同的服务器
<a id="copy-a-sql-database-to-a-different-server" class="xliff"></a>
...
### 监视复制操作的进度
<a id="monitor-the-progress-of-the-copy-operation" class="xliff"></a>
...
## 解析登录名
<a id="resolve-logins" class="xliff"></a>
...
若要了解如何在将数据库复制到其他逻辑服务器时管理用户和登录名，另请参阅[灾难恢复后如何管理 Azure SQL 数据库安全性](sql-database-geo-replication-security-config.md)。

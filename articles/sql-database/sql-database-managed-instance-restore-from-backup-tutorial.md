...
本教程演示如何使用 Wide World Importers - 标准备份文件将存储在 Azure blob 存储中的数据库备份还原到托管实例。 此方法需要一些停机时间。  有关各种迁移方法的讨论，请参阅[将 SQL Server 实例迁移到 Azure SQL 数据库托管实例](sql-database-managed-instance-migrate.md)。

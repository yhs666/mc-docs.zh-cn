...
ms.date: 07/03/2017
ms.author: v-johch
ms.openlocfilehash: 50dc89a7e210732b531bbe616cdc08dea9f7d3ff
...
下表显示一些即席测试结果。 这些测试执行具有事务和不具有事务的相同的顺序插入。 为了更具对比性，第一组测试是从笔记本电脑针对 Microsoft Azure 中的数据库远程运行的。 第二组测试是从位于同一 Microsoft Azure 数据中心（中国北部）的云服务和数据库运行的。 下表显示具有和不具有事务的一系列顺序插入所用的时间（毫秒）。

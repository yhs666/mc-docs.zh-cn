...
为了简化客户部署，Umbraco 添加了 Umbraco 即服务 (UaaS)：一个软件即服务 (SaaS) 产品，它消除了本地部署的需要，提供内置缩放，使开发人员能够专注于产品创新而不是解决方案管理，免除了管理开销。Umbraco 依赖于 Microsoft Azure 提供的弹性平台即服务 (PaaS) 模型提供所有这些优势。
...
- 有关虚拟网络的详细信息，请参阅 [virtual networking](../virtual-network/index.md)（虚拟网络）。

- 有关备份与恢复的详细信息，请参阅 [business continuity](./sql-database-business-continuity.md)（业务连续性）。

- 有关监视池的详细信息，请参阅 [monitoring pools](./sql-database-elastic-pool-manage-portal.md)（监视池）。

...
ms.date: 07/03/2017
ms.author: v-johch
ms.openlocfilehash: 771cd128be3348189eab801ddab087cec94123f7
...
---
# Azure SQL 数据库连接体系结构
<a id="azure-sql-database-connectivity-architecture" class="xliff"></a> 
...
## 连接体系结构
<a id="connectivity-architecture" class="xliff"></a>

下图提供 Azure SQL 数据库连接体系结构的高级概述。 

![体系结构概述](./media/sql-database-connectivity-architecture/architecture-overview.png)

以下步骤介绍如何通过 Azure SQL 数据库软件负载均衡器 (SLB) 和 Azure SQL 数据库网关建立与 Azure SQL 数据库的连接。

- Azure 内或 Azure 外的客户端连接到该 SLB，它具有一个公共 IP 地址并侦听端口 1433。
...
### 从 Azure 内连接
<a id="connectivity-from-within-azure" class="xliff"></a>
...
### 从 Azure 外连接
<a id="connectivity-from-outside-of-azure" class="xliff"></a>
...
<a id="change-azure-sql-database-connection-policy" class="xliff"></a>
...
- 如果将连接策略设置为“重定向”，所有网络数据包将直接流向中间件代理。 对于此设置，需要允许出站到多个 IP。 

## 用于更改连接设置的脚本
<a id="script-to-change-connection-settings" class="xliff"></a>

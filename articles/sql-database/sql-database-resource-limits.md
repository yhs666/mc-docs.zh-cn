...
ms.date: 10/02/2017
ms.author: v-johch
ms.openlocfilehash: f3a7f05d7e7d7d89dbf4e26f7524713529f744f9
...
| 每个数据库的最大存储 |池中一个数据库的最大存储空间。 入池数据库共享池的存储空间，因此数据库存储空间限制为小于池的剩余存储空间和每个数据库的最大存储空间。 每个数据库的最大存储是指数据文件的最大大小，不包括日志文件使用的空间。 |
|||

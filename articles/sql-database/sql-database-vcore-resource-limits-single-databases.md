...
ms.date: 09/09/2019
ms.openlocfilehash: c97098270c01109ee59badef9b4644655e3d87b5
ms.sourcegitcommit: 2610641d9fccebfa3ebfffa913027ac3afa7742b
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/05/2019
ms.locfileid: "70373011"
---
...
## <a name="general-purpose-service-tier-for-provisioned-compute"></a>用于预配计算的“常规用途”服务层级
...
|存储类型|远程 SSD|远程 SSD|远程 SSD|远程 SSD|远程 SSD|远程 SSD|
|IO 延迟（近似）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）
|最大数据 IOPS (64 KB)|3500|4000|4500|5000|7000|7000|
...
|存储类型|远程 SSD|远程 SSD|远程 SSD|远程 SSD|远程 SSD|远程 SSD|远程 SSD|
|IO 延迟（近似）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|
|最大数据 IOPS (64 KB)|7000|7000|7000|7000|7000|7000|7000|
...
## <a name="general-purpose-service-tier-for-serverless-compute"></a>用于无服务器计算的“常规用途”服务层级

[无服务器计算层级](sql-database-serverless.md)目前为预览版。
...
## <a name="business-critical-service-tier-for-provisioned-compute"></a>用于预配计算的“业务关键”服务层级
...
### <a name="gen5-compute-compute-part-1"></a>第 5 代计算（第 1 部分）
...
## <a name="hyperscale-service-tier-for-provisioned-compute"></a>用于预配计算的“超大规模”服务层级
...
|最大数据 IOPS (64 KB)| [注释 1](#note-1) |[注释 1](#note-1)|[注释 1](#note-1) |[注释 1](#note-1) |[注释 1](#note-1) |[注释 1](#note-1) |[注释 1](#note-1) | [注释 1](#note-1) |

---
title: Azure 门户预览：将 Azure SQL 数据库导出到 BACPAC 文件 | Azure
...
本文说明了如何使用 [Azure 门户预览](https://portal.azure.cn)将 Azure SQL 数据库导出到 BACPAC 文件（存储在 Azure Blob 存储中）。如需大致了解如何导出到 BACPAC 文件，请参阅[导出到 BACPAC](./sql-database-export.md)。

> [!NOTE]
也可使用 [SQL Server Management Studio](./sql-database-export-ssms.md)、[PowerShell](./sql-database-export-powershell.md) 或 [SQLPackage](./sql-database-export-sqlpackage.md) 将 Azure SQL 数据库文件导出到 BACPAC 文件。
>
...
3. 确保在导出过程中没有事务处理。
    ...
    > [!IMPORTANT]
    >若要确保获得事务处理一致性 BACPAC 文件，可以[创建数据库的副本](./sql-database-copy.md)，然后从该数据库副本进行导出。
    > 

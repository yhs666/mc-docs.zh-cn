...
ms.date: 08/19/2019
ms.openlocfilehash: 9e4ad4eebf9ca6011ecb2847d69fa12e12a1da27
ms.sourcegitcommit: 52ce0d62ea704b5dd968885523d54a36d5787f2d
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 08/16/2019
ms.locfileid: "69543943"
---
# <a name="monitoring-and-performance-tuning"></a>监视和性能优化

使用 Azure SQL 数据库可以轻松监视使用情况、添加或删除资源 (CPU、内存、I/O)、排查潜在问题，以及查找可提高数据库性能的建议。 若要让数据库适应你的工作负荷并自动优化性能，可以使用 Azure SQL 数据库提供的许多功能来自动修复数据库中的问题。 不过，可能需要排查某些自定义的问题。 本文介绍一些最佳做法，以及可用于排查性能问题的工具。

需要执行两个主要活动才能确保数据库正常运行：
- [监视数据库性能](#monitoring-database-performance)，以确保分配给数据库的资源能够处理工作负荷。 如果你发现即将达到资源限制，则需要识别资源消耗量最大的查询并对其进行优化，或者通过升级服务层来添加更多的资源。
- [排查性能问题](#troubleshoot-performance-issues)，以识别某些潜在问题的原因，并识别问题的根本原因和该问题的解决措施。

## <a name="monitoring-database-performance"></a>监视数据库性能

若要监视 Azure 中的 SQL 数据库的性能，首先需要监视所选数据库性能级别相关的资源利用率。 需要监视以下资源：
 - **CPU 使用率** - 需要检查在较长时间段内是否会达到 100% 的 CPU 使用率。 这可能表示需要升级数据库或实例，或者识别并优化使用大部分计算能力的查询。
 - **等待统计信息** - 需要检查查询为何正在等待某些资源。 查询正在等待提取数据或者将数据保存到数据库文件、由于达到了某些资源限制而发生等待，等等。
 - **IO 使用率** - 需要检查是否即将达到底层存储的 IO 限制。
 - **内存使用率** - 数据库或实例的可用内存量与 vCore 数目成正比；需要检查内存量对于工作负荷而言是否足够。 页生存期是可以指示是否很快从内存中删除页面的参数之一。

Azure SQL 数据库**提供可帮助你排查和解决潜在性能问题的建议**。 可以通过查看[性能优化建议](sql-database-advisor.md)轻松找到机会来改进和优化查询性能，而无需更改资源。 缺少索引与查询优化不足是数据库性能不佳的常见原因。 可以应用这些优化建议来改进你的工作负荷的性能。 还可以通过应用所有已确定的建议并确认它们改进了数据库性能来让 Azure SQL 数据库[自动优化查询性能](sql-database-automatic-tuning.md)。
...
原因或**运行相关**的问题可能包括：
- **编译问题** - 由于使用过时的统计信息、错误估算了要处理的行数，或者错误估算了所需的内存，SQL 查询优化器可能会生成欠佳的计划。 如果你知道该查询在过去或者在其他实例（托管实例或 SQL Server 实例）上的执行速度较快，请采用实际的执行计划并对其进行比较，以找到其差异。 尝试应用查询提示，或者重新生成统计信息或索引，以获得更好的计划。 在 Azure SQL 数据库中启用“自动更正计划”，以自动缓解这些问题。
- **执行问题** - 如果查询计划是最佳的，则原因可能是即将达到数据库中的某些资源限制（例如日志写入吞吐量限制），或者该计划可能使用了应重新生成的碎片化索引。 大量占用资源的并发查询也可能会导致执行问题。 在大多数情况下，**等待**相关的问题都与执行问题有关，因为未有效执行的查询可能正在等待某些资源。
...
- **TempDB 问题** - 如果你正在使用大量的临时表，或者在查询计划中看到大量的 TempDB 溢出，原因可能是 TempDB 吞吐量出现了问题。 
- **内存相关的问题** - 可能没有足够的内存用于工作负荷，因此页生存期可能会缩短，或者查询获得的内存授予量小于所需的量。 在某些情况下，查询优化器中的内置智能可解决这些问题。
...
- **更改了物理设计**。 例如，如果查询优化器认为利用创建的、可更有效地解决查询要求的新索引，比最初选择用于第一个版本的查询执行的数据结构更有利，则可以在新的编译中使用这些新索引。  对引用的对象进行任何物理更改可能会导致在编译时生成新的计划选项。

...
ms.date: 05/20/2019
ms.openlocfilehash: 9baf7270682e568be61b2b054fc9ab483b4e6fee
...
- **计费的资源**：CPU 和内存
- **计费的量 (CNY)**：vCore 单位价格 * 最大值（最小 vCore 数、使用的 vCore 数、最小内存量 (GB) * 1/3、使用的内存量量 (GB) * 1/3） 
- **计费频率**：每秒
...
假设计算单位的价格为 0.000544 CNY/vCore/秒。 然后，可以使用以下公式确定这一个小时内应付的计算费用：0.000544 CNY/vCore/秒 * 1631 vCore 秒 = 0.887264 CNY

...
ms.date: 08/15/2016
ms.author: v-johch
ms.openlocfilehash: 3036b7fd45fc5a3a962853471213000b99753f2e
...
| *类型* |必需。 身份验证的类型。对于 SSL 客户端证书，该值必须是 `ClientCertificate`。 |
...
|:--- |:--- |
| *authentication（父元素）* |用于使用 SSL 客户端证书的身份验证对象。 |
| *类型* |身份验证的类型。 对于 SSL 客户端证书，该值为 `ClientCertificate`。 |
...
| *类型* |必需。 身份验证的类型。 对于基本身份验证，该值必须是 `Basic`。 |

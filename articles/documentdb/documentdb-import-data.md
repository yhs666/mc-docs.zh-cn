...
ms.date: 05/31/2017
ms.author: v-junlch
ms.openlocfilehash: 5b0398a8cf19bb7c0f8fd0b62e23e00ac5f7f1e9
ms.sourcegitcommit: b1d2bd71aaff7020dfb3f7874799e03df3657cd4
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 06/23/2017
---
# 如何将 DocumentDB API 的数据导入 DocumentDB？
<a id="how-to-import-data-into-documentdb-for-the-documentdb-api" class="xliff"></a>
...
仅在将数据导入 DocumentDB 以用于 DocumentDB API 时，数据迁移工具才起作用。 目前不支持将导入的数据用于表 API 或图形 API。 

若要导入用于 MongoDB API 的数据，请参阅 [DocumentDB：如何迁移 MongoDB API 的数据？](documentdb-mongodb-migrate.md)。
...
借助 DocumentDB 批量导入程序，可以使用 DocumentDB 存储的过程从所有可用的源选项中导入，以提高效率。 该工具支持导入到一个单分区 DocumentDB 集合，并支持分片导入，通过这种方法可跨多个单分区 DocumentDB 集合对数据进行分区。 有关数据分区的详细信息，请参阅 [DocumentDB 中的分区和扩展](documentdb-partition-data.md)。 该工具将创建、执行，然后删除目标集合中存储的过程。  
...
指定集合名称后，请选择集合所需的吞吐量（400 RU 到 10,000 RU）。 为了获得最佳导入性能，请选择更高的吞吐量。 有关性能级别的信息信息，请参阅 [DocumentDB中的性能级别](documentdb-performance-levels.md)。
...
借助 DocumentDB 顺序记录导入程序，可以从任何可用的源选项中逐条导入记录。 如果要导入到已达到存储过程配额的现有集合中，可以选择此选项。 该工具支持导入到单个（单分区和多分区）DocumentDB 集合以及分片导入，从而可跨多个单分区和/或多分区 DocumentDB 集合对数据进行分区。 有关数据分区的详细信息，请参阅 [DocumentDB 中的分区和扩展](documentdb-partition-data.md)。
...
指定集合名称后，请选择集合所需的吞吐量（400 RU 到 250,000 RU）。 为了获得最佳导入性能，请选择更高的吞吐量。 有关性能级别的信息信息，请参阅 [DocumentDB中的性能级别](documentdb-performance-levels.md)。 任何导入到吞吐量大于 10,000 RU 的集合的内容都需要分区键。 如果选择拥有超过 250,000 个 RU，则需要在门户中提交一个请求来增加你的帐户。

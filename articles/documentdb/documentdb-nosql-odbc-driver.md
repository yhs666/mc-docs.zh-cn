...
3. 在 Windows 搜索框中键入“ODBC 数据源”，在计算机上打开“ODBC 数据源管理器”应用。 
    ...
    ![Azure Cosmos DB ODBC 数据源管理器](./media/documentdb-nosql-odbc-driver/documentdb-nosql-odbc-driver.png)

## <a id="connect"></a>第 2 步：连接到 Azure Cosmos DB 数据库

1. [安装 Azure Cosmos DB ODBC 驱动程序](#install)后，在“ODBC 数据源管理器”窗口中单击“添加”。 可以创建一个用户 DSN 或系统 DSN。 在本示例中，我们将创建一个用户 DSN。
2. 在“创建新数据源”窗口中选择“Azure Cosmos DB ODBC 驱动程序”，然后单击“完成”。
3. 在“Azure Cosmos DB ODBC 驱动程序 SDN 设置”窗口中填写以下信息： 

    ![“Azure Cosmos DB ODBC 驱动程序 DSN 设置”窗口](./media/documentdb-nosql-odbc-driver/documentdb-nosql-odbc-driver-dsn-setup.png)
    - **数据源名称**：ODBC DSN 的友好名称。 此名称对于 Azure Cosmos DB 帐户是唯一的，因此，如果有多个帐户，请适当地为数据源命名。
    - **说明**：数据源的简短说明。
    - **主机**：你的 Azure Cosmos DB 帐户的 URI。 可通过 Azure 门户中的“Azure Cosmos DB 密钥”边栏选项卡检索此信息，如以下屏幕截图所示。 
    - **访问密钥**：从 Azure 门户中的“Azure Cosmos DB 密钥”边栏选项卡获取的主要或辅助读写/只读密钥，如以下屏幕截图所示。 如果 DSN 用于只读数据的处理和报告，我们建议使用只读密钥。
    ![“Azure Cosmos DB 密钥”边栏选项卡](./media/documentdb-nosql-odbc-driver/documentdb-nosql-odbc-driver-keys.png)
...
6. 完成并关闭“Azure Cosmos DB ODBC 驱动程序 DSN 设置”窗口后，新的用户 DSN 会添加到“用户 DSN”选项卡。

    ![“用户 DSN”选项卡上的新 Azure Cosmos DB ODBC DSN](./media/documentdb-nosql-odbc-driver/documentdb-nosql-odbc-driver-user-dsn.png)
...
1. 完成[连接到 Azure Cosmos DB 数据库](#connect)中的步骤 1-4 后，请在“Azure Cosmos DB ODBC 驱动程序 DSN 设置”窗口中单击“架构编辑器”。
    ...
    ![“Azure Cosmos DB ODBC 驱动程序 DSN 设置”窗口中的架构编辑器按钮](./media/documentdb-nosql-odbc-driver/documentdb-nosql-odbc-driver-schema-editor.png)
2. 在“架构编辑器”窗口中单击“新建”。
...
5. 完成定义架构后，请单击“文件” | “保存”，导航到用于保存该架构的目录，然后单击“保存”。
    ...
    以后如果想要配合 DSN 使用此架构，请打开“Azure Cosmos DB ODBC 驱动程序 DSN 设置”窗口（通过“ODBC 数据源管理器”），单击“高级选项”，然后在“架构文件”框中导航到保存的架构。 将架构文件保存到现有 DSN 会将 DSN 连接范围修改为架构定义的数据和结构。
...
2. 在“编辑视图”窗口中，输入一个 Azure Cosmos DB 查询。 这必须是 Azure Cosmos DB SQL 查询（例如 `SELECT c.City, c.EmployeeName, c.Level, c.Age, c.Gender, c.Manager FROM c WHERE c.City = “Seattle”`），然后单击“确定”。
...
若要了解有关 Azure Cosmos DB 的详细信息，请参阅[什么是 Azure Cosmos DB？](documentdb-introduction.md)。

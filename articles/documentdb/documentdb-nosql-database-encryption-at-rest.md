...
description: "了解 Azure Cosmos DB 如何为所有数据提供默认加密。"
services: cosmosdb
author: voellm
manager: jhubbard
editor: mimig
...
“静态加密”这个短语通常指非易失性存储设备（例如固态硬盘 (SSD) 和硬盘 (HDD)）上的数据的加密。  Azure Cosmos DB 将其主数据库存储在 SSD 中，将其媒体附件和备份存储在通常由 HDD 支持的 Azure Blob 中。  随着 Azure Cosmos DB 的静态加密功能的发布，现在，所有数据库、媒体附件以及备份都是加密的。  这意味着，现在数据在传输中（通过网络）以及在静态时（非易失性存储）都处于加密状态，从而实现端到端加密。

Azure Cosmos DB 是一项 PaaS 服务，我们一直在努力使其变得非常易于使用。  秉承着这一理念，我们已使 Azure Cosmos DB 中存储的所有用户数据在静态时和传输中都处于加密状态，不需要用户执行任何操作。  实现这一理念的另一方式是默认“开启”静态加密。  没有开启或关闭此功能的控制。我们在提供此功能的同时，会继续满足可用性和性能 SLA 方面的要求。
...
答：Azure Cosmos DB 是一项 PaaS 服务，我们一直在努力使该服务易于使用。  我们注意到此问题经常被作为一个与满足符合性（例如 PCI-DSS）相关的代理问题问起。  在构建此功能时，我们一直与符合性审核机构合作，确保使用 Azure Cosmos DB 的客户满足相关要求，不需要客户自己管理密钥。
...
答：模拟器是一项独立的开发/测试工具，不使用托管 DocumentDB 服务使用的密钥管理服务。 建议在要存储敏感的模拟器测试数据的驱动器上启用 BitLocker。 (模拟器支持更改默认的数据目录)[https://docs.microsoft.com/azure/documentdb/documentdb-nosql-local-emulator]以及使用已知的位置。

## <a name="next-steps"></a>后续步骤

有关 Azure Cosmos DB 安全性和最新改进的概述，请参阅 [Azure Cosmos DB 数据库安全性](documentdb-nosql-database-security.md)。

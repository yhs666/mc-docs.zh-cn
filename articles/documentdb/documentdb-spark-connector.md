...
description: "使用本教程来了解 Azure Cosmos DB Spark 连接器：使用该连接器可将 Apache Spark 连接到 Azure Cosmos DB，以便在针对云设计的 Microsoft 多租户全局分布式的数据库系统中执行分布式聚合和数据科学。"
...
Spark 到 Azure Cosmos DB 的连接器能使 Cosmos DB 充当 Apache Spark 作业的输入源或输出接收器。 将 [Spark](http://spark.apache.org/) 连接到 [Azure Cosmos DB](https://azure.microsoft.com/services/documentdb/) 后，可以更快地解决瞬息万变的数据科学问题，使用 Cosmos DB 快速保存和查询数据。 Spark 到 Azure Cosmos DB 的连接器有效利用本机 Cosmos DB 托管的索引，可在针对 IoT、数据科学和分析方案等各种快速变化的全局分布式数据执行分析、下推谓词筛选时启用可更新的列。 

## <a name="download"></a>下载

首先，请从 GitHub 上的 [azure-documentdb-spark](https://github.com/Azure/azure-documentdb-spark/) 存储库下载 Spark 到 Azure Cosmos DB 的连接器（预览版）。

## <a name="connector-components"></a>连接器组件

连接器利用以下组件：

- [Azure Cosmos DB](http://documentdb.com)：专门为云设计的 Microsoft 多租户[全局分布式数据库系统](documentdb-distribute-data-globally.md)。 使用 Azure Cosmos DB，客户可跨数目不限的地理区域弹性缩放吞吐量与存储。 该服务向开发员提供 99% 的低延迟、99.99% 的高可用性保证，以及[多个完善定义的一致性模型](documentdb-consistency-levels.md)。

- [Apache Spark](http://spark.apache.org/)：专为速度、易用性和复杂分析打造的强大开源处理引擎。 

- [Azure HDInsight 上的 Apache Spark](../hdinsight/hdinsight-apache-spark-jupyter-spark-sql.md) 可以使用 [Azure HDInsight](https://azure.microsoft.com/services/hdinsight/apache-spark/) 在云中为任务关键型部署方案部署 Apache Spark。
...
![通过 pyCosmos DB 的 Spark 到 Cosmos DB 数据流。](./media/documentdb-spark-connector/azure-documentdb-spark-pydocumentdb.png)
...
1. 通过 pyCosmos DB 建立从 Spark 主节点到 Cosmos DB 网关节点的连接。  请注意，用户只需指定 Spark 和 Cosmos DB 连接，连接到相应主节点和网关节点的过程对用户是透明的。
2. 针对 Cosmos DB 执行查询（通过网关节点），然后，查询针对数据节点中的集合分区运行查询。 这些查询的响应将发回到网关节点，结果集将返回到 Spark 主节点。
3. 所有后续查询（例如，针对 Spark 数据框架执行的查询）将发送到 Spark 工作节点进行处理。

要点在于，Spark 与 Cosmos DB 之间的通信仅限于 Spark 主节点和 Cosmos DB 网关节点。  查询的速度与这两个节点之间的传输层速度相同。
...
### <a name="connecting-spark-to-cosmos-db-via-pycosmos-db"></a>通过 pyCosmos DB 将 Spark 连接到 Cosmos DB 
由于通信传输的简单性，使用 pyCosmos DB 执行从 Spark 到 Cosmos DB 的查询相对简单。
...
以下示例使用前面代码片段中通过指定的只读密钥创建的 Cosmos DB 实例。  以下代码片段连接到 **airports.codes** 集合（前面指定的 DoctorWho 帐户），并运行一个查询来提取华盛顿州的机场城市信息。 
...
在以下场合下，通常使用 pyCosmos DB 将 Spark 连接到 Cosmos DB：
...
Spark 到 Cosmos DB 的连接器利用 [Azure Cosmos DB Java SDK](https://github.com/Azure/azure-documentdb-java)，并在 Spark 辅助角色节点与 Cosmos DB 之间移动数据，如下图所示：

![Spark 到 Cosmos DB 的连接器中的数据流](./media/documentdb-spark-connector/azure-documentdb-spark-connector.png)

数据流如下所示：

1. 建立从 Spark 主节点到 Cosmos DB 网关节点的连接，以获取分区映射。  请注意，用户只需指定 Spark 和 Cosmos DB 连接，连接到相应主节点和网关节点的过程对用户是透明的。
2. 此信息将返回给 Spark 主节点。  此时，你应该能够分析查询，确定需要访问 Cosmos DB 中的哪些分区（及其位置）。
3. 此信息将传送到 Spark 工作节点 ...
4. 这将允许 Spark 工作节点直接连接到 Cosmos DB 分区以提取所需的数据，并将数据传送回到 Spark 辅助角色节点中的 Spark 分区。

要点在于，Spark 与 Cosmos DB 之间的通信速度会大幅提高，因为数据移动在 Spark 辅助角色节点与 Cosmos DB 数据节点（分区）之间发生。

### <a name="building-the-spark-to-cosmos-db-connector"></a>构建 Spark 到 Cosmos DB 的连接器
...
### <a name="connecting-spark-to-cosmos-db-using-the-connector"></a>使用连接器将 Spark 连接到 Cosmos DB
...
在以下场合下，通常使用连接器将 Spark 连接到 Cosmos DB：
...
本部分举例说明如何结合使用 Apache Spark 和 Azure Cosmos DB 来执行分布式聚合与分析。  请注意，如 [Planet scale aggregates with Azure Cosmos DB blog](https://azure.microsoft.com/blog/planet-scale-aggregates-with-azure-documentdb/)（使用 Azure Cosmos DB 实现全球级聚合）博客文章中所述，Azure Cosmos DB 已能支持聚合，下面介绍了如何将它与 Apache Spark 配合使用，以进一步发挥它的优势。

请注意，这些聚合与 [Spark 到 Cosmos DB 的连接器 Notebook](https://github.com/Azure/azure-documentdb-spark/blob/master/samples/notebooks/Spark-to-DocumentDB_Connector.ipynb) 相关。

### <a name="connecting-to-flights-sample-data"></a>连接到航班示例数据
在这些聚合示例中，我们要访问 **DoctorWho** Cosmos DB 数据库中存储的一些航班绩效数据。  若要连接到这些数据，需要利用以下代码片段：
...
我们还要使用此代码片段运行一个基本查询，以便将 Cosmos DB 中我们需要的筛选数据集传输到 Spark（后者可以执行分布式聚合）。  在本例中，我们要查询从西雅图 (SEA) 出发的航班。
...
从 [azure-documentdb-spark](https://github.com/Azure/azure-documentdb-spark) GitHub 存储库下载 Spark 到 Cosmos DB 的连接器（如果尚未下载），并浏览该存储库中的其他资源：

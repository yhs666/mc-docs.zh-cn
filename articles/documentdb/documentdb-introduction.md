...
使用  [Azure DocumentDB Emulator](./documentdb-nosql-local-emulator.md) 可在本地开发和测试应用程序，无需创建 Azure 订阅且不会产生任何费用。 如果对应用程序在 DocumentDB 模拟器中的工作情况感到满意，则可以切换到在云中使用 Azure DocumentDB 帐户。
...
已有 Azure 帐户？ 那么可以在 [Azure 门户](https://portal.azure.cn/#gallery/Microsoft.DocumentDB)中通过[创建 DocumentDB 数据库帐户](./documentdb-create-account.md)开始使用 DocumentDB。

还没有 Azure 帐户？ 你可以：

- 注册 [Azure 试用版](https://www.azure.cn/pricing/1rmb-trial/)，以获取 30 天试用期和 200 美元，可试用所有 Azure 服务。 

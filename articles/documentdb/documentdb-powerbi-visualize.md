...
ms.date: 05/31/2017
ms.author: v-junlch
ms.openlocfilehash: 16aa05f893d2de4b1debb3f3b2c4f4aad294c708
...
<a id="power-bi-tutorial-for-documentdb-visualize-data-using-the-power-bi-connector" class="xliff"></a>
...
- 在 DocumentDB 帐户中访问我们的演示帐户或数据。
  ...
  - 或者，若要创建自己的帐户，请参阅[使用 Azure 门户创建 DocumentDB 数据库帐户](./documentdb-create-account.md)。 然后，若要获取类似于本教程中所使用项的示例火山数据（但不包含 GeoJSON 块），请参阅 [NOAA 站点](https://www.ngdc.noaa.gov/nndc/struts/form?t=102557&s=5&d=5)，然后使用 [DocumentDB 数据迁移工具](./documentdb-import-data.md)导入数据。
...
6. 如下所示指定要从其中检索数据的 DocumentDB 帐户终结点 URL，然后单击“确定”。 可以在 Azure 门户的“密钥”边栏选项卡中的 URI 框检索 URL，或者可以使用演示帐户信息，其中 URL 为 `https://analytics.documents.azure.cn`。**[](documentdb-manage-account.md#keys)** 
...
11. 此时我们已经完成了将数据平展为表格格式。  可以利用查询编辑器中所有可用的功能在 DocumentDB 中形成和转换数据。  如果正在使用示例，可以通过在“主页”功能区上更改“数据类型”，将“海拔”数据类型更改为“整数”。
...
- 若要了解有关 DocumentDB 的详细信息，请参阅 [DocumentDB 文档登录页](./index.md)。

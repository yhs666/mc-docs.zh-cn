...
keywords: "最终一致性, azure cosmos db, azure, Azure"
services: cosmosdb
author: syamkmsft
manager: jhubbard
editor: cgronlun
...
Azure Cosmos DB 是从无到有开发出来的，其设计考虑到了每个数据模型的全局分发。 它旨在提供可预测的低延迟保证、99.99% 的可用性 SLA，以及多个定义完好的宽松一致性模型。 目前，Azure Cosmos DB 提供五种一致性级别：非常一致性、有限过期一致性、会话一致性和最终级别。 
...
![Azure Cosmos DB 提供了多个妥善定义的（宽松）一致性模型供你选择](./media/documentdb-consistency-levels/five-consistency-levels.png)

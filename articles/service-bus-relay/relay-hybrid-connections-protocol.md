...
ms.author: v-yiso
ms.date: 11/06/2017
ms.openlocfilehash: b3773fbed122fb33627342cc8996fa68285ccafa
...
该 URL 可以且必须直接由侦听器使用，无需其他作业。编码信息仅在短时间内有效，基本上为发送方愿意等待的端到端创建连接的时间，但是最长为 30 秒。 该 URL 只能用于一次成功的连接尝试。 使用集合 URL 建立 Web 套接字连接后，该 Web 套接字上的所有后续活动都会从发送方中继并中继到该发送方，无需服务任何干预或转译。
...
在检查“接受”消息后拒绝套接字，需要一个相似的握手，以使传达拒绝原因的状态代码和状态说明可以返回至发送方。

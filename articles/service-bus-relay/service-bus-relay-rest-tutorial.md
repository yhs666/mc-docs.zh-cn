...
ms.date: 02/17/2017
ms.author: v-yiso
ms.openlocfilehash: 7fd929918364a899a3c4e6afda29db76f17a68dc
ms.sourcegitcommit: 6728c686935e3cdfaa93a7a364b959ab2ebad361
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 06/21/2017
---
<a id="azure-wcf-relay-rest-tutorial" class="xliff"></a>

# Azure WCF 中继 REST 教程
...
<a id="step-1-create-a-service-namespace" class="xliff"></a>

## 步骤 1：创建服务命名空间

若要开始在 Azure 中使用中继功能，必须先创建一个服务命名空间。 命名空间提供了用于对应用程序中的 Azure 资源进行寻址的范围容器。 请按照[此处的说明](./relay-create-namespace-portal.md)创建中继命名空间。

<a id="step-2-define-a-rest-based-wcf-service-contract-to-use-with-azure-relay" class="xliff"></a>

## 步骤 2：定义基于 REST 的 WCF 服务协定用于 Azure 中继
...
<a id="to-create-a-contract-with-an-interface" class="xliff"></a>
...
<a id="step-3-implement-a-rest-based-wcf-service-contract-to-use-service-bus" class="xliff"></a>
...
<a id="to-implement-a-rest-style-service-bus-contract" class="xliff"></a>
...
<a id="to-define-the-configuration-for-running-the-web-service-on-service-bus" class="xliff"></a>

### 定义配置以便在服务总线上运行 Web 服务
1. 在“解决方案资源管理器”中，双击“App.config”文件以在 Visual Studio 编辑器中将其打开。
   
    **App.config** 文件包括服务名称、终结点（即，Azure 中继公开的、让客户端和主机相互通信的位置）和绑定（用于通信的协议类型）。 此处的主要差别在于，配置的服务终结点是指 [WebHttpRelayBinding](https://doc.microsoft.com/dotnet/api/microsoft.servicebus.webhttprelaybinding) 绑定。
2. `<system.serviceModel>` XML 元素是一个 WCF 元素，用于定义一个或多个服务。 在这里，它用于定义服务名称和终结点。 在 `<system.serviceModel>` 元素的下面（仍在 `<system.serviceModel>` 中）添加具有以下内容的 `<bindings>` 元素。 这样就定义了应用程序中使用的绑定。 你可以定义多个绑定，但在本教程中，你只要定义一个绑定。
    ...
    前面的代码定义了一个 WCF 中继 [WebHttpRelayBinding](https://doc.microsoft.com/dotnet/api/microsoft.servicebus.webhttprelaybinding) 绑定，其中“relayClientAuthenticationType” 设置为“None”。 此设置表明使用此绑定的终结点将不需要客户端凭据。
...
<a id="step-4-host-the-rest-based-wcf-service-to-use-azure-relay" class="xliff"></a>

## 步骤 4：托管基于 REST 的 WCF 服务以使用 Azure 中继
...
<a id="to-create-and-configure-the-web-service-host" class="xliff"></a>
...
<a id="to-run-the-web-service-host" class="xliff"></a>

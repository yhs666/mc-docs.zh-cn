...
你可以使用 PowerShell 命令（可在[此处](../service-bus-messaging/service-bus-manage-with-ps.md)的文章中找到）将命名空间从某个 Azure 订阅移到另一个 Azure 订阅。 若要执行此操作，命名空间必须已处于活动状态。 此外，执行这些命令的用户必须是源订阅和目标订阅的管理员。

...
ms.custom: it-pro
ms.openlocfilehash: 9b0e73b870c67cf9eb681451f06a90ea37645728
...
Azure 基于角色的访问控制 (RBAC) 附带以下可分配到用户、组和服务的内置角色。 不能修改内置角色的定义。 但是，可以创建 [Azure RBAC 中的自定义角色](custom-roles.md)，以满足组织的特定需要。
...
| [监视参与者](#monitoring-contributor) | 可以读取所有监视数据和编辑监视设置。 |
| [监视读取者](#monitoring-reader) | 可以读取所有监视数据（指标、日志等）。 |
...
| [安全管理器](#security-manager) | 允许管理安全性组件、安全策略和虚拟机 |
...
- [基于角色的访问控制故障排除](troubleshooting.md)：获取解决常见问题的建议。

...
ms.date: 10/30/2016
ms.author: v-yiso
ms.openlocfilehash: b8f6cef905acdb59695b6bfaf9b6230764b33cd9
...
* 增量同步：拉取操作的第一个参数是 query name，此参数只在客户端上使用。 如果使用非 null 查询名称，Azure Mobile SDK 将执行增量同步 。
  每次拉取操作返回结果集时，该结果集中最新的 `updatedAt` 时间戳将存储在 SDK 本地系统表中。 后续拉取操作只检索该时间戳以后的记录。
...
[Xamarin Android: Enable offline sync（Xamarin Android：启用脱机同步）]: ./app-service-mobile-xamarin-ios-get-started-offline-data.md

...
ms.date: 09/26/2016
ms.author: v-johch
ms.openlocfilehash: 46d8fd643aae1073d3af289bc2b486adb85f909b
...
3. 将后续 API 调用发布到新的 URL。 
    ...
    应该将后续 API 调用发布到 https://wamsbayclus001rest-hs.chinacloudapi.cn/api/。

##<a name="access-control-address"></a>访问控制地址

媒体服务访问控制地址是 https://wamsprodglobal001acs.accesscontrol.windows.net，中国北部地区除外的地区的地址是 https://wamsprodglobal001acs.accesscontrol.chinacloudapi.cn。
...
需要在此请求的正文中提供 client_id 和 client_secret 值；client_id 和 client_secret 分别对应于 AccountName 和 AccountKey 值。 在你设置帐户时，媒体服务将向你提供这些值。 

请注意，媒体服务帐户的 AccountKey 必须进行 URL 编码（当用作访问令牌请求中的 client_secret 值时，请参阅[百分号编码](http://tools.ietf.org/html/rfc3986#section-2.1)）。
...
>[!NOTE]
建议在外部存储中高速缓存“access_token”和“expires_in”值。 以后可以从存储中检索令牌数据，并在媒体服务 REST API 调用中重新使用。 这对于令牌可以在多个进程或多台计算机之间安全共享的方案尤其有用。
...
请注意，用于上传和下载资产文件的根 URI 为 https://yourstorageaccount.blob.core.chinacloudapi.cn/，其中的存储帐户名为在媒体服务帐户设置期间使用的同一帐户名。

以下示例演示了对媒体服务根 URI (https://media.chinacloudapi.cn/) 发出的 HTTP 请求。 该请求将在响应中获取 301 重定向。 后续请求是使用新的 URI (https://wamsbayclus001rest-hs.chinacloudapi.cn/api/)。     

...
ms.date: 12/11/2017
ms.author: v-nany
ms.openlocfilehash: 549e1d3cb9c62a87c3cec38f69478aa903e2ea02
...
当播放器请求流时，媒体服务使用指定的密钥通过 AES 或 PlayReady 加密动态加密你的内容。 为了解密流，播放器将从密钥传送服务请求密钥。 为了确定用户是否有权获取密钥，服务会评估为密钥指定的授权策略。
...
媒体服务不提供安全令牌服务。 可以创建自定义 STS 或使用 Azure Active Directory (AAD) 颁发令牌。 必须将 STS 配置为创建令牌，该令牌使用指定密钥以及你在令牌限制配置中指定的颁发声明进行签名（如本文所述）。 如果令牌有效，并且令牌中的声明与为内容密钥配置的声明相匹配，则媒体服务密钥传送服务会将加密密钥返回到客户端。

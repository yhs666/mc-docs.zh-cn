...
>[!NOTE]
> 目前，实时事件的最大建议持续时间为 8 小时。请注意，实时编码会影响计费，应记住，将实时编码通道保持为“正在运行”状态会产生费用。建议在实时流式处理事件完成之后立即停止正在运行的通道，以避免产生额外的小时费用。
...
[使用门户创建通道以执行从单比特率到自适应比特率流的实时编码](./media-services-portal-creating-live-encoder-enabled-channel.md)

[使用 .NET SDK 创建通道以执行从单比特率到自适应比特率流的实时编码](./media-services-dotnet-creating-live-encoder-enabled-channel.md)

...
> [!NOTE]
> 这些说明引用 Istio 版本 `1.3.2`，并至少使用 Helm 版本 `2.14.2`。
>
> Istio 团队已针对 Kubernetes 版本 `1.13`、`1.14` 和 `1.15` 测试了 `1.3.x` 版本。 可以在 [GitHub - Istio 版本][istio-github-releases]中找到其他 Istio 版本，在 [Istio 新闻][istio-release-notes]中找到有关每个版本的信息，在[一般的 Istio 常见问题解答][istio-faq]中找到支持的 Kubernetes 版本。
...
需要使用 [Helm][helm] 按照这些说明安装 Istio。 建议在群集中正确安装和配置最新的稳定版本。 安装 Helm 时如需帮助，请参阅 [AKS Helm 安装指南][helm-install]。 所有 Istio Pod 也必须按计划在 Linux 节点上运行。
...
为 Istio 版本添加 Istio Helm 图表存储库。 确保运行 `helm repo update`，以便更新图表存储库的本地信息。
...
[作业][kubernetes-jobs]将部署为 `istio-init` Helm 图表的一部分用于安装 CRD。 这些作业应可在 20 秒内完成，具体取决于群集环境。 可按如下所示验证作业是否已成功完成：
...
确认成功完成作业后，让我们验证是否安装了正确数目的 Istio CRD。 可以通过运行以下命令来验证是否安装了所有 23 个 Istio CRD。 该命令应返回数字 `23`。
...
应有三个状态为 `Completed` 的 `istio-init-crd-*` pod。 这些 pod 负责运行在前面步骤中创建 CRD 的作业。 其他所有 pod 应显示 `Running` 状态。 如果 Pod 没有这些状态，请在运行之前等待一两分钟。 如果任何 Pod 报告问题，请使用 [kubectl describe pod][kubectl-describe] 命令查看其输出和状态。
...
上述命令删除所有 Istio 组件和命名空间，但我们仍保留了 Istio CRD 和机密。 
...
<!--Update_Description: new articles on servicemesh istio install -->
<!--New.date: 11/04/2019-->
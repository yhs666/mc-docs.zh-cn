...
若要管理 Kubernetes 群集，请使用 Kubernetes 命令行客户端 [kubectl][kubectl]。 `kubectl` 客户端已预装在 Azure 本地 Shell 中。

使用 Azure 门户右上角的按钮打开 Cloud Shell。

![在门户中打开 Azure 本地 Shell](media/kubernetes-walkthrough-portal/aks-cloud-shell.png)
...
创建名为 `azure-vote.yaml` 的文件，并将其复制到以下 YAML 定义中。 在 Azure 本地 Shell 中，使用 `vi` 或 `Nano` 来创建此文件，就像在虚拟或物理系统中操作一样：

...
ms.topic: article
ms.date: 06/16/2018
ms.openlocfilehash: 04b8eefd8d5ad6c7a0d1db18e1626b2f6bc22e39
ms.sourcegitcommit: 044f3fc3e5db32f863f9e6fe1f1257c745cbb928
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 06/20/2018
ms.locfileid: "36270091"
---
...
请从 [DigiCert 官方网站](https://www.digicert.com/CACerts/DigiCertGlobalRootCA.crt)下载通过 SSL 与 Azure Database for MySQL 服务器通信所需的证书。

## <a name="step-2-download-and-install-openssl"></a>步骤 2：下载并安装 OpenSSL

转到 [Win32 OpenSSL](http://slproweb.com/products/Win32OpenSSL.html) 页，搜索并下载最新版 OpenSSL。

## <a name="step-3-move-the-local-certificate-file-to-the-openssl-directory"></a>步骤 3：将本地证书文件移到 OpenSSL 目录

将在步骤 1 中下载的证书置于 <...\OpenSSL-Win32\bin> 目录。

## <a name="step-4-convert-the-certificate-file-to-pem-format"></a>步骤 4：将证书文件转换为 .pem 格式

下载的根证书文件为 .cer 格式，必须进行转换，方法是使用命令行工具 openssl.exe 来执行以下命令：
...
```dos
mysql.exe -h mydemoserver.database.chinacloudapi.cn -u Username@mydemoserver -p --ssl-ca=c:\ssl\DigiCertGlobalRootCA.pem
```
...
```php
$conn = mysqli_init();
mysqli_ssl_set($conn,NULL,NULL, "/var/www/html/DigiCertGlobalRootCA.pem", NULL, NULL) ; 
mysqli_real_connect($conn, 'mydemoserver.database.chinacloudapi.cn', 'myadmin@mydemoserver', 'yourpassword', 'quickstartdb', 3306, MYSQLI_CLIENT_SSL, MYSQLI_CLIENT_SSL_DONT_VERIFY_SERVER_CERT);
...
try:
    conn=mysql.connector.connect(user='myadmin@mydemoserver', 
        ...
        ssl_ca='/var/www/html/DigiCertGlobalRootCA.pem')
...
conn = pymysql.connect(user = 'myadmin@mydemoserver', 
        ...
        database = 'quickstartdb', 
        host = 'mydemoserver.database.chinacloudapi.cn', 
        ssl = {'ssl': {'ca': '/var/www/html/DigiCertGlobalRootCA.pem'}})
```
### <a name="ruby"></a>Ruby
```ruby
...
pem, _ := ioutil.ReadFile("/var/www/html/DigiCertGlobalRootCA.pem")
if ok := rootCertPool.AppendCertsFromPEM(pem); !ok {
    log.Fatal("Failed to append PEM.")
}
mysql.RegisterTLSConfig("custom", &tls.Config{RootCAs: rootCertPool, InsecureSkipVerify: true})
var connectionString string
connectionString = fmt.Sprintf("%s:%s@tcp(%s:3306)/%s?allowNativePasswords=true&tls=custom",'myadmin@mydemoserver' , 'yourpassword', 'mydemoserver.database.chinacloudapi.cn', 'quickstartdb')  
...
url = String.format("jdbc:mysql://%s/%s?serverTimezone=UTC&useSSL=true", 'mydemoserver.database.chinacloudapi.cn', 'quickstartdb');
...
url = String.format("jdbc:mariadb://%s/%s?useSSL=true&trustServerCertificate=true", 'mydemoserver.database.chinacloudapi.cn', 'quickstartdb');
properties.setProperty("user", 'myadmin@mydemoserver');
properties.setProperty("password", 'yourpassword');
conn = DriverManager.getConnection(url, properties);
```

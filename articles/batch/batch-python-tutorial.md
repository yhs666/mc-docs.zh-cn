...
description: "了解 Azure 批处理的基本概念并使用 Python 构建简单的解决方案。"
services: batch
...
---
# <a name="get-started-with-the-batch-sdk-for-python"></a>适用于 Python 的批处理 SDK 入门
...
在介绍以 Python 编写的小型批处理应用程序时，我们了解了 [Azure 批处理][azure_batch]和[批处理 Python][py_azure_sdk] 客户端的基础知识。 我们将探讨两个示例脚本如何使用批处理服务来处理云中 Linux 虚拟机上的并行工作负荷，以及这些脚本如何与 [Azure 存储](../storage/storage-introduction.md)交互来暂存和检索文件。 你将了解常见的批处理应用程序工作流，并基本了解批处理的主要组件，例如作业、任务、池和计算节点。

![批处理解决方案工作流（基础）][11]<br/>

## <a name="prerequisites"></a>先决条件
本文假设你有 Python 的实践知识，并熟悉 Linux。 本文还假定，你能够满足下面为 Azure 和批处理及存储服务指定的帐户创建要求。
...
## <a name="batch-python-tutorial-code-sample"></a>批处理 Python 教程代码示例
批处理 Python 教程代码示例由两个 Python 脚本和若干数据文件组成。
...
下图演示了客户端和任务脚本执行的主要操作。 此基本工作流是通过批处理创建的许多计算解决方案中常见的工作流。 尽管它并未演示批处理服务提供的每项功能，但几乎每个批处理方案都包含此工作流的某些部分。
...
如前所述，并非每个批处理解决方案都会执行这些具体步骤，此类方案可能包含更多步骤，但本示例将演示批处理方案中的常见过程。
...
在以下部分中，我们将分析脚本处理批处理服务中的工作负荷使用的步骤。 建议你在执行本文余下部分所述的步骤时，经常在编辑器中查看脚本。
...
批处理包含的内置支持支持与 Azure 存储交互。 存储帐户中的容器将为批处理帐户中运行的任务提供所需的文件。 这些容器还提供存储任务生成的输出数据所需的位置。 *python_tutorial_client.py* 脚本执行的第一个操作是在 [Azure Blob 存储](../storage/storage-introduction.md#blob-storage)中创建三个容器：
...
> [!TIP]
> [如何通过 Python 使用 Azure Blob 存储](../storage/storage-python-how-to-use-blob-storage.md)对如何使用 Azure 存储容器和 Blob 做了全面的概述。 当你开始使用批处理时，它应该位于阅读列表顶部附近。
>
>
...
在文件上传操作中，*python_tutorial_client.py* 先定义**应用程序**和**输入**文件在本地计算机上的路径的集合， 然后将这些文件上传到上一步骤创建的容器。
...
[ResourceFile][py_resource_file] 提供批处理中的任务，以及 Azure 存储中将在任务运行之前下载到计算节点的文件的 URL。 [ResourceFile][py_resource_file].**blob_source** 属性指定存在于 Azure 存储的文件的完整 URL。 该 URL 还可以包含用于对文件进行安全访问的共享访问签名 (SAS)。 批处理中的大多数任务类型都包含 *ResourceFiles* 属性，这些类型包括：
...
本示例不使用 JobPreparationTask 或 JobReleaseTask 任务类型，但可以通过 [在 Azure 批处理计算节点上运行作业准备和完成任务](batch-job-prep-release.md)来详细了解这些任务类型。
...
然后，调用 `create_pool` 以在批处理帐户中创建计算节点池。
...
- 池的 **ID**（*id* - 必需）<p/>与批处理中的大多数实体一样，新池在批处理帐户中必须具有唯一 ID。 代码将使用池 ID 引用此池，这也是在 Azure [门户][azure_portal]中识别池的方式。
...
- **计算节点的大小**（*vm_size* - 必需）<p/>由于我们要为 [VirtualMachineConfiguration][py_vm_config] 指定 Linux 节点，因此应根据 [Azure 中虚拟机的大小](../virtual-machines/virtual-machines-linux-sizes.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json/)指定 VM 大小（在本示例中为 `STANDARD_A1`）。 同样，请参阅[在 Azure 批处理池中预配 Linux 计算节点](batch-linux-nodes.md)获取详细信息。
...
此外，在上述代码片段中，值得注意的问题是，StartTask 的 **command_line** 属性中使用了两个环境变量：`AZ_BATCH_TASK_WORKING_DIR` 和 `AZ_BATCH_NODE_SHARED_DIR`。 将自动为批处理池中的每个计算节点配置多个特定于批处理的环境变量。 由任务执行的任何进程都可以访问这些环境变量。
...
不仅可以使用作业来组织和跟踪相关工作负荷中的任务，还可以使用它来实施特定的约束，例如作业（并扩展到其任务）的最大运行时，以及批处理帐户中其他作业的相关作业优先级。 不过在本示例中，该作业仅与步骤 3 中创建的池关联。 未配置任何其他属性。

所有批处理作业都与特定的池关联。 此关联指示将要在其上执行作业任务的节点。 可以通过 [PoolInformation][py_poolinfo] 属性指定池，如以下代码片段所示。
...
由于你需要对位于 Azure 存储中的数据付费，因此我们建议删除批处理作业不再需要的所有 Blob。 在 *python_tutorial_client.py* 中，可通过调用 [BlockBlobService.delete_container][py_delete_container] 三次来实现此目的：

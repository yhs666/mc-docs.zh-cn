...
2. 在 Batch 帐户中创建计算节点的 Batch **池** - 这些节点是将执行任务的虚拟机。 需要指定属性，例如[节点大小](../cloud-services/cloud-services-sizes-specs.md)、其操作系统，以及节点加入池时要安装的应用程序在 Azure 存储中的位置（在步骤 1 中上载的应用程序）。 用户还可以配置池，使之随任务生成的工作负荷[自动缩放](batch-automatic-scaling.md)。 自动缩放功能可动态调整池中计算节点的数目。

...
- **批处理帐户** - Azure Batch 资源（包括池、计算节点、作业和任务）都与 Azure 批处理帐户关联。 如果应用程序对 Batch 服务发出请求，它将使用 Azure 批处理帐户名称、帐户 URL 和访问密钥对请求进行身份验证。 可以在 Azure 门户中 [创建 Batch 帐户](batch-account-create-portal.md) 。

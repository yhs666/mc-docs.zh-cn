...
4. 单击“添加”添加新的所需权限。 
5. 在步骤 1 中输入 **Azure Service Management API**，从结果列表中选择该 API，然后单击“选择”按钮。
...
在批处理应用程序中通过 Azure AD 进行身份验证之前，需要将应用程序注册到 Azure AD，并授予对批处理服务 API 的访问权限。 批处理服务 API 的 Azure AD 标识符为“Azure 批处理(MicrosoftAzureBatch)”。
...
4. 在步骤 1 中搜索 **MicrosoftAzureBatch**，选择“Azure 批处理(MicrosoftAzureBatch)”，然后单击“选择”按钮。

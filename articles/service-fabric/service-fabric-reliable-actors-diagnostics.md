...
ms.date: 06/15/2017
ms.author: v-johch
ms.openlocfilehash: 7861e7dd9ef71e15132c41a1eee2ca0cd8a924f3
...
Reliable Actors 运行时发出 [EventSource](https://msdn.microsoft.com/zh-cn/library/system.diagnostics.tracing.eventsource.aspx) 事件和[性能计数器](https://msdn.microsoft.com/zh-cn/library/system.diagnostics.performancecounter.aspx)。这些有助于深入了解运行时的运行状况以及进行故障排除和性能监视。
...
|0x2|描述执行组件方法调用的事件集。 有关详细信息，请参阅[执行组件主题简介](./service-fabric-reliable-actors-introduction.md#actors)。|
...
默认情况下在 Windows 操作系统中提供的 [Windows 性能监视器](https://technet.microsoft.com/zh-cn/library/cc749249.aspx) 应用程序可用于收集和查看性能计数器数据。 [Azure 诊断](../cloud-services/cloud-services-dotnet-diagnostics.md)是用于收集性能计数器数据并将其上传到 Azure 表的另一个选项。
...
Reliable Actors 运行时发出以下与[执行组件副本](./service-fabric-reliable-actors-platform.md#service-fabric-partition-concepts-for-stateful-actors)相关的事件。

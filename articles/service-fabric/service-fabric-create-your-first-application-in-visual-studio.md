...
ms.date: 08/14/2017
ms.author: v-yeche
ms.openlocfilehash: b38e4ed314ca6617dd09515bb9a6c6cb57c7cfc2
...
群集准备就绪时，会从 SDK 随附的本地群集系统托盘管理器应用程序收到通知。

![本地群集系统托盘通知][4]

应用程序启动后，Visual Studio 会自动显示**诊断事件查看器**，可在其中查看来自服务的跟踪输出。
...
本地群集包含在单台计算机上托管的五个节点。 在生产环境中，每个节点都托管在不同的物理或虚拟机上。 为了模拟丢失计算机的情况，并同时练习使用 Visual Studio 调试器，让我们在本地群集上停止一个节点。
...
右键单击“本地群集管理器”系统托盘应用程序，并选择“停止本地群集”，以便关闭群集。

### <a name="delete-the-cluster-and-all-data"></a>删除群集和所有数据

右键单击“本地群集管理器”系统托盘应用程序，并选择“删除本地群集”，以便删除群集。 

如果选择此选项，Visual Studio 会在下次运行应用程序时重新部署群集。 如果在一段时间内不想使用本地群集，或者需要回收资源，请选择此选项。

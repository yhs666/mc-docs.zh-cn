...
ms.date: 12/10/2018
ms.author: v-yeche
ms.openlocfilehash: 223c3a1f4336419b49477e7c030404a1bc58717b
ms.sourcegitcommit: 38f95433f2877cd649587fd3b68112fb6909e0cf
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 12/05/2018
ms.locfileid: "52901168"
---
...
[Azure 虚拟机规模集自动 OS 映像升级](/virtual-machine-scale-sets/virtual-machine-scale-sets-automatic-upgrade)是使操作系统保持在 Azure 中进行修补的最佳做法，而修补业务流程应用程序 (POA) 是 Service Fabrics RepairManager Systems 服务的包装器，它可为非 Azure 托管群集启用基于配置的 OS 修补计划。 对于非 Azure 托管群集，POA 不是必需的，但是，要想在不停机的情况下对 Service Fabric 群集主机进行修补，则需要按升级域来计划修补程序安装。POA 是一个 Azure Service Fabric 应用程序，它可以在不停机的情况下在 Service Fabric 群集上自动执行操作系统修补。
...
可以使用[独立 Windows 群集的配置设置](/service-fabric/service-fabric-cluster-manifest)在新的和现有的 Service Fabric 群集上启用修复管理器服务。
...
自动 Windows 更新可能导致可用性丢失，因为多个群集节点可能同时重启。 修补业务流程应用默认会尝试在每个群集节点上禁用自动 Windows 更新。 但是，如果设置由管理员或组策略管理，建议将 Windows 更新策略显式设置为“下载之前发出通知”。
...
若要删除应用，请按照[使用 PowerShell 部署和删除应用程序](/service-fabric/service-fabric-deploy-remove-applications)中的步骤操作。

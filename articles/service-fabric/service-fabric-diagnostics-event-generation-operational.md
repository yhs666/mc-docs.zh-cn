...
操作通道包含 Service Fabric 对节点和群集执行的高级操作生成的日志。 为群集启用“诊断”后，Azure 诊断代理便在群集上进行部署，并默认配置为读取操作通道中的日志。 详细了解如何配置 [Azure 诊断代理](service-fabric-diagnostics-event-aggregation-wad.md)，以便修改群集的诊断配置，从而选取更多日志或性能计数器。 

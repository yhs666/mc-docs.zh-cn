...
ms.date: 03/02/2017
ms.author: v-johch
ms.openlocfilehash: 674cd045e556c070b5b838bf848575a32d6d15b3
...
在 Visual Studio 中构建项目后，可以使用 PowerShell 命令 [Copy-ServiceFabricApplicationPackage](https://docs.microsoft.com/powershell/servicefabric/vlatest/copy-servicefabricapplicationpackage) 将应用程序包复制到 ImageStore。 如果要在本地验证应用包，请使用 [Test-ServiceFabricApplicationPackage](https://docs.microsoft.com/powershell/servicefabric/vlatest/test-servicefabricapplicationpackage) cmdlet。 下一个步骤是使用 [Register-ServiceFabricApplicationPackage](https://docs.microsoft.com/powershell/servicefabric/vlatest/register-servicefabricapplicationtype) cmdlet 将应用程序注册到 Service Fabric 运行时。 最后一个步骤是使用 [New-ServiceFabricApplication](https://docs.microsoft.com/powershell/servicefabric/vlatest/new-servicefabricapplication) cmdlet 启动应用程序的实例。  这三个步骤类似于使用 Visual Studio 中的“**部署**”菜单项。
...
现在，应用程序已生成并准备好进行升级。 如果以管理员身份打开 PowerShell 窗口并键入 [Get-ServiceFabricApplication](https://docs.microsoft.com/powershell/servicefabric/vlatest/get-servicefabricapplication)，将会看到已部署 **VisualObjects** 的 1.0.0.0 应用程序类型。  
...
下一步是将此应用程序注册到 Service Fabric，这可以使用 [Register-ServiceFabricApplicationType](https://docs.microsoft.com/powershell/servicefabric/vlatest/register-servicefabricapplicationtype) 命令来执行：
...
现在你可以使用 [Start-ServiceFabricApplicationUpgrade](https://docs.microsoft.com/powershell/servicefabric/vlatest/start-servicefabricapplicationupgrade) 命令开始升级应用程序：
...
现在，在应用程序升级进行时，你可以使用 Service Fabric Explorer 或 PowerShell 命令 [Get-ServiceFabricApplicationUpgrade](https://docs.microsoft.com/powershell/servicefabric/vlatest/get-servicefabricapplicationupgrade) 对其进行监视： 

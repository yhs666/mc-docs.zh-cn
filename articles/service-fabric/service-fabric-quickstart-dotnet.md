...
4. 按 F5 继续操作
    ...
    - 接下来，我们在事务中更新投票选项的相关键值，并提交操作（图中标识为 (3)）。 提交方法返回后，便会更新字典中的数据，并将数据复制到群集中的其他节点。 数据现在安全地存储在群集中，并且后端服务可以故障转移到其他节点，同时数据仍可用。
5. 按 F5 继续操作
...
2. 在“连接终结点”字段中，键入群集的连接终结点，再单击“发布”。 注册合作群集时，浏览器中会提供连接终结点。 例如，`winh1x87d1d.chinanorth.chinacloudapp.cn:19000`。

3. 打开浏览器，并键入群集地址（例如，`http://winh1x87d1d.chinanorth.chinacloudapp.cn`）。 此时，应该能够看到应用程序在 Azure 群集中运行。
...
1. 在群集中打开 Service Fabric Explorer（例如，`http://winh1x87d1d.chinanorth.chinacloudapp.cn:19080`）。
...
8. 打开浏览器，并转到端口 19080 上的群集地址（例如，`http://winh1x87d1d.chinanorth.chinacloudapp.cn:19080`）。

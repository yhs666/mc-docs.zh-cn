...
ms.date: 02/20/2017
ms.author: v-johch
ms.openlocfilehash: b15bafb6e51723de89deafa9587e9d5da80c69cf
...
4. 完成副本侦听程序的所有 `OpenAsync()` 调用并且已启动 `RunAsync()` 后（或者由于此副本当前是辅助节点而跳过了这些步骤时），将调用 `StatefulServiceBase.OnChangeRoleAsync()`。 （这是服务中不常见的重写。）

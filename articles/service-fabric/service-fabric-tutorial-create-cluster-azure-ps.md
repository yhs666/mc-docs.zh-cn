...
wacn.date: 
ms.author: v-yeche
ms.openlocfilehash: 57b7dfd2563417ccccb7425f0e03b26d9de50b01
...
2. 在 *azuredeploy.parameters.json* 参数文件中自定义以下参数：
   ...
   | 参数       | 说明 | 建议的值 |
   | --------------- | ----------- | --------------- |
   | clusterLocation | 要在其中部署群集的 Azure 区域。 | 例如 chinanorth、chinaeast、chinaeast |
...
5. 运行以下命令创建群集：
   ...
   >[!NOTE]
   >`-CertificateSubjectName` 参数应与参数文件中指定的 clusterName 参数以及连接到所选择的 Azure 区域的域保持一致，例如 `clustername.chinaeast.chinacloudapp.cn`。

    完成配置后，会输出有关在 Azure 中创建的群集的信息，并将证书复制到 -CertificateOutputFolder 目录。

6. 双击证书打开证书导入向导。

7. 使用默认设置，但务必勾选“将此密钥标记为可导出”。 复选框（在“私钥保护”步骤中）。 本教程中稍后将 Azure 容器注册表配置为 Service Fabric 群集身份验证时，Visual Studio 需要导出该证书。

8. 现在可以在浏览器中打开 Service Fabric Explorer。 URL 是 PowerShell CmdLet 输出中的 **ManagementEndpoint**，例如 *https://mycluster.chinanorth.chinacloudapp.cn:19080* 

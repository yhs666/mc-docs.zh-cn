...
ms.date: 11/13/2017
ms.author: v-yeche
ms.openlocfilehash: 550ad701002bd8d19eaf5ebdb47130957763d54f
...
使用 ReplicasHealthStateFilter 可以筛选分区上 ReplicaHealthState 对象的集合。 如果指定运行状况存储中不存在的分区，此 cmdlet 会返回错误。 上获取。
...
此 API 可用于测试故障转移。 如果用于针对无状态服务分区，RestartPartitionMode 必须是 AllReplicasOrInstances。 使用相同的 OperationId 调用 GetPartitionRestartProgress API 可获取进度。 上获取。
...
| --partition-id           [必需]| 分区的标识。|
| --restart-partition-mode [必需]| - Invalid - 保留值。  不传入 API。 - AllReplicasOrInstances - 重启分区中的所有副本或实例一次。 -OnlyActiveSecondaries - 仅重启辅助副本。 上获取。|
| --service-id             [必需]| 服务的标识。 这通常是不带“fabric:”URI 方案的服务全名。 从版本 6.0 开始，分层名称以“~”字符隔开。 例如，如果服务名称为“fabric://myapp/app1/svc1”，则 6.0 及更高版本中的服务标识为“myapp~app1~svc1”，在以前的版本中为“myapp/app1/svc1”。|
| --timeout -t                    | 服务器超时，以秒为单位。  默认值：60。|

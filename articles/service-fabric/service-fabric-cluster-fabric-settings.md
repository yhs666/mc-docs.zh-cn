...
- 使用 [PowerShell](/azure-resource-manager/resource-manager-export-template-powershell) 导出和更新资源管理器模板。
- 使用 [Azure CLI](/azure-resource-manager/resource-manager-export-template-cli) 导出和更新资源管理器模板。
...
- 使用 Azure CLI [az sf cluster setting](https://docs.azure.cn/zh-cn/cli/sf/cluster/setting?view=azure-cli-latest) 命令直接修改这些设置。
...
| **参数** | **允许的值** | **升级策略** | **指导或简短说明** |
| --- | --- | --- | --- |
|GlobalTicketLeaseDuration|TimeSpan，默认值为 Common::TimeSpan::FromSeconds(300)|静态|指定以秒为单位的时间跨度。 群集中的节点需要使用投票程序维护多区域租约。 投票程序在此持续时间内提交要在整个群集中传播的多区域租约。 如果该持续时间已过，则租约会丢失。 丢失租约仲裁会导致节点在此时间段内无法接收节点仲裁的通信，因而放弃群集。  需要根据群集大小调整此值。 |

---
title: "有关 Azure Service Fabric 的常见问题 | Microsoft Docs"
...
- 数据的跨分区分布大致均匀，或者你会向群集资源管理器报告负载指标。 默认情况下，Service Fabric 根据副本计数进行负载均衡。 在上述示例中，此操作会将 10 个主要副本和 20 个次要副本放在群集中的每个节点上。 这也适用于跨分区均匀分布的负载。 如果负载不均匀，则必须报告负载，以便 Resource Manager 将较小的副本打包在一起，让较大的副本在单个节点上使用更多内存。

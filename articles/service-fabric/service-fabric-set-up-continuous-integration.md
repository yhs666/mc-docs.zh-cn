...
3. 确保拥有可以向其中部署应用程序的 Service Fabric 群集，或者使用 [Azure 门户](./service-fabric-cluster-creation-via-portal.md)、[Azure Resource Manager 模板](./service-fabric-cluster-creation-via-arm.md)或 [Visual Studio](./service-fabric-cluster-creation-via-visual-studio.md) 创建群集。

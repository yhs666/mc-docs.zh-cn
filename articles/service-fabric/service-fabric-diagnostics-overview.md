...
这些通道包含组织有序的结构化事件用于更好地了解群集的状态。 创建群集时默认会启用“诊断”并设置一个 Azure 存储表，来自这些通道的事件将发送到其中，方便将来进行查询。 可以在[平台级别事件和日志生成](service-fabric-diagnostics-event-generation-infra.md)中详细了解如何监视群集。 我们建议使用 OMS Log Analytics 监视群集。 OMS Log Analytics 提供了 Service Fabric 分析，这是一个特定于 Service Fabric 的解决方案 ，它提供自定义仪表板用于监视 Service Fabric 群集，以及查询群集的事件和设置警报。

...
ms.date: 11/13/2017
ms.author: v-yeche
ms.openlocfilehash: 08cf199d4c3e09102aa6c1ab91ffa1e37613d440
...
在部署 API 管理和 Service Fabric 群集后，即可在 API 管理中配置安全设置和 Service Fabric 后端。 这样即可创建后端服务策略，将流量发送到 Service Fabric 群集。
...
如果后端策略中未指定任何服务名称，那么此处的 **url** 参数是群集中默认将请求路由到的服务的完全限定的服务名称。 如果不打算获取回退服务，可以使用一个假的服务名称，如“fabric:/fake/service”。
...
以管理员身份启动 Visual Studio 并创建 ASP.NET Core 服务：
 ...
 7. 最后，将应用程序部署到 Azure 中的群集。 [使用 Visual Studio](service-fabric-publish-app-remote-cluster.md#to-publish-an-application-using-the-publish-service-fabric-application-dialog-box)，右键单击“应用程序”项目，然后选择“发布”。 提供群集终结点（例如，`mycluster.chinaeast.cloudapp.chinacloudapi.cn:19000`），将应用程序部署到 Azure 中的 Service Fabric 群集。
...
3. 通过单击“+API”，单击“空白 API”框，并填写对话框来添加一个新的 API：
    ...
    - Web 服务 URL：对于 Service Fabric 后端，不使用此 URL 值。 可以在此处设置任何值。 在本教程中，使用“http://servicefabric”。

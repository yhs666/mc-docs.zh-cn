...
ms.date: 08/20/2018
ms.author: v-yeche
ms.openlocfilehash: 0c31bf92380bd82d67da0de19e3cbd7934628a99
ms.sourcegitcommit: 6174eee82d2df8373633a0790224c41e845db33c
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 08/17/2018
ms.locfileid: "41705250"
---
...
3. 使用步骤 2 中获得的 azure 存储详细信息更新 `setupentrypoint.sh` 脚本中的占位符值。
   ```sh
...
6. 在浏览器中转到 `http://PublicIPorFQDN:8081`。 该 URL 提供了登录时所需的初始管理员密码的路径。 
7. 查看 Service Fabric 资源管理器确定 Jenkins 容器在哪一节点上运行。 通过安全外壳 (SSH) 登录到此节点。
   ```sh
   ssh user@PublicIPorFQDN -p [port]
   ``` 
8. 使用 `docker ps -a` 获取容器实例 ID。
9. 通过安全外壳 (SSH) 登录到该容器，并粘贴 Jenkins 门户中显示的路径。 例如，如果门户中显示路径为 `PATH_TO_INITIAL_ADMIN_PASSWORD`，则可运行以下命令：
   ...
   ```
10. 在“Jenkins 入门”页上，选择“选择要安装的插件”选项，选择“无”复选框，单击“安装”。
11. 创建用户，或选择以管理员身份继续。
...
4. 对于“管理主机”，请输入群集的连接终结点，例如 `{your-cluster}.chinaeast.cloudapp.chinacloudapi.cn`。
...
1. 若要创建 Azure Active Directory 服务主体并在 Azure 订阅中分配其权限，请按照[使用门户创建 Azure Active Directory 应用程序和服务主体](/azure-resource-manager/resource-group-create-service-principal-portal)中的步骤操作。 请注意以下几点：
   ...
   * 在[为应用程序分配角色](/azure-resource-manager/resource-group-create-service-principal-portal#assign-application-to-role)部分中，可以在群集的资源组上为应用程序分配“读者”角色。
...
5. 在 Jenkins 凭据提供程序中，从“种类”下拉列表中选择“Azure 服务主体”。

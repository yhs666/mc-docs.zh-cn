...
3. 调用服务的 onOpenAsync 方法（如果存在）（具体而言，调用 `StatelessService.onOpenAsync()`。 这是一种不常见的重写，但这种调用是可行的）。
...
* 服务成功完成 `runAsync()` 并从中返回即可。 这不被视为一种错误状态，只是表示正在完成服务的后台工作。 对于有状态 Reliable Services，如果服务已从主副本降级，并重新升级到主副本，则会再次调用 `runAsync()`。

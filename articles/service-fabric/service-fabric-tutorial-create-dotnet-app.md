...
wacn.date: 
ms.author: v-yeche
ms.openlocfilehash: 74a862675a48a9683d93cc63099bf808d5504864
...
6. 下一页提供一组 ASP.NET Core 项目模板。 本教程中，选择“Web API”。 不过，也可运用相同的思路来构建完整的 Web 应用程序。
...
群集准备就绪时，会从系统托盘中的 Service Fabric 本地群集管理器应用程序收到通知。

1. 若要调试应用程序，请在 Visual Studio 中按 F5。
2. 部署完成后，Visual Studio 会启动浏览器并打开 ASP.NET Web API 服务的根路径。 ASP.NET Core Web API 模板不提供根目录的默认行为，因此会在浏览器中收到错误。
3. 将 `/api/values` 添加到浏览器中的 URL。 此请求会对 Web API 模板中的 ValuesController 调用 `Get` 方法。 它会返回模板提供的默认响应，即包含两个字符串的 JSON 数组：
...
2. 服务项目 - MyWebAPIFrontEnd
    - 该项目是 ASP.NET Core Web API 项目，包含服务的代码和配置。 查看 Controllers 文件夹中的 ValuesController.cs 代码文件，你会注意到，它是一个常规的 ASP.NET Core Web API 控制器。 在 Service Fabric 中将 ASP.NET Core Web API 作为 Reliable Services 运行时，不对作为控制器一部分编写的代码有任何具体要求。
...
2. 打开 `MyStatefulService.cs` 文件，对其进行扩展，以便实现已创建的 `ICounter` 接口。
...
现在，有状态服务已准备就绪，可以使用服务远程处理通过 RPC 接收来自其他服务的流量了。

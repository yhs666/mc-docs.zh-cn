...
部署模板的用户必须在包含 Key Vault 的范围内（包括资源组和 Key Vault）具有 `Microsoft.KeyVault/vaults/deploy/action` 权限。 [所有者](../role-based-access-control/built-in-roles.md#owner)和[参与者](../role-based-access-control/built-in-roles.md#contributor)角色均授予该访问权限。 如果你创建密钥保管库，则你是所有者，因此你具有权限。 如果密钥保管库位于不同的订阅中，则密钥保管库的所有者必须授予访问权限。

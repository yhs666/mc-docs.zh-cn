...
ms.date: 10/23/2017
ms.author: v-yeche
ms.openlocfilehash: 2f2b1dc14bc6d43048aab7e7f61c966eedc4989c
...
本主题介绍了你可能会遇到的一些常见 Azure 部署错误，并提供了解决这些错误所需的信息。 如果找不到部署错误的错误代码，请参阅[查找错误代码](#find-error-code)。
...
| BadRequest | 所发送的部署值与资源管理器所预期的不匹配。 请检查内部状态消息，获取故障排除帮助。 | [模板参考](/templates/)和[支持的位置](resource-manager-template-location.md) |
...
| InvalidRequestContent | 部署值包括非预期的值，或者缺少必需的值。 确认资源类型的值。 | [模板参考](/templates/) |
| InvalidRequestFormat | 在执行部署时启用调试日志记录，并验证请求的内容。 | [调试日志记录](resource-manager-troubleshoot-tips.md#enable-debug-logging) |
| InvalidResourceNamespace | 检查在 type 属性中指定的资源命名空间。 | [模板参考](/templates/) |
...
| InvalidResourceType | 检查在 type 属性中指定的资源类型。 | [模板参考](/templates/) |
...
| ResourceGroupNotFound | 检查部署的目标资源组的名称。 它必须已存在于订阅中。 请检查订阅上下文。 | [Azure CLI](https://docs.microsoft.com/cli/azure/account?#az_account_set) [PowerShell](https://docs.microsoft.com/powershell/module/azurerm.profile/set-azurermcontext) |

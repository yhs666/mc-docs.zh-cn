...
ms.topic: tutorial
ms.author: v-yeche
ms.openlocfilehash: 0f7e598d77ac4ee2aef5d658b4947c1aca0297a3
ms.sourcegitcommit: d75065296d301f0851f93d6175a508bdd9fd7afc
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 11/30/2018
ms.locfileid: "52645578"
---
<!-- Verify successfully-->
...
1. 运行以下 Azure PowerShell 或 Azure CLI 命令。  
    ...
    (Get-AzureRmADUser -UserPrincipalName $upn).Id
    openssl rand -base64 32
    ```
...
1. 从左窗格选择“编辑模板”，使用第 93 行中的 chinanorth 更新位置，然后单击“保存”。
    ...
    <!-- Notice: We ADD NEW PAGE due to Mooncake template is different with global-->
2. 选择或输入以下值。  输入值后不要选择“创建”。
    
    <!-- Notice: We update **Create** due to Mooncake template is different with global--> ![资源管理器模板 Key Vault 集成部署门户](./media/resource-manager-tutorial-use-key-vault/resource-manager-tutorial-create-key-vault-portal.png)

    * **订阅**：选择一个 Azure 订阅。
    * **资源组**：分配唯一的名称。 记下此名称，因为在下一个会话中将使用同一资源组来部署虚拟机。 将 Key Vault 和虚拟机放在同一资源组中可以在本教程结束时更轻松地清理资源。
    * **位置**：选择一个位置。  默认位置为“中国北部”。
...
<!-- Update_Description: new articles on resource manager tutorial use key vault -->
<!--ms.date: 11/19/2018-->
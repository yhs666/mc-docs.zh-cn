...
本文逐步讲解如何使用 VPN 网关和 Azure 门户预览在 Resource Manager 部署模型中创建 VNet 之间的连接。 使用 Azure 门户预览连接虚拟网络时，VNet 必须属于同一订阅。 如果虚拟网络属于不同订阅，仍然可以使用 [PowerShell](vpn-gateway-vnet-vnet-rm-ps.md) 步骤连接它们。

...
ms.date: 07/17/2017
ms.author: v-dazen
ms.openlocfilehash: 76bd695dc936550de638356bc1c9d34753b248cd
...
有关 VNet 到 VNet 连接的详细信息，请参阅本文末尾的 [VNet 到 VNet 常见问题解答](#faq) 。 请注意，如果 VNet 属于不同的订阅，则不能在门户中创建该连接。 可以使用 [PowerShell](vpn-gateway-vnet-vnet-rm-ps.md)。
...
TestVNet1 和 TestVNet4 的虚拟网络网关都已完成后，便可以创建虚拟网络网关连接。 在本部分中，将创建一个从 VNet1 到 VNet4 的连接。 这些步骤仅适用于同一订阅中的 VNet。 如果 VNet 属于不同的订阅，则必须使用 PowerShell 进行连接。 请参阅 [PowerShell](vpn-gateway-vnet-vnet-rm-ps.md) 一文。
...
7. 查看此边栏选项卡上列出的虚拟网络网关。 请注意，只会列出你的订阅中的虚拟网络网关。 如果想要连接到订阅外的虚拟网关，请参阅 [PowerShell 文章](vpn-gateway-vnet-vnet-rm-ps.md)。 
8. 单击要连接到的虚拟网络网关。

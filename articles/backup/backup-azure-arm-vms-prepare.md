...
2. 查看下面的支持详细信息和限制。
   ...
   **联网** | 备份的数据不包括附加到 VM 的网络装载驱动器。<br/><br/> 
   ...
   **存储帐户** | 如果使用网络受限的存储帐户，请确保启用“允许受信任的 Azure 服务访问此存储帐户”，使 Azure 备份服务能够访问该帐户。 网络受限的存储帐户不支持项级恢复。<br/><br/> 在存储帐户中，请确保“防火墙和虚拟网络”设置允许从“所有网络”进行访问。
...
- **NSG 规则**：允许 [Azure 数据中心 IP 范围](https://www.microsoft.com/download/details.aspx?id=42064)。 可以添加一个允许使用[服务标记](../virtual-network/security-overview.md#service-tags)访问 Azure 备份服务的规则，而无需单独允许每个地址范围并持续对其进行管理。
...
下面是设置代理的方式。 我们将使用示例值。 你应该将这些值替换为自己的值。

#### 设置系统帐户代理<a name="supported-operating-systems-for-backup"></a>
...
默认情况下，保管库采用[异地冗余存储 (GRS)](https://docs.microsoft.com/en-us/azure/storage/common/storage-redundancy-grs)。 我们建议对主要备份使用 GRS，但也可以使用更经济的[本地冗余存储](https://docs.microsoft.com/en-us/azure/storage/common/storage-redundancy-lrs?toc=%2fstorage%2fblobs%2ftoc.json)。 
...
- 无论 VM 是否在运行，备份服务都会安装备份扩展。
    ...
    -  但是，即使 VM 已关闭且无法安装扩展，也仍会备份 VM。 此类 VM 称为“脱机 VM”。 在这种情况下，恢复点会是崩溃一致性恢复点。

...
本文介绍了在准备你的环境以及将 Windows 服务器（或工作站）备份到 Azure 时要遵循的步骤。 此外，还说明了部署备份解决方案的注意事项。 如果对 Azure 备份感兴趣，本文可引导快速完成相关过程。
...
2. 在“快速启动”页上，单击“**下载保管库凭据**”。
   ...
   经典管理门户使用保管库名称和当前日期的组合生成保管库凭据。 保管库凭据文件仅在注册工作流中使用，48 小时后过期。
...
8. 在“保管库标识”页上，浏览到前面下载的保管库凭据文件并将其选中。
    ...
    确保将保管库凭据文件放置在安装应用程序可访问的位置。 如果用户遇到访问相关的错误，请将保管库凭据文件复制到同一台计算机中的临时位置，并重试操作。
...
10. 单击“完成” 。 注册服务器向导会在备份中注册服务器。
    ...
    > [!WARNING]
    > 若丢失或忘记通行短语，Microsoft 无法帮助你恢复备份数据。 加密通行短语由你拥有，Microsoft 看不到使用的通行短语。 请将该文件保存在安全位置，因为在恢复操作期间需要用到它。
    >
    >
...
备份策略完成初始备份之后，会创建在需要恢复数据时可用的备份点。 备份策略根据定义的计划执行此操作。

### <a name="to-schedule-the-backup"></a>计划备份
1. 打开 Azure 备份代理。 （如果在关闭注册服务器向导时让“启动 Azure 恢复服务代理”复选框保持选中状态，备份代理会自动打开。）可以通过在计算机中搜索“Azure 备份”找到该代理。
...
- [使用 Azure 备份服务器将工作负荷备份到 Azure](backup-azure-microsoft-azure-backup.md)

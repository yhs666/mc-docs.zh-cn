...
Azure 备份提供一个脚本框架，用于控制备份工作流和环境。 若要确保进行与应用程序一致的 Linux VM 备份，请使用此脚本框架来创建自定义操作前脚本和操作后脚本。 在创建 VM 快照前调用操作前脚本，然后在 VM 快照创建作业完成后调用操作后脚本。 有关详细信息，请参阅 [Linux VM 的应用程序一致性备份](/backup/backup-azure-linux-app-consistent)一文。

---
title: "使用 Azure 备份还原加密 VM 的 Key Vault 密钥和机密 | Microsoft Docs"
...
ms.openlocfilehash: 56fe458f29218b6ca307335ed2d7fc6808ba669e
...
> 2. 只有**将密钥和机密还原到 Key Vault 之后**，才应该从还原磁盘创建 VM。
>
>
...
使用先前生成的 JSON 文件来获取密钥名称和值，并将其他提供给设置密钥 cmdlet，以便将机密 (BEK) 放回 Key Vault。
...
如果已使用 Azure VM 备份备份已加密 VM，则可通过上面的 PowerShell cmdlet 将密钥和机密重新还原到密钥保管库中。 完成还原后，请参阅文章[使用 PowerShell 管理 Azure VM 的备份和还原](backup-azure-vms-automation.md#create-a-vm-from-restored-disks)，使用还原磁盘、密钥和机密创建加密 VM。
...
将密钥和机密还原回 Key Vault 后，请参阅文章[使用 PowerShell 管理 Azure VM 的备份和还原](backup-azure-vms-automation.md#create-a-vm-from-restored-disks)，使用还原磁盘、密钥和机密创建加密 VM。

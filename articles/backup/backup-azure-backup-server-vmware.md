...
4. 在“添加凭据”对话框中，输入新凭据的名称和说明。 然后指定用户名和密码。 名称 *Contoso Vcenter credential* 用于标识下一过程中的凭据。 所用用户名和密码必须与 vCenter 服务器所使用的相同。 如果 vCenter 服务器和 Azure 备份服务器不在同一域中，请在“用户名”中指定域。

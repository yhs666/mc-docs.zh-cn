...
ms.date: 02/07/2018
ms.author: v-junlch
ms.openlocfilehash: 1cb7d019705fb4e23b6ec65a1c3bdc932bd311ed
...
- 在 [Azure 门户](https://ms.portal.azure.cn)中自动为 Azure 导入/导出服务创建 Azure 导入作业。
...
- [下载 Azure 发布文件设置](https://portal.azure.cn/#blade/Microsoft_Azure_ClassicResources/PublishingProfileBlade) 。
...
1. 计划备份时，会看到以下屏幕（在 Windows Server、Windows 客户端或 System Center Data Protection Manager 中）。
    ...
    - **Azure 发布设置**：包含订阅配置文件相关信息的 XML 文件。 该文件中还包含与订阅关联的安全凭据。 可以 [下载该文件](https://portal.azure.cn/#blade/Microsoft_Azure_ClassicResources/PublishingProfileBlade)。 提供发布设置文件的本地路径。
    ...
    > [!NOTE]
    > 如果已通过 [Azure 门户](https://portal.azure.cn)在 Azure 恢复服务保管库中为备份注册了服务器，并且不是在云解决方案提供商 (CSP) 订阅上，则仍可以通过 Azure 门户创建存储帐户，然后将它用于脱机备份工作流。
    >
    >
...
5. 转到此工具显示的链接时，可以看到在**启动脱机备份**工作流中指定的 Azure 存储帐户。 可以在存储帐户的“导入/导出”选项卡中看到新建的导入作业。
    ...
    ![已创建导入作业](./media/backup-azure-backup-import-export/ImportJobCreated.png)<br/>
...
1. 在 [Azure 门户](https://portal.azure.cn/)中转到存储帐户，单击“导入/导出”，并单击任务窗格中的“创建导入作业”。

...
备份保管库是存储所有按时间创建的备份和恢复点的实体。 备份保管库还包含应用到要备份的虚拟机的备份策略。
...
向保管库注册 VM 之前，请运行发现过程以识别所有新 VM。 随后返回订阅中虚拟机的列表和其他信息，例如云服务名称和区域。
...
3. 单击页面底部的“保护”  。
    ...
    此时显示**受保护的项向导** ，其中 *只* 列出了已注册但未受保护的虚拟机。

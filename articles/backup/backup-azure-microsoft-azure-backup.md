...
---
title: 准备环境以使用 Azure 备份服务器备份工作负荷 | Azure
...
本文介绍如何准备环境以使用 Azure 备份服务器备份工作负荷。 使用 Azure 备份服务器，可以从单个控制台保护应用程序工作负荷，例如 Hyper-V VM、Microsoft SQL Server、SharePoint Server、Microsoft Exchange 和 Windows 客户端。 还可以在 Azure 中将信息作为服务器 (IaaS) 工作负荷（例如 VM）进行保护。
...
选择用于运行 Azure 备份服务器的服务器时，建议从 Windows Server 2012 R2 Datacenter 库映像开始。 [在 Azure 门户中创建第一个 Windows 虚拟机](..\virtual-machines\virtual-machines-windows-hero-tutorial.md)一文提供了如何在 Azure 中开始使用建议的虚拟机的教程，即使以前从未使用过 Azure 也没关系。 建议服务器虚拟机 (VM) 最低要求应为：带 2 核、3.5 GB RAM 的 A2 标准。

使用 Azure 备份服务器保护工作负荷有许多细微差异需要注意。 可通过[将 DPM 安装为 Azure 虚拟机](https://technet.microsoft.com/zh-cn/library/jj852163.aspx)一文了解这些细微差异。 部署计算机之前，请完整阅读此文章。
...
可以使用 Windows Server 重复数据删除来删除 DPM 存储中的重复数据。 了解有关在 Hyper-V VM 中部署时 [DPM 和重复数据删除](https://technet.microsoft.com/zh-cn/library/dn891438.aspx)如何配合工作的详细信息。
...
6. 在“要保护哪些工作负荷”中，选择要使用 Azure 备份服务器保护的工作负荷，然后单击“确定”。
    ...
    > [!NOTE]
    > 如果只是想要保护文件和文件夹，建议使用 Azure 备份代理。 如果除文件和文件夹外，还计划保护更多工作负荷，或如果计划在将来扩展保护需求，请选择所有这些工作负荷。

    这会更改“快速启动”向导中的设置，使其准备好基础结构，保护本地和 Azure 中的工作负荷。

    ![快速启动向导更改](./media/backup-azure-microsoft-azure-backup/getting-started-prep-infra.png)

7. 在打开的“准备基础结构”边栏选项卡中，单击用于安装 Azure 备份服务器和下载保管库凭据的“下载”链接。 在将 Azure 备份服务器注册到恢复服务保管库期间，请使用保管库凭据。 使用此链接转到“下载中心”，可从中下载软件包。

    ![为 Azure 备份服务器准备基础结构](./media/backup-azure-microsoft-azure-backup/azure-backup-server-prep-infra.png)

8. 选择所有文件，然后单击“**下一步**”。 下载 Azure 备份下载页中的所有文件，并将所有文件放在同一个文件夹中。
...
8. 安装将会分阶段进行。 第一个阶段，将在服务器上安装 Azure 恢复服务代理。 向导还会检查 Internet 连接。 如果可以连接到 Internet，则你可以继续安装，否则需要提供代理详细信息以连接到 Internet。
    ...
    下一个步骤是配置 Azure 恢复服务代理。 配置过程中，需要提供保管库凭据，用以将计算机注册到恢复服务保管库。 还需要提供通行短语来加密/解密 Azure 与本地之间发送的数据。 你可以自动生成通行短语，或提供自己的通行短语（最少包含 16 个字符）。 请继续运行向导，直到代理已完成配置。

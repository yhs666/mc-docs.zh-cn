...
ms.date: 07/24/2017
ms.author: v-yeche
ms.openlocfilehash: 94bbf241b9d96876aa09f6008c060916b7ca5c6b
...
复制的表在每个计算节点上存储表的完整副本。 复制表时，无需在执行联接或聚合前在计算节点之间传输数据。 复制表仅适用于小型表，因为将整个表存储在每个计算节点上需要额外的存储。  

下图显示了一个存储在每个计算节点上的复制表。 对于 SQL 数据仓库，复制的表将完整复制到每个计算节点上的分发数据库。 对于 Parallel 数据仓库，复制的表将跨分配给计算节点的所有磁盘进行存储。  此磁盘策略通过 SQL Server 文件组实现。  

...
ms.custom: loading
ms.author: v-yeche
ms.openlocfilehash: 0037b210af8480635621395a66be6477d6b7aa29
...
<a id="guide-for-using-polybase-in-sql-data-warehouse" class="xliff"></a>
本指南提供有关在 SQL 数据仓库中使用 PolyBase 的实用信息。

若要开始，请参阅 [使用 PolyBase 加载数据][Load data with PolyBase] 教程。

## 轮换存储密钥
<a id="rotating-storage-keys" class="xliff"></a>
...
## 查询 Azure Blob 存储数据
<a id="query-azure-blob-storage-data" class="xliff"></a>
...
> [!NOTE]
> 对外部表的查询可能因“查询已中止 -- 从外部源读取时已达最大拒绝阈值”错误而失败。 这表示外部数据包含脏  记录。 如果实际数据类型/列数目不匹配外部表的列定义，或数据不匹配指定的外部文件格式，则会将数据记录视为脏记录。 若要解决此问题，请确保外部表和外部文件格式定义正确，并且外部数据符合这些定义。 如果外部数据记录的子集是脏的，可通过使用 CREATE EXTERNAL TABLE DDL 中的拒绝选项，选择拒绝这些查询记录。
> 
> 
...
## 基于新加载的数据创建统计信息
<a id="create-statistics-on-newly-loaded-data" class="xliff"></a>
...
<a id="export-data-to-azure-blob-storage" class="xliff"></a>
...
```
## 隔离加载用户
<a id="isolate-loading-users" class="xliff"></a>

通常需要创建多个可将数据载入 SQL 数据仓库的用户。 由于 [CREATE TABLE AS SELECT (Transact-SQL)][CREATE TABLE AS SELECT (Transact-SQL)] 需要数据库的 CONTROL 权限，因此，最终会有多个用户对所有架构拥有控制访问权限。 若要限制这种情况，可以使用 DENY CONTROL 语句。

示例：假设部门 A 采用数据库架构 schema_A，部门 B 采用数据库架构 schema_B。让用户 user_A 和 user_B 分别充当部门 A 和 B 中可加载 PolyBase 的用户。 这些用户已被授予 CONTROL 数据库权限。
架构 A 和 B 的创建者现在使用 DENY 锁定其架构：

```sql
   DENY CONTROL ON SCHEMA :: schema_A TO user_B;
   DENY CONTROL ON SCHEMA :: schema_B TO user_A;
```   
 因此，user_A 和 user_B 现在应被锁定在其他部门的架构之外。

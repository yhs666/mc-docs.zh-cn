...
ms.date: 07/17/2017
ms.author: v-yeche
ms.openlocfilehash: 03da72c17ff30c38189c6a16327530c7be6c1977
...
---
# Azure SQL 数据仓库最佳实践
<a id="best-practices-for-azure-sql-data-warehouse" class="xliff"></a>
...
## 使用暂停和缩放来降低成本
<a id="reduce-cost-with-pause-and-scale" class="xliff"></a>
...
## 暂停或缩放之前清空事务
<a id="drain-transactions-before-pausing-or-scaling" class="xliff"></a>
...
## 维护统计信息
<a id="maintain-statistics" class="xliff"></a>
...
## 将 INSERT 语句分组为批
<a id="group-insert-statements-into-batches" class="xliff"></a>
...
<a id="use-polybase-to-load-and-export-data-quickly" class="xliff"></a>
SQL 数据仓库支持通过多种工具（包括 PolyBase 和 BCP）来加载和导出数据。  对于少量的数据，性能不是那么重要，任何工具都可以满足需求。  但是，当要加载或导出大量数据，或者需要快速的性能时，PolyBase 是最佳选择。  PolyBase 使用 SQL 数据仓库的 MPP（大规模并行处理）体系结构，因此加载和导出巨量数据的速度比其他任何工具更快。  可使用 CTAS 或 INSERT INTO 来运行 PolyBase 加载。  **使用 CTAS 可以减少事务日志记录，是加载数据最快的方法。**  PolyBase 支持各种不同的文件格式，包括 Gzip 文件。  **若要在使用 gzip 文本文件时获得最大的吞吐量，请将文件分成 60 个以上的文件让加载有最大化的并行度。**  若要更快的总吞吐量，请考虑并行加载数据。
...
## 加载并查询外部表
<a id="load-then-query-external-tables" class="xliff"></a>
...
## 哈希分布大型表
<a id="hash-distribute-large-tables" class="xliff"></a>
...
## 不要过度分区
<a id="do-not-over-partition" class="xliff"></a>
...
## 最小化事务大小
<a id="minimize-transaction-sizes" class="xliff"></a>
...
## 使用最小可能的列大小
<a id="use-the-smallest-possible-column-size" class="xliff"></a>
...
## 对暂时性数据使用临时堆表
<a id="use-temporary-heap-tables-for-transient-data" class="xliff"></a>
...
## 优化聚集列存储表
<a id="optimize-clustered-columnstore-tables" class="xliff"></a>
...
## 使用较大的资源类来改善查询性能
<a id="use-larger-resource-class-to-improve-query-performance" class="xliff"></a>
...
## 使用较小的资源类来增加并发性
<a id="use-smaller-resource-class-to-increase-concurrency" class="xliff"></a>
...
## 使用 DMV 来监视和优化查询
<a id="use-dmvs-to-monitor-and-optimize-your-queries" class="xliff"></a>

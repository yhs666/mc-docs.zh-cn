...
ms.date: 01/15/2018
ms.author: v-yeche
ms.openlocfilehash: f70d95a7e861570a1ef0e9b8af8bd4ff2c2e2e03
...
可以在系统上创建并使用以下视图来计算每个行组的平均行数，以及识别所有欠佳的聚集列存储索引。  此视图中的最后一列会生成为 SQL 语句，以用于重建索引。
...
立即提升段质量的快速方法是重建索引。  上述视图返回的 SQL 会返回可用于重建索引的 ALTER INDEX REBUILD 语句。  重建索引时，请确保将足够的内存分配给要重建索引的会话。  为此，请提高用户的资源类，该用户有权将此表中的索引重建为建议的最小值。  无法更改数据库所有者用户的资源类，因此，如果尚未在系统中创建用户，必须先这样做。  如果使用 DW300 或更少，建议的最小值为 xlargerc；如果使用 DW400 到 DW600，则建议的最小值为 largerc；如果使用 DW1000 和更高，则建议的最小值为 mediumrc。

...
[CREATE TABLE AS SELECT](https://docs.microsoft.com/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse) 或 CTAS 语句是所提供的最重要的 T-SQL 功能之一。 它是根据 SELECT 语句的输出创建新表的并行化操作。 CTASD 是创建表副本的最简单快速方法。 

## <a name="selectinto-vs-ctas"></a>SELECT..INTO 与CTAS
可将 CTAS 视为 [SELECT...INTO](https://docs.microsoft.com/sql/t-sql/queries/select-into-clause-transact-sql) 语句的超强版本。

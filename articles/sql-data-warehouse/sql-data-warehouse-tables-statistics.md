...
ms.date: 10/31/2016
ms.author: v-yeche
ms.openlocfilehash: a7050e641fe40878e298e8f3e3c48615629776f2
...
例如，**SQL Server**（不是 SQL 数据仓库）在以下情况下会自动更新统计信息：
...
在此示例中，直方图针对的是 product\_category。 跨列统计信息是根据 product\_category 和 product\_sub_c\ategory 计算的：
...
1. 未阐述的功能不受支持
2. 不能使用 Stats_stream
3. 不能联接特定统计数据子集的结果，例如(STAT_HEADER JOIN DENSITY_VECTOR)
4. 不能设置 NO_INFOMSGS 以进行消息隐藏
5. 不能在统计信息名称的前后使用方括号
6. 不能使用列名来标识统计信息对象
7. 不支持自定义错误 2767

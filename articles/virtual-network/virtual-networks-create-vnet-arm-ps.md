---
title: "创建虚拟网络 - Azure PowerShell | Azure"
...
ms.openlocfilehash: fad062834179f93ba81d0b610a9932077d8176fa
...
1. 请按[如何安装和配置 Azure PowerShell](https://docs.microsoft.com/powershell/azure/overview) 一文中的步骤安装和配置 Azure PowerShell。
...
4. 将虚拟网络对象存储在变量中：
   ...
   > [!TIP]
   > 可以通过运行 `$vnet = New-AzureRmVirtualNetwork -ResourceGroupName TestRG -Name TestVNet -AddressPrefix 192.168.0.0/16 -Location chinaeast` 将步骤 3 和步骤 4 结合使用。
   > 
...
- 通过阅读文章[创建 Windows VM](../virtual-machines/virtual-machines-windows-ps-create.md)，将虚拟机 (VM) 连接到虚拟网络。 可选择将 VM 连接到现有 VNet 和子网，而不按文章中的步骤创建 VNet 和子网。

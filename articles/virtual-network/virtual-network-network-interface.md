...
ms.date: 09/04/2017
ms.author: v-yeche
ms.openlocfilehash: 9aa21aa7acf4a600358484c472c74b6493b532b6
...
4. 在出现的“创建网络接口”边栏选项卡中，输入或选择以下设置的值，单击“创建”：
    ...
    |设置|必需？|详细信息|
    |---|---|---|
    |名称|是|名称在所选资源组中必须唯一。 随着时间推移，Azure 订阅中可能会有多个网络接口。 请阅读[命名约定](https://docs.microsoft.com/azure/architecture/best-practices/naming-conventions?toc=%2fazure%2fvirtual-network%2ftoc.json#naming-rules-and-restrictions)一文，获取有关创建命名约定来简化多个网络接口的管理的建议。 创建网络接口后，无法更改其名称。|
    ...
    |专用 IP 地址分配|是| 在此设置中，会为 IPv4 地址选择分配方法。 从以下分配方法中选择：**动态：**选择此选项时，Azure 将从所选子网的地址空间中自动分配可用地址。 当网络接口所在的虚拟机处于停止（解除分配）状态后启动时，Azure 会向网络接口分配一个不同的地址。 如果在虚拟机未处于停止（解除分配）状态时将其重启，地址会保持不变。 **静态：**选择此选项时，必须手动从所选子网的地址空间中手动分配一个可用的 IP 地址。 静态地址保持不变，除非手动更改或删除网络接口。 创建网络接口后，可更改分配方法。 Azure DHCP 服务器将此地址分配到虚拟机操作系统中的网络接口。|

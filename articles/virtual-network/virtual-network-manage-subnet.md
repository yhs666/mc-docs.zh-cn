---
title: "创建、更改或删除 Azure 虚拟网络子网 | Azure"
description: "了解如何在 Azure 中创建、更改或删除虚拟网络子网。"
...
ms.date: 07/24/2017
ms.author: v-dazen
ms.openlocfilehash: 4c33fa0ea9c7279e61604f6185252c69fae7200f
ms.sourcegitcommit: f2f4389152bed7e17371546ddbe1e52c21c0686a
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 07/14/2017
---
# <a name="create-change-or-delete-a-virtual-network-subnet"></a>创建、更改或删除虚拟网络子网

了解如何创建、更改或删除虚拟网络子网。 

如果你不熟悉虚拟网络，我们建议在创建、更改或删除子网之前参阅 [Azure 虚拟网络概述](virtual-networks-overview.md)和[创建、更改或删除虚拟网络](virtual-network-manage-network.md)。 可连接到虚拟网络的 Azure 资源将连接到虚拟网络中的子网。 在虚拟网络中创建多个子网通常是出于以下目的：
- **筛选子网之间的流量**。 可对子网应用网络安全组，筛选所有连接到虚拟网络的资源（例如虚拟机）的入站和出站网络流量。 若要详细了解如何创建网络安全组，请参阅[创建网络安全组](virtual-networks-create-nsg-arm-pportal.md)。
...
本文介绍如何为使用 Azure Resource Manager 部署模型创建的虚拟网络创建、更改和删除子网。

## <a name="before"></a>准备工作

在开始执行本文所述的任务之前，需满足以下先决条件：

- 如果你不熟悉虚拟网络的用法，我们建议学习[创建第一个 Azure 虚拟网络](virtual-network-get-started-vnet-subnet.md)中的练习。 此练习可帮助你熟悉虚拟网络。
...
- 使用 Azure 帐户登录到 Azure 门户、Azure 命令行工具 (Azure CLI) 或 Azure PowerShell。 如果你没有 Azure 帐户，可注册[试用帐户](https://www.azure.cn/pricing/1rmb-trial)。
...
|PowerShell|[](https://docs.microsoft.com/powershell/module/azurerm.network/new-azurermvirtualnetworksubnetconfig?view=azurermps-3.8.0?toc=%2fvirtual-network%2ftoc.json)New-AzureRmVirtualNetworkSubnetConfig、[](https://docs.microsoft.com/powershell/module/azurerm.network/add-azurermvirtualnetworksubnetconfig?view=azurermps-3.8.0?toc=%2fvirtual-network%2ftoc.json)Add-AzureRmVirtualNetworkSubnetConfig|

## <a name="change-subnet"></a>更改子网设置

可以通过管理已连接到子网的资源，来更改网络安全组、路由表以及用户对子网的访问权限。 若要了解这些设置，请参阅[创建子网](#create-subnet)中的步骤 6。 若要更改子网的地址空间，必须先删除连接到该子网的所有资源。 删除资源所要执行的步骤根据资源的不同而异。 若要了解如何删除已连接到子网的资源，请参阅要删除的每个资源类型的文档。 若要更改子网的地址范围，请执行以下操作：
...
只有在没有资源连接到子网的情况下，才能删除该子网。 如果有资源连接到子网，则必须先删除连接到该子网的资源，然后才能删除该子网。 删除资源所要执行的步骤根据资源的不同而异。 若要了解如何删除已连接到子网的资源，请参阅要删除的每个资源类型的文档。 若要删除子网，请执行以下操作：
...
若要创建 VM 并将其连接到子网，请参阅[创建虚拟网络并连接 VM](virtual-network-get-started-vnet-subnet.md#create-vms)。

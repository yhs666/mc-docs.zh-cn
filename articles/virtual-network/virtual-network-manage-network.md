...
ms.date: 03/12/2018
ms.author: v-yeche
ms.openlocfilehash: f614b5bbe970f7edeab5016e5c337427650e4495
...
2. 为以下设置输入或选择值，然后选择“创建”：
    ...
    - 订阅：选择[订阅](../azure-glossary-cloud-terminology.md?toc=%2fvirtual-network%2ftoc.json#subscription)。 不能在多个 Azure 订阅中使用同一个虚拟网络。 但是，可通过[虚拟网络对等](virtual-network-peering-overview.md)将一个订阅中的虚拟网络连接到另一个订阅中的虚拟网络。 任何连接到虚拟网络的 Azure 资源都必须与虚拟网络处于同一订阅中。
...
3. 将列出所选虚拟网络的以下设置：
    ...
    - 子网：显示存在于虚拟网络中的子网的列表。 若要了解如何添加和删除子网，请参阅[管理子网](virtual-network-manage-subnet.md)。
    ...
    - 图示：图示提供已连接到虚拟网络的所有设备的视觉表示形式。 图示提供一些有关设备的关键信息。 若要在此视图中管理设备，请选择设备。
...
若要在虚拟网络上执行任务，必须向帐户分配到[网络参与者](../active-directory/role-based-access-built-in-roles.md?toc=%2fvirtual-network%2ftoc.json#network-contributor)角色或分配到下表中列出适当的权限[自定义](../active-directory/role-based-access-control-custom-roles.md?toc=%2fvirtual-network%2ftoc.json)角色：

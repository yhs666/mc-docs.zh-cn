...
manager: corywink
ms.openlocfilehash: 99e10b641fecc986a30c805f058fa9326edd2d1e
ms.sourcegitcommit: 6ffa4d50cee80c7c0944e215ca917a248f2a4bcd
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 12/06/2019
ms.locfileid: "74882992"
---
...
本文介绍如何使用 Azure IoT Central 中的连续数据导出功能定期将数据导出到 **Azure Blob 存储帐户**或 **Azure Data Lake Storage Gen2 存储帐户**。 可将**度量**、**设备**和**设备模板**导出到 JSON 或 Apache Avro 格式的文件中。 导出的数据可用于冷路径分析，例如 Azure 机器学习中的训练模型或 Microsoft Power BI 中的长期趋势分析。
...
如果当前没有可导出到的存储，请执行以下步骤：

1. [在 Azure 门户中创建新的存储帐户](https://ms.portal.azure.cn/#create/Microsoft.StorageAccount-ARM)。 可以详细了解如何创建新的 [Azure Blob 存储帐户](https://aka.ms/blobdocscreatestorageaccount)或 [Azure Data Lake Storage v2 存储帐户](/storage/blobs/data-lake-storage-quickstart-create-account)。

    > [!Note] 
    > 如果选择将数据导出到 ADLS v2 存储帐户，则必须选择“Blob 存储”作为“帐户类型”。   
...
现在已经有了要将数据导出到的存储目标，接下来请遵循以下步骤设置连续数据导出。 
...
3. 选择右上角的“+ 新建”按钮。  选择“Azure Blob 存储”作为导出目标。  
...
4. 在下拉列表框中，选择自己的**存储帐户命名空间**。 也可选取列表中的最后一个选项，即“输入连接字符串”。  

    > [!NOTE] 
    > 只会在 **IoT Central 应用所在的订阅**中看到存储帐户命名空间。 若要导出到此订阅外部的某个目标，请选择“输入连接字符串”，然后参阅步骤 5。 

    > [!NOTE] 
    > 若要通过 7 天试用期的应用来配置连续事件导出，则唯一的方式是使用连接字符串。 这是因为 7 天试用期的应用没有关联的 Azure 订阅。

    ![创建导出到 Blob 的新设置](media/howto-export-data/export-create-blob2.png)

5. （可选）如果选中了“输入连接字符串”，则会出现一个用于粘贴连接字符串的新框。  若要获取存储帐户的连接字符串，请在 Azure 门户中转到自己的存储帐户：- 在“设置”下选择“访问密钥”- 复制密钥 1 连接字符串或密钥 2 连接字符串  
 
6. 从下拉列表框中选择一个容器。 如果没有容器，请在 Azure 门户中转到自己的存储帐户：
    - 在“Blob 服务”下选择“Blob”。   单击“+ 容器”并为容器命名。  选择数据的公共访问级别（任何级别都可用于连续数据导出）。 在 [Azure 存储文档](/storage/blobs/storage-quickstart-blobs-portal#create-a-container)中了解详细信息。

7. 选择偏好的**数据格式**：JSON 或 [Apache Avro](https://avro.apache.org/docs/current/index.html) 格式。

8. 在“要导出的数据”  下，通过将类型设置为“打开”  来指定要导出的各类数据。

9. 若要启用连续数据导出，请确保“数据导出”  开关为“打开”  。 选择“保存”  。

   ![配置连续数据导出](media/howto-export-data/export-list-blob2.png)

10. 几分钟后，数据便会出现在存储帐户中。


## <a name="path-structure"></a>路径结构

每分钟一次将度量、设备和设备模板数据导出到存储帐户，每个文件包含自上次导出文件以来所做的批量更改。 导出的数据以 JSON 或 Avro 格式放置在三个文件夹中。 存储帐户中的默认路径是：
...
导出的度量数据包含 IoT Central 在此期间从所有设备接收的所有新消息。 导出的文件所用格式与通过 [IoT 中心消息路由](/iot-hub/iot-hub-csharp-csharp-process-d2c)导出到 Blob 存储中的消息文件格式相同。

> [!NOTE]
> 请确保设备发送包含 `contentType: application/JSON` 和 `contentEncoding:utf-8`（或 `utf-16`、`utf-32`）的消息。 有关示例，请参阅 [IoT 中心文档](/iot-hub/iot-hub-devguide-routing-query-syntax#message-routing-query-based-on-message-body)。
...
- IoT Central 中设备的 `id`
- 设备的 `name`
- [设备预配服务](/iot-central/core/howto-connect-nodejs)中的 `deviceId`
- 设备模板信息
- 属性值
- 设置值
...
导出的文件为每条记录包含一行。 以下示例显示了已解码的采用 Avro 格式的记录：

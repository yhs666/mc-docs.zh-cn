---
title: "如何在内部虚拟网络中使用 Azure API 管理 | Azure"
...
ms.author: v-yiso
ms.date: 
ms.openlocfilehash: bb26715eaffcb67035b3c6de6276f0eb2ff0401e
...
内部虚拟网络中的 API 管理服务在内部负载均衡器 (ILB) 后面托管。 ILB 的 IP 地址在 [RFC1918](http://www.faqs.org/rfcs/rfc1918.html) 范围内。  
...
* **在 VNET 内创建 API 管理服务**：使用 cmdlet [New-AzureRmApiManagement](https://docs.microsoft.com/en-us/powershell/module/azurerm.apimanagement/new-azurermapimanagement) 在 VNET 内创建 Azure API 管理服务，并将其配置为使用内部 VNET 类型。

* **在 VNET 内部署现有 API 管理服务**：使用 cmdlet [Update-AzureRmApiManagementDeployment](https://docs.microsoft.com/en-us/powershell/module/azurerm.apimanagement/update-azurermapimanagementdeployment) 将现有 Azure API 管理服务移到虚拟网络内，并将其配置为使用内部 VNET 类型。

## <a name="apim-dns-configuration"></a>DNS 配置
在外部虚拟网络模式下使用 API 管理时，由 Azure 托管 DNS。 使用内部虚拟网络模式时，必须自行管理 DNS。

> [!NOTE]
> API 管理服务不会侦听 IP 地址上的传入请求， 而只响应到 Hostname 的请求，该主机名在其服务终结点（包括网关、开发人员门户、发布者门户、直接管理终结点，以及 GIT）上配置。

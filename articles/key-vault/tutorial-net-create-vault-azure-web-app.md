...
ms.author: pryerram
ms.custom: mvc
ms.openlocfilehash: 1239a274b091372e145f35ab1737e9f15839702e
ms.sourcegitcommit: 547436d67011c6fe58538cfb60b5b9c69db1533a
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 11/30/2018
ms.locfileid: "52676958"
---
...
* 所有平台：
  ...
  * Azure 订阅 <br />（如果你没有 Azure 订阅，请在开始前创建一个[试用帐户](https://www.azure.cn/pricing/1rmb-trial)。 ）
  * 适用于 Windows、Mac 和 Linux 的 [Azure CLI](https://docs.azure.cn/zh-cn/cli/install-azure-cli?view=azure-cli-latest) 2.0.4 或更高版本
...
为 Azure 服务（例如：应用服务）启用 MSI 时，Azure 会创建一个[服务主体](key-vault-whatis.md#basic-concepts)。 MSI 针对 Azure Active Directory (Azure AD) 中的服务实例提供启动标识，并将服务主体的凭据注入该实例。
...
遵循[此教程](../app-service/app-service-web-get-started-dotnet.md)创建一个 .NET Core Web 应用，并将其**发布**到 Azure。 
...
虽然 Azure Key Vault 提供安全存储凭据及其他机密的方式，但代码需要对 Key Vault 进行身份验证才能检索这些凭据和机密。 
...
1. 在 Azure CLI 中运行 assign-identity 命令，为此应用程序创建标识：
   ...
   >[!NOTE]
   >必须将 \<YourAppName\> 替换为 Azure 上发布的应用的名称。 例如，如果发布的应用名为 **MyAwesomeapp.chinacloudsites.cn**，请将 \<YourAppName\> 替换为 **MyAwesomeapp**。
...
>[!NOTE]
>此过程中的命令等同于转到[门户](https://portal.azure.cn)并在 Web 应用程序属性中将“标识/系统分配”设置切换为“打开”。
...
>[Azure Key Vault 开发人员指南](https://docs.azure.cn/zh-cn/key-vault/key-vault-developers-guide)

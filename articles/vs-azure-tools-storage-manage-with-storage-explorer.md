...
ms.date: 08/22/2019
ms.author: v-lingwu
ms.openlocfilehash: dfb696d35f103c21daf44116f57d971d8e403c77
ms.sourcegitcommit: c72fba1cacef1444eb12e828161ad103da338bb1
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/30/2019
ms.locfileid: "71674838"
---
...
存储资源管理器在最常用 Linux 分发版的 [Snap Store](https://snapcraft.io/storage-explorer) 中可用，并且是建议的安装方法。 将新版本发布到 Snap Store 时，存储资源管理器内嵌项会自动安装其所有依赖项和更新。

有关支持的分发版列表，请参阅 [snapd 安装页](https://snapcraft.io/docs/installing-snapd)。
...
存储资源管理器还以 .tar.gz 下载文件格式提供，但必须手动安装依赖项。 以下 Linux 分发版支持 .tar.gz 安装：
...
在 Linux 上安装存储资源管理器时如需更多帮助，请参阅[故障排除指南](https://docs.microsoft.com/azure/storage/common/storage-explorer-troubleshooting#linux-dependencies)。
...
> [!NOTE]
> 登录后，存储资源管理器要求拥有管理（Azure 资源管理器）和数据层权限才能全面访问资源。 这意味着，你需要拥有 Azure Active Directory (Azure AD) 权限，可以访问自己的存储帐户、该帐户中的容器以及该容器中的数据。 如果你仅在数据层拥有权限，请考虑[通过 Azure AD 添加资源](#add-a-resource-via-azure-ad)。 若要详细了解存储资源管理器所需的具体权限，请参阅[故障排除指南](https://docs.microsoft.com/azure/storage/common/storage-explorer-troubleshooting?tabs=1804#role-based-access-control-permission-issues)。

1. 在存储资源管理器中选择“管理帐户”，转到“帐户管理”面板。 

    ![管理帐户][1]

2. 左窗格随即显示已登录的所有 Azure 帐户。 若要连接到另一个帐户，请选择“添加帐户”  。

3. 若要登录到某个国家/地区云或 Azure Stack，请选择“Azure 环境”下拉列表以选择要使用的 Azure 云。  选择环境后，选择“登录”按钮。  有关详细信息，请参阅[将存储资源管理器连接到 Azure Stack 订阅](/azure-stack/user/azure-stack-storage-connect-se)。

    ![登录选项][2]

4. 使用 Azure 帐户成功登录后，该帐户以及与该帐户关联的 Azure Stack 订阅会添加到左窗格中。 选择要使用的 Azure 订阅，并选择“应用”  。 （选择“所有订阅”会选择所有列出的 Azure 订阅，或者一个都不选。） 
...
* [使用连接字符串](#use-a-connection-string)。 如果已获取存储帐户的连接字符串，请使用此选项。 存储资源管理器支持密钥和 [SAS](storage/common/storage-dotnet-shared-access-signature-part-1.md) 连接字符串。
* [使用 SAS URI](#use-a-sas-uri)。 如果已获取 Blob 容器、文件共享、队列或表的 [SAS URI](storage/common/storage-dotnet-shared-access-signature-part-1.md)，请使用它附加到资源。 若要获取 SAS URI，可以使用[存储资源管理器](#generate-a-sas-in-storage-explorer)或 [Azure 门户](https://portal.azure.com)。
* [使用名称和密钥](#use-a-name-and-key)。 如果你知道存储帐户的任一帐户密钥，则可以使用此选项快速建立连接。 存储帐户的密钥位于 [Azure 门户](https://portal.azure.com)上的“存储帐户访问密钥”面板中。 
...
1. 选择左侧垂直工具栏上的“连接”按钮打开“连接”对话框：  

    ![“连接到 Azure 存储”选项][9]

2. 使用“添加 Azure 帐户”选项登录到有权访问资源的 Azure 帐户（如果尚未执行此操作）。  登录后，返回到“连接”对话框。 

3. 依次选择“通过 Azure Active Directory (Azure AD)添加资源”、“下一步”。  

4. 选择有权访问所要附加到的存储资源的 Azure 帐户和租户。 选择“**下一步**”。

5. 选择要附加的资源类型，然后输入用于建立连接的信息。 在此页上输入的信息取决于要添加的资源类型。 请确保选择正确的资源类型。 输入所需的信息后，选择“下一步”。 

6. 检查连接摘要，确保所有信息正确。 如果正确，请选择“连接”。  否则，请使用“后退”按钮返回到前一页，以修复任何错误的信息。 

成功添加连接后，资源树中将自动打开表示该连接的节点。 如果未打开该节点，请在查看“本地和附加” > “存储帐户” > “(附加的容器)” > “Blob 容器”。     如果存储资源管理器无法添加你的连接，或者在成功添加连接后你无法访问自己的数据，请参阅[故障排除指南](https://docs.microsoft.com/azure/storage/common/storage-explorer-troubleshooting)。
...
3. 选择连接的显示名称，并输入连接字符串。 然后，选择“下一步”  。

4. 检查连接摘要，确保所有信息正确。 如果正确，请选择“连接”。  否则，请使用“后退”按钮返回到前一页，以修复任何错误的信息。 

成功添加连接后，资源树中将自动打开表示该连接的节点。 如果未打开该节点，请查看“本地和附加” > “存储帐户”。   如果存储资源管理器无法添加你的连接，或者在成功添加连接后你无法访问自己的数据，请参阅[故障排除指南](https://docs.microsoft.com/azure/storage/common/storage-explorer-troubleshooting)。
...
2. 依次选择“使用共享访问签名(SAS) URI”、“下一步”。  

3. 选择连接的显示名称，并输入 SAS URI。 适用于要附加的资源类型的服务终结点应会自动填充。 如果使用的是自定义终结点，有可能不会填充该终结点。 选择“**下一步**”。

4. 检查连接摘要，确保所有信息正确。 如果正确，请选择“连接”。  否则，请使用“后退”按钮返回到前一页，以修复任何错误的信息。 

成功添加连接后，资源树中将自动打开表示该连接的节点。 如果未打开该节点，请查看“本地和附加” > “存储帐户” > “(附加的容器)” >  适用于附加容器类型的服务节点。     如果存储资源管理器无法添加你的连接，或者在成功添加连接后你无法访问自己的数据，请参阅[故障排除指南](https://docs.microsoft.com/azure/storage/common/storage-explorer-troubleshooting)。
...
如果仿真器侦听默认端口，则你可以使用“仿真器 - 默认端口”节点（可在“本地和附加” > “存储帐户”下找到该节点）来快速访问仿真器。   
...
2. 选择左侧垂直工具栏上的“连接”按钮打开“连接”对话框：  

    ![“连接到 Azure 存储”选项][9]

3. 依次选择“附加到本地仿真器”、“下一步”。  

4. 选择连接的显示名称，并输入每种服务类型的仿真器所侦听的端口。 文本框中首先会包含大多数仿真器的默认端口值。 “文件端口”框保留空白，因为目前没有任何官方仿真器支持文件服务。  如果使用的仿真器不支持文件服务，你可以输入所用的端口。 然后，选择“下一步”  。

5. 检查连接摘要，确保所有信息正确。 如果正确，请选择“连接”。  否则，请使用“后退”按钮返回到前一页，以修复任何错误的信息。 

成功添加连接后，资源树中将自动打开表示该连接的节点。 如果未打开该节点，请查看“本地和附加” > “存储帐户”。   如果存储资源管理器无法添加你的连接，或者在成功添加连接后你无法访问自己的数据，请参阅[故障排除指南](https://docs.microsoft.com/azure/storage/common/storage-explorer-troubleshooting)。

#### <a name="connect-to-an-azure-cosmos-db-account-by-using-a-connection-string"></a>使用连接字符串连接到 Azure Cosmos DB 帐户

除了通过 Azure 订阅管理 Azure Cosmos DB 帐户以外，还可以使用连接字符串连接到 Azure Cosmos DB。 为此，请执行以下步骤：

1. 在资源树的左侧，展开“本地和附加”，右键单击“Azure Cosmos DB 帐户”，然后选择“连接到 Azure Cosmos DB”。   

    ![通过连接字符串连接到 Azure Cosmos DB][21]

2. 选择“Azure Cosmos DB API”，输入“连接字符串”数据，然后选择“确定”以连接到 Azure Cosmos DB 帐户。   有关如何检索连接字符串的信息，请参阅[获取连接字符串](https://docs.microsoft.com/azure/cosmos-db/manage-account)。

    ![连接字符串][22]

#### <a name="connect-to-azure-data-lake-store-by-uri"></a>通过 URI 连接到 Azure Data Lake Store

若要访问不在订阅中的资源，需要请求某个有权访问该资源的人员提供资源 URI。 登录后，可以使用该 URI 连接到 Data Lake Store。 为此，请执行以下步骤：
...
2. 在“生成共享访问签名”对话框中，指定要授予该帐户的时间范围和权限，然后选择“创建”。  
...
> [!NOTE]
> 若要加快搜索速度，请使用“帐户管理”面板取消选择不包含所要搜索的项的订阅。 还可以右键单击某个节点，然后选择“从此处搜索”，从特定的节点开始搜索。 
>
>

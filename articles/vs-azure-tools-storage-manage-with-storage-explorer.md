...
ms.date: 08/22/2019
ms.author: v-lingwu
ms.openlocfilehash: 387d4df2026e5d2628fda222a42841e2b7da29dd
ms.sourcegitcommit: 01788fd533b6de9475ef14e84aa5ddd55a1fef27
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 08/30/2019
ms.locfileid: "70169530"
---
...
如需如何在 Linux 上安装存储资源管理器的更多帮助，请参阅[故障排除指南](/storage/common/storage-explorer-troubleshooting?tabs=1804#linux-dependencies)。
...
* [使用名称和密钥](#use-a-name-and-key)：如果你知道存储帐户的任一帐户密钥，则可以使用此选项快速建立连接。 存储帐户的密钥位于 [Azure 门户](https://portal.azure.com)上的“存储帐户访问密钥”边栏选项卡中。 
...
4. 选择有权访问所要附加的存储资源的 Azure 帐户以及该资源所在的订阅，然后单击“下一步”。 

5. 选择要附加的资源类型，然后输入用于建立连接的信息。 此页上的输入将会根据你要添加的资源类型而更改。 请确保选择正确的资源类型。 填写所需的信息后，单击“下一步”。 

6. 检查连接摘要，确保所有信息正确。 如果所有信息看起来正确，请单击“连接”，否则请使用“后退”按钮返回上一页，以更正任何有错误的信息。  

成功添加连接后，资源树将自动导航到表示连接的节点。 如果由于某种原因未导航到该位置，请查看“本地和附加”→“存储帐户”→“(附加的容器)”→“Blob 容器”。     如果存储资源管理器无法添加你的连接，或者在成功添加连接后你无法访问自己的数据，请参阅[故障排除指南](https://docs.microsoft.com/azure/storage/common/storage-explorer-troubleshooting)获取帮助。
...
3. 选择连接的显示名称，并输入连接字符串。  。

4. 检查连接摘要，确保所有信息正确。 如果所有信息看起来正确，请单击“连接”，否则请使用“后退”按钮返回上一页，以更正任何有错误的信息。  

成功添加连接后，资源树将自动导航到表示连接的节点。 如果由于某种原因未导航到该位置，请查看“本地和附加”→“存储帐户”。   如果存储资源管理器无法添加你的连接，或者在成功添加连接后你无法访问自己的数据，请参阅[故障排除指南](https://docs.microsoft.com/azure/storage/common/storage-explorer-troubleshooting)获取帮助。
...
3. 选择连接的显示名称，并输入 SAS URI。 适用于要附加的资源类型的服务终结点应会自动填充。 如果使用的是自定义终结点，则可能不会自动填充。 单击“下一步”  。

4. 检查连接摘要，确保所有信息正确。 如果所有信息看起来正确，请单击“连接”，否则请使用“后退”按钮返回上一页，以更正任何有错误的信息。  

成功添加连接后，资源树将自动导航到表示连接的节点。 如果由于某种原因未导航到该位置，请查看“本地和附加”→“存储帐户”→“(附加的容器)”→ **适用于附加容器类型的服务节点**。    如果存储资源管理器无法添加你的连接，或者在成功添加连接后你无法访问自己的数据，请参阅[故障排除指南](https://docs.microsoft.com/azure/storage/common/storage-explorer-troubleshooting)获取帮助。
...
存储资源管理器支持所有平台上的仿真器。 目前，官方提供的仿真器有两个：
...
如果仿真器在默认端口上运行，则你可以使用“仿真器 - 默认端口”节点（始终可在“本地和附加”→“存储帐户”下找到该节点）来快速访问仿真器。    若要为连接使用不同的名称，或者仿真器不是在默认端口上运行，请执行以下步骤。

1. 启动仿真器。 执行此操作时，请记下每种服务类型的仿真器所侦听的端口。 稍后需要知道此信息。
...
4. 选择连接的显示名称，并输入每种服务类型的仿真器所侦听的端口。 默认情况下，文本框中会包含大多数仿真器的默认端口值。 默认情况下，“文件端口”也会保留空白，因为目前没有任何官方仿真器支持文件服务。  不过，如果所用的仿真器确实支持文件服务，则可以输入正在使用的端口。 单击“下一步”  。

5. 检查连接摘要，确保所有信息正确。 如果所有信息看起来正确，请单击“连接”，否则请使用“后退”按钮返回上一页，以更正任何有错误的信息。  

...
ms.custom: mvc
ms.date: 10/30/2017
ms.openlocfilehash: 482316ccf7d34566172fb8fd3ae582ac09de3818
...
此步骤在[用于 MySQL 的 Azure 数据库（预览版）](/mysql)中创建一个 MySQL 数据库。 稍后需要将 PHP 应用程序配置为连接到此数据库。
...
在以下命令中，请将 &lt;mysql_server_name> 占位符替换为你自己的唯一 MySQL 服务器名称（有效字符是 `a-z`、`0-9` 和 `-`）。 此名称是 MySQL 服务器主机名 (`<mysql_server_name>.database.chinacloudapi.cn`) 的一部分，必须全局唯一。
...
在本地终端窗口中，连接到 Azure 中的 MySQL 服务器。 使用前面为 &lt;mysql_server_name> 指定的值。 出现输入密码的提示时，请使用在 Azure 中创建数据库时指定的 _MySQLAzure2017_。
...
使用 [az webapp config set](https://docs.microsoft.com/cli/azure/webapp/config#set) 命令设置应用程序所需的 PHP 版本。
...
在应用服务中，使用 [az webapp config appsettings update ](https://docs.microsoft.com/cli/azure/webapp/config/appsettings#set) 命令将环境变量设置为应用设置。
...
使用 [az webapp config appsettings set](https://docs.microsoft.com/cli/azure/webapp/config/appsettings#set) 命令在应用服务 Web 应用中设置应用程序密钥。 替换占位符 _&lt;appname>_ 和 _&lt;outputofphpartisankey:generate>_。
...
使用 [az resource update](https://docs.microsoft.com/cli/azure/resource#update) 命令设置虚拟应用程序路径。 替换 _&lt;appname>_ 占位符。

...
可以直接使用下面提到的 API 管理连接到 Web 应用的混合连接。 
...
使用此信息的一种方式是通过可以从 [ARMClient][armclient] github 项目获取的 armclient 进行使用。 下面是有关将预先存在的混合连接附加到 Web 应用的示例。 按以上架构创建一个 JSON 文件，例如：
...
此功能的早期形式称为 BizTalk 混合连接。 此功能已于 2018 年 5 月 31 日走完一生并停止运营。 BizTalk 混合连接已从所有 Web 应用中删除且不可通过门户或 API 进行访问。 如果你仍然在混合连接管理器中配置了这些较旧的连接，则会看到已终止状态并且底部会显示寿命终止声明。

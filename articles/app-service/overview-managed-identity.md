...
可以使用门户、PowerShell 或 CLI 以与创建时相同的方式禁用该功能，从而删除标识。 可以单独删除用户分配的标识。 若要删除所有标识，请在 REST/ARM 模板协议中通过将类型设置为“无”来完成此操作：
...
以这种方式删除标识也将从 AAD 中删除它。 删除应用资源时，也将自动从 AAD 中删除系统分配的标识。

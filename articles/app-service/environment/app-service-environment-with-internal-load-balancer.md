...
ms.author: v-tawe
ms.custom: seodec18
ms.openlocfilehash: 3c9c551b068b7275314d57acd81847a560343794
ms.sourcegitcommit: 6a62dd239c60596006a74ab2333c50c4db5b62be
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/20/2019
ms.locfileid: "71156377"
---
...
选择“内部”后，无法向 ASE 添加更多 IP 地址，取而代之的是必须提供 ASE 的子域。 在使用外部 VIP 的 ASE 中，ASE 的名称会在子域中用于在该 ASE 中创建的应用。 如果 ASE 的名称为 ***contosotest***，该 ASE 中的应用名称为 ***mytest***，则子域的格式为 ***contosotest.p.chinacloudsites.cn***，该应用的 URL 为 ***mytest.contosotest.p.chinacloudsites.cn***。 如果将“VIP 类型”设置为“内部”，ASE 名称不会在 ASE 的子域中使用。 可以显式指定子域。 如果子域为 ***contoso.corp.cn***，而用户在该 ASE 中创建的应用名为 ***timereporting***，则该应用的 URL 为 ***timereporting.contoso.corp.cn***。
...
如果想要使用自己的证书试用流并测试对 ASE 进行 HTTP 和 HTTPS 访问，请执行以下操作：
...
使用外部 VIP 时，DNS 由 Azure 管理。 在 ASE 中创建的任何应用都会自动添加到 Azure DNS，这是一个公用 DNS。 在 ILB ASE 中，必须管理自己的 DNS。 针对指定的子域（例如 contoso.corp.cn），必须为以下项创建指向 ILB 地址的 DNS A 记录：
...
[vnetnsgs]: https://docs.azure.cn/virtual-network/virtual-network-vnet-plan-design-arm

...
ms.author: v-tawe
ms.custom: seodec18
ms.openlocfilehash: a13f8ad71cacdfac6c2abfabefd1409ab54aeb04
ms.sourcegitcommit: 6a62dd239c60596006a74ab2333c50c4db5b62be
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/20/2019
ms.locfileid: "71156347"
---
...
可以将计算资源用于你的四个资源池。  每个应用服务环境 (ASE) 都有一组前端和三个可能的工作节点池。 不需要三个辅助角色池都使用，如果需要，可以只使用其中一个或两个。

租户无法直接访问资源池（前端和辅助角色）中的主机。 无法使用远程桌面协议 (RDP) 来连接它们、更改其预配，或充当其管理员。

但是可以设置资源池的数量和大小。 在 ASE 中，有四个大小选项，标记为 P1 到 P4。 有关这些大小及其定价的详细信息，请参阅[应用服务定价](https://www.azure.cn/pricing/details/app-service/)。
...
* 如果更改辅助角色池中所用计算资源的大小，会导致在该辅助角色池中运行的应用冷启动。
...
如果虚拟网络配置了 ExpressRoute VPN，则应注意 ASE 的某些路由要求。 某些用户定义的路由 (UDR) 配置与 ASE 不兼容。 有关在使用 ExpressRoute 的虚拟网络中运行 ASE 的更多详细信息，请参阅[在使用 ExpressRoute 的虚拟网络中运行应用服务环境][ExpressRoute]。
...
“设置” > “属性”   ：“设置”边栏选项卡在显示 ASE 边栏选项卡时会自动打开  。 顶部是“属性”  。 此处显示了许多与“基本信息”  重复的项，但“虚拟 IP 地址”  和“出站 IP 地址”  是非常有用的项。

![“设置”边栏选项卡和“属性”][4]

“设置” > “IP 地址”   ：在 ASE 中创建 IP 安全套接字层 (SSL) 应用时，需要有一个 IP SSL 地址。 要获取该地址，ASE 需要拥有一些可分配的 IP SSL 地址。 创建的 ASE 最初有一个 IP SSL 地址用于此目的，但可以添加更多地址。 额外的 IP SSL 地址需要付费，如[应用服务定价][AppServicePricing]（在有关 SSL 连接的部分中）所述。 额外的价格是 IP SSL 价格。
...
若要使用特定资源池中的手动或自动缩放功能，请根据需要转到“设置”   > “前端池”   / “辅助角色池”  。 然后打开用户要更改的池。 转到“设置”   > “扩大”  或“设置”   > “增加”  。 通过“扩大”  边栏选项卡可控制实例数量。  。  
...
[virtualnetwork]: https://docs.azure.cn/virtual-network/virtual-networks-faq
[AppServicePricing]: https://www.azure.cn/pricing/details/app-service/

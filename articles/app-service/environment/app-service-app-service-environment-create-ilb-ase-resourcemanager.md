...
ms.author: v-tawe
ms.custom: seodec18
ms.openlocfilehash: 46dc63ec20306cf2f0b742f3220d656d7e20b6cf
ms.sourcegitcommit: 6a62dd239c60596006a74ab2333c50c4db5b62be
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/20/2019
ms.locfileid: "71156429"
---
...
GitHub 上（[此处][quickstartilbasecreate]）提供了示例 Azure 资源管理器模板及其相关联的参数文件。

*azuredeploy.parameters.json* 文件中的大部分参数都可用于创建 ILB ASE 以及绑定到公用 VIP 的 ASE。  创建 ILB ASE 时，以下列表会调出特殊注释的参数或唯一的参数：
...
* *dnsSuffix*：此参数定义要分配给 ASE 的默认根域。  在 Azure 应用服务的公共变体中，所有 Web 应用的默认根域均为 *chinacloudsites.cn*。  不过，ILB ASE 位于客户虚拟网络的内部，因此不适合使用公共服务的默认根域，  而应当具有适合在公司的内部虚拟网络中使用的默认根域。  例如，假定的 Contoso Corporation 可能会将 *internal-contoso.com* 的默认根域用于只能在 Contoso 虚拟网络内解析和访问的应用。 
...
一旦针对 ILB ASE 填入 *azuredeploy.parameters.json* 文件，就可以使用以下 Powershell 代码片段创建 ILB ASE。  更改文件 PATH，以匹配 Azure Resource Manager 模板文件在计算机上的位置。  此外，切记提供自己的 Azure 资源管理器部署名称值和资源组名称值。
...
* *使用者可选名称*：此属性必须同时包括 * *.your-root-domain-here.com* 和 * *.scm.your-root-domain-here.com*。  输入第二项是因为会使用 *your-app-name.scm.your-root-domain-here.com*形式的地址，建立与每个应用关联的 SCM/Kudu 站点的 SSL 连接。
...
完成模板后，可通过 HTTPS 访问 ILB ASE 上的应用，并使用默认 SSL 证书保护连接。  如果 ILB ASE 上的应用使用应用程序名称与默认主机名的组合来寻址，则会使用默认 SSL 证书。  例如， *https://mycustomapp.internal-contoso.com* 将为 * *.internal-contoso.com* 使用默认 SSL 证书。

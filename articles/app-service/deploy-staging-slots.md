...
ms.date: 03/18/2019
ms.author: v-biyu
ms.openlocfilehash: ea8e51c473985fe6e6f74c04b3ebf6b73d3480f4
ms.sourcegitcommit: 0ccbf718e90bc4e374df83b1460585d3b17239ab
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 03/05/2019
ms.locfileid: "57347119"
---
...
将 Web 应用、移动后端和 API 应用部署到[应用服务](overview.md)时，如果应用在“标准”或“高级”应用服务计划层下运行，则可以部署到单独的部署槽位而不是默认的生产槽位。 部署槽实际上是具有自身主机名的实时应用。 两个部署槽（包括生产槽）之间的应用内容与配置元素可以交换。 将应用程序部署到非生产槽具有以下优点：
...
2. 选择“部署槽”选项，并单击“添加槽”。
   ...
   > [!NOTE]
   > 如果应用尚未处于“标准”或“高级”层，则会收到消息，指示启用过渡支持的层。 此时，可选择“升级”，导航到应用的“缩放”选项卡，然后继续。
   > 
...
若要自动执行多阶段交换，请参阅[使用 PowerShell 进行自动化操作](https://docs.microsoft.com/en-us/azure/app-service/deploy-staging-slots#automate-with-azure-powershell)。
...
字符串 `x-ms-routing-name=self` 指定生产槽位。 客户端浏览器访问此链接后，不仅浏览器会重定向到生产槽，后续每个请求也都具有将会话固定到生产槽的 `x-ms-routing-name=self` cookie。

...
ms.date: 10/30/2017
ms.author: v-yiso
ms.openlocfilehash: 1f7f3ae936862bf4451619fed6973452423654b1
...
本文介绍 Azure Webapps 上运行的[节点应用程序](app-service-web-get-started-nodejs.md)的最佳实践和故障排除步骤（使用 [iisnode](https://github.com/azure/iisnode)）。
...
* nodeProcessCountPerApplication
    ...
    此设置控制每个 IIS 应用程序启动的节点进程数目。 默认值为 1。 可以将此值设置为 0，启动与 VM 核心计数一样多的 node.exe。 大部分应用程序的建议值为 0，以便利用计算机上的所有核心。 Node.exe 是单线程，因此一个 node.exe 最多耗用 1 个核心；若要让节点应用程序发挥最大性能，需要利用所有核心。
* nodeProcessCommandLine

    此设置控制 node.exe 的路径。 可以设置此值以指向 node.exe 版本。
* maxConcurrentRequestsPerProcess

    此设置控制 iisnode 发送给每个 node.exe 的并发请求数目上限。 在 Azure Webapps 上，此设置的默认值为“无限”。 不必担心此设置。 在 Azure Webapps 外部，默认值为 1024。 可能需要根据应用程序获取的请求数以及应用程序处理每个请求的速度来配置此设置。
...
* flushResponse
    ...
    支持刷新每个响应实体主体片段会增加性能开销，而降低约 5% 的系统吞吐量（从 v0.1.13 起），因此最好让此设置的范围仅限于需要响应流式处理的终结点（例如，在 web.config 中使用 <location> 元素）
...
* idlePageOutTimePeriod
    ...
    默认值为 0，这表示已禁用此功能。 如果设置为大于 0 的值，iisnode 会每隔“idlePageOutTimePeriod”毫秒将其所有子进程移出分页。 若要了解移出分页的意思，请参考[此文档](https://msdn.microsoft.com/library/windows/desktop/ms682606.aspx)。 对于耗用大量内存并偶尔想要将内存移出至磁盘以释放一些 RAM 的应用程序，此设置很有用。
...
* loggingEnabled
    ...
    此设置控制 iisnode 记录 stdout 和 stderr 的功能。 Iisnode 将从它所启动的节点进程捕获 stdout/stderr，并写入“logDirectory”设置中指定的目录。 一旦启用，应用程序便会将日志写入文件系统，这可能会影响性能，具体视应用程序完成的日志记录量而定。
* devErrorsEnabled
...
可能会在门户上获取 Azure Webapps 针对高 CPU 耗用量提供的建议。 也可以设置监视器，监视某些[指标](web-sites-monitor.md)。 在 Azure 门户仪表板上检查 CPU 使用率时，请检查 CPU 的 MAX 值，这样才不会错过峰值。
在认为应用程序耗用太多 CPU，但却无法解释的情况下，必须分析节点应用程序。
...
会看到一个如下所示的命令提示符。 进入 site/wwwroot 目录
...
上述更改将分析 WriteConsoleLog 函数，然后将配置文件输出写入站点 wwwroot 下的“profile.cpuprofile”文件。 将请求发送到应用程序。 站点 wwwroot 下将会显示所创建的“profile.cpuprofile”文件。

![](./media/app-service-web-nodejs-best-practices-and-troubleshoot-guide/scm_profile.cpuprofile.png)

下载此文件。需要使用 Chrome F12 工具打开此文件。 在 Chrome 上按 F12，然后单击“配置文件”选项卡。 单击“加载”按钮。 选择刚下载的 profile.cpuprofile 文件。 单击刚加载的配置文件。
...
可能会在门户上获得 Azure Webapps 针对高内存耗用量提供的建议。 也可以设置监视器，监视某些[指标](web-sites-monitor.md)。 在 Azure 门户仪表板上检查内存使用率时，请检查内存的 MAX 值，这样才不会错过峰值。
...
2. 应用程序耗用太多内存，其他进程无法开始运行。 如果 VM 内存总量接近 100%，进程管理器会终止 node.exe，让其他进程有机会执行一些操作。 若要解决此问题，请确保应用程序未泄漏内存，或者如果应用程序真的需要使用大量内存，请增加为具有更多 RAM 的较大 VM。
...
2. 尝试延迟加载 node\_modules，而不要在启动时加载所有模块。 这表示对 require('module') 的调用应该在尝试使用模块的函数中有实际需要时进行。
3. Azure Webapps 会提供一项名为本地缓存的功能。 此功能会将内容从网络共享复制到 VM 上的本地磁盘。 由于文件位于本地，因此 node\_modules 的加载速度要快很多。
...
| --- | --- | --- |
| 500 |1000 |将请求分派到 IISNODE 时存在一些问题 - 检查 node.exe 是否已启动。 Node.exe 可能在启动时已崩溃。 检查 web.config 配置是否有错误。 |
...
| 503 |1000 |内存不足，无法分配更多命名管道连接。 检查应用为何耗用大量内存。 检查 maxConcurrentRequestsPerProcess 设置值。 如果此值并非无限且拥有太多请求，则增加此值以免出现该错误。 |

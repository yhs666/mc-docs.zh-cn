...
- **使用内置平台映像的应用部署**：我们的内置平台映像包含常见 Web 应用运行时和依赖项，如 Node 和 PHP。 使用任意一种 [Azure App Service 部署方法](../../app-service-web/web-sites-deploy.md?toc=%2fazure%2fapp-service%2fcontainers%2ftoc.json)将应用部署到 Web 应用的存储，然后使用内置平台映像运行应用。
...
- **生成要求**：利用[持续部署](../../app-service-web/app-service-continuous-deployment.md?toc=%2fazure%2fapp-service%2fcontainers%2ftoc.json)，可以直接通过源代码启动应用并在 Azure 上运行。 无需外部生成或发布过程。 但是，[Kudu](https://github.com/projectkudu/kudu/wiki) 部署引擎中的生成工具在可定制性和可用性方面存在限制。 随着应用的依赖项或对自定义生成逻辑的要求的增加，Kudu 的功能可能无法满足应用。

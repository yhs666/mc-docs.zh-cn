...
在 Cloud Shell 中，使用 [az sql server create](https://docs.microsoft.com/cli/azure/sql/server#create) 命令创建 SQL 数据库逻辑服务器。
...
使用 [az sql server firewall create](https://docs.microsoft.com/cli/azure/sql/server#create) 命令创建 [Azure SQL 数据库服务器级防火墙规则](../../sql-database/sql-database-firewall-configure.md)。 若同时将起始 IP 和结束 IP 设置为 0.0.0.0，防火墙将仅对其他 Azure 资源开启。 
...
使用 [az sql db create](https://docs.microsoft.com/cli/azure/sql/db#create) 命令在服务器中创建 [S0 性能级别](../../sql-database/sql-database-service-tiers.md)的数据库。 
...
要为 Azure 应用设置连接字符串，请使用 Cloud Shell 中的 [az webapp config appsettings update](https://docs.microsoft.com/cli/azure/webapp/config/appsettings#update) 命令。 在下列命令中，将 \<app name> 和 \<connection_string> 参数替换为先前创建的连接字符串。
...
以下示例在 Azure Web 应用中配置 `ASPNETCORE_ENVIRONMENT` 应用设置。 替换 \<app_name> 占位符。
...
在浏览器中，导航到 http://local:5000/。 现在可以添加待办事项，然后单击“完成”。 然后，它应作为已完成项在主页中显示。 请记住，`Edit`视图不显示`Done`字段，因为没有更改`Edit`视图。

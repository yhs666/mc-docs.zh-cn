...
现在将使用 Azure CLI 2.0 创建在 Azure 应用服务中托管 Python 应用程序所需的资源。  使用 [az login](https://docs.microsoft.com/cli/azure/#login) 命令登录到 Azure 订阅，并按照屏幕上的说明进行操作。 
...
使用 [az group create](https://docs.microsoft.com/cli/azure/group#create) 创建[资源组](../../azure-resource-manager/resource-group-overview.md)。 
...
使用 [az postgres server create](https://docs.microsoft.com/cli/azure/documentdb#create) 命令创建 PostgreSQL 服务器。
...
使用 [az webapp create](https://docs.microsoft.com/cli/azure/webapp#create) 命令在 myAppServicePlan 应用服务计划中创建 web 应用。 
...
在应用服务中，使用 [az webapp config appsettings update ](https://docs.microsoft.com/cli/azure/webapp/config#set) 命令将环境变量设置为应用设置。 
...
![Azure 门户中的“应用服务”页](./media/tutorial-docker-python-postgresql-app/app-mgmt.png)

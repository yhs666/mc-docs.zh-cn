...
忽略 config.domain 警告。 当应用完全加载后，会看见类似下方所示的消息：
...
![MEAN.js 成功连接到 MongoDB](./media/tutorial-nodejs-mongodb-app/mongodb-connect-success.png)
...
在 Cloud Shell 中，使用 [az cosmosdb create](https://docs.microsoft.com/cli/azure/cosmosdb#create) 命令创建 Cosmos DB 帐户。

在下面命令中，将 *\<cosmosdb_name>* 占位符替换为一个唯一的 Cosmos DB 名称。 此名称将用作 Cosmos DB 终结点 `https://<cosmosdb_name>.documents.azure.com/` 的一部分，因此这个名称需要在 Azure 中的所有 Cosmos DB 帐户中具有唯一性。 此名称只能包含小写字母、数字以及连字符 (-)，同时长度必须为 3 到 50 个字符。
...
在本地终端窗口中运行以下命令，为生产环境缩小和捆绑脚本。 这一进程将生成生产环境所需的文件。
...
要设置应用设置，请使用 Cloud Shell 中的 [az webapp config appsettings update](https://docs.microsoft.com/cli/azure/webapp/config/appsettings#update) 命令。 
...
在本地 MEAN.js 存储库中，再次打开具有特定于生产环境的配置的 _config/env/production.js_。 请注意，默认 MEAN.js 应用已配置为使用所创建的 `MONGODB_URI` 环境变量。
...
![已向文章添加注释字段](./media/tutorial-nodejs-mongodb-app/added-comment-field.png)

...
此步骤使用 [Azure CLI](https://docs.azure.cn/zh-cn/cli/install-azure-cli) 创建[用于 MySQL 的 Azure 数据库](../mysql/quickstart-create-mysql-server-database-using-azure-cli.md)实例。 请配置示例应用程序，以便在本教程后面使用该数据库。

### <a name="create-a-resource-group"></a>创建资源组

使用 [`az group create`](/cli/group#az-group-create) 命令创建[资源组](../azure-resource-manager/resource-group-overview.md)。 Azure 资源组是在其中部署和管理相关资源（例如 Web 应用、数据库和存储帐户）的逻辑容器。 

以下示例在“中国东部”区域中创建一个资源组：

```azurecli
az group create --name myResourceGroup --location "China East"
```    

若要查看可以用于 `--location` 的可能值，请使用 [`az appservice list-locations`](/cli/appservice#list-locations) 命令。

### <a name="create-a-mysql-server"></a>创建 MySQL 服务器

在 Cloud Shell 中，使用 [`az mysql server create`](/cli/mysql/server?view=azure-cli-latest#az-mysql-server-create) 命令在 Azure Database for MySQL 中创建一个服务器。

在下列命令中，用唯一的服务器名称替换 *\<mysql_server_name>* 占位符，用用户名替换 *\<admin_user>* 占位符，并用密码替换 *\<admin_password>* 占位符。 此服务器名称将用作 PostgreSQL 终结点 (`https://<mysql_server_name>.mysql.database.azure.cn`) 的一部分，因此需要在 Azure 中的所有服务器之间保持唯一。

```azurecli
az mysql server create --resource-group myResourceGroup --name <mysql_server_name>--location "China East" --admin-user <admin_user> --admin-password <server_admin_password> --sku-name GP_Gen4_2
```
...
在终端窗口中，使用 [`az mysql server firewall-rule create`](/cli/mysql/server/firewall-rule#az-mysql-server-firewall-rule-create) 命令创建 MySQL 服务器的防火墙规则，以便建立客户端连接。 若同时将起始 IP 和结束 IP 设置为 0.0.0.0，防火墙将仅对其他 Azure 资源开启。 
...
使用 [`az appservice plan create`](/cli/appservice/plan#az-appservice-plan-create) CLI 命令通过**免费**定价层创建 Azure 应用服务计划。 appservice 计划定义用于托管应用的物理资源。 分配到 appservice 计划的所有应用程序共享这些资源，因此在托管多个应用时可以节省成本。 
...
在终端窗口中，使用 [`az webapp create`](/cli/webapp#az-webapp-create) CLI 命令在 `myAppServicePlan` 应用服务计划中创建 Web 应用定义。 Web 应用定义提供了一个用于访问应用程序的 URL，并配置了多个将代码部署到 Azure 的选项。 
...
在终端窗口中，使用 [`az webapp config set`](/cli/webapp/config#az-webapp-config-set) 命令，设置应用所需的 Java 运行时配置。
...
在终端窗口中，使用 CLI 中的 [`az webapp config appsettings`](https://docs.azure.cn/cli/webapp/config/appsettings) 设置应用程序设置：

```azurecli
az webapp config appsettings set --settings SPRING_DATASOURCE_URL="jdbc:mysql://<mysql_server_name>.mysql.database.azure.cn:3306/tododb?verifyServerCertificate=true&useSSL=true&requireSSL=false" --resource-group myResourceGroup --name <app_name>
```
...
若要启动日志流式处理，请在 Cloud Shell 中使用 [`az webapp log tail`](/cli/webapp/log?view=azure-cli-latest#az-webapp-log-tail) 命令。
...
默认情况下，Web 应用页显示“概览”页。 在此页中可以查看应用的运行状况。 在此处还可以执行管理任务，例如停止、启动、重启和删除。 页面左侧的选项卡显示可以打开的不同配置页。

![Azure 门户中的“应用服务”页](./media/app-service-web-tutorial-java-mysql/web-app-blade.png)

...
1. 显示你的 Web 应用后，请使用以下步骤指定 Java 版本：
   ...
   a.将新的虚拟硬盘附加到 VM。 单击“应用程序设置”菜单项。

   b.保留“数据库类型”设置，即设置为“共享”。 为 Java 版本选择“Java 8”。
...
1. 使用以下步骤指定 FTP 部署凭据：
   ...
   a.将新的虚拟硬盘附加到 VM。 单击“部署凭据”菜单项。

   b.保留“数据库类型”设置，即设置为“共享”。 指定用户名和密码。
...
1. 使用以下步骤检索 FTP 连接信息：
   ...
   b.保留“数据库类型”设置，即设置为“共享”。 复制并保存完整的 FTP 用户名和 URL，以便在本教程的下一部分使用。

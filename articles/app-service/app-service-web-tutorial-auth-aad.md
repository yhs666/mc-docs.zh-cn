...
导航到 `http://<front_end_app_name>.chinacloudapi.cn` 并添加一些项目，例如 `from front end 1` 和 `from front end 2`。

导航到 `http://<back_end_app_name>.chinacloudapi.cn`，此时会看到从前端应用添加的项目。 另请添加一些项目（例如 `from back end 1` 和 `from back end 2`），然后刷新前端应用，看其是否反映了所做的更改。
...
再次登录到 `http://<front_end_app_name>.chinacloudapi.cn.net`。 在用户数据使用协议页上，单击“接受”。

现在应该可以像以前一样通过后端应用创建、读取、更新和删除数据了。 唯一区别是，两种应用现在都受应用服务身份验证和授权的保护，包括在进行服务间调用时受到保护。

祝贺！ 服务器代码现在可以代表经身份验证的用户访问后端数据了。

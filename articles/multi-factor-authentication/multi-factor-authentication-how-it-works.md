...
ms.reviewer: yossib
ms.custom: it-pro
ms.openlocfilehash: a625c738863a6df192e1735f8b5153f081a0210b
...
| 移动应用通知 |将验证请求发送到用户的智能手机，要求他们通过在移动应用中选择“验证”来完成验证。 如果将应用通知选作主要验证方法，则会发生这种情况。  如果用户在未登录时收到通知，他们可以选择将通知举报为欺诈。 |
| 移动应用验证码 |在用户的智能手机上运行的移动应用会显示一个每隔 30 秒钟更改一次的 6 位数字验证码。 用户找到最新代码，并在登录页上输入该代码，以完成验证过程。 如果将验证码选作主要验证方法，则会发生这种情况。 |

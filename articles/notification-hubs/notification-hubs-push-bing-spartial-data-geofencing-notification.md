...
4. 在硬盘中创建包含以下内容的 **NotificationHubsGeofence.pipe** 文件：本教程使用基于管道的文件框住旧金山海滨区域：
...
8. 现在你已满足数据源的所有要求。 要获取有关 API 调用的请求 URL 的详细信息，请在必应地图开发人员中心内依次选择“数据源”、“数据源信息”。
...
6. 接下来，打开 `Controllers` > `NotificationsController.cs`。 更新 Post 调用以考虑目标经度和纬度。 为此，只需将以下两个字符串添加到函数签名：`latitude` 和 `longitude`。
    ...
    ```
7. 在名为 `ApiHelper.cs` 的项目中创建一个新类 - 使用它连接到必应，以检查位置点边界的交叉点。 如以下代码中所示实现 `IsPointWithinBounds` 函数：
...
1. 为推送通知注册 UWP 应用。 在 Visual Studio 中，选择“项目” > “应用商店” > “将应用与应用商店关联”。
...
7. 为了进行测试，可以再次创建 `MainPage_Loaded` 事件处理程序，并在其中添加以下代码片段：
...
8. 在 `LocationHelper` 的 `Geolocator_PositionChanged` 处理程序中，可以添加一段测试代码以强制将位置放入地域隔离区：

    ```csharp
    await LocationHelper.SendLocationToBackend("wns", "TEST_USER", "TEST", "37.7746", "-122.3858");
    ```

9. 由于我们未传递实际坐标（目前这可能不在边界内），并且使用的是预定义测试值，因此更新时会看到显示的通知：

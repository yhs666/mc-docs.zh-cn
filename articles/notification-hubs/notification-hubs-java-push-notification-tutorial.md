...
ms.date: 12/17/2018
ms.author: v-biyu
ms.openlocfilehash: 142e64ffc779417b10011825c30f57b53af77e75
ms.sourcegitcommit: 6e07735318eb5f6ea319b618863259088eab3722
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 12/06/2018
ms.locfileid: "52981661"
---
...
安装 API 是一种注册管理的替代机制。 其现在可以使用“单个”安装对象，而不必维护多个注册，后者不但工作量较大，而且容易出错且效率低下。 安装包含所需一切内容：推送通道（设备标记）、标记、模板、辅助磁贴（用于 WNS 和 APNS）。 不必再调用该服务以获取 ID - 只需生成 GUID 或任何其他标识符，将其保存在设备上并与推送通道（设备标记）一起发送到后端即可。 在后端，应当只做一个调用：CreateOrUpdateInstallation，它完全是幂等的，因此，如果需要，可随时重试。

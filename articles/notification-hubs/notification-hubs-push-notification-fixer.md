---
title: Microsoft Azure 通知中心 - 诊断已删除的通知
...
ms.date: 04/29/2019
ms.author: v-biyu
ms.openlocfilehash: b141ca584ccd3a133b5d1798d1f3ee9120beb24d
ms.sourcegitcommit: f9d082d429c46cee3611a78682b2fc30e1220c87
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 04/15/2019
ms.locfileid: "59566271"
---
# <a name="azure-notification-hubs---diagnose-dropped-notifications"></a>Microsoft Azure 通知中心 - 诊断已删除的通知

Azure 通知中心客户提出的最常见问题之一是如何排查以下问题：客户端设备上不显示从应用程序发送的通知。 他们想知道删除通知的位置和原因以及如何修复该问题。 本文标识了通知被删除或设备收不到通知的各种原因。 了解如何分析并确定根本原因。

首先务必了解通知中心服务如何将通知推送到设备。
...
确定目标之后，通知中心服务将通知推送到设备平台的“推送通知服务”。 示例包括适用于 Apple 的 Apple Push Notification 服务 (APNs)。 通知中心推送跨多批注册拆分的通知。 通知中心基于你在 Azure 门户的“配置通知中心”下设置的凭据，向各自的推送通知服务验证身份。 然后，推送通知服务将通知转发到各自的*客户端设备*。 

通知传递的最后一步在平台推送通知服务与设备之间进行。 推送通知过程中四个主要组件（客户端、应用程序后端、通知中心和平台推送通知服务）的任意一个都有可能导致通知被删除。 有关通知中心体系结构的详细信息，请参阅[通知中心概述]。
...
若要成功将通知发送到各自的推送通知服务，通知中心服务需要在开发人员的应用程序环境中对自身进行身份验证。 为此，开发人员可在各自的平台（Apple、Windows 等）中创建开发人员帐户。 然后，开发人员向从中获取凭据的平台注册自己的应用程序。 
...
如果通知中心配置正确，所有标记或标记表达式的使用也正确，则会找到有效的目标。 应将通知发送到这些目标。 接着，通知中心服务会并行启动几个处理批次。 每个批次将消息发送到一组注册。
...
由于平台通知服务非常强大，这些通知往往在几秒内就能从推送通知服务到达设备。 如果推送通知服务进行限制，通知中心服务会应用指数回退策略。 如果推送通知服务在 30 分钟之内都无法访问，我们会实施一个策略，让消息过期并永久删除它们。
...
在各自的推送通知服务开发人员门户（APNs、Windows 通知服务等）中验证凭据。 有关详细信息，请参阅 [Azure 通知中心入门]。
...
可查看和管理中心的所有注册，这些注册已经按照平台、本机或模板注册、任意标记、推送通知服务标识符、注册 ID 以及过期日期进行分类。 还可以在此页面中编辑注册。 它对于编辑标记特别有用。

在**服务器资源管理器**中右键单击你的**通知中心**，然后选择“诊断”。 
...
![Visual Studio - 测试性发送](./media/notification-hubs-diagnosing/test-send-vs.png)
...
> [!NOTE]
> 使用 `EnableTestSend` 属性受到严重限制。 此选项只能在开发/测试环境中与一组有限的注册结合使用。 我们仅向 10 台设备发送调试通知。 此外，我们每分钟向 10 台设备发送有限的处理调试。
...
2. 在“监视器”选项卡上，可以添加许多其他平台特定指标，以便进行深入了解。 可以专门查看在通知中心服务尝试将通知发送给推送通知服务时返回的任何与推送通知服务相关的错误。

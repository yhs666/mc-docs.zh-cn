...
可以将业务逻辑转变为用于 Azure IoT Edge 的模块。 本文介绍如何使用 Visual Studio Code (VS Code) 作为主要工具来开发和调试 Python 模块。

## <a name="prerequisites"></a>先决条件
本文假设使用运行 Windows 或 Linux 的计算机或虚拟机作为开发计算机。 在开发计算机上使用 IoT Edge 安全守护程序模拟 IoT Edge 设备。

> [!NOTE]
> 本调试文章演示了如何在模块容器中附加进程并使用 VS Code 对它进行调试。 只能调试 Linux amd64 容器中的 Python 模块。 如果你不熟悉 Visual Studio Code 的调试功能，请阅读有关[调试](https://code.visualstudio.com/Docs/editor/debugging)的信息。 
...
执行以下步骤，使用 Visual Studio Code 和 Azure IoT Edge 扩展创建基于 Azure IoT Python SDK 的 IoT Edge 模块。 首先创建一个解决方案，然后生成该解决方案中的第一个模块。 每个解决方案可以包含多个模块。 
...
解决方案附带的默认 Python 模块代码位于“模块”> [你的模块名称] >“main.py”。 设置模块和 deployment.template.json 文件，以便可以生成解决方案，将其推送到容器注册表，然后将其部署到设备以开始测试而无需触及任何代码。 该模块构建为只需从源（在此示例中，为模拟数据的 tempSensor 模块）获取输入并通过管道将其传送到 IoT Hub。 
...
每个模块文件夹中都有适用于不同容器类型的多个 Docker 文件。 使用任何以扩展名 **.debug** 结尾的文件来生成用于测试的模块。 目前，Python 模块仅支持在 Linux amd64 容器中进行调试。 
...
3. 导航到 `main.py`，并在 import 节后面添加以下代码
...
4. 将以下单行代码添加到要调试的回调
   ...
   例如，若要调试 `receive_message_callback` 方法， 可以插入如下所示的单行代码。
...
1. 导航到 VS Code 调试视图。 选择你的模块的调试配置文件。 调试选项名称应类似于“ModuleName 远程调试(Python)”

2. 导航到 `main.c`。 在已插入 `ptvsd.break_into_debugger()` 的回调方法中添加一个断点。

3. 选择“开始调试”或选择 **F5**。 选择要附加到的进程。

4. 在 VS Code 调试视图中，将在左侧面板中看到变量。 

上面的示例展示了如何调试容器上的 Python IoT Edge 模块。 它在模块容器 createOptions 中添加了公开的端口。 完成 Python 模块的调试后，我们建议删除针对生产就绪型 IoT Edge 模块公开的端口。

...
对于本教程中，需要部署两个模块。 第一个是 **tempSensor**，这是一个用于模拟温度和湿度传感器的模块。 第二个是流分析作业。 传感器模块提供了作业查询将要分析的数据流。 

1. 在 Azure 门户的 IoT 中心转到“IoT Edge”，然后打开 IoT Edge 设备的详细信息页。

1. 选择“设置模块”。  

1. 如果以前在此设备上部署了 tempSensor 模块，则它可以自动填充数据。 如果该模块不存在，请通过以下步骤来添加该模块：
...
1. 流分析作业发布到已创建的存储容器后，请单击该模块名称以查看流分析模块的构造方式。 
...
1. 记下流分析模块的名称（因为将在下一步中需要它），然后选择“下一步”以继续。

1. 将“Routes”中的默认值替换为以下代码。 使用你的 Azure 流分析模块名称更新 _{moduleName}_ 的所有三个实例。 
   ...
   此处声明的路由定义通过 IoT Edge 设备的数据流。 来自 tempSensor 的遥测数据将发送到 IoT 中心和在流分析作业中配置的 **temperature** 输入。 **alert** 输出消息将发送到 IoT 中心和 tempSensor 模块以触发重置命令。 

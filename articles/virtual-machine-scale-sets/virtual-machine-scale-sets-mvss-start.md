...
ms.date: 07/24/2017
ms.author: v-dazen
ms.openlocfilehash: 9c265340d774b0ccfee954de5eb3a551859ffab9
...
---
# 了解虚拟机规模集模板
<a id="learn-about-virtual-machine-scale-set-templates" class="xliff"></a>
...
## 查看模板
<a id="review-the-template" class="xliff"></a>
...
## 定义 $schema 和 contentVersion
<a id="define-schema-and-contentversion" class="xliff"></a>
...
## 定义资源
<a id="define-resources" class="xliff"></a>
...
## 指定位置
<a id="specify-location" class="xliff"></a>
为了指定虚拟网络的位置，我们使用了 [Resource Manager 模板函数](../azure-resource-manager/resource-group-template-functions.md)。 必须将此函数括在引号和方括号中，如下所示：`"[<template-function>]"`。 在本例中，我们使用了 `resourceGroup` 函数。 该函数不使用任何参数，返回 JSON 对象以及有关要将部署部署到的资源组的元数据。 资源组在部署时由用户进行设置。 然后使用 `.location` 为此 JSON 对象建立索引，以便从此 JSON 对象中获取位置。
...
## 指定虚拟网络属性
<a id="specify-virtual-network-properties" class="xliff"></a>
...
## 添加 dependsOn 列表
<a id="add-dependson-list" class="xliff"></a>
...
```
## 指定规模集属性
<a id="specify-scale-set-properties" class="xliff"></a>
...
### 提供 VM 大小和容量
<a id="supply-vm-size-and-capacity" class="xliff"></a>
...
### 选择更新类型
<a id="choose-type-of-updates" class="xliff"></a>
...
### 选择 VM 操作系统
<a id="choose-vm-operating-system" class="xliff"></a>
...
<a id="specify-computernameprefix" class="xliff"></a>
...
### 指定 VM 网络配置
<a id="specify-vm-network-configuration" class="xliff"></a>

...
ms.date: 03/05/2018
ms.author: v-junlch
ms.openlocfilehash: 50f1127ee7533cba41047129498fb2506242b08a
...
REST API：`GET https://management.chinacloudapi.cn/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Compute/virtualMachineScaleSets/{vmScaleSetName}?api-version={apiVersion}`（有关详细信息，请参阅 [REST API 文档](https://docs.microsoft.com/rest/api/compute/virtualmachinescalesets/get)）
...
CLI：`az vmss show -g {resourceGroupName} -n {vmSaleSetName}`（有关详细信息，请参阅 [CLI 文档](/cli/vmss?view=azure-cli-latest#az_vmss_show)）

也可使用 [Azure SDK](/downloads/) 来查询规模集的模型。
...
REST API：`GET https://management.chinacloudapi.cn/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Compute/virtualMachineScaleSets/{vmScaleSetName}/instanceView?api-version={apiVersion}`（有关详细信息，请参阅 [REST API 文档](https://docs.microsoft.com/rest/api/compute/virtualmachinescalesets/getinstanceview)）
...
CLI：`az vmss get-instance-view -g {resourceGroupName} -n {vmSaleSetName}`（有关详细信息，请参阅 [CLI 文档](/cli/vmss?view=azure-cli-latest#az_vmss_get_instance_view)）

也可使用 [Azure SDK](/downloads/) 来查询规模集的实例视图。
...
REST API：`GET https://management.chinacloudapi.cn/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Compute/virtualMachineScaleSets/{vmScaleSetName}/virtualmachines/{instanceId}?api-version={apiVersion}`（有关详细信息，请参阅 [REST API 文档](https://docs.microsoft.com/rest/api/compute/virtualmachinescalesetvms/get)）
...
CLI：`az vmss show -g {resourceGroupName} -n {vmSaleSetName} --instance-id {instanceId}`（有关详细信息，请参阅 [CLI 文档](/cli/vmss?view=azure-cli-latest#az_vmss_show)）

也可使用 [Azure SDK](/downloads/) 来查询规模集中 VM 的模型。
...
可以看到，这些属性描述的是 VM 本身的配置，而不是规模集作为一个整体的配置。 例如，规模集模型使用 `overprovision` 作为属性，而规模集中 VM 的模型则不是这样。 之所以存在这种差异，是因为过度预配是规模集作为一个整体的属性，而不是规模集中各个 VM 的属性（有关过度预配的详细信息，请参阅[此文档](/virtual-machine-scale-sets/virtual-machine-scale-sets-design-overview#overprovisioning)）。
...
REST API：`GET https://management.chinacloudapi.cn/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Compute/virtualMachineScaleSets/{vmScaleSetName}/virtualmachines/{instanceId}/instanceView?api-version={apiVersion}`（有关详细信息，请参阅 [REST API 文档](https://docs.microsoft.com/rest/api/compute/virtualmachinescalesetvms/getinstanceview)）
...
CLI：`az vmss get-instance-view -g {resourceGroupName} -n {vmSaleSetName} --instance-id {instanceId}`（有关详细信息，请参阅 [CLI 文档](/cli/vmss?view=azure-cli-latest#az_vmss_get_instance_view)）

也可使用 [Azure SDK](/downloads/) 来查询规模集中 VM 的实例视图。
...
REST API：`PUT https://management.chinacloudapi.cn/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Compute/virtualMachineScaleSets/{vmScaleSetName}?api-version={apiVersion}`（有关详细信息，请参阅 [REST API 文档](https://docs.microsoft.com/rest/api/compute/virtualmachinescalesets/createorupdate)）
...
CLI。 若要修改属性，请使用：`az vmss update --set {propertyPath}={value}`。 若要向规模集中的列表属性添加对象，请使用：`az vmss update --add {propertyPath} {JSONObjectToAdd}`。 若要从规模集中的列表属性删除对象，请使用：`az vmss update --remove {propertyPath} {indexToRemove}`。 （有关详细信息，请参阅 [CLI 文档](/cli/vmss?view=azure-cli-latest#az_vmss_update)）。 或者，如果此前已使用 `az vmss create` 命令部署规模集，则可再次运行 `az vmss create` 命令来更新规模集。 为此，需确保 `az vmss create` 命令中的所有属性都与以前的一样，要修改的属性除外。



也可使用 [Azure SDK](/downloads/) 来更新规模集模型。
...
REST API：`POST https://management.chinacloudapi.cn/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Compute/virtualMachineScaleSets/{vmScaleSetName}/manualupgrade?api-version={apiVersion}`（有关详细信息，请参阅 [REST API 文档](https://docs.microsoft.com/rest/api/compute/virtualmachinescalesets/updateinstances)）
...
CLI：`az vmss update-instances -g {resourceGroupName} -n {vmScaleSetName} --instance-ids {instanceIds}`（有关详细信息，请参阅 [CLI 文档](/cli/vmss?view=azure-cli-latest#az_vmss_update_instances)）。

也可使用 [Azure SDK](/downloads/) 在规模集中的 VM 上进行手动升级。

>[!NOTE]
> Service Fabric 群集只能使用“自动”模式，但采用不同方式来处理更新。 有关 Service Fabric 更新的详细信息，请参阅 [Service Fabric 文档](/service-fabric/service-fabric-application-upgrade)。

>[!NOTE]
> 有一类对全局规模集属性的修改不遵循升级策略。 这些是对规模集 OS 配置文件（例如，管理员用户名和密码）的更改。 这些更改仅适用于在对规模集模型进行更改后创建的 VM。 若要更新现有的 VM，必须对每个现有的 VM 执行“重置映像”操作。 可通过以下方式执行此重置映像操作：
...
也可使用 [Azure SDK](/downloads/) 对规模集中的 VM 重置映像。
...
>[!NOTE]
> 使用平台映像时，通常指定 "latest" 作为映像引用版本。 这意味着，在完成规模集的创建、横向扩展和重置映像操作期间，VM 使用最新发布的版本创建。 但是，这**并不**意味着 OS 映像会随新映像版本的发布自动进行更新。 这是独立的功能，目前为预览版。 

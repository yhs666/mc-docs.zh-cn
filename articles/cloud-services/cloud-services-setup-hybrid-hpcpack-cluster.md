...
ms.date: 10/09/2017
ms.author: v-yiso
ms.openlocfilehash: c3ac93e70e459d5971a857b700595d06565fda87
...
9. 在安装完毕后，取消选中“启动 HPC 群集管理器”，然后单击“完成”。 （可在后面的步骤中启动 HPC 群集管理器。）
   
    ![完成][install_hpc7]

## <a name="prepare-the-azure-subscription"></a>准备 Azure 订阅
在 [Azure 门户](https://portal.azure.cn)中执行以下 Azure 订阅步骤。 完成这些步骤后，可从本地头节点部署 Azure 节点。 
...
HPC Pack 将在头节点上安装名为“默认 Microsoft HPC Azure 管理证书”的自签名证书，可将其上传为 Azure 管理证书。 此证书用于测试和概念证明部署，以保护头节点和 Azure 之间的连接。

1. 从头节点计算机登录到 [Azure 门户](https://portal.azure.cn)。
...
6. 在“安装凭据”对话框中，键入要用于安装 HPC Pack 的域帐户的凭据  。 。

    ![安装凭据][config_hpc6]
   
7. 在“部署待办事项列表”中，单击“提供安装凭据” 中单击“云服务”&gt; 。

8. 在“指定节点命名序列”对话框中，接受默认命名系列，然后单击“确定”。 完成此步骤，即使已自动命名在本教程中添加的 Azure 节点。
   
    ![节点名称][config_hpc8]
   
9. 在“部署待办事项列表”中，单击“提供安装凭据” 中单击“云服务”&gt; 。 稍后本教程中将使用节点模板向群集添加 Azure 节点。
...
10. 在创建节点模板向导中，执行以下操作：
    ...
    c. 在“提供订阅信息”页上，输入 Azure 订阅 ID（在 Azure 帐户信息中提供）。 然后，在“管理证书”中，浏览“默认 Microsoft HPC Azure 管理”。 。
    
    ![节点模板][config_hpc12]

    d. 在“提供服务信息”页上，选择在前一步中创建的云服务和存储帐户。 。
...
3. 几分钟后，Azure 节点完成设置并处于“脱机”状态  。 在此状态下，角色实例尽管在运行，但不会接受群集作业。

4. 要确认角色实例正在运行，在 Azure 门户中，单击“云服务（经典）” > your_cloud_service_name。
   
   应会看到服务中正在运行两个“HpcWorkerRole”实例（节点）。 HPC Pack 还将自动部署两个 HpcProxy 实例（中等大小），以便处理头节点和 Azure 之间的通信。

    ![运行实例][view_instances1]

5. 如果要使 Azure 节点联机以便运行群集作业，请选择这些节点，右键单击，并单击“联机” 。
...
现在，提交在混合群集上运行的测试作业。 此示例是一个简单的参数扫描作业（一种固有的并行计算）。 此示例运行多个子任务，这些子任务通过使用 **set /a** 命令向其本身添加整数。 群集中的所有节点都参与完成针对 1 到 100 的整数的子任务。
...
试用群集后，可停止 Azure 节点，以免向帐户收取不必要的费用。 此步骤将停止云服务并且删除 Azure 角色实例。
...
3. 节点转换到“正在停止”状态  。 几分钟后，HPC 群集管理器会显示节点处于“未部署”状态 。

...
ms.date: 07/10/2017
ms.author: v-yeche
ms.openlocfilehash: 209d6cd1ef2753049a360113cc4a057e41b431cb
...
---
# 对 Azure 负载均衡器进行故障排除
<a id="troubleshoot-azure-load-balancer" class="xliff"></a>

此页介绍有关 Azure 负载均衡器常见问题的疑难解答信息。 负载均衡器连接不可用时，最常见的症状如下： 
- 负载均衡器后端的 VM 不响应运行状况探测 
- 负载均衡器后端的 VM 不响应已配置端口上的流量

## 症状：负载均衡器后端的 VM 不响应运行状况探测
<a id="symptom-vms-behind-the-load-balancer-are-not-responding-to-health-probes" class="xliff"></a>
...
### 原因 1：负载均衡器后端池 VM 不正常
<a id="cause-1-load-balancer-backend-pool-vm-is-unhealthy" class="xliff"></a> 

验证及解决方法

要解决此问题，请登录参与的 VM，检查 VM 状态是否正常，能否从池中的另一 VM 响应 PsPing 或 TCPing。 如果 VM 不正常或无法响应探测，必须先解决此问题并使 VM 恢复正常状态，该 VM 才可参与负载均衡。

### 原因 2：负载均衡器后端池 VM 未侦听探测端口
<a id="cause-2-load-balancer-backend-pool-vm-is-not-listening-on-the-probe-port" class="xliff"></a>
...
###原因 3：防火墙或网络安全组阻止负载均衡器后端池 VM 上的端口
<a id="cause-3-firewall-or-a-network-security-group-is-blocking-the-port-on-the-load-balancer-backend-pool-vms" class="xliff"></a>  
...
### 原因 4：负载均衡器中的其他错误配置
<a id="cause-4-other-misconfigurations-in-load-balancer" class="xliff"></a>
...
## 症状：负载均衡器后端的 VM 不响应已配置数据端口上的流量
<a id="symptom-vms-behind-load-balancer-are-not-responding-to-traffic-on-the-configured-data-port" class="xliff"></a>
...
### 原因 1：负载均衡器后端池 VM 未侦听数据端口
<a id="cause-1-load-balancer-backend-pool-vm-is-not-listening-on-the-data-port" class="xliff"></a> 
...
### 原因 2：网络安全组阻止负载均衡器后端池 VM 上的端口
<a id="cause-2-network-security-group-is-blocking-the-port-on-the-load-balancer-backend-pool-vm" class="xliff"></a>  
...
### 原因 3：从相同的 VM 和网络接口访问负载均衡器
<a id="cause-3-accessing-the-load-balancer-from-the-same-vm-and-network-interface" class="xliff"></a> 
...
### 原因 4：从参与的负载均衡器后端池 VM 访问 Internet 负载均衡器 VIP
<a id="cause-4-accessing-the-internet-load-balancer-vip-from-the-participating-load-balancer-backend-pool-vm" class="xliff"></a>
...
## 附加网络捕获
<a id="additional-network-captures" class="xliff"></a>

...
| --------- | ----------- | ----------- | --------------------------- | ----------------------------- | --------- |
| **单个言语 + 匹配的脚本** | 单个言语形式的音频文件 (.wav) 集合 (.zip)。 每个音频文件的长度不应超过 15 秒，并与带格式的脚本 (.txt) 配对。 | 包含匹配脚本的专业录制内容 | 做好训练准备。 | 对于 en-US 和 zh-CN 无硬性要求。 对于其他区域设置，需要 2,000 个以上的相异言语。 | 所有自定义语音区域设置 |
| **长音频 + 脚本 (beta)** | 长的未分段音频文件（长度超过 20 秒）集合 (.zip)，与包含所有口语的脚本 (.txt) 配对。 | 有音频文件和匹配的脚本，但它们未分段成言语。 | 分段（使用批量听录）。<br>根据需要进行音频格式转换。 | 对于 en-US 和 zh-CN 无硬性要求。 | `en-US` 和 `zh-CN` |
| **仅限音频 (beta)** | 不包含脚本的音频文件集合 (.zip)。 | 只有音频文件，没有脚本。 | 分段 + 脚本生成（使用批量听录）。<br>根据需要进行音频格式转换。| 对 `en-US` 和 `zh-CN` 没有硬性要求。 | `en-US` 和 `zh-CN` |
...
所有音频文件应分组成一个 .zip 文件。 可将 .wav 文件和 .mp3 文件放入一个音频 zip，但不允许 zip 文件中包含任何子文件夹。 例如，上传的 zip 文件可以包含一个名为“kingstory.wav”、长度为 45 秒的音频文件，以及一个名为“queenstory.mp3”、长度为 200 秒且不包含任何子文件夹的文件。 处理后，所有 .mp3 文件将转换为 .wav 格式。

...
了解如何部署语言检测容器。 此过程说明如何创建本地 Docker 容器，将容器推送到自己的专用容器注册表，在 Kubernetes 群集中运行容器，以及在 Web 浏览器中对其进行测试。 使用容器可以将开发人员的注意力从管理基础结构转移到应用程序开发上。
...
1. 根据下表更改 `language.yml` 的语言部署行，以添加自己的容器注册表映像名称、客户端密码和文本分析设置。
    ...
    |第 92 行<br> `billing` 属性|文本分析资源的账单终结点。<br>`https://<your region>.api.cognitive.azure.cn/text/analytics/v2.1`|

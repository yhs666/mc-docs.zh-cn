...
可以。使用托管磁盘的 Azure VM 允许这样做。 向支持复制的 Azure VM 添加新磁盘时，VM 的复制运行状况会显示一个警告，并在说明中指出 VM 上的一个或多个磁盘需要进行保护。 可以为添加的磁盘启用复制。
...
例如，如果你有一个应用程序，并且每层的应用、数据库和 Web 位于不同的资源组中，则必须三次单击[复制向导](/site-recovery/azure-to-azure-how-to-enable-replication#enable-replication)来保护所有层。 Site Recovery 会将这三个层复制到三个不同的资源组中。

...
ms.date: 03/21/2017
ms.author: v-johch
ms.openlocfilehash: 9b807701c2042a147a7741dbe09b96c91c1dc08c
...
<a id="multi-tenant-support-in-azure-site-recovery-for-replicating-vmware-virtual-machines-to-azure-through-the-csp-program" class="xliff"></a>
...
## 多租户环境
<a id="multi-tenant-environments" class="xliff"></a>
有三种主要的多租户模型：
...
## 共享托管多租户指南
<a id="shared-hosting-multi-tenant-guidance" class="xliff"></a>
...
<a id="every-cs-under-the-multi-tenant-scenario-uses-two-accounts" class="xliff"></a>
...
### vCenter 访问帐户的要求
<a id="requirements-for-vcenter-access-account" class="xliff"></a>
...
## 其他多租户环境
<a id="other-multi-tenant-environments" class="xliff"></a>

上述指南详述了如何设置共享托管解决方案的多租户环境。 其他两个主要解决方案为专用托管解决方案和托管服务解决方案。 这些解决方案的体系结构如下所示：

### 专用托管解决方案
<a id="dedicated-hosting-solution" class="xliff"></a>
...
### 托管服务解决方案
<a id="managed-service-solution" class="xliff"></a>
...
## CSP 计划概述
<a id="csp-program-overview" class="xliff"></a>
...
## 创建和管理租户帐户
<a id="creating-and-managing-tenant-accounts" class="xliff"></a>

### 步骤 0：先决条件检查
<a id="step-0-prerequisite-check" class="xliff"></a>
...
### 步骤 1：创建租户帐户
<a id="step-1-create-tenant-account" class="xliff"></a>
...
### 步骤 2：访问租户帐户
<a id="step-2-access-tenant-account" class="xliff"></a>
...
3.  单击“所有资源”按钮，系统就会授予你访问租户的 Azure 订阅的权限，而用户则可通过查看显示在 Azure 门户右上角的 AAD 来验证这一点。

    ![aad-admin](./media/site-recovery-multi-tenant-support-vmware-using-csp/aad-admin-display.png)

现在可以通过 Azure 门户执行针对租户的所有 Site Recovery 操作，以及管理 DR 操作。 每次通过 CSP 访问租户订阅以实现托管 DR 时，都必须遵循上述详细过程。

### 步骤 3：将资源部署到租户订阅
<a id="step-3-deploy-resources-to-tenant-subscription" class="xliff"></a>
...
<a id="step-4-register-site-recovery-infrastructure-to-recovery-services-vault" class="xliff"></a>
...
### 步骤 5：分配租户对订阅的访问权限
<a id="step-5-assign-tenant-access-to-subscription" class="xliff"></a>

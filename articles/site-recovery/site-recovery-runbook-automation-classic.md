...
如果还没有听说过 Azure 自动化，请单击[此处](https://www.azure.cn/home/features/automation/)进行注册，并从[此处](https://www.azure.cn/documentation/scripts/)下载示例脚本。 若要详细了解 [Azure Site Recovery](https://www.azure.cn/home/features/site-recovery/)，以及如何使用恢复计划来安排 Azure 恢复，请单击[此处](https://www.azure.cn/blog/?p=166264)。

这篇简短的教程介绍如何将 Azure 自动化 Runbook 集成到恢复计划。 我们自动执行以前需要手动干预的简单任务，并了解如何将多步骤恢复转换成单击恢复操作。 我们还会了解如何解决已出错的简单脚本。
...
在本教程中，我们为 Fabrikam HRweb 应用程序创建一个恢复计划，以将应用程序故障转移到 Azure。 然后，我们会将它与某个 Runbook 集成，该 Runbook 将在故障转移的 Azure 虚拟机上创建一个终结点，以便为端口 80 上的网页提供服务。

首先，让我们为应用程序创建恢复计划。

## <a name="create-the-recovery-plan"></a>创建恢复计划
要将应用程序恢复到 Azure，你需要创建一个恢复计划。
...
若要了解有关恢复计划的详细信息，请阅读 [此处](https://msdn.microsoft.com/library/azure/dn788799.aspx "此处")。

接下来，让我们在 Azure 自动化中创建必要的项目。

## <a name="create-the-automation-account-and-its-assets"></a>创建自动化帐户及其资产
需要使用一个 Azure 自动化帐户来创建 Runbook。 如果你还没有帐户，请导航到 ![](media/site-recovery-runbook-automation/02.png) 指示的“Azure 自动化”选项卡，并创建一个新帐户。
...
若要详细了解如何通过 PowerShell 连接到订阅，请单击[此处](https://docs.microsoft.com/powershell/azure/overview)。
...
3. 在故障转移到 Azure 脚本时，选择 Azure 自动化帐户名。

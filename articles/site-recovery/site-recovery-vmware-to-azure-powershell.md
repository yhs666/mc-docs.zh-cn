---
title: "使用 Azure Site Recovery PowerShell 将 VMware 虚拟机复制和故障转移到 Azure | Azure"
description: "使用 Azure Site Recovery PowerShell 将 VMware 虚拟机复制和故障转移到 Azure"
...
> - 创建恢复服务保管库。
> - 设置保管库上下文。
> - 验证配置服务器和横向扩展进程服务器是否已注册到保管库。
> - 创建并映射复制策略，以便在配置服务器上使用。
...
开始之前：
- 请确保了解[方案体系结构和组件](concepts-vmware-to-azure-architecture.md)。
...
使用 Login-AzureRmAccount -EnvironmentName AzureChinaCloud -EnvironmentName AzureChinaCloud cmdlet 登录到 Azure 订阅
...
* 创建要在其中创建恢复服务保管库的资源组。 在以下示例中，资源组名为 VMwareDRtoAzurePS，它是在中国东部区域创建的。
...
* 创建恢复服务保管库。 在以下示例中，恢复服务保管库名为 VMwareDRToAzurePs，它是在中国东部区域创建的，并位于上一步骤创建的资源组中。
...
* 下载保管库的保管库注册密钥。 保管库注册密钥用于将本地配置服务器注册到此保管库。 注册属于配置服务器软件安装过程的一部分。
...
使用下载的保管库注册密钥并遵循下面所列文章中的步骤，完成配置服务器的安装和注册。
* [选择保护目标](site-recovery-set-up-vmware-to-azure.md#choose-your-protection-goals)
* [设置源环境](site-recovery-set-up-vmware-to-azure.md#set-up-the-source-environment) 
...
## <a name="validate-that-your-configuration-server-and-scale-out-process-servers-are-registered-to-the-vault"></a>验证配置服务器和横向扩展进程服务器是否已注册到保管库

 假设：
- 已将名为 *ConfigurationServer* 的配置服务器注册到此保管库
- 已将名为 *ScaleOut ProcessServer* 的附加进程服务器注册到 *ConfigurationServer*
- 已在配置服务器上设置名为 *vCenter_account*、*WindowsAccount* 和 *LinuxAccount* 的帐户。 这些帐户用于添加 vCenter 服务器来发现虚拟机，并在要复制的 Windows 和 Linux 服务器上推送安装移动服务软件。

已注册的配置服务器由 Azure Site Recovery 中的某个结构对象表示。 此步骤获取保管库中的结构对象列表，并标识配置服务器。
...
## <a name="create-a-replication-policy-and-map-it-for-use-with-the-configuration-server"></a>创建并映射复制策略，以便在配置服务器上使用
...
* 创建名为 *ReplicationPolicy* 的复制策略，以使用指定的属性将 VMware 虚拟机复制到 Azure。
...
## <a name="add-a-vcenter-server-and-discover-vmware-virtual-machines"></a>添加 vCenter 服务器并发现 VMware 虚拟机
...
## <a name="configure-failover-settings-for-replicating-virtual-machines"></a>配置故障转移设置用于复制虚拟机
...
```
成功完成测试故障转移作业后，可以看到，Azure 中创建了一个虚拟机，其名称带有后缀 *-Test*（在本例中为 Win2K12VM1-Test）。 

现在，可以连接到测试故障转移虚拟机并验证测试故障转移。

使用 Start-ASRTestFailoverCleanupJob cmdlet 清理测试故障转移。 此操作会删除执行测试故障转移操作过程中创建的虚拟机。
...
此步骤将虚拟机 Win2K12VM1 故障转移到特定的恢复点。
...
成功完成故障转移后，可以提交故障转移操作，并设置从 Azure 到本地 VMware 站点的反向复制。

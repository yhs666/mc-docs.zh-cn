...
ms.date: 07/10/2017
ms.author: v-yeche
ms.openlocfilehash: c6ce9c8eb6e023a4ac39db18f7c4d218e696b912
...
<a id="replicate-between-on-premises-hyper-v-virtual-machines-and-azure-by-using-powershell-and-azure-resource-manager" class="xliff"></a>
...
另请参阅有关[将 Azure PowerShell 与 Azure Resource Manager 配合使用](../azure-resource-manager/powershell-azure-resource-manager.md)的更多信息。

> [!NOTE]
> 参与云解决方案提供商 (CSP) 计划的 Microsoft 合作伙伴可以根据客户各自的 CSP 订阅（租户订阅）情况对客户服务器的保护措施进行配置和管理。
>
>
...
* 一个 [Azure](https://www.azure.cn/) 帐户。 可以从[试用版](https://www.azure.cn/pricing/1rmb-trial/)开始。 此外，还可以参阅 [Azure Site Recovery Manager 定价](https://www.azure.cn/pricing/details/site-recovery/)。
* Azure PowerShell 1.0。 若要深入了解此版本及其安装方法，请参阅 [Azure PowerShell 1.0。](../powershell-install-configure.md)
* [AzureRM.SiteRecovery](https://www.powershellgallery.com/packages/AzureRM.SiteRecovery/) 和 [AzureRM.RecoveryServices](https://www.powershellgallery.com/packages/AzureRM.RecoveryServices/) 模块。 你可以从 [PowerShell 库](https://www.powershellgallery.com/)
...
## 步骤 1：登录到你的 Azure 帐户
<a id="step-1-sign-in-to-your-azure-account" class="xliff"></a>
1. 打开 PowerShell 控制台，然后运行以下命令以登录到 Azure 帐户。 该 cmdlet 会打开一个网页，提示你输入帐户凭据。
    ...
    此外，也可以通过 `-Credential` 参数将帐户凭据作为参数包含在 `Login-AzureRmAccount -EnvironmentName AzureChinaCloud` cmdlet 中。
...
## 步骤 2：设置恢复服务保管库
<a id="step-2-set-up-the-recovery-services-vault" class="xliff"></a>
1. 创建一个可在其中创建该保管库的 Azure Resource Manager 资源组，或者使用现有资源组。 可以使用以下命令创建新的资源组：
    ...
    其中，$ResourceGroupName 变量包含需要创建的资源组的名称，$Geo 变量包含要在其中创建资源组的 Azure 区域（例如：“中国东部”）。
...
> [!NOTE]
> 如果希望对通过经典管理门户或 Azure 服务管理 PowerShell 模块创建的 Site Recovery 保管库执行操作，可以使用 `Get-AzureRmSiteRecoveryVault` cmdlet 检索此类保管库列表。 你需要对所有新操作都创建新的恢复服务保管库。 你此前创建的 Site Recovery 保管库将继续受到支持，但不会有最新功能。
>
>
...
<a id="step-4-create-a-hyper-v-site-and-generate-a-new-vault-registration-key-for-the-site" class="xliff"></a>
...
<a id="step-5-install-the-azure-site-recovery-provider-and-azure-recovery-services-agent-on-your-hyper-v-host" class="xliff"></a>
...
<a id="step-6-create-a-replication-policy-and-associate-it-with-the-protection-container" class="xliff"></a>
...
## 步骤 8：运行测试故障转移
<a id="step-8-run-a-test-failover" class="xliff"></a>

...
在多种场合下，我们希望将现有的 Azure IaaS 虚拟机从一个区域移到另一个区域，以提高现有 VM 的可靠性和可用性、提高可管理性、或者满足监管要求等，具体请参阅[此文](azure-to-azure-move-overview.md)中的详述。 除了使用 [Azure Site Recovery](site-recovery-overview.md) 服务管理和协调本地计算机和 Azure VM 的灾难恢复以实现业务连续性和灾难恢复 (BCDR) 外，还可以使用 Site Recovery 来管理以另一个区域为目的地的 Azure VM 移动。       
...
> [!IMPORTANT]
> 本文档指导你将 Azure VM 按现样从一个区域移动到另一个区域。
...
- 验证帐户权限：如果你刚刚创建了试用版 Azure 帐户，那么你就是订阅的管理员。 如果你不是订阅管理员，请与管理员一起分配你所需的权限。 若要为 VM 启用复制，并使用 Azure Site Recovery 按原样复制数据，必须：
...
4. 标识源网络布局以及当前正在使用的所有资源 - 包括但不限于负载均衡器、NSG、公共 IP，等等。
...
3. 选择要将 Azure VM 移到的目标 Azure 虚拟网络，以测试配置。 
    ...
    > [!IMPORTANT]
    > 我们建议使用单独的 Azure VM 网络来测试故障，而不要使用 VM 最终要移到的目标 VM 中的生产网络。 启用复制时设置。

---
title: "准备使用 Azure Site Recovery 将本地 VMware 资源复制到 Azure | Azure"
...
ms.date: 08/28/2017
ms.author: v-yeche
ms.openlocfilehash: 6d68ddc3cdd125fe651e029f096f04bb5539cfc5
...
使用本文中的说明准备本地 VMware 服务器，以与 Azure Site Recovery 进行交互，并准备 VMWare VM 以用于安装移动服务。 必须在要复制到 Azure 的所有本地 VM 上安装移动服务代理。

## <a name="prepare-for-automatic-discovery"></a>准备自动发现

Site Recovery 可自动发现在 vSphere ESXi 主机（有或没有 vCenter 服务器）上运行的 VM。 若要进行自动发现，Site Recovery 需要一个用于访问主机和服务器的帐户：

1. 若要使用专用帐户，请创建一个角色（在 vCenter 级别，具有下表所述的权限）。 为其指定一个名称，例如 Azure_Site_Recovery。
...
## <a name="prepare-for-push-installation-of-the-mobility-service"></a>准备移动服务的推送安装

必须在要复制的所有 VM 上安装移动服务。 安装该服务的方法有数种，包括手动安装、从 Site Recovery 进程服务器推送安装，以及使用 System Center Configuration Manager 等方法进行安装。

---
title: "将 VMware VM 从 Azure 故障回复到本地 | Azure"
...
* 当从 Azure 故障转移回复到 VMware 时，受保护的物理计算机会回复成虚拟机。 受保护且已故障转移到 Azure 的 Windows Server 2008 R2 SP1 物理计算机无法故障回复。 作为本地虚拟机启动的 Windows Server 2008 R2 SP1 计算机可执行故障回复。
...
如果已将虚拟机作为经典资源进行保护（即，在 Azure 中恢复的 VM 是使用经典部署模型创建的 VM），则需要在 Azure 中安装进程服务器。 如果已使用 Azure 资源管理器将 VM 恢复为部署类型，则进程服务器必须为资源管理器部署类型。 在其中部署进程服务器的 Azure 虚拟网络会选择相应部署类型。
...
6. 如果选择了经典部署模型网络，则通过 Azure Marketplace 创建 VM 并将进程服务器安装于其中。
 ...
 创建进程服务器时，请注意以下事项：
 * 映像的名称为“Azure Site Recovery 进程服务器 V2”。 选择“经典”作为部署模型。
...
7. 如果选择资源管理器 Azure 网络，请通过提供以下信息部署进程服务器：
...
1. 在管理服务器 VM 上至少安装 CentOS 6.6 操作系统。 将 ISO 留在 DVD 驱动器中，并启动系统。 跳过介质测试。 选择“美国英语”作为语言，选择“基本存储设备”，确保硬盘驱动器上没有任何重要数据，并单击“是”，丢弃所有数据。 输入管理服务器的主机名，并选择服务器网络适配器。  在“编辑系统”对话框中选择“自动连接”，并添加静态 IP 地址、网络和 DNS 设置。 指定时区。 要访问管理服务器，请输入根密码。
2. 当系统询问安装类型时，应选择“创建自定义布局”作为分区。 单击“下一步”。 选择“免费”，并单击“创建”。 使用 **FS Type:** **ext4** 创建 **/**、**/var/crash** 和 **/home 分区**。 创建交换分区，格式为 **FS Type: swap**。
...
2. 选择“高级” > “常规项”，并单击“配置参数”。 当计算机正在运行时，此选项不可用。 若要使其可用，必须关闭计算机。
3. 执行以下操作之一：
 * 如果 **disk.EnableUUID** 这一行存在，请确保将值设置为 **True**（区分大小写）。 如果已将值设置为 True，则可先取消，并在启动来宾操作系统后对其中的 SCSI 命令进行测试。
...
故障回复完成后，提交虚拟机以确保删除在 Azure 中恢复的 VM。 为此，请右键单击受保护的项，并单击“提交”。 此操作会触发一个作业，该作业会删除之前在 Azure 中恢复的虚拟机。
...
* 应在 Azure 虚拟网络上设置 ExpressRoute 连接，源计算机会故障转移到该网络，而故障转移完成后，Azure VM 也会位于其中。

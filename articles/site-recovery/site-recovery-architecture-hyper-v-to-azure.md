---
title: "如何使用 Azure Site Recovery 将 Hyper-V 复制到 Azure？ | Azure"
...
ms.date: 07/31/2017
ms.author: v-johch
ms.openlocfilehash: e7711f7147a8f17b5faddf09a83e0e8b328cb948
ms.sourcegitcommit: 66db84041f1e6e77ef9534c2f99f1f5331a63316
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 07/28/2017
---
# <a name="how-does-hyper-v-replication-to-azure-work-in-site-recovery"></a>将 Hyper-V 复制到 Azure 的工作原理是什么？
...
**组件** | **位置** | **详细信息**
--- | --- | ---
**Azure** | 在 Azure 中，需要一个 Azure 帐户、Azure 存储帐户和 Azure 网络。 | 复制的数据存储在存储帐户中；从本地站点故障转移时，将使用复制的数据创建 Azure VM。<br/><br/> 创建 Azure VM 后，它们将连接到 Azure 虚拟网络。
...
2. 该作业将检查计算机是否符合先决条件，并调用 [CreateReplicationRelationship](https://msdn.microsoft.com/library/hh850036.aspx)方法，以使用用户配置的设置来设置复制。
...
3. 在执行初始和增量复制期间，可以在 VM 视图中监视 VM。 [了解详细信息](site-recovery-monitoring-and-troubleshooting.md#monitor-replication-health-for-virtual-machines)。  
...
**类别** | **详细信息**
--- | ---
**不可恢复的错误** | 不尝试重试操作。 VM 状态为“严重”，并且需要管理员干预。 这些错误的示例包括：VHD 链断裂；副本 VM 的状态无效；网络身份验证错误：授权错误；“找不到 VM”错误（适用于独立的 Hyper-V 服务器）

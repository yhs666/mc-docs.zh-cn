---
title: "如何从 Azure 故障回复到 VMware | Azure"
...
1. [重新保护](site-recovery-how-to-reprotect.md) Azure 中的虚拟机，使其开始复制到本地站点中运行的 VMware 虚拟机。 在此过程中，还需要：
...
2. 在“确认故障转移”中，验证故障转移方向（从 Azure 故障转移），并选择要用于故障转移的恢复点（最新恢复点，或最新的应用一致性恢复点）。 应用一致性点会在最新的时间点之后，并会导致丢失部分数据。
...
如果选择最新处理的时间点，则所有虚拟机都会故障转移到其最新的可用时间点。 如果恢复计划中有一个复制组，则复制组的每个虚拟机都会故障转移到其独立的最新时间点。
...
如果选择应用程序一致恢复点，则单个虚拟机故障回复会恢复到其最新的可用应用程序一致恢复点。 如果恢复计划具有复制组，则每个复制组都会恢复到其公共的可用恢复点。
...
完成提交后，虚拟机会回到本地站点，但不受保护。 若要再次开始复制到 Azure，请执行以下操作：
...
重新保护作业完成后，虚拟机会复制回 Azure，然后可以执行故障转移。

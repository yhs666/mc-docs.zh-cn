---
title: "查看使用 Azure Site Recovery 执行 Hyper-V 到 Azure 的复制（不使用 System Center VMM）的体系结构 | Azure"
...
在没有 VMM 的情况下，将 Hyper-V VM 复制到 Azure 时涉及多个组件。

**组件** | **位置** | **详细信息**
--- | --- | ---
**Azure** | 在 Azure 中，需要一个 Azure 帐户、Azure 存储帐户和 Azure 网络。 | 复制的数据存储在存储帐户中；从本地站点故障转移时，将使用复制的数据创建 Azure VM。<br/><br/> 创建 Azure VM 后，这些 Azure VM 将连接到 Azure 虚拟网络。
**Hyper-V** | 将 Hyper-V 主机和群集收集到 Hyper-V 站点中。 Azure Site Recovery 提供程序和恢复服务代理安装在每个 Hyper-V 主机上。 | 提供程序通过 Internet 使用 Site Recovery 来安排复制。 恢复服务代理处理数据复制。<br/><br/> 来自提供程序和代理的通信都是安全且经过加密的。 Azure 存储中的复制数据也已加密。

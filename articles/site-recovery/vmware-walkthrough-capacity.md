...
ms.date: 08/28/2017
ms.author: v-yeche
ms.openlocfilehash: ecf2a7a3787036aafe21d7f5c58f187cb3d3e1b7
...
1. 下载 [Deployment Planner 工具](https://aka.ms/asr-deployment-planner)，以进行 VMware 复制。
2. [阅读此文](site-recovery-deployment-planner.md)了解如何运行该工具。
3. 使用该工具可以收集有关兼容的和不兼容 VM、每个 VM 的磁盘以及每个磁盘数据变化的信息。 该工具还介绍了成功复制和测试故障转移的网络带宽要求，以及所需的 Azure 基础结构。
...
按照这些说明设置额外的进程服务器。 设置服务器后，可以迁移源计算机来使用它。

1. 在“Site Recovery 服务器”中，单击“配置服务器”>“+进程服务器”。
2. 在“服务器类型”中，单击“进程服务器(本地)”。

    ![进程服务器](./media/vmware-walkthrough-capacity/migrate-ps2.png)
3. 下载 Site Recovery 统一安装程序文件。
4. 运行安装程序以安装进程服务器，并将其注册到保管库中。
5. 在“准备阶段”中选择“添加额外的进程服务器以扩大部署”。
6. 在“配置服务器详细信息”中，指定配置服务器的 IP 地址和密码。 如果没有密码，请在配置服务器上运行 **[SiteRecoveryInstallationFolder]\home\sysystems\bin\genpassphrase.exe –n** 来获取密码。

    ![配置服务器](./media/vmware-walkthrough-capacity/add-ps2.png)
7. 完成剩余的安装步骤，完成方式与安装配置服务器时采用的方式相同。

### <a name="migrate-machines-to-use-the-process-server"></a>对计算机进行迁移，以使用进程服务器
...
运行 [Deployment Planner 工具](site-recovery-deployment-planner.md)计算复制（初始复制和增量复制）所需的带宽后，可以使用下列几个选项来控制用于复制的带宽量：
...
4. 设置工作和非工作小时数限制。 有效范围为 512 Kbps 到 102 Mbps。
    ...
    ![限制](./media/vmware-walkthrough-capacity/throttle2.png)

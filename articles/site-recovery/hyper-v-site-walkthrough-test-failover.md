---
title: "为到 Azure 的 Hyper-V 复制（无 System Center VMM）运行测试故障转移 | Azure"
...
ms.date: 07/31/2017
ms.author: v-yeche
ms.openlocfilehash: 8efe9655783ffe7eb1951f263dfc8d4a9f01f5c8
...
本文介绍如何使用 Azure 门户中的 [Azure Site Recovery](site-recovery-overview.md) 服务为从本地 Hyper-V 虚拟机（未由 System Center VMM 管理的）到 Azure 的复制运行测试故障转移。
...
- 可以设置故障转移后的 Azure VM 要使用的目标 IP 地址。 如果未提供地址，故障转移的计算机使用 DHCP。 如果设置了无法用于故障转移的地址，故障转移将会失败。 如果地址可用于测试故障转移网络，则同一个目标 IP 地址可用于测试故障转移。

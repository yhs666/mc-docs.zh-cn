...
ms.date: 08/28/2017
ms.author: v-yeche
ms.openlocfilehash: 11668470a5b33de3235357bdd0102ddcfd9fede9
...
| **先决条件** | **详细信息** |
| --- | --- |
| **Virtual Machine Manager** |必须具有一台或多台运行 System Center 2012 R2 或更高版本的 VMM 服务器。 每台 VMM 服务器必须配置有一个或多个云。 <br/><br/>云必须具有：<br/>- 一个或多个 VMM 主机组。<br/>- 每个主机组中有一个或多个 Hyper-V 主机服务器或群集。<br/><br/>有关设置 VMM 云的详细信息，请参阅[如何在 Virtual Machine Manager 2012 中创建云](http://social.technet.microsoft.com/wiki/contents/articles/2729.how-to-create-a-cloud-in-vmm-2012.aspx)。 |
...
|--- | --- | --- | --- | --- |
|``*.accesscontrol.chinacloudapi.cn`` | 允许 | 允许 | 允许 | 允许 |
|``*.backup.windowsazure.cn`` | 不是必需 | 允许 | 允许 | 允许 |
|``*.hypervrecoverymanager.windowsazure.cn`` | 允许 | 允许 | 允许 | 允许 |
|``*.store.core.chinacloudapi.cn`` | 允许 | 允许 | 允许 | 允许 |
|``*.blob.core.chinacloudapi.cn`` | 不是必需 | 允许 | 允许 | 允许 |
|``https://dev.mysql.com/get/archives/mysql-5.5/mysql-5.5.37-win32.msi`` | 不是必需 | 不是必需 | 不是必需 | 允许进行 SQL 下载 |

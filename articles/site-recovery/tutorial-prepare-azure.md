...
如果你刚刚创建了试用版 Azure 帐户，那么你就是订阅的管理员。 如果你不是订阅管理员，请要求管理员分配你所需的权限。 若要为新虚拟机启用复制，必须有权执行以下操作：
...
已复制计算机的映像保存在 Azure 存储中。 在从本地故障转移到 Azure 时，会从该存储中创建 Azure VM。 存储帐户必须位于与恢复服务保管库相同的区域。 在本教程中，我们使用“中国北部”。

1. 在 [Azure 门户](https://portal.azure.cn)菜单中，选择“创建资源” > “存储” > “存储帐户 - blob、文件、表、队列”。
...
- [了解](/storage/common/storage-introduction#types-of-storage-accounts) Azure 存储的类型。
- [详细了解](/storage/common/storage-redundancy-grs#read-access-geo-redundant-storage)存储冗余和存储的[安全传输](/storage/common/storage-require-secure-transfer)。

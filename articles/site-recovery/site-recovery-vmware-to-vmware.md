---
title: "将 VMware VM 或物理服务器复制到另一站点（经典 Azure 门户）| Azure"
...
ms.date: 08/28/2017
ms.author: v-yeche
ms.openlocfilehash: 70f9372a028930c932ebfada374ef92c912ce3e0
...
**Azure 帐户**：需要一个 [Azure](https://azure.microsoft.com/) 帐户。 你可以从[试用版](https://www.azure.cn/pricing/1rmb-trial/)开始。 [详细了解](https://www.azure.cn/pricing/details/site-recovery/) Site Recovery 定价。
...
5. 在“位置”中，选择保管库的地理区域。 若要查看支持的区域，请参阅 [Azure Site Recovery 定价](https://www.azure.cn/pricing/details/site-recovery/)。
6. 如果要从仪表板快速访问保管库，请单击“固定到仪表板”，并单击“创建”。
...
1. 下载[更新](https://aka.ms/asr-scout-update5) .zip 文件。 此 .zip 文件包含以下文件：
...
9. **Linux 源服务器**：要更新统一代理，请将相应版本的 UA 文件复制到 Linux 服务器并将它解压缩。 在解压缩的文件夹中运行 **/Install**。  示例：对于 RHEL 6.7 64 位服务器，将 **UA_RHEL6-64_8.0.4.0_GA_Update_4_9035261_26Sep16.tar.gz** 复制到服务器并进行文件提取。 在解压缩的文件夹中运行 **/Install**。
...
从 ASR Scout update4 到 update5 期间添加的修复特定于主目标和 vContinuum 组件。 如果所有源服务器、主目标、配置服务器、进程服务器和 RX 都已处于 ASR Scout update4，则只需要在主目标服务器上应用 update 5。 

**新的平台支持**
* SUSE Linux Enterprise Server 11 Service Pack 4(SP4)

> [!NOTE]
> SLES 11 SP4 64 位 **InMage_UA_8.0.1.0_SLES11-SP4-64_GA_13Apr2017_release.tar.gz** 与基础 Scout GA 包 **InMage_Scout_Standard_8.0.1 GA.zip** 打包在一起。 可根据 [步骤 1](#step-1-create-a-vault)中所述，从门户下载 Scout GA 包。
>
...
Scout Update 4 是累积更新。 其中包含 update1 到 update3 的所有修复程序，以及下面的 bug 修复程序和增强功能。

**新的平台支持**

* 添加了对 vCenter/vSphere 6.0、6.1 和 6.2 的支持
* 添加了对以下 Linux 操作系统的支持
...
> [!NOTE]
> RHEL/CentOS 7 64 位 **InMage_UA_8.0.1.0_RHEL7-64_GA_06Oct2016_release.tar.gz** 与基础 Scout GA 程序包 **InMage_Scout_Standard_8.0.1 GA.zip** 一起打包。 可根据 [步骤 1](#step-1-create-a-vault)中所述，从门户下载 Scout GA 包。
>
>

**Bug 修复和增强功能**

* 改进了以下 Linux OS 和副本的关机处理方式，以防止意外的重新同步问题。
  * Red Hat Enterprise Linux (RHEL) 6.x
  * Oracle Linux (OL) 6.x
* 对于 Linux，统一代理安装目录中的完整文件夹访问权限现在仅限为本地用户。
* 在 Windows 上，在重负载分布式应用程序（例如 SQL 和 Share Point 群集）上发出通用分布式一致性书签时会出现的超时问题。
* 在 CX 基本安装程序中添加了日志相关的修复程序。
* 在 Windows 主目标基本安装程序中添加了 VMware vCLI 6.0 下载链接。
* 针对故障转移和灾难恢复演练期间的网络配置更改添加了更多的检查和日志。
* 有时不会向 CX 报告保留信息。  
* 对于物理群集，在发生源卷收缩时，通过 vContinuum 向导执行卷调整大小操作会失败。
* 当群集磁盘是 PRDM 磁盘时，群集保护失败并出现错误“找不到磁盘签名”。
* 由于发生超出范围异常，cxps 传输服务器崩溃。
* 现在，可在 vContinuum 向导的推送安装页中调整服务器名称和 IP 列的大小。
* RX API 增强功能
  * 提供了五个最新可用的通用一致性点（仅适用于“保证”标记）。
  * 提供所有受保护设备的容量和可用空间详细信息。
  * 提供源服务器上的 Scout 驱动程序状态。

> [!NOTE]
> * **InMage_Scout_Standard_8.0.1_GA.zip** 基础数据包现在包含更新的 CX 基础安装程序 **InMage_CX_8.0.1.0_Windows_GA_26Feb2015_release.exe** 和 Windows 主目标基础安装程序 **InMage_Scout_vContinuum_MT_8.0.1.0_Windows_GA_26Feb2015_release.exe**。 对于所有新安装，请使用新的 CX 和 Windows 主目标 GA 软件。
...
请在 [Azure 恢复服务论坛](https://social.msdn.microsoft.com/Forums/en-US/home?forum=hypervrecovmgr)上发布任何问题。

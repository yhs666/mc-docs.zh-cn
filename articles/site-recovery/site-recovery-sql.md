...
本文介绍如何结合使用 SQL Server 业务连续性和灾难恢复 (BCDR) 技术和 [Azure Site Recovery](./site-recovery-overview.md) 来保护应用程序的 SQL Server 后端。
...
> [!NOTE]
> 此功能目前以预览版提供。当主站点中的 Hyper-V 主机服务器在 System Center VMM 云中管理时，可以使用此功能。
>
>
...
>[!NOTE]
> 在执行未计划的故障转移之后，应在可用性组上触发反向复制以继续复制。在完成此操作之前，复制会保持暂停状态。
...
对于使用分布式事务的应用程序，建议使用 [SAN 复制](./site-recovery-vmm-san.md)部署 Site Recovery。
...
- 如果应用使用分布式事务，则建议针对 Hyper-V 环境使用 [SAN 复制](./site-recovery-vmm-san.md)。

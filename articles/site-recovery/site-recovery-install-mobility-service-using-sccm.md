...
* [使用命令行手动安装](./site-recovery-vmware-to-azure.md#install-mobility-service-on-a-windows-server-using-the-command-prompt)
* [使用配置服务器执行推送安装](./site-recovery-vmware-to-azure.md#install-the-mobility-service)
* [使用 Azure 自动化和 Desired State Configuration 自动安装](./site-recovery-automate-mobility-service-install.md)

## 后续步骤
现在，可为虚拟机[启用保护](./site-recovery-vmware-to-azure.md#step-6-replicate-applications)。

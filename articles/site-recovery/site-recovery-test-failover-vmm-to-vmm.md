...
| --- | --- | --- | --- |
| 故障转移到辅助 VMM 站点 — 无网络 |不选择 VM 网络。 |故障转移将检查是否已创建测试计算机。<br/><br/>在副本虚拟机所在的主机上创建测试虚拟机。 不会将它添加到副本虚拟机所在的云。 |<p>故障转移计算机未连接到任何网络。<br/><br/>创建后，计算机可连接到 VM 网络。 |
| 故障转移到辅助 VMM 站点 — 有网络 |选择现有 VM 网络。 |故障转移将检查是否已创建虚拟机。 |在副本虚拟机所在的主机上创建测试虚拟机。 不会将它添加到副本虚拟机所在的云。<br/><br/>创建与生产网络隔离的 VM 网络。<br/><br/>如果使用基于 VLAN 的网络，建议在 VMM 中创建一个单独逻辑网络（不在生产中使用）以实现此目的。 此逻辑网络用于创建实现测试故障转移所需的 VM 网络。<br/><br/>逻辑网络应至少与所有托管虚拟机的 Hyper-V 服务器中的一个网络适配器关联。<br/><br/>对于 VLAN 逻辑网络，应隔离添加到逻辑网络中的网络站点。<br/><br/>如果使用基于 Windows 网络虚拟化的逻辑网络，Azure Site Recovery 将自动创建隔离的 VM 网络。 |
| 故障转移到辅助 VMM 站点 — 创建网络 |根据在“逻辑网络”及其相关网站中指定的设置，自动创建一个临时测试网络。 |故障转移将检查是否已创建虚拟机。 |如果恢复计划使用多个 VM 网络，则使用此选项。 如果使用 Windows 网络虚拟化网络，此选项可使用副本虚拟机网络中的相同设置（子网和 IP 地址池）自动创建 VM 网络。 测试故障转移完成后，将自动清理这些 VM 网络。</p><p>在副本虚拟机所在的主机上创建测试虚拟机。 不会将它添加到副本虚拟机所在的云。 |

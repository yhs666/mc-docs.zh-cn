---
title: "连接工厂 Azure IoT 套件解决方案演练 | Microsoft Docs"
description: "介绍 Azure IoT 连接工厂预配置解决方案及其体系结构。"
services: 
suite: iot-suite
documentationcenter: 
author: dominicbetts
...
ms.openlocfilehash: f6e9ede58a5df802edb4c6ab7ded57e21f0e5399
ms.contentlocale: zh-cn
ms.lasthandoff: 05/19/2017


---
# <a name="connected-factory-preconfigured-solution-walkthrough"></a>连接工厂预配置解决方案演练

IoT 套件连接工厂[预配置解决方案][lnk-preconfigured-solutions]是端到端工业解决方案的实现，具有以下功能：

* 同时连接到在仿真工厂生产线中运行 OPC UA 服务器的仿真工业设备以及真实的 OPC UA 服务器设备。
...
本文将分步讲解连接工厂解决方案的一些关键要素，以帮助你了解其工作原理。 该知识有助于：
...
仿真工位和仿真制造执行系统 (MES) 构成了工厂生产线。 仿真设备和 OPC 发布服务器模块基于 OPC Foundation 发布的 [OPC UA .NET 标准][lnk-OPC-UA-NET-Standard]。

OPC 代理和 OPC 发布服务器实现为基于 [Azure IoT Edge][lnk-Azure-IoT-Gateway] 的模块。 每条仿真生产线都附加了一个指定的网关。

所有仿真组件都在 Azure Linux VM 中托管的 Docker 容器中运行。 默认情况下，仿真配置为运行八条模拟生产线。

## <a name="simulated-production-line"></a>仿真生产线

生产线制造各种部件。 它包括不同的工位：装配工位、测试工位和包装工位。

仿真运行并更新通过 OPC UA 节点公开的数据。 所有仿真生产线工位都由 MES 通过 OPC UA 进行安排。

## <a name="simulated-manufacturing-execution-system"></a>仿真制造执行系统

MES 通过 OPC UA 监视生产线中的每个工位来检测工位状态更改。 它调用 OPC UA 的方法来控制工位并将产品从一个工位传送到下一个工位，直到产品完成为止。

## <a name="gateway-opc-publisher-module"></a>网关 OPC 发布服务器模块

OPC 发布服务器模块连接到工位 OPC UA 服务器并订阅要发布的 OPC 节点。 该模块将节点数据转换为 JSON 格式，对其进行加密，然后将其作为 OPC UA 发布/订阅消息发送到 IoT 中心。
...
TSI 使用一个 SearchSpan（Time.From、Time.To）对节点数据进行查询并按 OPC UA ApplicationUri、OPC UA NodeId 或 OPC UA DisplayName 进行聚合。

...
ms.date: 01/09/2017
ms.author: v-dazen
ms.openlocfilehash: 992a153b22f4819f38ddc2d6faa2e1b3441685f8
...
### <a name="tags"></a>标记
使用标记可以指定不同的单词和短语用于帮助标识 Runbook。 例如，在向 [Runbook 库](automation-runbook-gallery.md)提交 Runbook 时，可以指定特定的标记，标识应将该 Runbook 列入的类别。 可为一个 Runbook 指定多个标记并用逗号分隔各个标记。
...
### <a name="changing-runbook-settings-with-the-azure-classic-management-portal"></a>使用 Azure 经典管理门户更改 Runbook 设置
...
可以使用 [Set-AzureAutomationRunbook](https://msdn.microsoft.com/library/dn690275.aspx) cmdlet 更改 Runbook 的设置。 如果想要指定多个标记，可以向 Tags 参数提供一个数组，或者一个包含逗号分隔值的字符串。 可以使用 [Get-AzureAutomationRunbook](https://msdn.microsoft.com/library/dn690278.aspx) 获取当前标记。

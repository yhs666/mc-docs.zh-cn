...
可以获取包含 cmdlet 的 [PowerShell 模块](automation-runbook-gallery.md)，这些 cmdlet 可以用在 [PowerShell 库](http://www.powershellgallery.com/)中的 Runbook 中。 可以手动下载并导入它们。 你不能直接从 Azure 经典管理门户安装这些模块，但可以在下载之后进行安装，就像使用任何其他模块一样。 

...
如果模块应包含 Azure 自动化连接类型，则还必须包含名为 `<ModuleName>-Automation.json` 的文件，以便指定连接类型属性。 这是一个 json 文件，它位于 .zip 压缩文件的 module 文件夹中。 此文件包含连接到模块所代表的系统或服务所需的**连接**的字段。 此配置最终会在 Azure 自动化中创建一个连接类型。 你可以使用此文件为模块的连接类型设置字段名称、类型，以及这些字段是否应加密和/或可选。 以下示例是采用 json 文件格式的模板：
...
2. 如果模块是针对远程系统运行的：
   ...
   2. 模块中的每个 cmdlet 都应能够使用连接对象（该连接类型的实例）作为参数。  
    ...
    如果你允许将对象传递给模块中的 cmdlet 并使用连接类型的字段作为参数，则这些 cmdlet 在 Azure 自动化中将更易于使用。 这让用户能够在每次调用 cmdlet 时将连接资产的参数映射到 cmdlet 的相应参数。

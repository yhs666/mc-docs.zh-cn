...
ms.date: 03/28/2017
ms.author: v-dazen
ms.openlocfilehash: a764ce0ca959808430da0b11f6d73fd12fd912f6
...
## <a name="configure-input-parameters-in-powershell-workflow-runbooks"></a>在 PowerShell 工作流 Runbook 中配置输入参数
...
#### <a name="start-a-published-runbook-by-using-the-azure-classic-management-portal-and-assign-parameters"></a>使用 Azure 经典管理门户启动已发布的 Runbook 并分配参数
[启动 Runbook](automation-starting-a-runbook.md#starting-a-runbook-with-the-azure-classic-management-portal) 时，“启动 Runbook”弹出窗口将会打开，可以为刚刚创建的参数配置值。
...
* Azure 服务管理 cmdlet：可以使用 [Start-AzureAutomationRunbook](https://msdn.microsoft.com/library/dn690259.aspx) 启动在默认资源组中创建的自动化 Runbook。
...
* **Azure 服务管理方法：** 可以使用编程语言的 SDK 启动 Runbook。 以下 C# 代码片段用于在自动化帐户中启动 Runbook。 可以在 [GitHub 存储库](https://github.com/Azure/azure-sdk-for-net/blob/master/src/ServiceManagement/Automation/Automation.Tests/TestSupport/AutomationTestBase.cs)中查看完整代码。  
...
可[将计划链接](automation-schedules.md)到 Runbook，以便在特定的时间启动 Runbook。 可以在创建计划时分配输入参数，Runbook 在按计划启动时，将使用这些值。 只有在提供所有必需参数值之后，才可以保存计划。

...
ms.date: 04/27/2017
ms.author: v-dazen
ms.openlocfilehash: 5a2a95b7f06e9f39b2179908f31e7e2482b1521e
...
首次启动 Azure 自动化时，你必须创建至少一个自动化帐户。 使用自动化帐户，可以将自动化资源（Runbook、资产、配置）与其他自动化帐户中包含的资源隔离。 可以使用自动化帐户将资源隔离到独立的逻辑环境中。 例如，你可以在开发环境中使用一个帐户，在生产环境中使用另一个帐户，并在本地环境中使用另一个账户。 Azure 自动化帐户不同于 Azure 帐户或在你的 Azure 订阅中创建的帐户。
...
| 方法 | 环境 | 文章 |
| --- | --- | --- |
| Azure AD 用户帐户 |Azure Resource Manager 和 Azure 服务管理 |[使用 Azure AD 用户帐户进行 Runbook 身份验证](automation-create-aduser-account.md) |
| Azure 运行方式帐户 |Azure Resource Manager | 无 |

...
对于序列链接，只会对条件评估一次，因为会返回单个包含源活动中所有对象输出的数组。 因此，不能使用序列链接来进行管道链接这样的筛选，但可以通过该链接来直接确定是否会运行下一活动。 以下面的活动集（位于 Start VM Runbook 中）为例。
...
“获取运行方式连接”活动（即 Get-AutomationConnection）配置了名为 AzureRunAsConnection 的常量值数据源。

![运行方式连接配置](media/automation-graphical-authoring-intro/authenticate-runas-parameterset.png)

下一个活动（即 Connect-AzureRmAccount）添加经过身份验证的可在 Runbook 中使用的运行方式帐户。
...
用户可以将此功能添加到图形 Runbook，只需将凭据资产添加到画布，并完成 Add-AzureAccount 活动即可。 Add-AzureAccount 使用凭据活动作为其输入。 以下示例对此进行了演示：
...
由任何没有传出链接的活动创建的数据将保存到 [Runbook 的输出](automation-runbook-output-and-messages.md)。  输出与 Runbook 作业一起保存，在该 Runbook 作为子 Runbook 使用的情况下，还可供父 Runbook 使用。  

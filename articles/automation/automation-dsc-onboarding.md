...
要以一般方式将任何计算机加入 Azure Automation State Configuration，可以生成应用时告知计算机上的 DSC 代理从 Azure Automation State Configuration 提取和/或报告的 [DSC 元配置](https://docs.microsoft.com/powershell/dsc/metaconfig)。 Azure Automation State Configuration 的 DSC 元配置可以使用 PowerShell DSC 配置或 Azure 自动化 PowerShell cmdlet 来生成。

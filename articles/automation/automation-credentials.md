...
description: Azure 自动化中的凭据资产包含可用于向 Runbook 所访问的资源进行身份验证的安全凭据。 本文介绍如何创建凭据资产并在 Runbook 中使用它们。
services: automation
author: yunan2016
manager: digimobile
...
下表中的 cmdlet 用于通过 Windows PowerShell 创建和管理自动化凭据资产。  可在自动化 Runbook 配置中使用的 [Azure PowerShell 模块](/powershell/azure/overview)已随附了这些 cmdlet。
...
对于 AzureRM，下表中的 cmdlet 用于通过 Windows PowerShell 创建和管理自动化凭据资产。  可在自动化 Runbook 配置中使用的 [AzureRM.Automation 模块](/powershell/azure/overview)已随附了这些 cmdlet。

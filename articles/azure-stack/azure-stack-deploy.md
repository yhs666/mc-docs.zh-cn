---
title: "Azure Stack 开发工具包部署先决条件 | Microsoft Docs"
description: "查看 Azure Stack 开发工具包的环境和硬件要求（云操作员）。"
...
ms.date: 03/02/2018
ms.author: v-junlch
ms.openlocfilehash: f1a50fd717c292fc7851c14d851396b47abe49cb
ms.sourcegitcommit: 34925f252c9d395020dc3697a205af52ac8188ce
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 03/02/2018
---
# <a name="azure-stack-deployment-prerequisites"></a>Azure Stack 部署先决条件

适用于：Azure Stack 开发工具包

在部署 [Azure Stack 开发工具包](azure-stack-poc.md)之前，请确保计算机满足以下要求：
...
| 磁盘驱动器：常规开发工具包数据* |4 磁盘。 每个磁盘提供至少 140 GB 的容量（SSD 或 HDD）。 将使用所有可用的磁盘。 |4 磁盘。 每个磁盘提供至少 250 GB 的容量（SSD 或 HDD）。 将使用所有可用的磁盘。 |
...
\*如果计划从 Azure 添加多个 [Marketplace 项](azure-stack-download-azure-marketplace-item.md)，则需要的容量比这个建议的容量要大。
...
\* 没有直通功能的 RAID 控制器无法识别此介质类型。 此类控制器会将 HDD 和 SSD 都标记为“未指定”。 在这种情况下，SSD 将用作持久存储而不是缓存设备。 因此，可以在这些 SSD 上部署开发工具包。
...
1. 创建一个 Azure AD 帐户，该帐户是至少一个 Azure AD 的目录管理员。 如果已经有一个这样的帐户，则可以使用该帐户。 否则，可以在 [https://account.windowsazure.cn/organization](https://account.windowsazure.cn/organization) 上创建一个。 如果打算以后[将 Azure Stack 注册到 Azure](azure-stack-register.md)，则还必须在这个新创建的帐户中有一个订阅。
   
    保存这些凭据，以便在[部署开发工具包](azure-stack-run-powershell-script.md)的步骤 6 中使用。 此*服务管理员*帐户可以配置和管理资源云、用户帐户、租户计划、配额和定价。 在门户中，该帐户可以创建网站云和虚拟机专用云、创建计划，以及管理用户订阅。
2. [创建](azure-stack-add-new-user-aad.md)至少一个帐户，以便以租户身份登录到开发工具包。
...
开发工具包计算机支持连接到交换机访问端口或 Trunk 端口。 交换机上不要求使用专用功能。 若要使用 Trunk 端口，或者需要配置 VLAN ID，则需以部署参数的形式提供 VLAN ID。 可以在[部署参数列表](azure-stack-run-powershell-script.md)中查看示例。
...
### <a name="dhcp"></a>DHCP
确保可以在 NIC 连接到的网络上使用 DHCP 服务器。 如果 DHCP 不可用，则除了主机使用的静态 IPv4 网络，还必须准备另一个此类网络。 必须提供该 IP 地址和网关作为部署参数。 可以在[部署参数列表](azure-stack-run-powershell-script.md)中查看示例。

### <a name="internet-access"></a>Internet 访问
Azure Stack 需要访问 Internet，不管是直接访问还是通过透明代理进行访问。 Azure Stack 不支持通过配置 Web 代理来启用 Internet 访问。 主机 IP 和分配到 MAS-BGPNAT01 的新 IP（通过 DHCP 或静态 IP 的方式进行分配）必须能够访问 Internet。 在 graph.chinacloudapi.cn 和 login.partner.microsoftonline.cn 域下使用端口 80 和 443。

...
ms.openlocfilehash: c9fa4dab7aaaed933e260f08fe779ccb352b34e1
ms.sourcegitcommit: 34925f252c9d395020dc3697a205af52ac8188ce
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 03/02/2018
ms.locfileid: "29731074"
---
...
- 直接连接到 Internet 的 Azure Stack 集成系统（多节点）部署。 这意味着，必须可从公共 Internet 直接连接到外部公共 IP 地址范围。
...
网络示例值表显示本文中使用的示例值。 可以使用这些值或参考这些值，以更好地了解本文中的示例。
...
5. 从资源列表中选择“本地网络网关”。
6. 在“名称”中，键入 **Azs-GW**。
7. 在“IP 地址”中，键入前面网络配置表中列出的 Azure Stack 虚拟网络网关的公共 IP 地址。
8. 在 Azure Stack 的“地址空间”中，键入 **AzureVNet** 的 **10.1.0.0/24** 和 **10.1.1.0/24** 地址空间。
...
9. 在“共享密钥(PSK)”中，键入 **12345**。 如果选择其他值，请记住，该值必须与在连接另一端创建的共享密钥值匹配。 选择“确定” 。
10. 查看“摘要”部分，然后选择“确定”。
...
8. 在“设置”部分，可以接受默认值。 确保选择 **AzureVnet** 虚拟网络，并确认子网已设置为 **10.100.0.0/24**。 选择“确定” 。
...
在 Azure Stack 中，“本地网络网关”的概念稍微不同于 Azure 部署。

在 Azure 部署中，本地网关代表一个本地（位于用户位置）物理设备，用于连接到 Azure 中的虚拟网络网关。 在 Azure Stack 中，连接的两端都是虚拟网络网关！
...
若要验证通过 VPN 连接传输的数据，需要在两端创建虚拟机，以通过 VPN 隧道发送和接收数据。 
...
建立站点到站点连接以后，应验证其流量是否正常。 若要验证，请登录到在 Azure Stack 中创建的虚拟机之一。 然后，ping Azure 中创建的虚拟机。 
...
4. 在虚拟机部分，单击“连接”，然后打开 Azs-VM.rdp 文件。
...
10. 在 Azure 上的虚拟机中，通过隧道 ping Azure Stack 中的虚拟机。 为此，请 ping 从 Azs-VM 中记录的 DIP。
   在示例环境中，该地址为 **10.1.0.4**，但请确保 ping 实验室中记下的地址。 应会看到以下屏幕截图所示的结果：
...
4. “连接”部分显示了“传入数据”和“传出数据”的统计信息。 在以下屏幕截图中，较大的数字是附加的文件传输造成的。 应会看到其中有一些非零值。

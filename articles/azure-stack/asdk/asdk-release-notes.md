...
ms.openlocfilehash: 8a333934b6cbcfb30d7f74bd0264ae26d1ad0473
...
- 内部负载均衡 (ILB) 对后端 VM 的 MAC 地址处理不当，导致使用 Linux 实例时后端网络中丢弃数据包。
...
#### <a name="deployment"></a>部署
- 在部署期间，必须使用 IP 地址指定时间服务器。
- 从版本 1711 开始，**CloudAdmin** 是保留的帐户名，在部署开发包时不应手动指定该帐户名。 

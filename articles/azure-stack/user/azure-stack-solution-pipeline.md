...
请按照[创建服务主体](/active-directory/develop/active-directory-integrating-applications)中的说明来创建服务主体，然后选择“Web 应用/API”作为“应用程序类型”。
...
要访问订阅中的资源，必须将应用程序分配到角色。 确定哪个角色体现应用程序的最佳权限。 若要了解有关可用角色的信息，请参阅 [RBAC：内置角色](/role-based-access-control/built-in-roles)。

可将作用域设置为订阅、资源组或资源级别。 较低级别的作用域会继承权限。 例如，将某个应用程序添加到资源组的“读取者”角色意味着该应用程序可以读取该资源组及其任意资源。
...
在 VSTS 中使用托管生成代理是生成并部署 Web 应用的便捷做法。 Azure 会自动执行代理维护和升级，使开发周期持续，不间断。

...
ms.openlocfilehash: adf5ca76e692acb89a3465f71541e11d763c8dcf
ms.sourcegitcommit: 98142af6eb83f036d72e26ebcea00e2fceb673af
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 12/14/2018
ms.locfileid: "53396333"
---
...
- 使用 Azure Stack 集成系统，或部署 Azure Stack 开发工具包。
    - 可以在[安装 Azure Stack 开发工具包](/azure-stack/asdk/asdk-install)中找到有关安装 Azure Stack 的说明。
...
1. 使用在 VSTS 上拥有项目创建权限的帐户登录到 Visual Studio Team Services (VSTS)。
...
Visual Studio Team Services 和 Team Foundation Server (TFS) 提供高度可配置、可管理的管道用于发布到多个环境（例如开发、过渡、QA 和生产环境）；在特定的阶段要求审批。
...
使用 [Azure 资源管理器模板](https://azure.microsoft.com/resources/templates/)（例如 VSTS 中的 Web 应用代码）部署到这两个云。
...
在 VSTS 中使用托管生成代理是生成并部署 Web 应用的便捷做法。 Microsoft Azure 会自动执行代理维护和升级，使开发周期持续而不间断。
...
> [!Note]  
> 发布任务的某些设置将在从模板创建发布定义时自动定义为[环境变量](https://docs.microsoft.com/vsts/build-release/concepts/definitions/release/variables?view=vsts#custom-variables)。 无法在任务设置中修改这些设置，但可以在父环境项中修改。

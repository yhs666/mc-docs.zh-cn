...
ms.date: 03/08/2018
ms.author: v-junlch
ms.openlocfilehash: 390f20f1274c4c14634bb93abe6fc55073ec2ebf
...
8. （可选）选择**高级访问策略**以启用功能，如访问虚拟机 (VM) 进行部署，访问资源管理器进行模板部署，并访问 Azure 磁盘加密进行卷加密。 
...
成功创建密钥后，可以在“密钥”下选择该密钥，并查看或修改其属性。 属性部分包含**密钥标识符**，即外部应用程序可用来访问此密钥的统一资源标识符 (URI)。 若要限制对此密钥的操作，请在“允许的操作”下配置设置。
...
成功创建机密后，可以在“机密”下选择该机密，并查看或修改其属性。 属性部分包含**机密标识符**，即外部应用程序可用来访问此机密的 URI。 

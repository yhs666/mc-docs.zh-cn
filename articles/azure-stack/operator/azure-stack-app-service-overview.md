...
Azure Stack 上的 Azure 应用服务是 Azure 的一种可用于 Azure Stack 的平台即服务 (PaaS) 产品。 该服务可让你的内部或外部客户为任何平台或设备创建 Web 应用、API 应用和 Azure Functions 应用程序。 他们可以将你的应用与本地应用程序集成，并可实现业务流程的自动化。 Azure Stack 云操作员可以在完全托管的虚拟机 (VM) 上，使用他们选择的共享 VM 资源或专用 VM 运行客户的应用。

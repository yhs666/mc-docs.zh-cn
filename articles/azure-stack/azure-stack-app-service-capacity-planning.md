...
ms.date: 03/01/2018
ms.author: v-junlch
ms.openlocfilehash: 3f94e1c95f6068b7fb960000666633d63a2078ed
ms.sourcegitcommit: 34925f252c9d395020dc3697a205af52ac8188ce
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 03/02/2018
ms.locfileid: "29731037"
---
...
若要在 Azure Stack 上预配 Azure 应用服务的生产就绪部署，则必须计划你期望系统支持的容量。  下面提供了有关应当为任何生产部署使用的最少实例和计算 SKU 数量的指南。
...
如果许多用户同时发布，则发布服务器角色可能会经历很高的 CPU 使用率。 为实现高可用性，提供了一个以上发布服务器角色。  发布服务器仅处理 FTP/FTPS 流量。
...
- **平均内存占用**：在生产环境中观察到的应用程序平均内存占用大约为 70 MB。 因此，可以按以下方式计算在所有 Web 辅助角色的计算机或 VM 中分配的内存：

    *预配的应用程序数 * 70 MB * 5% - (Web 辅助角色数 * 1044 MB)*

   例如，如果在运行 10 个 Web 辅助角色的环境中有 5,000 个应用程序，则每个 Web 辅助角色 VM 应当具有 7060 MB RAM：

   5,000 * 70 * 0.05 - (10 * 1044) = 7060 (=大约 7 GB)

## <a name="file-server-role"></a>文件服务器角色

对于文件服务器角色，可以使用独立的文件服务器进行开发和测试，例如，当在 Azure Stack 开发工具包上部署 Azure 应用服务时，可以使用以下模板：https://aka.ms/appsvconmasdkfstemplate。 对于生产用途，应当使用预先配置的 Windows 文件服务器，或使用预先配置的非 Windows 文件服务器。
...
- 非 Windows 文件服务器
- 文件服务器群集
- NAS（网络连接存储）设备 有关详细信息，请参阅[预配文件服务器](azure-stack-app-service-before-you-get-started.md#prepare-the-file-server)。

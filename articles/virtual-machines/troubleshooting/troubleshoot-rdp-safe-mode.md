...
不能与 Azure 中的 VM 建立 RDP 连接和其他连接（如 HTTP），因为已将该 VM 配置为启动到安全模式。 在 Azure 门户的[启动诊断](../troubleshooting/boot-diagnostics.md)中检查屏幕截图时，可能会看到 VM 启动正常，但网络接口不可用：
...
2. 检查启动配置数据： 
    ...
    如果 VM 配置为启动到安全模式，则可在“Windows 启动加载程序”部分下看到一个名为 **safeboot** 的额外标志。 如果未看到 **safeboot** 标志，则 VM 未处于安全模式。 本文不适用于这种情况。

    safeboot 标志可能显示有以下值：
    - 最少
    - 网络

    在上述任一模式下，RDP 都不会启动。 因此，修复方法保持不变。

    ![有关“安全模式”标志的图像](./media/troubleshoot-rdp-safe-mode/safe-mode-tag.png)

3. 删除 **safemoade** 标志，使 VM 启动到正常模式：

        bcdedit /deletevalue {current} safeboot

4. 检查启动配置数据，确保删除 safeboot 标志：

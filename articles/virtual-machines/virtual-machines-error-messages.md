...
|  OperationNotAllowed  |  无法重设 VM 大小，因为请求的大小 {0} 不可用于当前分配有 VM 的群集。 若要将 VM 的大小调整为 {1}，请解除分配（这在 Azure 门户预览版中为“停止”操作），然后重试调整大小操作。 访问 https://aka.ms/azure-resizevm 详细了解重设 VM 大小的策略。  |

...
ms.openlocfilehash: a51c605a2dc821319076f4b1bfa1057dd4b99154
...
## <a name="understand-public-ip-address"></a>了解公共 IP 地址
...
可以在创建 VM 时使用 [az vm create](https://docs.microsoft.com/cli/azure/vm#create) 命令创建网络安全组。 执行此操作时，会将 NSG 与 VM 网络接口相关联，并自动创建一个 NSG 规则来允许任何目标发到 *22* 端口上的流量。 在本教程的前面部分中，已在前端 VM 上自动创建了前端 NSG。 也会为端口 22 自动创建 NSG 规则。 

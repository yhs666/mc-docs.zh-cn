...
ms.date: 07/30/2018
ms.author: v-yeche
ms.openlocfilehash: 0ecf412edd62655abf4f83263d2eca53053ebca7
ms.sourcegitcommit: 35889b4f3ae51464392478a72b172d8910dd2c37
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 07/26/2018
ms.locfileid: "39261859"
---
# <a name="information-for-non-endorsed-distributions"></a>有关未认可发行版的信息
[!INCLUDE [learn-about-deployment-models](../../../includes/learn-about-deployment-models-both-include.md)]

仅当使用某个[认可的发行版](endorsed-distros.md?toc=%2fvirtual-machines%2flinux%2ftoc.json)时，Azure 平台 SLA 才适用于运行 Linux OS 的虚拟机。 对于这些认可的分发版，将在 Azure 市场中提供具有所需配置的 Linux 映像。
...
所有正在 Azure 上运行的分发都需要满足多个先决条件才能在平台上正常运行。  本文并未涵盖所有信息，因为每个分发都是不同的；即使满足以下所有条件，也可能仍需显著调整 Linux 系统以确保其在平台上正常运行。

正是出于这个原因，建议尽可能地从 [Azure 认可分发版上的 Linux](endorsed-distros.md?toc=%2fvirtual-machines%2flinux%2ftoc.json) 开始操作。 以下文章指导完成如何准备 Azure 上支持的各种认可的 Linux 分发：
...
<!-- Notice: Red Hat Enterprise Linux is correct when it show how to on existing red hat vhd-->

本文的其余部分重点介绍有关在 Azure 上运行 Linux 分发的一般准则。
...
重新生成 initrd 或 initramfs 映像的机制可能会因发行版而有所不同。 查阅分发的文档或相应过程的支持。  以下是使用 `mkinitrd` 实用程序如何重新生成 initrd 的示例：
...
Azure 上的 VHD 映像必须已将虚拟大小调整为 1MB。  通常，应正确调整使用 Hyper-V 创建的 VHD。  如果未正确调整 VHD，在尝试基于 VHD 创建映像时，可能会收到如下错误消息：
...
若要修正此行为，可使用 Hyper-V 管理器控制台或 [Resize-VHD](http://technet.microsoft.com/library/hh848535.aspx) Powershell cmdlet 调整 VM 大小。  如果未在 Windows 环境中运行，建议使用 qemu-img 转换（如果需要）并调整 VHD 大小。
...
2. 计算磁盘映像所需的大小，确保虚拟大小已调整为 1MB。  可以使用以下 bash shell 脚本帮助解决此问题。  此脚本使用“`qemu-img info`”来确定磁盘映像的虚拟大小，然后将大小计算到下个 1MB：
...
Hyper-V 和 Azure 的 Linux 集成服务 (LIS) 驱动程序会直接影响上游 Linux 内核。 包括最新 Linux 内核版本（即 3.x）在内的许多分发已提供这些驱动程序，或以其他方式为其内核提供了这些驱动程序的向后移植版本。  这些驱动程序会不断地在上游内核中使用新的修补程序和功能进行更新，因此，如果可能，建议运行包含这些修补程序和更新的[认可的分发版](endorsed-distros.md?toc=%2fvirtual-machines%2flinux%2ftoc.json)。
...
如果需要自定义内核，建议使用较新的内核版本（即 **3.8+**）。 对于这些分发或维护自己的内核的供应商，需要执行一些操作，以便定期将 LIS 驱动程序从上游内核向后移植到自定义内核。  即使已经运行相对较新的内核版本，也强烈建议跟踪 LIS 驱动程序中的任何上游修复，并根据需要向后移植这些修复。 LIS 驱动程序源文件的位置可在 Linux 内核源树的 [MAINTAINER](https://git.kernel.org/cgit/linux/kernel/git/torvalds/linux.git/tree/MAINTAINERS) 文件中找到：
...
至少，已知道缺少以下修补程序会在 Azure 上导致问题，因此必须在内核中包含这些修补程序。 对于所有发行版，此列表并不详尽或完整：
...
[Azure Linux 代理](../extensions/agent-linux.md) (waagent) 是在 Azure 中正确预配 Linux 虚拟机所必需的。 可以在 [Linux 代理 GitHub 存储库](https://github.com/Azure/WALinuxAgent)中获取最新版本、文件问题或提交拉取请求。

* Linux 代理根据 Apache 2.0 许可证发布。 许多分发已为该代理提供 RPM 或 deb 包，因此，在某些情况下不费吹灰之力即可安装和更新该代理。
...
* 修改 GRUB 或 GRUB2 中的内核引导行，以便包含以下参数。 这还可确保所有控制台消息都发送到第一个串行端口，从而可以协助 Azure 支持人员调试问题：
...
* 安装 Azure Linux 代理
    ...
    在 Azure 上预配 Linux 映像需要 Azure Linux 代理。  许多发行版将该代理提供为 RPM 或 Deb 包（该包通常称为“WALinuxAgent”或“walinuxagent”）。  还可以按照 [Linux 代理指南](../extensions/agent-linux.md)中的步骤手动安装该代理。

...
若要创建 *PUT* 请求，必须使用 `{subscription-id}` 参数。 如果拥有多个订阅，请参阅[使用多个订阅](https://docs.azure.cn/zh-cn/cli/manage-azure-subscriptions-azure-cli?view=azure-cli-latest#working-with-multiple-subscriptions)。 你将定义资源的 `{resourceGroupName}` 和 `{vmName}` 以及 `api-version` 参数。 本文使用的是 `api-version=2017-12-01`。

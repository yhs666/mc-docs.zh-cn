...
ms.date: 04/27/2017
ms.author: v-dazen
ms.openlocfilehash: 66aa9372c81fa6acfa743dc2cd523e3e3a73a714
...
3. 通过运行以下命令卸载 NetworkManager：
    ...
    **注意：** 如果尚未安装此包，则该命令将失败，并显示错误消息。 这是正常情况。
...
9. 在 grub 配置中修改内核引导行，以使其包含 Azure 的其他内核参数。 为此，在文本编辑器中打开“/boot/grub/menu.lst”，并确保默认内核包含以下参数：
...
8. 运行以下命令清除当前 yum 元数据并安装所有更新：
        ...
        # sudo yum clean all
        # sudo yum -y update
9. 在 grub 配置中修改内核引导行，以使其包含 Azure 的其他内核参数。 为此，在文本编辑器中打开“/etc/default/grub”并编辑 `GRUB_CMDLINE_LINUX` 参数，例如：

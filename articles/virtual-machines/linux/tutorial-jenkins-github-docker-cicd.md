...
使用 [az group create](https://docs.microsoft.com/cli/azure/group#create) 创建资源组，然后才能创建 VM。 以下示例在 *chinaeast* 位置创建名为 *myResourceGroupJenkins* 的资源组：

```azurecli 
az group create --name myResourceGroupJenkins --location chinaeast
```

现在，请使用 [az vm create](https://docs.microsoft.com/cli/azure/vm#create) 创建 VM。 使用 `--custom-data` 参数传入 cloud-init 配置文件。 如果已将 *cloud-init-jenkins.txt* 文件保存在现有工作目录的外部，请提供该文件的完整路径。
...
若要允许 Web 流量抵达 VM，请使用 [az vm open-port](https://docs.microsoft.com/cli/azure/vm#open-port) 为 Jenkins 流量打开端口 *8080*，并为用于运行示例应用的 Node.js 应用打开端口 *1337*：

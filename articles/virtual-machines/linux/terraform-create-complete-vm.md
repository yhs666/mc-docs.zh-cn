...
ms.date: 08/28/2017
ms.author: v-haiqya
ms.openlocfilehash: 1491f1827feef359a0676cbad295e5ebb479789b
...
若要开始，请在所选的文本编辑器（Visual Studio Code/Sublime/Vim/ 等）中创建名为 \terraform_azure101.tf 的文件。 文件的确切名称并不重要，因为 terraform 接受使用文件夹名称作为参数：会执行文件夹中的所有脚本。 将以下代码粘贴到新文件中：
...
terraform plan
```
我们使用了 `plan` Terraform 命令来查找脚本中定义的资源。 该命令将这些资源与 Terraform 保存的状态进行比较，并输出计划的执行，但不会在 Azure 中实际创建资源。 
...
为方便起见，下面提供了用于预配本文中讨论的所有基础结构的完整 Terraform 脚本。

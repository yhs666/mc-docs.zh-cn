...
ms.date: 01/08/2018
ms.author: v-yeche
ms.openlocfilehash: ad66b989e49a898fc8a9ac480b6bedae892b5cb4
...
若要将用户添加到 Linux VM，请在当前 shell 中创建名为 *cloud_init_add_user.txt* 的文件并粘贴以下配置。 可使用任何想要使用的编辑器。 输入 `sensible-editor cloud_init_add_user.txt` 以创建文件并查看可用编辑器的列表。 选择 #1 以使用 nano 编辑器。 请确保已正确复制整个 cloud-init 文件，尤其是第一行。  需要提供自己的公钥（例如 *~/.ssh/id_rsa.pub* 的内容）作为 `ssh-authorized-keys:` 的值 - 为简化示例，此处已缩短。

...
ms.openlocfilehash: b717346f48b9179f6e0aa48c8d8989afccc73dd9
...
* 定义控制资源组访问所需的用户访问角色。
* 使用命名约定来创建资源组集。 可以使用 Azure CLI 或门户。
...
资源组的一项关键功能，是能够使用用于声明存储、网络和计算资源的 JSON 文件来构建环境。 还可以定义要应用的任何相关自定义脚本或配置。 通过使用这些 JSON 模板，可以为应用程序创建一致且可重现的部署。 此方法可让用户构建开发环境，并使用相同的模板创建生产部署，反之亦然。 若要更好地了解模板的使用方式，请参阅[模板演练](../../azure-resource-manager/resource-manager-template-walkthrough.md)，它将引导用户完成构建 JSON 模板的每个步骤。

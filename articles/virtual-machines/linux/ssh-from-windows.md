...
5. 生成公钥后，可选择输入并确认通行短语。 使用 SSH 密钥面向 VM 进行身份验证时，系统会提示输入通行短语。 如果有人获取了私钥但没有通行短语，他们可以登录到使用该密钥的任何 VM 或服务。 我们建议创建一个通行短语。 但是，如果你忘记了通行短语，将没有办法恢复它。

6. 公钥会显示在窗口的顶部。 创建 Linux VM 时，将此单行格式公钥复制并粘贴到 Azure 门户或 Azure 资源管理器模板中。 也可以单击“保存公钥”将一个副本保存到计算机：

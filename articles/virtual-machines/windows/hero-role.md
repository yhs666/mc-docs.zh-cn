...
ms.date: 10/17/2016
ms.author: v-dazen
ms.openlocfilehash: 4c03ba6d769dc24eb4c2feb6470a38fbdb72abd4
...
如果尚未创建第一个 VM，应先返回 [Create your first Windows virtual machine in the Azure portal](../virtual-machines-windows-hero-tutorial.md?toc=%2fvirtual-machines%2fwindows%2ftoc.json)（在 Azure 门户中创建第一个 Windows 虚拟机），然后继续学习本教程。
...
如果在尝试连接时遇到故障，请参阅[对基于 Windows 的 Azure 虚拟机的远程桌面连接进行故障排除](troubleshoot-rdp-connection.md?toc=%2fvirtual-machines%2fwindows%2ftoc.json)。
...
5. 在 NSG 的“概要”边栏选项卡中，应有一个现有的 **default-allow-rdp** 默认输入规则允许你登录到 VM。 你将添加另一个允许 IIS 流量的入站规则。 单击“入站安全规则”。
...
有关 NSG、入站和出站规则的详细信息，请参阅 [Allow external access to your VM using the Azure portal](nsg-quickstart-portal.md?toc=%2fvirtual-machines%2fwindows%2ftoc.json)（使用 Azure 门户对 VM 实现外部访问）。

## <a name="connect-to-the-default-iis-website"></a>连接到默认 IIS 网站
1. 在 Azure 门户中，单击“虚拟机”，然后选择你的 VM。
2. 在“概要”边栏选项卡中，复制你的“公共 IP 地址”。
...
4. 浏览器应会打开默认的 IIS 网页， 它应如下所示：

    ![显示默认 IIS 页面在浏览器中的外观的屏幕截图](./media/hero-role/iis-default.png)

## <a name="next-steps"></a>后续步骤
* 还可以体验[将数据磁盘附加到](attach-disk-portal.md?toc=%2fvirtual-machines%2fwindows%2ftoc.json)虚拟机。 数据磁盘可为虚拟机提供更多存储空间。

...
ms.date: 12/16/2016
ms.author: v-dazen
ms.openlocfilehash: 487b052eae6b3998a144935c6aff6c037c191749
...
> * [PowerShell - 模板](extensions-configuration-samples.md?toc=%2fvirtual-machines%2fwindows%2ftoc.json)
> * [CLI - 模板](../linux/extensions-configuration-samples.md?toc=%2fvirtual-machines%2flinux%2ftoc.json)
> 
> 
...
若要详细了解这些扩展，请参阅 [Azure VM 扩展概述](extensions-features.md?toc=%2fvirtual-machines%2fwindows%2ftoc.json)。

若要详细了解如何创作扩展模板，请参阅[创作扩展模板](extensions-authoring-templates.md?toc=%2fvirtual-machines%2fwindows%2ftoc.json)。

本文列出了一些 Windows 扩展的预期配置值。

## <a name="sample-template-snippet-for-vm-extensions-with-iaas-vms"></a>适用于 IaaS VM 的 VM 扩展的示例模板代码段。
用于部署扩展的模板代码段如下所示：
...
* fileUris：通过该扩展下载到 VM 上的文件的 URL 列表（以逗号分隔）。 如果不指定，则不会下载任何文件。 如果文件位于 Azure 存储中，可以将 fileURL 标记为私有，且相应的 storageAccountName 和 storageAccountKey 可以作为私有参数传递，以访问这些文件。
...
请参考 CustomScript 1.4 版本，了解参数说明。 版本 1.7 引入了将脚本参数 (commandToExecute) 作为 protectedSettings 发送的支持，在此情况下，这些参数将在发送前加密。 “commandToExecute”参数可以在 settings 或 protectedSettings 中指定（但不能同时在两者中指定）。
...
### <a name="barracuda-vpn-connectivity-agent-for-azure"></a>适用于 Azure 的 Barracuda VPN 连接代理
          {
...
有关如何配置诊断的详细信息，请参阅 [Azure 诊断扩展](extensions-diagnostics-template.md?toc=%2fvirtual-machines%2fwindows%2ftoc.json)

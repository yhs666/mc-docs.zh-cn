...
使用 [New-AzureRMVMConfig](https://docs.microsoft.com/powershell/module/azurerm.compute/new-azurermvmconfig) 通过配置创建 VM 时，可以使用 `-AvailabilitySetId` 参数指定可用性集，以指定该可用性集的 ID。

在可用性集中使用 [New-AzureRmVM](https://docs.microsoft.com/powershell/module/azurerm.compute/new-azurermvm) 创建 2 个 VM。
...
创建和配置这两个 VM 需要几分钟的时间。 完成后，将拥有 2 个跨基础硬件分布的虚拟机。 

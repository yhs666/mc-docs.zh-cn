...
# <a name="complete-prerequisites-for-creating-always-on-availability-groups-in-azure-virtual-machines"></a>满足在 Azure 虚拟机中创建 Always On 可用性组的先决条件

本教程说明如何完成必备组件配置，以便[在 Azure 虚拟机上中创建 SQL Server Always On 可用性组](virtual-machines-windows-portal-sql-availability-group-tutorial.md)。 满足先决条件后，可在单个资源组中获得一个域控制器、两个 SQL Server 和一个见证服务器。
...
本教程假设读者对 SQL Server Always On 可用性组有一个基本的了解。 如果读者不熟悉此技术，请参阅 [Always On 可用性组概述 (SQL Server)](http://msdn.microsoft.com/library/ff877884.aspx)。
...
>[!IMPORTANT]
>只能在创建 VM 时将 VM 放入可用性集。 创建 VM 后，无法更改可用性集。 请参阅[管理虚拟机的可用性](../../virtual-machines-windows-manage-availability.md)。
...
14. 单击“下一步”浏览向导中的其他页。 在“必备项检查”页上，确认看到以下消息：“所有先决条件检查都成功通过”。 查看任何适用的警告消息，但可以继续安装。
15. 单击“安装” 。 **ad-primary-dc** 虚拟机将自动重新启动。

### <a name="note-ip-address-of-primary-domain-controller"></a>记下主域控制器的 IP 地址
...
### <a name="grant-required-permissions-to-the-installation-account"></a>向安装帐户授予所需的权限
...
6. 在“网络连接”窗口中，选择网络图标。
7. 在命令栏中，单击“更改此连接的设置”。 如果看不到此选项，单击双右箭头。
...
1. 等待 VM 重新启动，然后从主域控制器重新启动 RDP 文件，使用 **CORP\DomainAdmin** 帐户登录到 **sqlserver-0**。
   >[!TIP]
   >请确保使用域管理员帐户登录。 在前面的步骤中，使用的是 BUILT IN 管理员帐户。 既然服务器已加入域，现在就应该使用域帐户。 在 RDP 会话中，指定*域*\\*用户名*。
...
### <a name="create-login-on-each-sql-server-for-installation-account"></a>在每个 SQL Server 上创建安装帐户的登录名

使用安装帐户配置可用性组。 此帐户需是每个 SQL Server 上的 **sysadmin** 固定服务器角色的成员。 以下步骤将创建安装帐户的登录名：
...
### <a name="open-a-tcp-port-on-a-firewall"></a>在防火墙中打开 TCP 端口 

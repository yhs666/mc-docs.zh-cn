...
在开始学习本教程之前，需要[满足在 Azure 虚拟机中创建 Always On 可用性组的先决条件](../../windows/sql/virtual-machines-windows-portal-sql-availability-group-prereq.md)。如果已满足这些先决条件，可转到[创建群集](#CreateCluster)。
...
1. 使用在两个 SQL Server 上和见证服务器上充当管理员的域帐户通过 RDP 连接到第一个 SQL Server。
    ...
    >[!TIP]
    如果已完成[预备文档](../../windows/sql/virtual-machines-windows-portal-sql-availability-group-prereq.md)，则已创建名为 **CORP\\Install** 的帐户。请使用此帐户。

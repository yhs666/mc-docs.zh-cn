...
ms.openlocfilehash: 7dcef7d45a37cace461409463a1d42645c30f8b3
...
自动备份将在运行 SQL Server 2014 Standard 或 Enterprise 的 Azure VM 上自动为所有现有数据库和新数据库配置[托管备份到 Azure](https://msdn.microsoft.com/library/dn449496.aspx)。 这样，你便可以配置使用持久 Azure Blob 存储的定期数据库备份。 自动备份依赖于 [SQL Server IaaS 代理扩展](virtual-machines-windows-sql-server-agent-extension.md)。
...
下表描述了可为自动备份配置的选项。 实际配置步骤会根据使用的是 Azure 门户还是 Azure Windows PowerShell 命令而有所不同。

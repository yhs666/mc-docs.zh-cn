...
成功登录后，屏幕上会显示一些信息，其中包括默认订阅的订阅名称和 ID。 除非更改为其他订阅，否则本教程中的资源都在该订阅中创建。 如果有多个订阅，请运行以下 cmdlet 返回所有订阅的列表：
...
可以使用 Get-AzureRmVMImageSku 来查看可用于产品的 SKU。 以下命令显示 SQL2016SP1-WS2016 产品可用的所有 SKU。
...
首先指定虚拟机大小。 对于本教程，我们应指定 DS13。 我们将使用 [New-AzureRmVMConfig](https://docs.microsoft.com/powershell/module/azurerm.compute/new-azurermvmconfig) cmdlet，以前面初始化的变量所定义的名称与大小来创建可配置的虚拟机对象。
...
现在已完成配置步骤，接下来可以创建虚拟机。 我们将通过 [New-AzureRmVM](https://docs.microsoft.com/powershell/module/azurerm.compute/new-azurermvm) cmdlet，使用已定义的变量来创建虚拟机。

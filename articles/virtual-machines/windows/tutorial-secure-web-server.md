...
ms.author: v-haiqya
ms.custom: mvc
ms.openlocfilehash: 03baed922b8c9c9d4fd43243c3a9c2c246d286b4
...
创建 Key Vault 和证书之前，需使用 [New-AzureRmResourceGroup](https://docs.microsoft.com/powershell/module/azurerm.resources/new-azurermresourcegroup) 创建资源组。 以下示例在“中国北部”位置创建名为 *myResourceGroupSecureWeb* 的资源组。
...
创建 VM 需要几分钟时间。 最后一个步骤通过 [Set-AzureRmVmExtension](https://docs.microsoft.com/powershell/module/azurerm.compute/set-azurermvmextension) 使用 Azure 自定义脚本扩展来安装 IIS Web 服务器。
...
使用 [Get-AzureRmPublicIPAddress](https://docs.microsoft.com/powershell/resourcemanager/azurerm.network/get-azurermpublicipaddress) 获取 VM 的公共 IP 地址。 以下示例获取前面创建的 `myPublicIP` 的 IP 地址：

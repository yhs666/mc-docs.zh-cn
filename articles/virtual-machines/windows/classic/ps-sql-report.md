...
2. 单击左侧窗格中的“虚拟机”。
    ...
    ![Microsoft Azure 虚拟机](./media/ps-sql-report/IC660124.gif)  

3. 单击“新建”。

    ![新建按钮](./media/ps-sql-report/IC692019.gif)  

4. 单击“从库中”。

    ![从库创建 VM](./media/ps-sql-report/IC692020.gif)  

5. 单击“SQL Server 2014 RTM Standard - Windows Server 2012 R2”，然后单击箭头继续。

    ![下一步](./media/ps-sql-report/IC692021.gif)  
...
6. 在“虚拟机配置”页上，编辑以下字段：
    ...
    * 单击“下一步”。![下一步](./media/ps-sql-report/IC692021.gif)
7. 在下一页上，编辑以下字段：
    ...
    * **终结点**：保留“远程桌面”和 **PowerShell** 终结点，然后添加一个 HTTP 或 HTTPS 终结点，具体取决于你的环境。
        ...
        * **HTTPS**：默认公共和专用端口均为 **443**。最佳安全方案是更改私有端口并配置你的防火墙和报表服务器以使用私有端口。有关终结点的详细信息，请参阅[如何设置与虚拟机的通信](setup-endpoints.md)。请注意，如果你使用非 443 的端口，请更改 HTTPS 脚本中的参数 **$HTTPsport = 443**。
    * 单击“下一步”。![下一步](./media/ps-sql-report/IC692021.gif)
...
9. 单击“完成”。![确定](./media/ps-sql-report/IC660122.gif)
...
1. 若要信任本地 VM 上的证书的根 CA，请将该证书添加到**受信任的根证书颁发机构**。以下是所需步骤的摘要。有关如何信任 CA 的详细步骤，请参阅[安装服务器证书](https://technet.microsoft.com/zh-cn/library/cc740068)。
    ...
    1. 从 Azure 经典管理门户中，选择 VM 并单击“连接”。根据你的浏览器配置，可能会向你提示保存一个用于连接到 VM 的 .rdp 文件。
       ...
       ![连接到 Azure 虚拟机](./media/ps-sql-report/IC650112.gif) 使用你在创建 VM 时配置的用户 VM 名称、用户名和密码。

       例如，在下图中，VM 名称是 **ssrsnativecloud**，用户名是 **testuser**。

       ![登录名包含 VM 名称](./media/ps-sql-report/IC764111.png)  
...
有关更多详细步骤，请参阅[连接到虚拟机并启动 Reporting Services 配置管理器](../sqlclassic/virtual-machines-windows-classic-ps-sql-bi.md#connect-to-the-virtual-machine-and-start-the-reporting-services-configuration-manager)。
...
1. 从 Azure 经典管理门户中，选择 VM 并单击“连接”。使用在创建 VM 时配置的用户名和密码。
    ...
    ![连接到 Azure 虚拟机](./media/ps-sql-report/IC650112.gif)  

---
title: "在 VM 中配置 Oracle GoldenGate | Azure"
...
ms.date: 07/10/2017
ms.author: v-dazen
ms.openlocfilehash: 613afa347040eaff59441e4232870913ae4493f6
...
Oracle GoldenGate 包含以下主要组件：提取、数据抽取、复制、跟踪/提取文件、检查点、管理进程和收集进程。 若要在两个站点之间进行双向复制，需要在两个站点上创建并启动所有组件。 有关 Oracle GoldenGate 体系结构的详细信息，请参阅 [Oracle GoldenGate 指南](http://docs.oracle.com/goldengate/1212/gg-winux/index.html)。
...
* 已从 [Oracle 下载](http://www.oracle.com/us/downloads/index.html)网站下载了 Oracle GoldenGate 软件。 已选择产品包“Oracle Fusion Middleware – Data Integration”。 然后，已选择适用于 Oracle 11g 数据库的 Oracle GoldenGate on Oracle v11.2.1 Media Pack for Microsoft Windows x64（64 位）。 接下来，请下载适用于 Windows 2008（64 位）的 Oracle GoldenGate V11.2.1.0.3（Oracle 11g 64 位）。
* 已使用 Windows Server 上的 Oracle Enterprise Edition 在 Azure 中创建了两个虚拟机 (VM)。 确保这些虚拟机位于[相同的云服务](../../virtual-machines-linux-load-balance.md)和相同的[虚拟网络](/virtual-network/)中，以确保它们可以通过持久性专用 IP 地址相互访问。
* 已在 Azure 门户中将站点 A 的虚拟机名称设置为“MachineGG1”，将站点 B 的虚拟机名称设置为“MachineGG2”。
...
5. 在站点 A 和站点 B 上创建提取组和数据抽取进程
   ...
   1. 在站点 A 上创建提取和数据抽取进程
   2. 在站点 B 上创建 GoldenGate 检查点表
   3. 在站点 B 上添加复制进程
   4. 在站点 B 上创建提取和数据抽取进程
   5. 在站点 A 上创建 GoldenGate 检查点表
   6. 在站点 A 上添加复制进程
   7. 在站点 A 和站点 B 上添加 trandata
   8. 在站点 A 上启动提取和数据抽取进程
   9. 在站点 B 上启动提取和数据抽取进程
   10. 在站点 A 上启动复制进程
   11. 在站点 B 上启动复制进程
6. 验证双向复制过程

> [!IMPORTANT]
> 本教程已针对以下软件配置进行了设置和测试：
> 
> |  | 站点 A 数据库 | 站点 B 数据库 |
> | --- | --- | --- |
...
然后，需要创建一个将被 Oracle GoldenGate 管理进程、提取和复制进程使用的数据库用户。 请注意，可以为每个进程创建单独的用户，也可以只配置一个通用用户。 本教程将创建一个名为 ggate 的用户。 然后，我们将授予该用户必要的特权。 请注意，必须在站点 A 和站点 B 上执行以下操作。
...
显示系统上所有管理进程、提取和复制进程的状态与滞后时间（如适用）：

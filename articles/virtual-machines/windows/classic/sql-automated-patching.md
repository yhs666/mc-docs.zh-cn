...
>- [经典](sql-automated-patching.md)

自动修补将为运行 SQL Server 的 Azure 虚拟机建立一个维护时段。只能在此维护时段内安装自动更新。对于 SQL Server，这可以确保在数据库的最佳可能时间进行系统更新和任何关联的重新启动。自动修补依赖于 [SQL Server IaaS 代理扩展](sql-server-agent-extension.md)。
...
* [安装 SQL Server IaaS 扩展](sql-server-agent-extension.md)。
...
## 后续步骤
若要了解其他可用的自动化任务，请参阅 [SQL Server IaaS 代理扩展](sql-server-agent-extension.md)。

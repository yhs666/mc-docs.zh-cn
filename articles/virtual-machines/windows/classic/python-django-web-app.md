---
title: "在 Azure VM 上构建 Django 应用 | Azure"
description: "本教程演示如何使用经典部署模型，在 Azure 中使用 Windows Server 2012 R2 Datacenter 虚拟机托管基于 Django 的网站。"
...
ms.date: 07/10/2017
ms.author: v-dazen
ms.openlocfilehash: be2de6e20d9d6aabcaba4cbd296b970e98bcf283
...
> [!IMPORTANT] 
> Azure 提供两个不同的部署模型用于创建和处理资源：[Resource Manager 和经典模型](../../../resource-manager-deployment-model.md)。 本文介绍如何使用经典部署模型。 Azure 建议大多数新部署使用 Resource Manager 模型。 有关用于部署 Django 的 Resource Manager 模板，请参阅[此处](https://github.com/Azure/azure-quickstart-templates/tree/master/django-app/)。
...
2. 指示 Azure 将来自 Web 的端口 80 通信定向到虚拟机上的端口 80：
   ...
   * 在 Azure 门户中，转到仪表板并选择新创建的虚拟机。
   * 单击“终结点”，然后单击窗格顶部的“添加”。

     ![添加终结点](./media/python-django-web-app/django-helloworld-add-endpoint-new-portal.png)

   * 输入 `HTTP` 作为“名称”。 将公共和专用 TCP 端口设置为 80。

     ![将端口设置为 80](./media/python-django-web-app/django-helloworld-add-endpoint-set-ports-new-portal.png)

   * 完成后，单击窗格底部的“确定” 。

3. 在“仪表板”中，选择 VM，然后单击窗格顶部的“连接”以使用远程桌面远程登录到新创建的 Azure 虚拟机。  

**重要说明：** 下面的所有说明假定你已正确登录到虚拟机并在从虚拟机而非你的本地计算机发布命令。

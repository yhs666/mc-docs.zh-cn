...
* 如果要复制通用化 VM，请参阅 [如何从现有的通用化 Azure VM 创建 VM 映像](capture-image.md)。
* 如果想要从本地 VM（例如，使用 Hyper-V 创建的 VM）上载 VHD，请参阅 [Upload a Windows VHD from an on-premises VM to Azure](upload-image.md)（将 Windows VHD 从本地 VM 上载到 Azure）。
...
## 后续步骤
* 可以通过[将 VHD 的副本作为 OS 磁盘附加到 VM](create-vm-specialized.md) 来创建新 VM。

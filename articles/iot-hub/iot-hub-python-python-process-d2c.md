...
ms.date: 06/11/2018
ms.author: v-yiso
ms.openlocfilehash: 6d3fc9eae81ffb3b2cbaf8833fa747fe04dc9022
ms.sourcegitcommit: b8fb6890caed87831b28c82738d6cecfe50674fd
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 03/29/2019
ms.locfileid: "58626504"
---
...
* **SimulatedDevice.py**（在 [IoT 中心入门]教程中创建的应用的修改版本）每秒发送一次设备到云的数据点消息，根据随机时间间隔发送设备到云的交互式消息。 此应用使用 MQTT 协议与 IoT 中心通信。
...
1. 使用文本编辑器打开 **SimulatedDevice.py** 文件。 本文件包含用于 [IoT 中心入门]教程中创建的 **SimulatedDevice** 应用的代码。
...
1. 现在单击 IoT 中心的“路由”  。 单击边栏选项卡顶部的“添加” ，创建将消息路由到刚添加的队列的路由规则。 
    ...
    输入名称，并选择“设备消息”作为数据源。 选择自定义终结点 **CriticalQueue** 作为路由规则终结点，并输入 `level="critical"` 作为查询字符串。 单击底部的“保存”。
...
在本部分中，将创建一个 Python 控制台应用，用于读取来自 IoT 服务总线的关键消息。 在[队列入门][lnk-sb-queues-node]中查看更多信息。 
...
本部分创建一个存储帐户并将其连接到 IoT 中心，还会配置 IoT 中心，根据消息上的现有属性发送消息到帐户。 有关如何管理存储的详细信息，请参阅 [Azure 存储入门][Azure 存储]。
 ...
 > [!NOTE]
   > _F1 免费_层下创建的 IoT 中心帐户局限于一个**终结点**。 如果可以使用不止一个**终结点**，则除了 **CriticalQueue**，还可以设置 **StorageContainer**，然后同时运行这二者。
...
3. 在“终结点”边栏选项卡中，选择“CriticalQueue”终结点，然后单击“删除”。 单击“是”，然后单击“添加”。 将终结点命名为 **StorageContainer**，从下拉列表中选择“Azure 存储容器”，然后创建**存储帐户**和**存储容器**。  记下名称。  完成后，单击底部的“确定”。 
   ...
   > [!NOTE]
   > _F1 免费_层下创建的 IoT 中心帐户局限于一个**终结点**。 如果可以使用不止一个**终结点**，则不需删除 **CriticalQueue**。
...
5. 确保以前的应用程序 **SimulatedDevice.py** 仍在运行。 

6. 在 Azure 门户中转到存储帐户，然后在“Blob 服务”下单击“浏览 Blob...”。选择容器，导航到 JSON 文件并单击该文件，然后单击“下载”查看数据。

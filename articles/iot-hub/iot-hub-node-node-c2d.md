...
ms.openlocfilehash: aa9fd72085a86e3aa847aa9d39810a0445d18776
...
在本部分中，你将创建一个 Node.js 控制台应用，用于将云到设备的消息发送到模拟设备应用。 需要使用 [Get started with IoT Hub]教程中添加的设备的设备 ID。 还需要中心的 IoT 中心连接字符串（位于 [Azure 门户]）。
...
1. 创建名为 **sendcloudtodevicemessage**的空文件夹。 在命令提示符下的 **sendcloudtodevicemessage** 文件夹中，使用以下命令创建 package.json 文件。 接受所有默认值：
    ...
    npm init
    ```
2. 在 **sendcloudtodevicemessage** 文件夹的命令提示符下，运行以下命令以安装 **azure-iothub** 包：
...
4. 在 **SendCloudToDeviceMessage.js** 文件的开头添加以下 `require` 语句：
    ...
    ```
5. 将以下代码添加到 **SendCloudToDeviceMessage.js** 文件。 将 IoT 中心连接字符串占位符值替换为在 [Get started with IoT Hub]教程中创建的中心的 IoT 中心连接字符串。 将目标设备占位符替换为在 [Get started with IoT Hub]教程中添加的设备的设备 ID：

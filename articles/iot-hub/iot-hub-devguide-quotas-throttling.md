...
| 新设备连接（此限制适用于建立_新连接_的速率，而不是连接总数） | 高于 100/秒或 12/秒/单位 <br/> 例如，两个 S1 单位是 2\*12 = 24 个新连接/秒，但你的单位至少有 100 个新连接/秒。 如果有 9 个 S1 单位，则你的单位就有 108 个新连接/秒 (9\*12)。 | 120 个新连接/秒/单位 | 6000 个新连接/秒/单位 |
...
| 作业<sup>1</sup> | 作业历史记录最多保留 30 天 <br/> 最大并发作业数为 1（对于免费版和 S1）、5（对于 S2）、10（对于 S3）。 |

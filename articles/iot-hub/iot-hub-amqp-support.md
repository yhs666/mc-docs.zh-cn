...
ms.date: 06/17/2019
ms.author: rezas
ms.openlocfilehash: 819096f61657d3b558fd38282c81c75cc8086993
...
---
# <a name="communicate-with-your-iot-hub-using-the-amqp-protocol"></a>使用 AMQP 协议来与 IoT 中心通信

IoT 中心支持使用 [AMQP 版本 1.0](http://docs.oasis-open.org/amqp/core/v1.0/os/amqp-core-complete-v1.0-os.pdf) 通过面向设备和面向服务的终结点来交付各种功能。 本文档介绍如何使用 AMQP 客户端连接到 IoT 中心，以使用 IoT 中心功能。

## <a name="service-client"></a>服务客户端

### <a name="connection-and-authenticating-to-iot-hub-service-client"></a>建立连接并在 IoT 中心（服务客户端）进行身份验证
...
| 信息 | Value | 
|-------------|--------------|
| IoT 中心主机名 | `<iot-hub-name>.azure-devices.cn` |
| 密钥名称 | `service` |
| 访问密钥 | 与服务关联的主要密钥或辅助密钥 |
| 共享访问签名 | 采用以下格式的短生存期 SAS：`SharedAccessSignature sig={signature-string}&se={expiry}&skn={policyName}&sr={URL-encoded-resourceURI}`（可在[此处](./iot-hub-devguide-security.md#security-token-structure)找到用于生成此签名的代码）。
...
# Use generate_sas_token implementation available here: /iot-hub/iot-hub-devguide-security#security-token-structure
...
### <a name="invoking-cloud-to-device-messages-service-client"></a>调用云到设备的消息（服务客户端）
[此处](iot-hub-devguide-messages-c2d.md)介绍了服务与 IoT 中心之间，以及设备与 IoT 中心之间的云到设备消息交换。 服务客户端使用下述两个链接发送消息，以及接收先前从设备发送的消息的反馈。

| 创建者 | 链接类型 | 链接路径 | 说明 |
|------------|-----------|-----------|-------------|
| 服务 | 发送方链接 | `/messages/devicebound` | 服务会将发往设备的 C2D 消息发送到此链接。 通过此链接发送的消息的 `To` 属性设置为目标设备的接收方链接路径：例如 `/devices/<deviceID>/messages/devicebound`。 |
| 服务 | 接收方链接 | `/messages/serviceBound/feedback` | 服务在此链接上收到的、来自设备的完成、拒绝和丢弃反馈消息。 有关反馈消息的详细信息，请参阅[此文](./iot-hub-devguide-messages-c2d.md#message-feedback)。 |

以下代码片段演示如何使用 [Python 中的 uAMQP 库](https://github.com/Azure/azure-uamqp-python)创建一条 C2D 消息并将其发送到设备。
...
若要接收反馈，服务客户端需创建一个接收方链接。 以下代码片段演示如何使用 [Python 中的 uAMQP 库](https://github.com/Azure/azure-uamqp-python)执行此操作。
...
如上所示，C2D 反馈消息的内容类型为 `application/vnd.microsoft.iothub.feedback.json`，其 JSON 正文中的属性可用于推断原始消息的传送状态：
* 反馈正文中的键 `statusCode` 包含以下值之一：`['Success', 'Expired', 'DeliveryCountExceeded', 'Rejected', 'Purged']`。
* 反馈正文中的键 `deviceId` 包含目标设备的 ID。
* 反馈正文中的键 `originalMessageId` 包含服务发送的原始 C2D 消息的 ID。 可以使用此 ID 将反馈关联到 C2D 消息。
...
* 一个用于在分区中指定起点的可选筛选谓词（可以采用序列号、偏移量或排队时间戳的形式）。
...
# Use generate_sas_token implementation available here: /azure/iot-hub/iot-hub-devguide-security#security-token-structure
from helper import generate_sas_token
...
### <a name="connection-and-authenticating-to-iot-hub-device-client"></a>建立连接并在 IoT 中心（设备客户端）进行身份验证
...
| 共享访问签名 | 采用以下格式的短生存期 SAS：`SharedAccessSignature sig={signature-string}&se={expiry}&sr={URL-encoded-resourceURI}`（可在[此处](./iot-hub-devguide-security.md#security-token-structure)找到用于生成此签名的代码）。
...
| 创建者 | 链接类型 | 链接路径 | 说明 |
|------------|-----------|-----------|-------------|
| 设备 | 接收方链接 | `/devices/<deviceID>/messages/devicebound` | 每个目标设备将在此链接上接收发往设备的 C2D 消息。 |
| 设备 | 发送方链接 | `/devices/<deviceID>messages/events` | 设备发送的 D2C 消息将通过此链接发送。 |
| 设备 | 发送方链接 | `/messages/serviceBound/feedback` | 设备通过此链接将 C2D 消息反馈发送到服务。 |


### <a name="receive-c2d-commands-device-client"></a>接收 C2D 命令（设备客户端）
发送到设备的 C2D 命令抵达 `/devices/<deviceID>/messages/devicebound` 链接。 设备可以分批接收这些消息，并根据需要在消息中使用消息数据有效负载、消息属性、批注或应用程序属性。

以下代码片段使用 [Python 中的 uAMQP 库](https://github.com/Azure/azure-uamqp-python)接收设备发送的 C2D 消息。
...
设备也可以通过 AMQP 发送遥测消息。 设备可以选择性地提供应用程序属性的字典或各种消息属性，例如消息 ID。

以下代码片段使用 [Python 中的 uAMQP 库](https://github.com/Azure/azure-uamqp-python)从设备发送 D2C 消息。
...
* 在某些情况下，客户端必须能够正确处理链接重定向。 有关如何处理此操作，请参阅 AMQP 客户端文档。

## <a name="next-steps"></a>后续步骤

若要详细了解 AMQP 协议，请参阅 [AMQP v1.0 规范](http://www.amqp.org/sites/amqp.org/files/amqp.pdf)。

若要详细了解 IoT 中心消息传递，请参阅：

* [云到设备的消息](./iot-hub-devguide-messages-c2d.md)
* [支持其他协议](iot-hub-protocol-gateway.md)
* [MQTT 协议支持](./iot-hub-mqtt-support.md)

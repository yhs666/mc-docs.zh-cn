---
title: "连接到云的 Raspberry Pi (Node.js) - 将 Raspberry Pi 连接到 Azure IoT 中心 | Azure"
...
ms.openlocfilehash: 9cefc406214989d1f3c1285c986b42756090c8c5
...
在本教程中，你首先将学习有关使用运行 Raspbian 的 Raspberry Pi 的基础知识。 然后将学习如何使用 [Azure IoT 中心](./iot-hub-what-is-iot-hub.md)将设备无缝连接到云。 有关 Windows 10 IoT Core 的示例，请访问 [Windows 开发人员中心](http://www.windowsondevices.com/)。
...
将 Raspberry Pi 连接到你创建的 IoT 中心。 然后，在 Pi 上运行示例应用程序，以从 BME280 传感器收集温度和湿度数据。 最后，将传感器数据发送到 IoT 中心。
...
| 始端（传感器和 LED 灯）     | 末端（开发板）            | 线缆颜色   |
| -----------------------  | ---------------------- | ------------: |
| VDD（引脚 5G）             | 3.3 伏 PWR（引脚 1）       | 白线   |
| GND（引脚 7G）             | GND（引脚 6）            | 棕色线   |
...
1. 使用以下任一 SSH 客户端从主计算机连接到 Raspberry Pi。

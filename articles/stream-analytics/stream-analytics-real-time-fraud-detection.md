...
2. 最佳做法是让每个流分析作业都从单个事件中心使用者组进行读取。我们稍后会引导完成创建使用者组的过程。[了解有关使用者组的详细信息](../event-hubs/event-hubs-what-is-event-hubs.md)。若要创建使用者组，请转到新创建的事件中心，单击“使用者组”选项卡，然后单击页面底部的“创建”，为使用者组提供一个名称。

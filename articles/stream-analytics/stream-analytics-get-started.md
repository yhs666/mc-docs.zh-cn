...
ms.date: 07/10/2017
ms.author: v-yeche
ms.openlocfilehash: 2bd645be07446b9ecc4924e16a147882e0942fa6
...
<a id="get-started-using-azure-stream-analytics-real-time-fraud-detection" class="xliff"></a>
...
<a id="scenario-telecommunications-and-sim-fraud-detection-in-real-time" class="xliff"></a>
...
在标准的物联网 (IoT) 方案中，会生成大量的遥测或传感器数据，客户需要对这些数据进行聚合分析，或在出现异常情况时实时提醒用户。
...
<a id="create-an-azure-event-hubs-input-and-consumer-group" class="xliff"></a>
...
## 配置并启动事件生成器应用程序
<a id="configure-and-start-event-generator-application" class="xliff"></a>
...
## 创建流分析作业
<a id="create-stream-analytics-job" class="xliff"></a>
现在，我们已获得电信事件流，因此可以设置一个流分析作业来实时分析这些事件。

### 预配流分析作业
<a id="provision-a-stream-analytics-job" class="xliff"></a>
...
### 指定作业输入
<a id="specify-job-input" class="xliff"></a>
...
### 指定作业查询
<a id="specify-job-query" class="xliff"></a>
...
#### 可选：示例输入数据
<a id="optional-sample-input-data" class="xliff"></a>
...
#### 传递查询
<a id="passthrough-query" class="xliff"></a>
...
### 列投影
<a id="column-projection" class="xliff"></a>
...
<a id="count-of-incoming-calls-by-region-tumbling-window-with-aggregation" class="xliff"></a>
...
1. 在代码编辑器中将查询更改为：
    ...
    此查询使用 **Timestamp By** 关键字在要用于临时计算的负载中指定时间戳字段。 如果未指定此字段，将根据每个事件到达事件中心的时间执行时间范围操作。 请参阅 [流分析查询语言参考中的“到达时间与应用程序时间”](https://msdn.microsoft.com/library/azure/dn834998.aspx)。
...
### 使用自联接进行 SIM 欺诈行为检测
<a id="sim-fraud-detection-with-a-self-join" class="xliff"></a>
...
### 创建输出接收器
<a id="create-output-sink" class="xliff"></a>
...
## 指定作业输出
<a id="specify-job-output" class="xliff"></a>
...
## 启动要进行实时处理的作业
<a id="start-job-for-real-time-processing" class="xliff"></a>
...
## 查看欺诈检测输出
<a id="view-fraud-detection-output" class="xliff"></a>

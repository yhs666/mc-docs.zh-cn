...
Azure 资源组项目提供的 PowerShell 脚本在此版本中已更新，可以配合新的 Azure PowerShell cmdlet 1.0 版使用。  使用 2.8 以前的 SDK 版本时，无法从 Visual Studio 使用此新脚本。  
...
* 当客户在创建应用服务时，如果要部署的目标 App Insights 实例位于中国东部以外的区域，可能会看到错误。 在这种情况下，在门户中创建应用服务和下载发布配置文件会启用发布方案。 
...
1. 以槽为目标的触发 Web 作业发布会显示错误，并且不会设置计划，但会将 Web 作业推送到 Azure。 然后，需要计划作业的客户可以使用 Azure 门户设置 Web 作业的计划。 

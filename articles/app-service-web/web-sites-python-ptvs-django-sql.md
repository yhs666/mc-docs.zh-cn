...
ms.date: 03/17/2017
ms.author: v-dazen
ms.openlocfilehash: 9b036aef8c8b45637c68d6d8eed3bba9ca82ae9b
ms.sourcegitcommit: d75065296d301f0851f93d6175a508bdd9fd7afc
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 11/30/2018
ms.locfileid: "52648750"
---
...
我们要了解如何使用在 Azure 上托管的 SQL 数据库、如何将 Web 应用配置为使用 SQL 数据库，以及如何将 Web 应用发布到 [Azure 应用服务 Web 应用](/app-service-web/app-service-changes-existing-services)中。

请访问 [Python 开发人员中心] ，查看更多有关使用 PTVS 以及 Bottle、Flask 和 Django Web 框架、Azure 表存储、MySQL、SQL 数据库服务开发 Azure 应用服务 Web 应用的文章。 虽然本文着重介绍应用服务，但步骤与 [Azure 云服务]的开发步骤类似。
...
在此部分中，我们使用样本模板创建 Visual Studio 项目。 我们创建虚拟环境并安装所需软件包。 我们将创建一个使用 sqlite 的本地数据库。 然后，我们将在本地运行 Web 应用。
...
6. 在“防火墙设置”边栏选项卡中，添加“起始 IP”和“结束 IP”已设置为开发计算机的公共 IP 地址的防火墙规则。 单击“保存” 。
   ...
   这会允许从您的开发计算机连接到数据库服务器。
7. 返回数据库边栏选项卡，并依次单击“属性”和“显示数据库连接字符串”。
8. 使用复制按钮将 **ADO.NET** 的值复制到剪贴板上。

## <a name="configure-the-project"></a>配置项目
在此部分中，我们把 Web 应用配置为使用我们刚刚创建的 SQL 数据库。 我们还会安装结合使用 SQL 数据库和 Django 所需的其他 Python 包。 然后，我们将在本地运行 Web 应用。
...
1. 在“解决方案资源管理器”的“Python 环境”下，右键单击“虚拟环境”，并选择“安装 Python 包”。
2. 使用 **pip** 安装包`pyodbc`。
...
4. 在“解决方案资源管理器”中，右键单击项目节点，选择“Python”，并选择“Django 迁移”。  然后选择“Django 创建超级用户”。

   这会创建我们在上一节中创建的 SQL 数据库的表。 按照提示操作以创建用户，其并不一定要匹配第一节中创建的 sqlite 数据库中的用户。
...
5. 接受其他所有默认值，并单击 **发布**。
6. 此时，Web 浏览器会自动打开已发布的 Web 应用。 应该会看到 Web 应用使用在 Azure 上托管的 **SQL** 数据库按预期方式运行。

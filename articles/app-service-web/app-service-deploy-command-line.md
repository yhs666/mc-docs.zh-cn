...
可以从 [Windows PowerShell](http://msdn.microsoft.com/library/dd835506.aspx) 执行 MSBuild 或 FTP 部署功能。 如果这样做，则还可以使用 Windows PowerShell cmdlet 集合（可使 Azure REST 管理 API 易于调用）。
...
可以编写 C# 代码以执行 MSBuild 或 FTP 功能进行部署。 如果这样做，则可以访问 Azure 管理 REST API 来执行站点管理功能。
...
通过使用 FTP，可以在 Mac 或 Linux 计算机中通过 Windows 中的命令行来进行部署。 如果这样做，则还可以访问使用 Azure CLI 的 Azure REST 管理 API。

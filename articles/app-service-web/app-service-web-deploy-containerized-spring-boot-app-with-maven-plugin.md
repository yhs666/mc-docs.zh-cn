...
ms.author: v-yiso
ms.date: 09/04/2017
ms.openlocfilehash: 87ba12cadaa07ff241f9bde60393966623cdadb1
ms.sourcegitcommit: d75065296d301f0851f93d6175a508bdd9fd7afc
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 11/30/2018
ms.locfileid: "52661295"
---
...
本文演示如何使用适用于 Azure Web 应用的 Maven 插件将 Docker 容器中的示例 Spring Boot 应用程序部署到 Azure 应用服务。
...
* 一个 Azure 订阅；如果没有 Azure 订阅，可以注册 [试用 Azure 帐户]。
* [Azure 命令行接口 (CLI)]。
...
本部分创建 Maven 插件在将容器部署到 Azure 时使用的 Azure 服务主体。
...
本部分使用 Azure 服务主体中的值配置 Maven 在将容器部署到 Azure 时要使用的身份验证。
...
1. 使用 Docker 帐户名更新 `${docker.image.prefix}` 值：
   ```xml
   <containerSettings>
...
1. 选择以下部署方法之一：
   ...
   * 使用 Maven 在本地构建容器映像，并使用 Docker 将容器推送到 Docker 中心：
      ```shell
...
在文本编辑器中打开 Spring Boot 应用程序的 `pom.xml` 文件，并找到 `azure-webapp-maven-plugin` 的 `<plugin>` 元素。 该元素应类似于以下示例：
...
可以为 Maven 插件修改几个值，[适用于 Azure Web 应用的 Maven 插件]文档中提供了这些元素各自的详细说明。 尽管如此，在本文中有仍几个值得注意的值：
...
配置了本文前面部分中的所有设置后，就可将容器部署到 Azure。 为此，请使用以下步骤：
...
Maven 会将 Web 应用部署到 Azure；如果 Web 应用不存在，则会创建一个。
...
* [通过 Azure CLI 登录到 Azure](https://docs.microsoft.com/en-us/azure/xplat-cli-connect)

* [如何使用适用于 Azure Web 应用的 Maven 插件将 Spring Boot 应用部署到 Azure 应用服务](./app-service-web-deploy-spring-boot-app-with-maven-plugin.md)

* [使用 Azure CLI 2.0 创建 Azure 服务主体](https://docs.microsoft.com/en-us/cli/azure/create-an-azure-service-principal-azure-cli)
...
[Azure 命令行接口 (CLI)]: https://docs.microsoft.com/en-us/cli/azure/overview

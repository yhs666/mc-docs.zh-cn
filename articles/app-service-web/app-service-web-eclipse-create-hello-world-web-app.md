...
ms.date: 07/10/2017
ms.author: v-dazen
ms.openlocfilehash: c45babbe70483fe0748cd4d45557541375cdd484
ms.sourcegitcommit: f119d4ef8ad3f5d7175261552ce4ca7e2231bc7b
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 06/30/2017
---
# 使用 Eclipse 创建基本的 Azure Web 应用
<a id="create-a-basic-azure-web-app-using-eclipse" class="xliff"></a>
...
7. 在 Eclipse 中打开 index.jsp 文件后，添加文本以将 **Hello World!** 动态显示。 现有 `<body>` 元素中。 更新后的 `<body>` 内容应类似于以下示例：
...
6. 如果前面尚未创建 Azure Web 应用容器，或你想要将应用程序发布到新的容器，请使用以下步骤。 否则，请选择现有的 Web 应用容器，并跳到下面的步骤 7。
   ...
   6. 在“资源组”下拉菜单中，选择要与 Web 应用关联的资源组。 （使用 Azure 资源组可以将相关资源组织在一起，以便于将它们一起删除。）
      ...
      * 可选：默认情况下，Azure 自动将最新的 Java 8 分发版作为 JVM 部署到 Web 应用容器。 但是，可以根据 Web 应用的需要指定 JVM 的其他版本和分发版。 若要指定 Web 应用的 JDK，请单击“JDK”  选项卡，然后选择下列选项之一：
        ...
        * **部署 Azure 上可用的第三方 JDK**：使用此选项可从由 Azure 提供的 JDK 列表中选择。
        * **从此下载位置部署我自己的 JDK**：此选项可以指定自己的 JDK 分发版，此 JDK 必须打包为 ZIP 文件并上传到公开可用的下载位置或你可以访问的 Azure 存储帐户。
...
8. 接下来，应会看到“Azure 活动日志”视图，其中指示了 Web 应用的部署状态。
    ...
    将 Web 应用部署到 Azure 的过程只需几秒钟即可完成。 当应用程序准备就绪时，可在“状态”列中看到名为“已发布”的链接。 单击该链接时，将转到已部署的 Web 应用的主页。

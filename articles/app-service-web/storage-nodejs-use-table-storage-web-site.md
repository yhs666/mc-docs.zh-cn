...
ms.date: 03/28/2017
ms.author: v-dazen
ms.openlocfilehash: ace2f26306b09f4fd55fbf694cf03a8b80db4151
ms.sourcegitcommit: d75065296d301f0851f93d6175a508bdd9fd7afc
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 11/30/2018
ms.locfileid: "52666829"
---
...
2. 单击门户左下角的“新建”图标，并单击“数据 + 存储” > “存储”。 为存储帐户命名一个唯一名称，并为其新建一个[资源组](../azure-resource-manager/resource-group-overview.md)。
...
本节涉及创建新的 Node 应用程序并使用 npm 添加模块包。 对于此应用程序，将使用 [Express] 和 [Azure] 模块。 Express 模块为 Node 提供模型视图控制器框架，而 Azure 模块提供与表服务的连接。
...
**express** 创建的其中一个文件是 **package.json**。 此文件包含模块依赖项的列表。 之后，将应用程序部署到应用服务 Web 应用时，此文件会确定需要在 Azure 上安装的模块。
...
使用 **--save** 标志将这些模块的条目添加到 **package.json** 文件中。
...
2. 在 **models** 目录中，创建一个名为 **task.js** 的新文件。 此文件将包含应用程序创建的任务的模型。
...
2. 将以下代码添加到 **tasklist.js**。 这会加载 **tasklist.js**使用的 azure 和 async 模块。 这还将定义 **TaskList** 函数，会向该函数传递我们之前定义的 **Task** 对象的一个实例：
...
2. 在文件开头，添加以下代码来加载 azure 模块，设置表名称、分区键，并设置此示例使用的存储凭据：
   ...
   > [!NOTE]
   > nconf 将从环境变量或我们稍后会创建的 **config.json** 文件中加载配置值。
   > 
   > 
3. 在 app.js 文件中，向下滚动到以下行：
    ...
    将上面的行替换为下面显示的代码。 这会通过连接到你的存储帐户来初始化 <strong>Task</strong> 的实例。 这是 <strong>TaskList</strong>的密码，TaskList 使用该密码与表服务进行通信：
...
**views** 目录中的 **Layout.jade** 文件是其他 **.jade** 文件的全局模板。 在此步骤中，对其进行修改，以使用 [Twitter Bootstrap](https://github.com/twbs/bootstrap)（一个可以轻松设计美观 Web 应用的工具包）。
...
要在本地运行应用，我们将 Azure 存储凭据放入 config 文件中。 使用以下 JSON 创建名为 *config.json* 的文件：
...
3. 打开 Web 浏览器并导航到 http://127.0.0.1:3000。
    ...
    ![显示空白 tasklist 的网页][node-table-finished]
4. 要创建新的待办事项，请输入名称和类别，并单击“添加项”。 
5. 要将任务标记为完成，请选中“完成”，然后单击“更新任务”。
...
本节中的步骤使用 Azure 命令行工具在应用服务中创建新的 Web 应用，并使用 Git 部署应用程序。 要执行这些步骤，必须具有 Azure 订阅。

> [!NOTE]
> 还可以使用 [Azure 门户](https://portal.azure.cn/)执行这些步骤。 请参阅 [在 Azure 应用服务中构建和部署 Node.js Web 应用]（在 Azure 应用服务中构建和部署 Node.js Web 应用）。
> 
> 如果这是你创建的第一个 Web 应用，则必须使用 Azure 门户部署此应用程序。
> 
> 
...
1. 输入以下命令：
    ...
    文件下载会自动开始；如果没有自动开始，可以单击该页面开头的链接手动下载文件。 保存文件并记下文件路径。
2. 输入以下命令以导入设置：
...
2. 使用以下命令创建新的 Web 应用。
    ...
    系统会提示输入 Web 应用的名称和位置。 提供唯一名称，并选择与 Azure 存储帐户相同的地理位置。
    ...
    此命令运行完成后，会看到类似如下的输出。 请注意，以 **Website created at** 开头的行包含 Web 应用的 URL。
   ...
   > [!NOTE]
   > 如果这是订阅的第一个应用服务 Web 应用，系统会指示使用 Azure 门户创建 Web 应用。 有关详细信息，请参阅 [在 Azure 应用服务中构建和部署 Node.js Web 应用]（在 Azure 应用服务中构建和部署 Node.js Web 应用）。
   > 
   > 

### <a name="set-environment-variables"></a>设置环境变量。
在此步骤中，环境变量会添加到 Azure 上的 Web 应用配置中。
在命令行中，输入以下信息：
...
1. 通过单击“浏览” > “Web 应用”> 应用名称打开 Web 应用的边栏选项卡。
2. 在 Web 应用边栏选项卡中，单击“所有设置” > “应用程序设置”。

     <!-- ![Top Menu](./media/storage-nodejs-use-table-storage-web-site/PollsCommonWebSiteTopMenu.png) -->
3. 向下滚动到“应用设置”部分，并添加键/值对。
...
要发布应用，将代码文件提交到 Git 中，随后推送到 azure/master。
...
3. 将提交的文件推送到应用服务 Web 应用：
    ...
    使用 **master** 作为目标分支。 部署结束后，会看到类似于以下示例的语句：

        To https://username@tabletasklist.chinacloudsites.cn/TableTasklist.git
          * [new branch]      master -> master
4. 推送操作完成后，浏览到 `azure create site` 命令之前返回的 Web 应用 URL，以查看应用程序。
...
[在 Azure 应用服务中构建和部署 Node.js Web 应用]: app-service-web-get-started-nodejs.md

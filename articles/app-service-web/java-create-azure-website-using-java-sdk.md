...
* 第 2 部分演示如何创建简单的 JSP“Hello World”应用程序，并使用 FTP 客户端将代码部署到应用服务。
...
在开始此过程之前，需要拥有有效的 Azure 订阅，并在 Azure 上设置默认的 Active Directory (AD)。

### <a name="create-an-active-directory-ad-in-azure"></a>在 Azure 中创建 Active Directory (AD)
如果 Azure 订阅中还没有 Active Directory (AD)，请使用 Microsoft 帐户登录 [Azure 经典管理门户][Azure Classic Management Portal]。 如有多个订阅，请单击“订阅”并选择要用于此项目的订阅的默认目录。 然后单击“应用”切换到该订阅视图。
...
Azure SDK for Java 使用管理证书在 Azure 订阅中进行身份验证。 对于使用服务管理 API 代表订阅所有者管理订阅资源的客户端应用程序，可以使用这些 X.509 v3 证书来对其进行身份验证。
...
完成此过程后，CER 证书驻留在 Azure 订阅中，JKS 证书将驻留在本地驱动器中。 有关管理证书的详细信息，请参阅[创建并上传 Azure 的管理证书][Create and Upload a Management Certificate for Azure]。
...
1. 出现提示时，输入目标密钥库密码；这会是 JKS 文件的密码。
...
5. 打开“全局存储库”，右键单击“中央”存储库，然后选择“重新生成索引”。
    ...
    此步骤可能需要几分钟时间，具体取决于连接速度。 重新生成索引后， **中心** Maven 存储库中应会显示 Azure 包。
...
2. 在“新建 Java 类”中，将类命名为 `WebCreator`，并选中“public static void main”复选框。 所选内容应如下所示：
    ...
    ![][2]
3. 单击“完成” 。 WebCreator.java 文件会在项目资源管理器中出现。
...
为 Web 应用和 Web 空间添加以下参数定义。 需要提供自己的 Azure 订阅 ID 和证书信息。
...
代码输出指示成功或失败的 HTTP 响应状态；如果成功，则输出创建的 Web 应用的名称。
...
若要验证应用程序是否运行，请单击“运行”>“运行”。 在应用程序完成运行后，应该会在 Eclipse 控制台中看到以下输出：
...
运行 AzureWebDemo 并创建新 Web 应用后，请登录经典管理门户，单击“Web 应用”，然后在“Web 应用”列表中选择“WebDemoWebApp”。 在 Web 应用的仪表板页上，单击“浏览”（或单击 URL `webdemowebapp.chinacloudsites.cn`）导航到该网站。 会看到一个空白的占位符页，因为尚未将任何内容发布到 Web 应用。

接下来，要创建一个“Hello World”应用程序并将其部署到 Web 应用。
...
选择第三方 FTP 客户端来发布应用程序。 此过程介绍两个选项：Azure 中内置的 Kudu 控制台；FileZilla，这是一个带有便捷式图形 UI 的常用工具。
...
确保已运行 **AzureWebDemo** 应用程序来创建 Web 应用。 会将文件转移到此位置。
...
2. 在“仪表板”页的“速览”下，单击“设置部署凭据”（如果已有部署凭据，则此选项应为“重置部署凭据”）。
    ...
    部署凭据与某个 Microsoft 帐户关联。 需要指定可用于使用 Git 和 FTP 进行部署的用户名和密码。 可以使用这些凭据部署到与 Microsoft 帐户关联的所有 Azure 订阅中的任何 Web 应用。 在对话框中提供 Git 和 FTP 部署凭据，并记下用户名和密码以供将来使用。

#### <a name="get-ftp-connection-information"></a>获取 FTP 连接信息
要使用 FTP 将应用程序文件部署到新建的 Web 应用，你需要获取连接信息。 可通过两种方法获取连接信息。 一种方法是访问 Web 应用的“仪表板”  页；另一种方法是下载 Web 应用的发布配置文件。 发布配置文件是一个 XML 文件，它提供 Azure 应用服务中 Web 应用的 FTP 主机名和登录凭据等信息。 可以使用此用户名和密码部署到与 Azure 帐户关联的所有订阅中的任何 Web 应用，而不仅仅是此 Web 应用。
...
发布应用程序之前，需要更改几项配置设置，使 Web 应用可以托管 Java 应用程序。
...
不久之后（可能小于 5 分钟），Tomcat 服务器会将 WAR 文件解压缩到解包的 JSPHello 目录中。 单击根目录以查看 index.jsp 是否已解压缩并复制到此处。 如果是，请导航回到 webapps 目录，查看是否已创建解包的 JSPHello 目录。 如果看不到这些项，请等待片刻后重复操作。
...
此过程会创建应用服务 Web 应用。 只要 Web 应用存在，就要支付资源的费用。 除非打算继续使用该 Web 应用进行测试或开发，否则应考虑停止或删除它。 已停止的 Web 应用仍会产生较小的费用，但你随时可以重新启动它。 删除某个 Web 应用会清除已上传到该 Web 应用的所有数据。

...
Microsoft Flow 和 Azure 逻辑应用都是以配置为首的集成服务。 二者都可以创建工作流，与各种 SaaS 和企业应用程序集成。 
...
| 管理员体验 |管理流环境和数据丢失防护 (DLP) 策略，跟踪许可 [https://admin.flow.microsoft.com](https://admin.flow.microsoft.com) |管理资源组、连接、访问管理和日志记录 [https://portal.azure.cn](https://portal.azure.cn) |
| 安全性 |Office 365 安全性和符合性审核日志、数据丢失防护 (DLP)、针对敏感数据的[静态加密](https://wikipedia.org/wiki/Data_at_rest#Encryption)等。 |Azure 的安全保证：Azure 安全性、[安全中心](https://azure.microsoft.com/services/security-center/)、[审核日志](https://azure.microsoft.com/blog/azure-audit-logs-ux-refresh/)等等。 |

<a name="function"></a>

## <a name="functions-vs-webjobs"></a>函数 vs.Web 作业

与 Azure Functions 一样，包含 WebJobs SDK 的 Azure 应用服务是一项代码优先的集成服务，专为开发人员设计。 二者都是在 [Azure 应用服务](../app-service/app-service-web-overview.md)上构建的，并支持[源代码管理集成](../app-service/app-service-continuous-deployment.md)和[身份验证](../app-service/app-service-authentication-overview.md)等功能。

### <a name="webjobs-vs-the-webjobs-sdk"></a>WebJobs 与 WebJobs SDK 对比
...
WebJobs 和 WebJobs SDK 在一起使用时效果最佳，但也可在没有 WebJobs SDK 的情况下使用 WebJobs，反之亦然。 WebJob 可以运行任何程序或脚本，只要该程序或脚本可以在应用服务沙盒中运行。 WebJobs SDK 控制台应用程序可以在任何允许控制台应用程序运行的位置（例如本地服务器）运行。
...
流可以调用逻辑应用。 逻辑应用可以调用函数，而函数也可以调用逻辑应用。 

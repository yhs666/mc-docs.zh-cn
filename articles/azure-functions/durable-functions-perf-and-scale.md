...
ms.date: 04/13/2018
ms.author: v-junlch
ms.openlocfilehash: 992594e6590ee5c4f37b0bb5fb85d82b33fcb00f
...
Durable Functions 支持通过 [Azure Functions 缩放控制器](/azure-functions/functions-scale#runtime-scaling)进行自动缩放。 缩放控制器监视工作项队列和每个控制队列的长度，可相应地添加或删除 VM 实例。 如果控制队列的长度逐渐增加，缩放控制器会持续添加 VM 实例，直到达到控制队列分区计数为止。 如果工作项队列的长度逐渐增加，缩放控制器会持续添加 VM 实例，直到能够与负载相匹配为止，而不考虑控制队列分区计数。

...
如果需要将函数应用使用的出站 IP 地址加入白名单，另一种做法是将函数应用的数据中心（Azure 区域）加入白名单。 可以[下载列出所有 Azure 数据中心 IP 地址的 XML 文件](https://www.microsoft.com/en-us/download/details.aspx?id=42064)。 然后，找到应用于运行函数应用的区域的 XML 元素。

例如，中国北部区域的 XML 元素可能如下所示：

```
  <Region Name="chinanorth">
...
```
 ...
 有关此文件何时更新以及 IP 地址何时更改的信息，请展开[下载中心页](https://www.microsoft.com/en-us/download/details.aspx?id=42064)的“详细信息”部分。

...
本快速入门要求部署在不同 Azure 区域（中国东部和中国北部）运行的两个 Web 应用程序实例。 这两个 Web 应用程序实例充当流量管理器的主终结点和备份终结点。
...
2. 在“Web 应用”中输入或选择以下信息，并在未指定任何设置的地方输入默认设置：
    ...
    | 应用服务计划/位置         | 选择“新建”。  在应用服务计划中，输入 *myAppServicePlanChinaEast*，然后选择“确定”。 
    |      位置  |   中国东部        |
    |||
...
2. 在“创建流量管理器配置文件”中输入或选择以下信息，接受剩下的默认设置，然后选择“创建”：
    ...
    | 设置                 | 值                                              |
    | ---                     | ---                                                |
    | Name                   | 此名称必须在 trafficmanager.cn 区域中唯一，并会生成 DNS 名称 (**trafficmanager.cn**)，该名称用于访问流量管理器配置文件。|
    | 路由方法          | 选择“优先级”路由方法。|
    | 订阅            | 选择订阅。|
...
将“中国东部”区域的网站添加为用于路由所有用户流量的主终结点。 将“中国北部”区域的网站添加为备份终结点。 当主终结点不可用时，流量自动路由到辅助终结点。

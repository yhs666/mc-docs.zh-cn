...
* **可管理用户名和密码的 HDInsight SSH 服务**。 有关详细信息，请参阅[使用 SSH 连接到 HDInsight (Apache Hadoop)](/hdinsight/hdinsight-hadoop-linux-use-ssh-unix) 和[使用 SSH 隧道访问 Ambari Web UI、JobHistory、NameNode、Apache Oozie 和其他 Web UI](/hdinsight/hdinsight-linux-ambari-ssh-tunnel)。 

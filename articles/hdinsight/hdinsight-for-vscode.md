...
7. 范型 livy 终结点已成功链接。
   ...
   ![已链接的范型 livy 群集](./media/hdinsight-for-vscode/link-cluster-process-generic-livy.png)
...
通过用于 VS Code 的 HDInsight 工具，可以将交互式 Hive 查询、Hive 批处理脚本提交到 HDInsight 群集。

1. 创建一个新的工作文件夹和一个新的 Hive 脚本文件（如果还没有）。

2. 连接到 Azure 帐户或者链接群集。
...
默认情况下，当提交交互式 PySpark 查询时，HDInsight 工具将检查环境并安装所依赖的程序包。 若要禁用环境检查，请在“用户设置”下将 **hdinsight.disablePysparkEnvironmentValidation** 设置为 **yes**。

   ![从设置中设置环境检查](./media/hdinsight-for-vscode/hdi-azure-hdinsight-environment-check.png)

另外，还可以在对话框弹出时单击“禁用验证”按钮。

   ![从对话框中设置环境检查](./media/hdinsight-for-vscode/hdi-azure-hdinsight-environment-check-dialog.png)
...
可以找到“文件”菜单，选择“首选项”，然后在上下文菜单中选择“设置”。 单击“工作区设置”选项卡，你可以开始设置 livy 配置。

还可以提交一个文件，你会看到 .vscode 文件夹会自动添加到工作文件夹。 可以通过单击 **.vscode\settings.json** 来查找 livy 配置。
...
Azure HDInsight 已添加到左侧面板。 你可以直接浏览和管理群集。

1. 展开“AZURE HDINSIGHT”，如果未登录，则它将显示“登录到 Azure...”链接。

    ![登录链接图像](./media/hdinsight-for-vscode/hid-azure-hdinsight-sign-in.png)

2. 单击“登录到 Azure”，将会在右下角弹出登录链接和代码。

    ![针对其他环境的登录说明](./media/hdinsight-for-vscode/hdi-azure-hdinsight-azure-signin-code.png)

3. 单击“复制并打开”按钮将打开浏览器，请粘贴代码，单击“继续”按钮，你将会看到有关登录成功的提示。

4. 在登录后，可用的订阅和群集（支持 Spark、Hadoop 和 HBase）将在 **AZURE HDINSIGHT** 中列出。 

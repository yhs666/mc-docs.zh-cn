---
title: 将 BI 工具与 Azure HDInsight 上的 Apache Spark 配合使用 | Azure
...
7. 验证创建表是否成功。可以使用 `%%sql` 幻数直接运行 Hive 查询。有关 `%%sql` 幻数以及 PySpark 内核提供的其他幻数的详细信息，请参阅[包含 Spark HDInsight 群集的 Jupyter Notebook 上可用的内核](./hdinsight-apache-spark-jupyter-notebook-kernels.md#parameters-supported-with-the-sql-magic)。

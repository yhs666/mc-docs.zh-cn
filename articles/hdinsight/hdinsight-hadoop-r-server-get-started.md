...
ms.openlocfilehash: 95c27f49a027439f0ae283796cade45f5ef82b8a
ms.contentlocale: zh-cn
ms.lasthandoff: 05/26/2017


---
# <a name="get-started-using-r-server-on-hdinsight"></a>HDInsight 上的 R Server 入门
...
* **Azure 订阅**：在开始学习本教程之前，必须有一个 Azure 订阅。 请参阅 [Get a Azure trial](https://www.azure.cn/pricing/1rmb-trial/)（获取 Azure 试用版）一文以了解详细信息。
...
## <a name="create-the-cluster-using-the-azure-portal-preview"></a>使用 Azure 门户创建群集
...
4. 选择“群集类型”以打开“群集配置”边栏选项卡。 在“**群集配置**”边栏选项卡上，选择以下选项：
...
5. 输入“群集登录用户名”和“群集登录密码”。
    ...
    将“使用与群集登录相同的密码”对应的框保留为选中状态，以使用“密码”作为身份验证类型（除非更喜欢使用公钥）。  如果想要通过远程客户端（例如 RTVS、RStudio 或其他桌面 IDE）访问群集上的 R Server，则需要使用公钥/私钥对。 如果安装 RStudio Server 社区版，则需要选择 SSH 密码。     
...
6. 快速创建会转换到“存储”边栏选项卡以选择要用于群集使用的 HDFS 文件系统主位置的存储帐户设置。 选择新的或现有的 Azure 存储帐户。
    ...
    1. 通过选择“选择存储帐户”并选择一个帐户来选择现有存储帐户。 或者使用“选择存储帐户”部分中的“新建”链接创建新帐户。

        > [!NOTE]
        > 如果选择“新建”，则必须输入新存储帐户的名称。 如果该名称可接受，将会出现一个绿色对号。
...
7. “摘要”边栏选项卡随后会显示以验证所有设置。 在此处可以更改**群集大小**以修改群集中的服务器数，还可指定要运行的任何**脚本操作**。 除非确定需要更大的群集，否则请保留辅助角色节点数目的默认值 `4`。 该群集的预估成本将显示在边栏选项卡内。
    ...
    > [!NOTE]
    > 以后，如果需要，可以通过门户调整群集的大小（“群集”->“设置”->“缩放群集”），以增加或减少辅助角色节点的数目。  这种方法可将不再使用的群集置于空闲状态，或者增加容量来满足更大任务的需要。
    >
    >
    ...
    * R Server 分析的性能与所要分析的数据大小呈线性关系。 例如：  
    ...
    > [!NOTE]
    > 创建群集需要一些时间，通常为 20 分钟左右。 使用启动板上的磁贴或页面左侧的“通知”项检查创建过程。
    >
    >
...
2. 在“设置”边栏选项卡中，依次选择“脚本操作”、“提交新”以提交新的脚本操作。
...
4. 选择“创建”来运行脚本。 脚本完成后，可在所有辅助角色节点上使用 R 包。
...
Microsoft R Server 当前未通过 Yarn 管理。 如果未解除辅助节点，Yarn Resource Manager 将无法正常工作，因为它不知道服务器所占用的资源。 为了避免这种情况，建议解除要将计算节点缩放到的辅助节点。
...
* 使用 `dotnet /usr/lib64/microsoft-deployr/9.0.1/Microsoft.DeployR.Utils.AdminUtil/Microsoft.DeployR.Utils.AdminUtil.dll`
...
* [适用于 HDInsight 上的 R Server 的 Azure 存储选项](hdinsight-hadoop-r-server-storage.md)

...
ms.date: 09/24/2018
ms.author: v-yiso
ms.openlocfilehash: f4ed88e21291694afe2b7265f4cef71e6b626aaf
ms.sourcegitcommit: bae4e9e500e3e988ef8fa0371777ca9cc49b4e94
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/14/2018
ms.locfileid: "45584867"
---
...
* 位于同一 Azure 虚拟网络中的任何资源（通过使用资源的内部 DNS 名称）。 例如，在使用默认的名称解析时，下面是分配给 HDInsight 工作节点的内部 DNS 名称示例：
...
3. 查找 Azure 为你的虚拟网络分配的 DNS 后缀。 该值类似于 `0owcbllr5hze3hxdja3mqlrhhe.ex.internal.chinacloudapp.cn`。 有关查找 DNS 后缀的信息，请参阅[示例：自定义 DNS](#example-dns) 一节。
...
2. 使用以下文本作为自定义 DNS 服务器上 `/etc/bind/named.config.local` 文件的内容。 在这两个虚拟网络中的自定义 DNS 服务器上进行此更改。
    ...
    将 `0owcbllr5hze3hxdja3mqlrhhe.ex.internal.chinacloudapp.cn` 值替换为另一个虚拟网络的 DNS 后缀。 此条目将远程网络 DNS 后缀的请求路由到该网络中的自定义 DNS。

...
此 xml 配置解决方案，以将输出打包到 uber jar。 该 jar 包含项目代码和所需的依赖项。 它还用于：
...
4. 若要查看记录的数据，请转到 https://CLUSTERNAME.azurehdinsight.cn/stormui，其中 __CLUSTERNAME__ 是 HDInsight 群集的名称。 选择拓扑，深入到组件。 选择组件实例的 __port__ 项即可查看记录的信息。

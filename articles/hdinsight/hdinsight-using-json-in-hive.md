...
ms.date: 10/23/2017
ms.author: v-yiso
ms.openlocfilehash: b0b8f1c22cf0029feab6c1535515d0662e705fba
...
可以在 wasb://processjson@hditutorialdata.blob.core.chinacloudapi.cn/上找到该文件。 有关将 Azure Blob 存储与 HDInsight 配合使用的详细信息，请参阅[将 HDFS 兼容的 Azure Blob 存储与 HDInsight 中的 Hadoop 配合使用](hdinsight-hadoop-use-blob-storage.md)。 可以将该文件复制到群集的默认容器。
...
SerDe 是用于分析嵌套 JSON 文档的最佳选择，不但可定义 JSON 架构，而且还可使用架构分析文档。 有关说明，请参阅 [How to use a Custom JSON Serde with Microsoft Azure HDInsight](https://blogs.msdn.microsoft.com/bigdatasupport/2014/06/18/how-to-use-a-custom-json-serde-with-microsoft-azure-hdinsight/)（如何将自定义 JSON Serde 与 Microsoft Azure HDInsight 配合使用）。

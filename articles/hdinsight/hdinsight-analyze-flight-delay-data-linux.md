...
* **Azure CLI**。 如果尚未安装 Azure CLI，请参阅[安装 Azure CLI](https://docs.azure.cn/cli/install-azure-cli?view=azure-cli-latest)了解详细步骤。
...
1. 打开命令提示符，使用以下命令将 zip 文件上传到 HDInsight 群集头节点：
    ...
    ```bash
    scp <FILENAME>.zip <SSH-USERNAME>@<CLUSTERNAME>-ssh.azurehdinsight.cn:<FILENAME.zip>
    ```

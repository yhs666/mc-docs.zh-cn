...
> [!NOTE]
> HDInsight 还提供了一个 IntelliJ IDEA 插件工具，用于简化在 Linux 上创建应用程序并提交到 HDInsight Spark 群集的过程。 有关详细信息，请参阅[使用适用于 IntelliJ IDEA 的 HDInsight 工具插件创建和提交 Spark 应用程序](hdinsight-apache-spark-intellij-tool-plugin.md)。
> 
> 
...
7. 后续步骤将更新 pom.xml 以定义 Spark Scala 应用程序的依赖项。 若要自动下载并解析这些依赖项，必须相应地配置 Maven。
   ...
   4. 单击“应用”，并单击“确定”。
8. 更新 Scala 源文件以包含应用程序代码。 打开现有示例代码，并将其替换为以下代码，保存所做更改。 此代码从 HVAC.csv（所有 HDInsight Spark 群集均有该文件）中读取数据，检索第六列中只有一个数字的行，并将输出写入群集的默认存储容器下的 **/HVACOut**。

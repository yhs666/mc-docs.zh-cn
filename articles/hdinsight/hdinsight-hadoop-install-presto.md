...
> 
> 自定义组件（例如 Presto）可获得合理范围的支持，以便进一步排查问题。 此支持也许能够解决问题。 支持人员可能要求你参与可用的开源技术渠道，在该处可以找到该技术的深入专业知识。 有许多可以使用的社区站点。 例如，[面向 HDInsight 的 MSDN 论坛](https://social.msdn.microsoft.com/Forums/azure/home?forum=hdinsight)、[Azure CSDN](http://azure.csdn.net/)。 此外，Apache 项目在 [https://apache.org](https://apache.org) 上提供了项目站点，例如：[Hadoop](https://hadoop.apache.org/)。
> 
...
3. 针对所有 HDInsight 群集默认提供的示例表 **hivesampletable** 运行查询：
    ...
    默认情况下，已配置适用于 Presto 的 [Apache Hive](https://prestodb.io/docs/current/connector/hive.html) 和 [TPCH](https://prestodb.io/docs/current/connector/tpch.html) 连接器。 Hive 连接器配置为使用默认的 Hive 安装， 因此 Hive 中的所有表会自动在 Presto 中显示。
...
4. 使用[此模板](https://portal.azure.cn/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Fhdinsight%2Fpresto-hdinsight%2Fmaster%2Fairpal-deploy.json)创建 HDInsight 群集边缘节点。 按以下屏幕截图所示提供值。

    ![自定义部署](./media/hdinsight-hadoop-install-presto/hdinsight-install-airpal.png)

5. 选择“购买”。

6. 将更改应用到群集配置后，请执行以下步骤，从 [Azure 门户](https://portal.azure.cn)访问 Airpal Web 接口：
...
2. 在 `/var/lib/presto/presto-hdinsight-master/appConfig-default.json` 文件中进行配置更改。 有关 Presto 配置的详细信息，请参阅[基于 YARN 的群集的 Presto 配置选项](https://prestodb.io/presto-yarn/installation-yarn-configuration-options.html)。

...
* 已加入域的 HDInsight 群集。 请参阅 [Configure Domain-joined HDInsight clusters](apache-domain-joined-configure.md)（配置已加入域的 HDInsight 群集）。
...
1. 从浏览器中连接到 Ranger 管理 UI。 URL 为 https://&lt;ClusterName>.azurehdinsight.cn/Ranger/。
...
2. 使用群集管理员域用户名和密码登录：
    ...
    ![已加入域 HDInsight 的 Ranger 主页](./media/apache-domain-joined-run-hive/hdinsight-domain-joined-ranger-home-page.png)
...
3. 单击“添加新策略” ，并输入以下值：
   ...
   * 权限：select
     ...
     ![已加入域 HDInsight 的 Ranger Hive 策略配置](./media/apache-domain-joined-run-hive/hdinsight-domain-joined-configure-ranger-policy.png)上获取。
...
在最后一个部分中，配置了两个策略。  hiveuser1 对所有列具有 select 权限，而 hiveuser2 对两个列具有 select 权限。 在此部分中，将模拟这两个用户将数据导入到 Excel 中。
...
7. 单击“完成” 。
8. 在“导入数据”对话框中，可以更改或指定查询。 为此，请单击“属性” 。 这可能需要几秒钟时间。
9. 单击“定义”  选项卡。命令文本为：
...
12. 重新输入 hiveuser1 的密码，并单击“确定”。 需要几秒钟时间才能将数据导入到 Excel 中。 完成后，应看到 11 个列的数据。

...
2. 从页面顶部的菜单中，选择“Hive 编辑器” 。 此时会显示包含以下示例性查询的页面。
...
3. 选择“提交”  按钮。 页面底部的“作业会话”部分会显示查询的状态。 一旦状态更改为“已完成”，即可选择“查看详细信息”链接查看结果。 “作业输出”  应如下所示：
...
1. 在 [Azure 门户](https://portal.azure.cn)中，选择 HDInsight 群集。 在 HDInsight 边栏选项卡的顶部，选择“远程桌面”  图标。 此链接会显示远程桌面边栏选项卡
...
2. 在“远程桌面”边栏选项卡中，选择“连接”  连接到群集头节点。 出现提示时，使用群集远程桌面用户名和密码验证连接。
   ...
   > [!NOTE]
   > 如果尚未启用远程桌面连接，请提供用户名、密码和到期日期，然后选择“启用”，即可启用远程桌面。 一旦启用，即可使用前面的步骤进行连接。
   >
   >
...
5. 在 Internet Explorer 中浏览到 http://headnodehost:8188/tezui/#/。 此时会显示 Tez UI
    ...
    如果只有一个条目，则该条目是前一部分运行的查询的结果。 如果有多个条目，用户可以在 DAG 上方的字段中输入搜索条件进行搜索，并按 **Enter**。
...
8. 选择“图形视图” 。 此视图显示 DAG 的图形表示形式。 将鼠标放在视图中的单个顶点上即可显示其信息。

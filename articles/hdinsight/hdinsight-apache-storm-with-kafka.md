...
description: "Apache Kafka 将随 Apache Storm on HDInsight 一起安装。 了解如何使用 Storm 随附的 KafkaBolt 和 KafkaSpout 组件向 Kafka 写入数据，然后从中读取数据。 此外，还了解如何使用 Flux 框架来定义和提交 Storm 拓扑。"
services: hdinsight
documentationcenter: 
author: Blackmist
manager: paulettm
...
3. 使用以下命令将包复制到 Storm on HDInsight 群集。 将 **USERNAME** 替换为群集的 SSH 用户名。 将 **BASENAME** 替换为创建群集时使用的基名称。

...
收到此错误并不表示数据已损坏或丢失。笔记本仍在 `/var/lib/jupyter` 中的磁盘上，可以通过 SSH 连接到群集进行访问。对于 Windows 客户端，请参阅[在装有 PuTTY 的 Windows 中的 HDInsight 上将 SSH 与 Hadoop 配合使用](./hdinsight-hadoop-linux-use-ssh-windows.md)；对于 Linux、Unix 或 OS X，请参阅[在 Linux、Unix 或 OS X 中的 HDInsight 上将 SSH 与 Hadoop 配合使用](./hdinsight-hadoop-linux-use-ssh-unix.md)

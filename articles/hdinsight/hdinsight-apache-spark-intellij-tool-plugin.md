---
title: "用于 IntelliJ 的 Azure 工具包 - 为 HDInsight Spark 创建 Scala 应用程序 | Azure"
...
3. 单击“确定”下载 Scala 插件。 请遵照说明重启 IntelliJ。 
   ...
   ![Scala 安装](./media/hdinsight-apache-spark-intellij-tool-plugin/Choose-Scala-Plugin.PNG)
...
若要解决此错误，必须[下载该可执行文件](http://public-repo-1.hortonworks.com/hdp-win-alpha/winutils.exe)到相应位置（例如 **C:\WinUtils\bin**）。 然后添加环境变量 **HADOOP_HOME**，并将其值设置为 **C\WinUtils**。
...
2. 在下一个窗口中提供以下项目详细信息，然后单击“完成”。
    ...
    - 对于“Spark 版本”，请选择 Scala 要使用的版本：“适用于 Spark 2.0 的 Scala 2.11.x”和“适用于 Spark 1.6 的 Scala 2.10.x”。
![选择 Spark SDK](./media/hdinsight-apache-spark-intellij-tool-plugin/Create-local-project.PNG)

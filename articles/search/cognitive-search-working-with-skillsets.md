...
description: 技能集是在认知搜索中创作 AI 扩充管道的位置。本文将会介绍技能集的几个概念、其工作原理，以及如何生成简单或复杂的技能集
manager: eladz
author: vkurpad
services: search
ms.service: search
...
ms.date: 09/26/2019
ms.author: v-tawe
ms.openlocfilehash: 84f56c56c07b2e0f2e71354f4b75be3e77bd76ad
ms.sourcegitcommit: a5a43ed8b9ab870f30b94ab613663af5f24ae6e1
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/30/2019
ms.locfileid: "71674454"
---
# <a name="working-with-skillsets"></a>使用技能集
本文面向需要深入了解扩充管道工作原理的开发人员，并假设读者在概念上对认知搜索过程有所了解。 如果你不熟悉认知搜索，请从以下文章着手：
+ [什么是 Azure 搜索中的“认知搜索”？](cognitive-search-concept-intro.md)
+ [什么是 Azure 搜索中的知识存储？](knowledge-store-concept-intro.md)

## <a name="specify-the-skillset"></a>指定技能集
技能集是 Azure 搜索中的可重用资源，在索引编制期间指定用于分析、转换和扩充文本或图像内容的认知技能集合。 通过创建技能集，可以在数据引入阶段附加文本和图像扩充，并从原始内容提取和创建新的信息与结构。
...
技能集是以 JSON 格式创作的。 可以使用[表达式语言](cognitive-search-skill-conditional.md)生成包含循环和[分支](cognitive-search-skill-conditional.md)的复杂技能集。 表达式语言使用 [JSON 指针](https://tools.ietf.org/html/rfc6901)路径表示法，做出少量的修改来标识扩充树中的节点。 ```"/"``` 遍历树中的较低级别，```"*"``` 充当上下文中的 for-each 运算符。 我们将通过一个示例来全面描述这些概念。 为了演示某些概念和功能，我们将演练[酒店评论示例](knowledge-store-create-portal.md)技能集。 若要在遵循导入数据工作流后查看技能集，需要使用 REST API 客户端来[获取技能集](https://docs.microsoft.com/rest/api/searchservice/get-skillset)。
...
本文档的余下内容假设使用的是[酒店评论示例](knowledge-store-create-portal.md)，但相同的概念也适用于从所有其他数据源扩充文档。
...
现在，让我们逐步了解酒店评论技能集；可以遵循该[教程](knowledge-store-create-portal.md)创建技能集或[查看](https://github.com/Azure-Samples/azure-search-postman-samples/blob/master/samples/skillset.json)技能集。 我们将会探讨：
...
![完成文档破解之后的扩充树](media/cognitive-search-working-with-skillsets/enrichment-tree-doc-cracking.png "完成文档破解之后、执行技能之前的扩充树")
...
技能集还会定义一个知识存储，可在其中将扩充的文档投影为表或对象。 若要将扩充数据保存到知识存储中，可为扩充的文档定义一组投影。 若要详细了解知识存储，请参阅[什么是 Azure 搜索中的知识存储？](knowledge-store-concept-intro.md)

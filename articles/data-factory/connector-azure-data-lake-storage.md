...
ms.date: 06/10/2019
ms.author: v-jay
ms.openlocfilehash: ccc45ae37b071abdfa20bc35e8350683b012138b
ms.sourcegitcommit: 1ebfbb6f29eda7ca7f03af92eee0242ea0b30953
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 06/06/2019
ms.locfileid: "66732598"
---
...
>[!NOTE]
>如果在 Azure 存储防火墙设置上启用“允许受信任的 Microsoft 服务访问此存储帐户”  选项，则使用 Azure Integration Runtime 连接 Data Lake Storage Gen2 将失败并显示“已禁止”错误，因为 ADF 不被视为受信任的服务。 请改用自承载集成运行时进行连接。
...
1. 向服务主体授予适当的权限。
    ...
    - **作为源**：在存储资源管理器中，至少授予“读取 + 执行”权限以列出和复制文件夹和子文件夹中的文件，或授予“读取”权限以复制单个文件。   或者，在访问控制 (IAM) 中至少授予“存储 Blob 数据读取者”角色。 
    - **作为接收器**：在存储资源管理器中，至少授予“写入 + 执行”权限以在文件夹中创建子项。  或者，在访问控制 (IAM) 中至少授予“存储 Blob 数据参与者”角色。 

>[!NOTE]
>若要列出从帐户级别开始的文件夹或测试连接，需要在 IAM 中使用“执行”权限为服务主体授予对存储帐户的权限。  对于以下情况需要这样做：
>- 使用**复制数据工具**创作复制管道。
>- 在创作期间使用**数据工厂 UI** 测试连接和浏览文件夹。 
>如果对于在帐户级别授予权限存在顾虑，可以在创作过程中跳过测试连接，然后手动输入路径。 只要针对要复制的文件为服务主体授予适当的权限，则仍可使用复制活动。
...
可将数据工厂与代表此特定数据工厂的 [Azure 资源托管标识](data-factory-service-identity.md)相关联。 可以像使用自己的服务主体一样，直接使用此托管标识进行 ADLS Gen2 身份验证。 此指定工厂可通过此方法访问以及从/向 ADLS Gen2 存储复制数据。
...
>[!NOTE]
>若要列出从帐户级别开始的文件夹或测试连接，需要在 IAM 中使用“执行”权限为托管标识授予对存储帐户的权限。  对于以下情况需要这样做：
>- 使用**复制数据工具**创作复制管道。
>- 在创作期间使用**数据工厂 UI** 测试连接和浏览文件夹。 
>如果对于在帐户级别授予权限存在顾虑，可以在创作过程中跳过测试连接，然后手动输入路径。 只要针对要复制的文件为托管标识授予适当的权限，则仍可使用复制活动。

>[!IMPORTANT]
>如果在使用 ADLS Gen2 托管标识身份验证时使用 PolyBase 将 ADLS Gen2 中的数据载入 SQL 数据仓库，请确保同时遵循[此指南](../sql-database/sql-database-vnet-service-endpoint-rule-overview.md#impact-of-using-vnet-service-endpoints-with-azure-storage)中的步骤 #1 和 #2，以将 SQL 数据库服务器注册到 Azure Active Directory (AAD)，并将存储 Blob 数据参与者 RBAC 角色分配到 SQL 数据库服务器；剩余的任务将由 ADF 处理。 如果在 ADLS Gen2 中配置了 VNet 服务终结点，若要使用 PolyBase 从中加载数据，必须使用托管标识身份验证。
...
若要以 **Parquet 或带分隔符的文本格式**将数据复制到 ADLS Gen2 或从中复制数据，请参阅 [Parquet 格式](format-parquet.md)和[带分隔符的文本格式](format-delimited-text.md)一文，了解基于格式的数据集和支持的设置。 基于格式的数据集中 `location` 设置下的 ADLS Gen2 支持以下属性：

| 属性   | 说明                                                  | 必选 |
| ---------- | ------------------------------------------------------------ | -------- |
| type       | 数据集中 `location` 下的 type 属性必须设置为 **AzureBlobFSLocation**。 | 是      |
| fileSystem | ADLS Gen2 文件系统名称。                              | 否       |
...
> [!NOTE]
> Copy/Lookup/GetMetadata 活动仍然按原样支持下一部分中提到的带有 Parquet/Text 格式的 **AzureBlobFSFile** 类型数据集，以实现向后兼容，但它不适用于映射数据流。 建议你继续使用此新模型，并且 ADF 创作 UI 已切换为生成这些新类型。
...
若要以 **ORC/Avro/JSON/Binary 格式**将数据复制到 ADLS Gen2 或从中复制数据，需要支持以下属性：

| 属性 | 说明 | 必选 |
|:--- |:--- |:--- |
| type | 数据集的 type 属性必须设置为 AzureBlobFSFile  。 |是 |
| folderPath | Data Lake Storage Gen2 中的文件夹的路径。 如果未指定，它指向根目录。 <br/><br/>支持通配符筛选器，允许的通配符为：`*`（匹配零个或更多个字符）和 `?`（匹配零个或单个字符）；如果实际文件夹名中包含通配符或此转义字符，请使用 `^` 进行转义。 <br/><br/>示例：filesystem/folder/，请参阅[文件夹和文件筛选器示例](#folder-and-file-filter-examples)中的更多示例。 |否 |
...
若要以 **Parquet 或带分隔符的文本格式**从 ADLS Gen2 复制数据，请参阅 [Parquet 格式](format-parquet.md)和[带分隔符的文本格式](format-delimited-text.md)一文，了解基于格式的复制活动源和支持的设置。 基于格式的复制源中 `storeSettings` 设置下的 ADLS Gen2 支持以下属性：
...
| wildcardFolderPath       | 数据集中配置的给定文件系统下包含通配符的文件夹路径，用于筛选源文件夹。 <br>允许的通配符为：`*`（匹配零个或更多个字符）和 `?`（匹配零个或单个字符）；如果实际文件夹名称中包含通配符或此转义字符，请使用 `^` 进行转义。 <br>请参阅[文件夹和文件筛选器示例](#folder-and-file-filter-examples)中的更多示例。 | 否                                            |
| wildcardFileName         | 给定的文件系统 + folderPath/wildcardFolderPath 下包含通配符的文件名，用于筛选源文件。 <br>允许的通配符为：`*`（匹配零个或更多个字符）和 `?`（匹配零个或单个字符）；如果实际文件夹名称中包含通配符或此转义字符，请使用 `^` 进行转义。  请参阅[文件夹和文件筛选器示例](#folder-and-file-filter-examples)中的更多示例。 | 如果数据集中未指定 `fileName`，则为“是” |
...
> [!NOTE]
> 对于 Parquet/带分隔符的文本格式，仍然按原样支持下一部分中提到的 **AzureBlobFSSource** 类型复制活动源，以实现向后兼容性。 建议你继续使用此新模型，并且 ADF 创作 UI 已切换为生成这些新类型。
...
若要以 **ORC/Avro/JSON/Binary 格式**从 ADLS Gen2 复制数据，复制活动 **source** 节需要支持以下属性：
...
| maxConcurrentConnections | 用于同时连接到数据存储的连接数。 仅在要限制与数据存储的并发连接时指定。 | 否 |
...
- 若要复制到其他格式（如 **ORC/Avro/JSON/Binary 格式**），请参阅[其他格式接收器](#other-format-sink)部分。

#### <a name="parquet-and-delimited-text-format-sink"></a>Parquet 和带分隔符的文本格式接收器

若要将数据复制到 **Parquet 或带分隔符的文本格式**的 ADLS Gen2，请参阅 [Parquet 格式](format-parquet.md)和[带分隔符的文本格式](format-delimited-text.md)一文，了解基于格式的复制活动接收器和支持的设置。 基于格式的复制接收器中 `storeSettings` 设置下的 ADLS Gen2 支持以下属性：
...
> [!NOTE]
> 对于 Parquet/带分隔符的文本格式，仍然按原样支持下一部分中提到的 **AzureBlobFSSink** 类型复制活动接收器，以实现向后兼容性。 建议你继续使用此新模型，并且 ADF 创作 UI 已切换为生成这些新类型。
...
若要以 **ORC/Avro/JSON/Binary 格式**将数据复制到 ADLS Gen2，**sink** 节中需要支持以下属性：

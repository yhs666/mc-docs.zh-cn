...
manager: digimobile
ms.openlocfilehash: 6780d7228a0b9fe6f5ba658939c69d1ff3d14abc
ms.sourcegitcommit: 5191c30e72cbbfc65a27af7b6251f7e076ba9c88
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 07/05/2019
ms.locfileid: "67569742"
---
...
[教程：将 SSIS 包部署到 Azure](tutorial-create-azure-ssis-runtime-portal.md) 介绍了如何使用 Azure SQL 数据库服务器来承载 SSIS 目录数据库 (SSISDB)，以创建 Azure-SSIS IR。 本文对该教程的内容做了延伸，介绍了如何执行以下操作：

- （可选）使用 Azure SQL 数据库服务器与虚拟网络服务终结点/托管实例来托管 SSISDB。 有关如何选择用于托管 SSISDB 的数据库服务器类型的指导，请参阅[比较 Azure SQL 数据库单一数据库/弹性池和托管实例](create-azure-ssis-integration-runtime.md#compare-sql-database-single-databaseelastic-pool-and-sql-database-managed-instance)。 作为先决条件，需将 Azure-SSIS IR 加入虚拟网络，并根据需要配置虚拟网络权限/设置。 请参阅[将 Azure-SSIS IR 加入虚拟网络](/data-factory/join-azure-ssis-integration-runtime-virtual-network)。

- （可选）对 ADF 的托管标识使用 Azure Active Directory (AAD) 身份验证，以连接到数据库服务器。 作为先决条件，需将 ADF 的托管标识添加为能够在 Azure SQL 数据库服务器/托管实例中创建 SSISDB 的包含的数据库用户。具体请参阅[为 Azure-SSIS IR 启用 AAD 身份验证](/data-factory/enable-aad-authentication-azure-ssis-ir)。
...
创建 Azure-SSIS IR 时，ADF 服务将连接到 Azure SQL 数据库服务器/托管实例以准备 SSISDB。 它还会配置虚拟网络的权限/设置（如果已指定），并将 Azure-SSIS IR 加入虚拟网络中。

预配 Azure-SSIS IR 时，还会安装 Azure Feature Pack for SSIS 和 Access Redistributable。 除了内置组件支持的数据源外，这些组件还提供与 Excel/Access 文件和各种 Azure 数据源的连接。 还可以安装其他组件。 有关详细信息，请参阅 [Azure-SSIS 集成运行时的自定义设置](how-to-configure-azure-ssis-ir-custom-setup.md)。
...
- **Azure PowerShell**。 若要运行 PowerShell 脚本来预配 Azure-SSIS IR，请遵照[如何安装和配置 Azure PowerShell](https://docs.microsoft.com/powershell/azure/install-az-ps) 中的说明。
...
3. 在“SQL 设置”  页上，完成以下步骤：
   ...
   ![SQL 设置](./media/tutorial-create-azure-ssis-runtime-portal/sql-settings.png)
    ...
    d. 在“使用 AAD 身份验证...”复选框中，选择数据库服务器用来承载 SSISDB 的身份验证方法：  SQL或 Azure 数据工厂托管标识的 Azure Active Directory (AAD)。 如果勾选它，则需将 ADF 的托管标识添加到有权访问数据库服务器的 AAD 组中，具体请参阅[为 Azure-SSIS IR 启用 AAD 身份验证](/data-factory/enable-aad-authentication-azure-ssis-ir)。
    ...
    h.如果该值不存在，请单击“添加行”。 单击“测试连接”  ，如果成功，则单击“下一步”  。
...
5. 在“选择虚拟网络...”复选框中，选择是否要将集成运行时加入虚拟网络。  如果结合虚拟网络服务终结点/托管实例使用 Azure SQL 数据库来托管 SSISDB，或者需要访问本地数据（即，SSIS 包中包含本地数据源/目标），请选中此项。具体请参阅[将 Azure-SSIS IR 加入虚拟网络](/data-factory/join-azure-ssis-integration-runtime-virtual-network)。 如果选中此项，请完成以下步骤：
...
如果不结合虚拟网络服务终结点/托管实例使用 Azure SQL 数据库来托管 SSISDB，也不需要访问本地数据，则可以省略 VNetId 和 Subnet 参数，或传递这些参数的空值。 否则不能省略这些参数，而必须传递虚拟网络配置中的有效值。具体请参阅[将 Azure-SSIS IR 加入虚拟网络](/data-factory/join-azure-ssis-integration-runtime-virtual-network)。
...
如果对 Azure 数据工厂的托管标识使用 Azure Active Directory (AAD) 身份验证来连接到数据库服务器，则可以省略 CatalogAdminCredential 参数，但必须将 ADF 的托管标识添加到有权访问数据库服务器的 AAD 组，具体请参阅[为 Azure-SSIS IR 启用 AAD 身份验证](/data-factory/enable-aad-authentication-azure-ssis-ir)。 否则，不能忽略它，且必须传递由 SQL 身份验证的服务器管理员用户名和密码构成的有效对象。
...
现在，使用 SQL Server Data Tools (SSDT) 或 SQL Server Management Studio (SSMS) 来将 SSIS 包部署到 Azure。 连接到承载 SSIS 目录 (SSISDB) 的数据库服务器。 数据库服务器的名称采用以下格式：&lt;Azure SQL 数据库服务器名称&gt;.database.chinacloudapi.cn，或 &lt;托管实例名称.DNS 前缀&gt;.database.chinacloudapi.cn。 有关说明，请参阅[部署包](https://docs.microsoft.com/sql/integration-services/packages/deploy-integration-services-ssis-projects-and-packages#deploy-packages-to-integration-services-server)一文。

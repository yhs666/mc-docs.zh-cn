...
由于策略或计划可分配到不同的范围，因此在表中请注意每个分配的范围，以及分配到该范围的定义类型。 还提供每个分配项中不合规资源和不合规策略的数量。 单击表中的某个策略或计划可以更深入地了解该特定分配的符合性。

![策略符合性详细信息](../media/getting-compliance-data/compliance-details.png)

“资源符合性”选项卡（默认为“不合规”，但可进行筛选）上的资源列表反映了当前分配项中现有资源的评估状态，而资源创建请求所触发的事件（追加、审核、拒绝、部署）则显示在“事件”选项卡下。

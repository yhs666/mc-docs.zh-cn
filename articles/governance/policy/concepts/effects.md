...
资源可能会受到多个分配的影响。 这些分配可能处于相同范围（特定资源、资源组、订阅或管理组）或处于不同范围内。 这些分配中的每一个也可能具有不同的定义效果。 无论如何，每项策略的条件和效果（直接分配或作为初始分配的一部分）均为独立评估。 例如，如果策略 1 的条件为将订阅 A 的资源位置限制为
...
- “chinaeast”中订阅 A/资源组 B 中的任何新资源将在策略 2 中标记为不符合，但会被创建（符合策略 1，策略 2 为审核而不是拒绝）。

如果策略 1 和策略 2 都具有拒绝效果，情况将变为：

- 在资源组 B 中而不在“chinanorth”中的任何资源将被标记为不符合策略 2。
- 在资源组 B 中而不在“chinaeast”中的任何资源将被标记为不符合策略 1。
- 订阅 A 中任何不在“chinaeast”中的新资源将会被策略 1 拒绝。

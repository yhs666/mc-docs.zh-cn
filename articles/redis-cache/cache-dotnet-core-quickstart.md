...
description: 本快速入门将介绍如何在 .NET Core 应用中访问 Azure Redis 缓存
...
将以下成员添加到 *Program.cs* 中的 `Program` 类。 此代码为 redis 缓存连接字符串初始化用于访问用户机密的配置。
...
在本快速入门中，你已学习了如何从 .NET Core 应用程序中使用 Azure Redis 缓存。 请继续学习下一快速入门来将 Redis 缓存与 ASP.NET Web 应用配合使用。

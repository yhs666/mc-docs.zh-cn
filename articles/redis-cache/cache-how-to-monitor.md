...
若要查看缓存度量值，请[浏览到](./cache-configure.md#configure-redis-cache-settings) [Azure 门户](https://portal.azure.cn)中的缓存实例。可在“Redis 指标”边栏选项卡上访问 Azure Redis 缓存实例的度量值。
...
>[!NOTE]
> Azure 门户中仅显示存储在所选存储帐户中的度量值。如果更改存储帐户，以前配置的存储帐户中的数据仍可供下载，但并不会显示在 Azure 门户中。

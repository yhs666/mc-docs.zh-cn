...
如果尚未安装 Azure CLI，请参阅[安装 Azure CLI](../cli-install-nodejs.md)。 如果不熟悉 Azure CLI，可在[将适用于 Mac、Linux 和 Windows 的 Azure CLI 与 Azure 资源管理器配合使用](../azure-resource-manager/xplat-cli-azure-resource-manager.md)中了解相关详细信息。
...
运行此命令后，必须按照屏幕上的说明进行登录。 之后，会看到你的帐户、TenantId 和默认订阅 ID。所有命令都在默认订阅的上下文中工作。

...
若要将数据存储比上面列出的时间段更长的时间，可以使用 Azure 存储。 监视数据根据你设置的保留策略保留在存储帐户中。 你必须为数据在 Azure 存储中占用的空间付款。 
...
> [!NOTE]
> 随着 Azure 上的警报功能的不断发展，现在推出了针对警报的统一体验。 有关[新的 Azure 警报](monitoring-overview-unified-alerts.md)的更多详细信息

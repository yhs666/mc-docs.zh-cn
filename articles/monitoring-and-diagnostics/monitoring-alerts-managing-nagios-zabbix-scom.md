...
1. 对于 **Nagios** 和 **Zabbix** 警报，[配置这些服务器](/log-analytics/log-analytics-linux-agents)以将警报发送到 Log Analytics。
1. 对于 **System Center Operations Manager** 警报，[将 Operations Manager 管理组连接到 Log Analytics 工作区](/log-analytics/log-analytics-om-agents)。 System Center Operations Manager 中创建的任何警报均导入 Log Analytics。
...
> [!NOTE]
>  统一警报体验中的 Nagios 警报不具有状态 - 例如，警报的[监视条件](https://aka.ms/azure-alerts-overview)不会从“已触发”变为“已解决”， 但会将“已触发”和“已解决”都显示为单独的警报实例。
...
ms.date: 07/30/2018
ms.author: v-yiso
ms.openlocfilehash: 3a7467222f4686a448445fc03c68bdcca368bca2
ms.sourcegitcommit: 6d4ae5e324dbad3cec8f580276f49da4429ba1a7
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 07/20/2018
ms.locfileid: "39168015"
---
...
集成技术有许多可能的应用方式：从简单的点到点应用程序，到完整的企业服务总线。*此体系结构系列阐述了可能适用于构建一般集成应用程序的可重用组件成分 – 架构师应考虑他们需要为其应用程序和基础结构实现哪些具体组件。*
...
- 资源组。 [资源组](/azure-resource-manager/resource-group-overview)是 Azure 资源的逻辑容器。
- Azure API 管理。 [Azure API 管理](/api-management/)是一个完全托管的平台，用于发布、保护和转换 HTTP API。
- Azure API 管理开发人员门户。 每个 Azure API 管理实例都附带一个[开发人员门户](/api-management/api-management-customize-styles)，用于访问文档、代码示例以及对 API 进行测试。
- Azure 逻辑应用。 [逻辑应用](/logic-apps/logic-apps-overview)是一个无服务器平台，用于构建企业工作流和集成。
- 连接器。 [连接器](/connectors/apis-list)由逻辑应用用来连接到常用的服务。 逻辑应用已有数百个不同的连接器，但也可以使用自定义连接器创建逻辑应用。
- IP 地址。 Azure API 管理服务具有固定的公共 [IP 地址](/virtual-network/virtual-network-ip-addresses-overview-arm)和域名。 域名是 azure-api.net 的子域，例如 contoso..azure-api.net。 逻辑应用和服务总线还有一个公共的 IP 地址；但是，在此体系结构中，仅限 API 管理的 IP 地址调用逻辑应用终结点（出于安全考虑）。 对服务总线的调用受共享访问签名的保护。
...
- Azure Active Directory (Azure AD)。 使用 [Azure AD](/active-directory/) 或其他标识提供者进行身份验证。 Azure AD 提供身份验证来访问 API 终结点（通过传递 [API 管理的 JSON Web 令牌](/policies/authorize-request-based-on-jwt-claims)进行验证），并可以保护对 API 管理开发人员门户的访问（仅限标准和高级层）。

此体系结构具有一些基本的运行模式：

2. 使用逻辑应用构建复合 API；协调对 SAAS 系统、Azure 服务和发布到 API 管理的任何 API 的调用。 [逻辑应用](/api-management/import-logic-app-as-api)也通过 API 管理开发人员门户进行发布。
3. 应用程序[获取所需的 OAuth 2.0 安全令牌](/api-management/api-management-howto-protect-backend-with-aad)，以便能够使用 Azure Active Directory 访问 API。
4. Azure API 管理[验证安全令牌](/api-management/api-management-howto-protect-backend-with-aad)，并将请求传递给后端 API/逻辑应用。

## <a name="recommendations"></a>建议

你的具体要求可能与此处所述的通用体系结构不同。 请使用本部分中的建议作为入手点。

### <a name="azure-api-management-tier"></a>Azure API 管理层

使用基本、标准或高级层，因为这些提供生产 SLA，并支持 Azure 区域中的横向扩展（单位数因层而异）。 高级层还支持跨多个 Azure 区域横向扩展。 请根据所需的吞吐量级别和功能集选择层。 有关详细信息，请参阅 [API 管理定价](https://www.azure.cn/pricing/details/api-management/)。

需要对所有运行的 API 管理实例付费。 如果已纵向扩展，并且始终不需要该性能级别，请考虑利用 API 管理的按小时计费和纵向缩减。

### <a name="logic-apps-pricing"></a>逻辑应用定价

逻辑应用以[无服务器](logic-apps-serverless-overview.md)模型的形式工作 – 根据操作和连接器执行计算费用。 有关详细信息，请参阅[逻辑应用定价](https://www.azure.cn/pricing/details/logic-apps/)。 目前，逻辑应用没有层方面的注意事项。

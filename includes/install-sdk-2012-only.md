...
首先，通过安装 Azure SDK for .NET 来设置开发环境。

1. 若要安装 SDK，请单击以下链接。 本教程需要安装 Visual Studio 2012。 如果尚未安装 Visual Studio，则将与 SDK 一起安装 Visual Studio Express for Web。

    - [Azure SDK for Visual Studio 2012][]

    **注意：**根据计算机上已有 SDK 依赖项数量的不同，安装 SDK 可能耗时较长，从几分钟到半小时或更长时间不等。

2. 当系统提示是要运行还是保存可执行安装文件时，请单击“运行”。

...
<sup>4</sup>高级层在使用应用服务环境时最多允许 50 个计算实例（取决于可用性）和 500 GB 的磁盘空间，否则为 20 个计算实例和 250 GB 的存储。 但是，Azure 中国区目前尚未支持应用服务环境。
...
<sup>8</sup>使用应用服务环境时，高级层允许将备份间隔下调为最多每隔 5 分钟，否则为每天 50 次。 但是，Azure 中国区目前尚未支持应用服务环境。

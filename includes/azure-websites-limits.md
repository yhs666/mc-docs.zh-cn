...
| --- | --- | --- | --- | --- | --- |
| 每个[应用服务计划](../articles/app-service/azure-web-sites-web-hosting-plans-in-depth-overview.md)的 [Web 应用、移动应用或 API 应用数](https://www.azure.cn/home/features/app-service/)<sup>1</sup> |10 |100 |无限制<sup>2</sup> |无限制<sup>2</sup> |无限制<sup>2</sup> |
...
| 自定义域 [SSL 支持](../articles/app-service-web/web-sites-configure-ssl-certificate.md) | | |无限制 |无限制，包含 5 个 SNI SSL 和 1 个 IP SSL 连接 |无限制，包含 5 个 SNI SSL 和 1 个 IP SSL 连接 |
| 集成负载均衡器 | |X |X |X |X |
...
<sup>4</sup>高级层在使用 App Service 环境时最多允许 50 个计算实例（取决于可用性）和 500 GB 的磁盘空间，否则为 20 个计算实例和 250 GB 的存储。 但是，Azure 中国区目前尚未支持应用服务环境。
...
<sup>8</sup>使用 App Service 环境时，高级层允许将备份间隔下调为最多每隔 5 分钟，否则为每天 50 次。 但是，Azure 中国区目前尚未支持应用服务环境。

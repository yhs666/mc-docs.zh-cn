...
MAC &lt;-&gt; IoT 中心 ID 模块充当转换器。 此示例使用 MAC 地址作为唯一的设备标识符，将其与 IoT 中心设备标识相关联。 不过，可以编写自己的模块，让其使用其他的唯一标识符。 例如，设备可能具有唯一序列号或者遥测数据可能包括唯一的嵌入式设备名。
...
* [创建 IoT 中心][lnk-create-hub] 。 在此示例演练中，需要 IoT 中心的名称。 如果没有帐户，只需几分钟即可创建一个[免费帐户][lnk-free-trial]。

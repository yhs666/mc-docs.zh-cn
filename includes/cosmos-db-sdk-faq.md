...
在 12 个月通知期之后，SDK 会停用。 DocumentDB 平台不允许使用已停用的 SDK 的应用程序对 DocumentDB 进行任何访问。 此外，Microsoft 不会对已停用的 SDK 提供客户支持。
...
新特性和功能只添加到新版本。 如果使用的是未停用的旧版本 SDK，则对 DocumentDB 进行的请求仍可与以前一样正常工作，但是你无权访问任何新功能。  

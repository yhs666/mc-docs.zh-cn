使用 [az appservice web create](https://docs.microsoft.com/cli/azure/webapp#create) 命令创建该 Web 应用。 在以下命令中，请将 `<app_name>` 占位符替换为唯一应用名称。 在默认 DNS 站点中，将 `<app_name>` 用于 Web 应用。 如果 `<app_name>` 不唯一，将收到友好错误消息“具有给定名称 <app_name> 的网站已存在”。
...
浏览到该站点 (`http://<app_name>.chinacloudsites.cn`) 查看新建的 Web 应用。
...
请使用 [az appservice web source-control config-local-git](https://docs.microsoft.com/cli/azure/webapp/source-control#config-local-git) 命令配置本地 Git 对 Web 应用的访问权限。
...
保存所显示的 URI。 在下一步中将使用它。 `<username>` 是在上一步中创建的[部署用户](#configure-a-deployment-user)。

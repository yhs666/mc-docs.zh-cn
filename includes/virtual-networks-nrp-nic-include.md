## <a name="nic"></a>NIC
网络接口卡 (NIC) 资源提供与 VNet 资源中现有子网的网络连接。 尽管可以将 NIC 创建为独立对象，但需要将其关联到另一个对象才能实际提供连接。 NIC 可用于将 VM 连接到一个子网、公共 IP 地址或负载均衡器。  
...
| dnsSettings |NIC 的 DNS 设置 |请参阅 [DNS 设置](#dns-settings) |

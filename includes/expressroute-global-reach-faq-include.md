...
ms.openlocfilehash: 6f9b028a9d9a93359376863921aed77f1eb5f7e9
...
否。 两条线路必须来自不同的对等位置。 如果受支持国家/地区的某个市区包含多个 ExpressRoute 对等位置，则可以将该市区内不同对等位置创建的 ExpressRoute 线路连接到一起。 
...
否。 若要在任意两个本地网络之间实现连接，必须显式连接对应的 ExpressRoute 线路。 在上述示例中，必须连接线路 X 和线路 Z。 

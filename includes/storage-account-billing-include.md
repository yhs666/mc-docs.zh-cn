我们根据你的存储帐户，针对你的 Azure 存储使用情况收费。 存储成本取决于以下几个因素：区域/位置、帐户类型、存储容量、复制方案、存储交易和数据流出量。
...
* 数据流出量指的是传出某个 Azure 区域的数据。 当不在同一区域中的应用程序访问存储帐户中的数据时，需要为数据流出量付费。 （对于 Azure 服务，可以采取措施将你的数据和服务通过分组分到相同的数据中心内，从而降低或避免数据流出量费用。）

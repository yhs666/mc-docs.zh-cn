...
路由表资源包含的路由用于定义流量在 Azure 基础结构中的流动方式。 可使用用户定义的路由 (UDR) 将所有流量从给定子网发送到某个虚拟设备，例如防火墙或入侵检测系统 (IDS)。 可将路由表关联到子网。 

路由表包含以下属性。

| 属性 | 说明 | 示例值 |
| --- | --- | --- |
| routes |路由表中用户定义的路由的集合 |请参阅[用户定义的路由](#User-defined-routes) |
...
| nextHopType |流量发送目标设备的类型 |VirtualAppliance、VPN 网关、Internet |

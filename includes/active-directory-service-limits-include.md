---
 title: include 文件 description: include 文件 services: active-directory author: curtand ms.service: active-directory ms.topic: include origin.date:02/21/2019 ms.date:03/06/2019 ms.author: v-junlch ms.custom: include 文件
---
...
| 对象 |<ul><li>Azure Active Directory 免费版用户最多可以在单个目录中创建 500,000 个对象。</li><li>非管理员用户最多可以创建 250 个对象。 活动对象和可供还原的已删除对象（删除时间少于 30 天）都会计入此配额。 不再可供还原的已删除对象在 30 天内按 1/4 的值计入此配额。 请考虑[将管理员角色分配给](../articles/active-directory/users-groups-roles/directory-assign-admin-roles.md)在执行常规职责过程中可能会重复超出此配额的非管理员用户。</li></ul> |

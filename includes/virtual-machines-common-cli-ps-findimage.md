...
将 `az vm image list` 与 [JMESPATH 查询筛选器](https://docs.microsoft.com/cli/azure/query-az-cli2) 结合使用可查找特定信息。 例如，下面显示了可用于 **Debian** 的 **SKU**（请记住，在没有 `--all` 开关时，它只搜索常用映像的本地缓存）：

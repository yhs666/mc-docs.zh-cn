...
模拟设备应用会将文件上传到 Blob，因此，必须拥有与 IoT 中心关联的 [Azure 存储](../articles/storage/storage-create-storage-account.md#create-a-storage-account) 帐户。 将 Azure 存储帐户与 IoT 中心相关联时，IoT 中心会生成一个 SAS URI。 设备可以使用此 SAS URI 安全地将文件上传到 Blob 容器。 IoT 中心服务和设备 SDK 协调生成 SAS URI 的过程，并使其可供设备用来上传文件。

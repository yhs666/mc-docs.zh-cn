...
3. 在“新建帐户”边栏选项卡中，为 Azure Cosmos DB 帐户指定所需的配置。 
    ...
    将以下屏幕截图中的信息仅作为指南，完成“新建帐户”边栏选项卡上的各字段。 由于你自己的值不会匹配屏幕截图上的值，因此请确保在设置帐户时选择唯一的值。 

    ![“Azure Cosmos DB”边栏选项卡](./media/cosmos-db-create-dbaccount-graph/create-nosql-db-databases-json-tutorial-2.png)

    设置|建议的值|说明
    ---|---|---
    ID|*唯一值*|选择用于标识 Azure Cosmos DB 帐户的唯一名称。 由于 documents.azure.cn 将追加到所提供的 ID 后面以创建 URI，因此，请使用唯一但可识别的 ID。 ID 必须仅包含小写字母、数字和连字符 (-) 字符，并且必须包含 3 到 50 个字符。

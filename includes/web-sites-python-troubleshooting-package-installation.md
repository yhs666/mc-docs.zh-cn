...
如果软件包由于需要编译器而未安装，用户可以在本地计算机上安装编译器，然后为软件包构建轮，随后将轮包含在用户的存储库中。
...
还可以使用 easy\_install 从 exe 安装程序进行安装（有些兼容 zip，所以 easy\_install 支持它们）。  将安装程序添加到存储库，并通过传递可执行文件的路径来调用 easy\_install。
...
如果存储库中包含虚拟环境，则可以通过创建一个空文件来防止部署脚本在 Azure 上执行虚拟环境管理：

...
<a id="common-errors-during-classic-to-azure-resource-manager-migration" class="xliff"></a>
本文编录了将 IaaS 资源从 Azure 经典部署模型迁移到 Azure Resource Manager 堆栈期间的最常见错误和缓解措施。

## 错误列表
<a id="list-of-errors" class="xliff"></a>
| 错误字符串 | 缓解措施 |
| --- | --- |
...
## 详细的缓解措施
<a id="detailed-mitigations" class="xliff"></a>

### VM 中数据磁盘的物理 Blob 字节大小与 VM 数据磁盘的逻辑字节大小不匹配。
<a id="vm-with-data-disk-whose-physical-blob-size-bytes-does-not-match-the-vm-data-disk-logical-size-bytes" class="xliff"></a>

当数据磁盘逻辑大小与实际 VHD Blob 大小不同步时，将发生这种情况。 可以轻松使用以下命令验证这种问题：

#### 验证问题
<a id="verifying-the-issue" class="xliff"></a>
...
#### 缓解问题
<a id="mitigating-the-issue" class="xliff"></a>
...
### 完成迁移后，将 VM 移动到其他订阅中
<a id="moving-a-vm-to-a-different-subscription-after-completing-migration" class="xliff"></a>

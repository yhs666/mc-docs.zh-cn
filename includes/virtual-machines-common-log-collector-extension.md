...
> [!NOTE]
> 可以在 http://blogs.msdn.com/b/kwill/archive/2013/08/09/windows-azure-paas-compute-diagnostics-data.asp 中找到大部分日志记录信息的说明。
> 
> 
...
5. 按如下所示调用 SetAzureServiceLogCollector.ps1（本文末尾提供），以便为云服务启用 AzureLogCollector 扩展。 执行完以后，你可以在 `https://YouareStorageAccountName.blob.core.chinacloudapi.cn/vmlogs`
...
2. 提供要向其上传所收集文件的 Azure 存储帐户名称和密钥。
        ...
        $StorageAccountName = 'YourStorageAccountName'
        $StorageAccountKey  = 'YouStorageAccountKey'
3. 按如下所示调用 SetAzureVMLogCollector.ps1（本文末尾提供），以便为云服务启用 AzureLogCollector 扩展。 执行完以后，可以在 https://YouareStorageAccountName.blob.core.chinacloudapi.cn/vmlogs 下找到上传的文件

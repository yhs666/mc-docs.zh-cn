* 如果非托管磁盘位于此前使用 Azure 存储服务加密加密的存储帐户中，则不能将该非托管磁盘转换成托管磁盘。 有关在托管磁盘中复制和使用这些虚拟硬盘 (VHD) 的步骤，请参阅本文后面的[托管磁盘和 Azure 存储服务加密](#managed-disks-and-azure-storage-service-encryption)部分。

...
虚拟机的大小会影响定价。 此外，大小还会影响虚拟机的处理能力、内存和存储容量。 存储成本根据存储帐户中的已使用页数进行单独计算。 有关详细信息，请参阅 [Virtual Machines Pricing Details](https://www.azure.cn/pricing/details/virtual-machines/)（虚拟机定价详细信息）和 [Azure Storage Pricing](https://www.azure.cn/pricing/details/storage/)（Azure 存储定价）。 

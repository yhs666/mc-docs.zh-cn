...
| Resource | 免费 | 基本 | S1 | S2 | S3 | S3 HD |
| --- | --- | --- | --- | --- | --- | --- |
| 服务级别协议 (SLA) |否 <sup>1</sup> |是 |是 |是 |是 |是 |
| 每个分区的存储空间 |50 MB |2 GB |25 GB |100 GB |200 GB |200 GB |
...
<sup>2</sup> S3 HD 的硬性限制为 3 个分区，低于 S3 的分区限制。 采用的分区限制较低是因为 S3 HD 的索引计数要高得多。 由于计算资源（存储和处理）和内容（索引和文档）都存在服务限制，因此会先达到内容限制。

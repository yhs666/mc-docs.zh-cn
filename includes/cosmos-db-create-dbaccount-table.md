...
3. 在“新建帐户”边栏选项卡中，为 Azure Cosmos DB 帐户指定所需的配置。 
    ...
    在本快速入门教程中，我们针对表 API 编程，因此，在填写表单时，请选择“表(键/值)”。 但如果有社交媒体应用的图形数据、来自目录应用的文档数据或从 MongoDB 应用迁移的数据，请意识到 Azure Cosmos DB 可以为所有任务关键型应用程序提供高度可用的全球分布式数据库服务平台。
    ...
    API|表（键/值）|在本文的后面部分中，我们针对[表 API](../articles/cosmos-db/table-introduction.md) 编程。|
    订阅|

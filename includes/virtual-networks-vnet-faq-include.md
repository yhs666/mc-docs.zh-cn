...
### <a name="what-is-an-azure-virtual-network-vnet"></a>什么是 Azure 虚拟网络 (VNet)？
可以使用 VNet 预配和管理 Azure 中的虚拟专用网 (VPN)，或者将 VNet 与 Azure 中的其他 VNet 链接，或与本地 IT 基础结构链接，以创建混合或跨界解决方案。 你创建的每个 VNet 都具有其自己的 CIDR 块，并且只要 CIDR 块不会冲突，则可链接到其他 Vnet 和本地网络。 还可以控制 VNet 的 DNS 服务器设置并将 VNet 分离到子网中。
...
请访问[虚拟网络概述](../articles/virtual-network/virtual-networks-overview.md)，查看可帮助用户确定最佳网络设计选项的决策表。

### <a name="how-do-i-get-started"></a>如何开始？
请从访问[虚拟网络文档](/virtual-network/)开始。 此页包含指向常见配置步骤的链接以及帮助你了解设计虚拟网络时需要考虑的事项的信息。
...
可以在 VNet 中使用基于 IP 的标准协议。 但是，VNet 内阻止多播、广播、IP-in-IP 封装数据包以及通用路由封装 (GRE) 数据包。 工作的标准协议包括：
...
* **内部 IP 地址 -** 如果将 VM 部署到 VNet，该 VM 从您指定的内部 IP 地址池接收内部 IP 地址。 VM 使用内部 IP 地址在 VNet 内进行通信。 虽然 Azure 分配动态内部 IP 地址，但你可以为你的 VM 请求静态地址。 若要详细了解静态内部 IP 地址，请访问[如何设置静态内部 IP](../articles/virtual-network/virtual-networks-reserved-private-ip.md)。
* **VIP -** 你的 VM 还与 VIP 相关联，不过永远不会将 VIP 直接分配到 该 VM。 VIP 是可以分配到云服务的公共 IP 地址。 还可以为云服务保留 VIP。
* **ILPIP**：还可以配置实例层级公共 IP 地址 (ILPIP)。 ILPIP 是直接与 VM 相关联，而非云服务。 若要详细了解 ILPIP，请访问[实例层级公共 IP 概述](../articles/virtual-network/virtual-networks-instance-level-public-ip.md)。
...
是的。 内部 IP 地址在生存期内仍属于 VM，除非 VM 被释放。 当 VM 被释放时，内部 IP 地址则释放，除非定义了 VM 的静态内部 IP 地址。 如果 VM 只是停机（而不是处于“已停止(已释放)”状态），IP 地址仍保持分配到 VM。
...
是的。 可在[此处](../articles/virtual-network/virtual-networks-move-vm-role-to-subnet.md)找到更多信息。
...
是，即使 VM 已停止（已释放）并重新启动，VM 的 MAC 地址也保持不变。
...
### <a name="what-services-can-i-use-with-vnets"></a>哪些服务可以与 VNet 共同使用？
只能在 VNet 中使用计算服务。 计算服务仅限于云服务（Web 角色和辅助角色）和 VM。

### <a name="can-i-use-web-apps-with-virtual-network"></a>是否可以共同使用 Web 应用和虚拟网络？
